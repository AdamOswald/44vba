...
public:
  Slot2InfoSimple(const char *_name, const char *_descr, const u8 _id)
      : mName(_name), mDescr(_descr), mID(_id) {}
...
public:
  ...
  // called once when the emulator starts up, or when the device springs into
  // existence
  ...
  // called when the emulator shuts down, or when the device disappears from
  // existence
...
// just disconnects and reconnects the device. ideally, the disconnection and
// connection would be called with sensible timing
void slot2_Reset();
...
template <u8 PROCNUM, typename T> bool slot2_write(u32 addr, T val);

template <u8 PROCNUM, typename T> bool slot2_read(u32 addr, T &val);
...
enum ADDON_CFLASH_MODE {
  ADDON_CFLASH_MODE_Path,
  ADDON_CFLASH_MODE_File,
...
inline bool CFlash_IsUsingPath() {
  return CFlash_Mode == ADDON_CFLASH_MODE_Path ||
...
extern void guitarGrip_setKey(bool green, bool red, bool yellow,
                              bool blue); // Guitar grip keys
extern void piano_setKey(bool c, bool cs, bool d, bool ds, bool e, bool f,
                         bool fs, bool g, bool gs, bool a, bool as, bool b,
                         bool hic); // piano keys
#endif                              //__SLOT_H__
