    ...
    case 0:		// 1 byte
        for (u32 i = 0; i < (4 * 1024 * 1024); i++)
        {
            ...
            if (statMem[addr] & (1<<offs))
            {
                ...
                {
                case 0:
                    res=(T1ReadByte(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) > T1ReadByte(mem, i));
                    break;
                case 1:
                    res=(T1ReadByte(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) < T1ReadByte(mem, i));
                    break;
    ...
    case 1:		// 2 bytes
        for (u32 i = 0; i < (4 * 1024 * 1024); i+=2)
        {
            ...
            if (statMem[addr] & (3<<offs))
            {
                ...
                {
                case 0:
                    res=(T1ReadWord(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) > T1ReadWord(mem, i));
                    break;
                case 1:
                    res=(T1ReadWord(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) < T1ReadWord(mem, i));
                    break;
    ...
    case 2:		// 3 bytes
        for (u32 i = 0; i < (4 * 1024 * 1024); i+=3)
        {
            ...
            if (statMem[addr] & (7<<offs))
            {
                ...
                {
                case 0:
                    res=((T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) & 0x00FFFFFF) > (T1ReadLong(mem, i) & 0x00FFFFFF) );
                    break;
                case 1:
                    res=((T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) & 0x00FFFFFF) < (T1ReadLong(mem, i) & 0x00FFFFFF) );
                    break;
    ...
    case 3:		// 4 bytes
        for (u32 i = 0; i < (4 * 1024 * 1024); i+=4)
        {
            ...
            if (statMem[addr] & (0xF<<offs))
            {
                ...
                {
                case 0:
                    res=(T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) > T1ReadLong(mem, i));
                    break;
                case 1:
                    res=(T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) < T1ReadLong(mem, i));
                    break;
            ...
            *address = i;
            ...
            case 1:
                *curVal=(u32)T1ReadWord(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i);
                return TRUE;
            case 2:
                *curVal=(u32)T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) & 0x00FFFFFF;
                return TRUE;
            case 3:
