...
BOOL CHEATS::add(u8 size, u32 address, u32 val, char *description,
                 BOOL enabled) {
...
BOOL CHEATS::update(u8 size, u32 address, u32 val, char *description,
                    BOOL enabled, u32 pos) {
  if (pos >= list.size())
    return FALSE;
...
BOOL CHEATS::move(u32 srcPos, u32 dstPos) {
  if (srcPos >= list.size() || dstPos > list.size())
    return false;
...
static void CheatWrite(int size, int proc, u32 addr, u32 val) {
  ...
  if (isDangerous) {
    // test dirtiness
    if (size == 8)
      dirty = _MMU_read08(proc, MMU_AT_DEBUG, addr) != val;
    if (size == 16)
      dirty = _MMU_read16(proc, MMU_AT_DEBUG, addr) != val;
    if (size == 32)
      dirty = _MMU_read32(proc, MMU_AT_DEBUG, addr) != val;
  }
  ...
  if (size == 8)
    _MMU_write08(proc, MMU_AT_DEBUG, addr, val);
  if (size == 16)
    _MMU_write16(proc, MMU_AT_DEBUG, addr, val);
  if (size == 32)
    _MMU_write32(proc, MMU_AT_DEBUG, addr, val);
...
void CHEATS::ARparser(CHEATS_LIST &list) {
  // primary organizational source (seems to be referenced by cheaters the most)
  // - http://doc.kodewerx.org/hacking_nds.html secondary clarification and
  // details (for programmers) -
  // http://problemkaputt.de/gbatek.htm#dscartcheatactionreplayds more here:
  // http://www.kodewerx.org/forum/viewtopic.php?t=98

  // general note: gbatek is more precise with the maths; it does not indicate
  // any special wraparound/masking behaviour so it's assumed to be standard
  // 32bit masking general note: <gbatek>  For all word/halfword accesses, the
  // address should be aligned accordingly -- OR ELSE WHAT? (probably the
  // default behaviour of our MMU interface)

  // TODO: "Code Hacks" from kodewerx (seems frail, but we should at least
  // detect them and print some diagnostics
  // TODO: v154 special stuff

  bool v154 = true; // on advice of power users, v154 is so old, we can assume
                    // all cheats use it
  bool vEmulator = true;
  ...
  for (u32 i = 0; i < list.num; i++) {
    ...
    //<gbatek> The condition register is checked, for all code types but the D0,
    //D1 and D2 code type <gbatek> and for the C5 code type it's checked AFTER
    //the counter has been incremented (so the counter is always incremented)
    // but first: we must run this for IFs regardless of the condition flag to
    // track the nest level
    if (type >= 0x03 && type <= 0x0A) {
      // pretty wild guess as to how this is implemented (I could be off by one)
      // we begin by assuming the current status is disabled, since we wont have
      // a chance to correct if this IF is not supposed to be evaluated
    ...
    case 0x00:
      ...
      // maybe a type "M" code -- impossible to enter into desmume? not
      // supported right now. otherwise:
    ...
    case 0x03:
      ...
      // If not, the code(s) following this one are not executed (ie. execution
      // status is set to false) until a code type D0 or D2 is encountered, or
      // until the end of the code list is reached.
      x = hi & 0x0FFFFFFF;
      y = lo;
      if (v154)
        if (x == 0)
    ...
    case 0x07:
      // Greater Than (Conditional 16-Bit + Masking RAM "Writes") (but WRITING
      // has nothing to do with this) 7XXXXXXX ZZZZYYYY Checks if (YYYY) > (not
      // (ZZZZ) & halfword at [XXXX]). If not, the code(s) following this one
      // are not executed (ie. execution status is set to false) until a code
      // type D0 or D2 is encountered, or until the end of the code list is
      // reached.
      x = hi & 0x0FFFFFFF;
      y = lo & 0xFFFF;
      z = lo >> 16;
    ...
    case 0x08:
      // Less Than (Conditional 16-Bit + Masking RAM "Writes") (but WRITING has
      // nothing to do with this) 8XXXXXXX ZZZZYYYY Checks if (YYYY) < (not
      // (ZZZZ) & halfword at [XXXX]). If not, the code(s) following this one
      // are not executed (ie. execution status is set to false) until a code
      // type D0 or D2 is encountered, or until the end of the code list is
      // reached.
    ...
    case 0x09:
      // Equal To (Conditional 16-Bit + Masking RAM "Writes") (but WRITING has
      // nothing to do with this) 9XXXXXXX ZZZZYYYY Checks if (YYYY) == (not
      // (ZZZZ) & halfword at [XXXX]). If not, the code(s) following this one
      // are not executed (ie. execution status is set to false) until a code
      // type D0 or D2 is encountered, or until the end of the code list is
      // reached.
    ...
    case 0x0A:
      // Not Equal To (Conditional 16-Bit + Masking RAM "Writes") (but WRITING
      // has nothing to do with this) AXXXXXXX ZZZZYYYY Checks if (YYYY) != (not
      // (ZZZZ) & halfword at [XXXX]). If not, the code(s) following this one
      // are not executed (ie. execution status is set to false) until a code
      // type D0 or D2 is encountered, or until the end of the code list is
      // reached.
    ...
    case 0xC0:
      ...
      // to be executed' and the 'Dx execution status'. Repeat will be executed
      // when a D1/D2 code is encountered. When repeat is executed, the AR
      // reloads the 'next code to be executed' and the 'execution status' from
      // the Dx registers. <gbatek> FOR loopcount=0 to YYYYYYYY  ;execute Y+1
      //times
      y = lo;
      st.loop.idx = 0; //<gbatek> any FOR statement does forcefully terminate
                       //any prior loop
      st.loop.iterations = y;
      st.loop.top =
          i; // current instruction is saved as top for branching back up
      //<gbatek> FOR does backup the current IF condidition flags, and NEXT does
      //restore these flags
    ...
    case 0xC4:
      ...
      // it seems this lets us rewrite the code at runtime. /his will be very
      // difficult to emulate. But it would be possible: we could copy whenever
      // it's activated and allow that to be rewritten we would try to select a
      // special sentinel pointer which couldn't be confused for a useful
      // address
      if (!v154)
        break;
    ...
    case 0xC5:
      ...
      // http://www.kodewerx.org/forum/viewtopic.php?t=98 has more details, but
      // this can't be executed readily without adding something to the cheat
      // engine
      if (!v154)
        break;
    ...
    case 0xD0:
      ...
      // D0000000 00000000
      // Loads the previous execution status. If none exists, the execution
      // status stays at 'execute codes'
      ...
      // in other words, a stack underflow results in the original state of
      // execution
    ...
    case 0xD1:
      ...
      // D1000000 00000000
      // Executes the next block of codes 'n' times (specified by the 0x0C
      // codetype), but doesn't clear the Dx register upon completion. <gbatek>
      //FOR does backup the current IF condidition flags, and NEXT does restore
      //these flags
    ...
    case 0xD2:
      ...
      // D2000000 00000000
      // Executes the next block of codes 'n' times (specified by the 0x0C
      // codetype), and clears all temporary data. (i.e. execution status,
      // offsets, code C settings, etc.) This code can also be used as a full
      // terminator, giving the same effects to any block of code. <gbatek> FOR
      //does backup the current IF condidition flags, and NEXT does restore
      //these flags
      st.status = st.loop.status;
      if (st.loop.idx < st.loop.iterations) {
        st.loop.idx++;
        ...
        //<gbatek> The NEXT+FLUSH command does (after finishing the loop) reset
        //offset=0, datareg=0, and does clear all condition flags, so further
    ...
    case 0xD6:
      ...
      // D6000000 XXXXXXXX
      // Writes the 'Dx data' word to [XXXXXXXX+offset], and increments the
      // offset by 4. <gbatek> word[XXXXXXXX+offset]=datareg, offset=offset+4
      x = lo;
      addr = x + st.offset;
    ...
    case 0xD7:
      ...
      // D7000000 XXXXXXXX
      // Writes the 'Dx data' halfword to [XXXXXXXX+offset], and increments the
      // offset by 2. <gbatek> half[XXXXXXXX+offset]=datareg, offset=offset+2
      x = lo;
      addr = x + st.offset;
    ...
    case 0xD8:
      ...
      // D8000000 XXXXXXXX
      // Writes the 'Dx data' byte to [XXXXXXXX+offset], and increments the
      // offset by 1. <gbatek> byte[XXXXXXXX+offset]=datareg, offset=offset+1
      x = lo;
      addr = x + st.offset;
    ...
    case 0xD9:
      ...
      // D9000000 XXXXXXXX
      // Loads the word at [XXXXXXXX+offset] and stores it in the'Dx data
      // register'.
      x = lo;
      addr = x + st.offset;
    ...
    case 0xDA:
      ...
      // DA000000 XXXXXXXX
      // Loads the halfword at [XXXXXXXX+offset] and stores it in the'Dx data
      // register'.
      x = lo;
      addr = x + st.offset;
    ...
    case 0xDB:
      ...
      // DB000000 XXXXXXXX
      // Loads the byte at [XXXXXXXX+offset] and stores it in the'Dx data
      // register'. This is a bugged code type. Check 'AR Hack #0' for the fix.
      x = lo;
      addr = x + st.offset;
      st.data = _MMU_read08(st.proc, MMU_AT_DEBUG, addr);
      //<gbatek> Before v1.54, the DB000000 code did accidently set
      //offset=offset+XXXXXXX after execution of the code
      if (!v154)
        st.offset = addr;
      break;
    ...
    case 0x0E:
      ...
      // EXXXXXXX YYYYYYYY
      // Copies YYYYYYYY bytes from (current code location + 8) to [XXXXXXXX +
      // offset]. <gbatek> Copy YYYYYYYY parameter bytes to [XXXXXXXX+offset...]
      //<gbatek> For the COPY commands, addresses should be aligned by four (all
      //data is copied with ldr/str, except, on odd lengths, the last 1..3 bytes
      //do use ldrb/strb). attempting to emulate logic the way they may have
...

      {
        ...
        while (y >= 4) {
          if (i == list.num)
            break; // if we erroneously went off the end, bail
        ...
        // the main loop will increment to the next cheat, but the loop above
        // may have gone one too far
        if (t == 0)
          i--;
      }
      break;
...
BOOL CHEATS::add_AR(char *code, char *description, BOOL enabled) {
  ...
  CHEATS_LIST temp;
  if (!CHEATS::XXCodeFromString(&temp, code))
    return FALSE;
...
BOOL CHEATS::update_AR(char *code, char *description, BOOL enabled, u32 pos) {
  ...
  if (code != NULL) {
    if (!CHEATS::XXCodeFromString(this->getItemByIndex(pos), code))
      return FALSE;
    this->setDescription(description, pos);
    list[pos].type = 1;
  }
...
BOOL CHEATS::add_CB(char *code, char *description, BOOL enabled) {
  ...
  if (!CHEATS::XXCodeFromString(this->getItemByIndex(num), code))
    return FALSE;
...
CHEATS_LIST *CHEATS::getListPtr() { return &this->list[0]; }
...
u32 CHEATS::getSize() { return list.size(); }
...
BOOL CHEATS::load() {
  ...
  while (!flist.eof()) {
    ...
    if (!strncasecmp(buf, "name=", 5))
      continue;
    if (!strncasecmp(buf, "serial=", 7))
      continue;

    memset(&tmp_cht, 0, sizeof(tmp_cht));
    if ((buf[0] == 'D') && (buf[1] == 'S')) // internal
      tmp_cht.type = 0;
    else if ((buf[0] == 'A') && (buf[1] == 'R')) // Action Replay
      tmp_cht.type = 1;
    else if ((buf[0] == 'B') && (buf[1] == 'S')) // Codebreaker
      tmp_cht.type = 2;
    else
      continue;
    ...
    if (descr_pos != 0) {
      strncpy(tmp_cht.description, (buf + descr_pos + 1),
              sizeof(tmp_cht.description));
    ...
    for (int i = 0; i < tmp_cht.num; i++) {
      ...
      if (tmp_cht.type == 0) {
        tmp_cht.size =
            std::min<u32>(3, ((tmp_cht.code[i][0] & 0xF0000000) >> 28));
...
void CHEATS::process(int targetType) {
  ...
  if (cheatsResetJit) {
    if (CommonSettings.use_jit) {
      printf("Cheat code operation potentially not compatible with JIT "
             "operations. Resetting JIT...\n");
...
BOOL CHEATS::XXCodeFromString(CHEATS_LIST *cheatItem,
...
BOOL CHEATS::XXCodeFromString(CHEATS_LIST *cheatItem, const char *codeString) {
  ...
  for (size_t i = 0; i < code_len; i++) {
    char c = codeString[i];
    // apparently 100% of pokemon codes were typed with the letter O in place of
    // zero in some places so let's try to adjust for that here
  ...
  case 2: // 3 bytes
    for (u32 i = 0; i < (4 * 1024 * 1024); i += 3) {
      ...
      if (statMem[addr] & (0x7 << offs)) {
        if ((T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) & 0x00FFFFFF) ==
            val) {
  ...
  case 0: // 1 byte
    for (u32 i = 0; i < (4 * 1024 * 1024); i++) {
      ...
      if (statMem[addr] & (1 << offs)) {
        switch (comp) {
        case 0:
          res = (T1ReadByte(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) >
                 T1ReadByte(mem, i));
          break;
        case 1:
          res = (T1ReadByte(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) <
  ...
  case 1: // 2 bytes
    for (u32 i = 0; i < (4 * 1024 * 1024); i += 2) {
      ...
      if (statMem[addr] & (3 << offs)) {
        switch (comp) {
        case 0:
          res = (T1ReadWord(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) >
                 T1ReadWord(mem, i));
          break;
        case 1:
          res = (T1ReadWord(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) <
  ...
  case 2: // 3 bytes
    for (u32 i = 0; i < (4 * 1024 * 1024); i += 3) {
      ...
      if (statMem[addr] & (7 << offs)) {
        switch (comp) {
        case 0:
          res = ((T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) & 0x00FFFFFF) >
                 (T1ReadLong(mem, i) & 0x00FFFFFF));
          break;
        case 1:
          res = ((T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) & 0x00FFFFFF) <
                 (T1ReadLong(mem, i) & 0x00FFFFFF));
  ...
  case 3: // 4 bytes
    for (u32 i = 0; i < (4 * 1024 * 1024); i += 4) {
      ...
      if (statMem[addr] & (0xF << offs)) {
        switch (comp) {
        case 0:
          res = (T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) >
                 T1ReadLong(mem, i));
          break;
        case 1:
          res = (T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) <
...
u32 CHEATSEARCH::getAmount() { return (amount); }
  ...
  for (u32 i = lastRecord; i < (4 * 1024 * 1024); i += step) {
    ...
    if (statMem[addr] & (stepMem << offs)) {
      *address = i;
      ...
      switch (_size) {
      case 0:
        *curVal = (u32)T1ReadByte(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i);
        return TRUE;
      case 1:
        *curVal = (u32)T1ReadWord(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i);
        return TRUE;
      case 2:
        *curVal =
            (u32)T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i) & 0x00FFFFFF;
        return TRUE;
      case 3:
        *curVal = (u32)T1ReadLong(MMU.MMU_MEM[ARMCPU_ARM9][0x20], i);
        return TRUE;
      default:
        return TRUE;
      }
...
void CHEATSEARCH::getListReset() { lastRecord = 0; }
...
void CHEATSEXPORT::R4decrypt(u8 *buf, u32 len, u32 n) {
  ...
  while (r < len) {
    ...
    for (i = 0; i < 512 && i < len - r; i++) {
      ...
      if (BIT_N(k, 18))
        key |= 0x0400;
      if (BIT_N(k, 17) != BIT_N(x, 31))
        key |= 0x0200;
      if (BIT_N(k, 16) != BIT_N(x, 30))
        key |= 0x0100;
      if (BIT_N(k, 30) != BIT_N(k, 29))
        key |= 0x0080;
      if (BIT_N(k, 29) != BIT_N(k, 28))
        key |= 0x0040;
      if (BIT_N(k, 28) != BIT_N(k, 27))
        key |= 0x0020;
      if (BIT_N(k, 27) != BIT_N(k, 26))
        key |= 0x0010;
      if (BIT_N(k, 26) != BIT_N(k, 25))
        key |= 0x0008;
      if (BIT_N(k, 25) != BIT_N(k, 24))
        key |= 0x0004;
      if (BIT_N(k, 25) != BIT_N(x, 26))
        key |= 0x0002;
      if (BIT_N(k, 24) != BIT_N(x, 25))
        key |= 0x0001;
      buf[i] ^= _xor;
    }
  ...
  while (1) {
    ...
    if (gameInfo.crcForCheatsDb == fat.CRC &&
      ...
      printf("Cheats: found %s CRC %08X at 0x%08llX, size %i byte(s)\n", buf,
             fat.CRC, fat.addr, dataSize - encOffset);
      return true;
    }
...
bool CHEATSEXPORT::getCodes() {
  ...
  u32 *cmd = (u32 *)(((intptr_t)gameTitlePtr +
                      strlen((const char *)gameTitlePtr) + 4) &
                     ptrMask);
  ...
  while (pos < numCheats) {
    ...
    if ((*cmd & 0xF0000000) == 0x10000000) // Folder
    {
      ...
      folderNote =
          (u8 *)((intptr_t)folderName + strlen((char *)folderName) + 1);
      pos++;
      cmd = (u32 *)(((intptr_t)folderName + strlen((char *)folderName) + 1 +
                     strlen((char *)folderNote) + 1 + 3) &
                    ptrMask);
    }
    ...
    for (u32 i = 0; i < folderNum; i++) // in folder
    {
      ...
      u8 *cheatNote =
          (u8 *)((intptr_t)cheatName + strlen((char *)cheatName) + 1);
      u32 *cheatData =
          (u32 *)(((intptr_t)cheatNote + strlen((char *)cheatNote) + 1 + 3) &
                  ptrMask);
      ...
      if (numberCodes <= MAX_XX_CODE) {
        ...
        strncpy(cheats[pos_cht].description, descriptionStr.c_str(),
                sizeof(cheats[pos_cht].description));
        cheats[pos_cht].description[sizeof(cheats[pos_cht].description) - 1] =
            '\0';
...
CHEATS_LIST *CHEATSEXPORT::getCheats() { return cheats; }
u32 CHEATSEXPORT::getCheatsNum() { return numCheats; }
