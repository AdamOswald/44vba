...
static const char *DESMUME_BACKUP_FOOTER_TXT =
    "|<--Snip above here to create a raw sav by excluding this DeSmuME "
    "savedata footer:";
...
//(can possibly be used to repair poorly chosen save types discovered late in
//gameplay i.e. pokemon gamers)
...
BackupDevice::BackupDevice() {
  ...
  // usually 0xFF, but occasionally others. If these exceptions could be related
  // to a particular backup memory type, that would be helpful. at first we
  // assumed it would be 0x00, but baby pals proved that it should be 0xFF:
  //  the game reads its initial sound volumes from uninitialized data, and if
  //  it is 0, the game will be silent if it is 0xFF then the game starts with
  //  its sound and music at max, as presumably it is supposed to. so in r3303
  //  we finally changed it (no$ appears definitely to initialize to 0xFF)
  ...
  if (fexists && CommonSettings.backupSave) {
    ...
    if (!in.fail()) {
      ...
      printf("BackupDevice: Could not read the save file for creating a "
             "backup.\n");
    }
  }

  if (!fexists) {
    printf("BackupDevice: DeSmuME .dsv save file not found. Trying to load a "
           ".sav file.\n");
        ...
        if (!fpOut.fail()) {
          ...
          if ((buf) && (fpTmp.fread(buf, sz) == sz)) {
            if (no_gba_unpack(buf, sz)) {
              ...
              // was this intended for egregiously over-sized save files? too
              // bad. sz = trim(buf, sz);
            }
  ...
  /*
      ...
      if (!fileCanReadWrite)
      {
              ...
              printf("BackupDevice: WARNING! Failed to get read/write access to
     the save file! Will operate in RAM instead.\n");
      } */
...
int BackupDevice::readFooter() {
  ...
  MCLOG("\t* padSize:\t%u\n", this->_info.padSize);
  MCLOG("\t* type (%u):\t%s\n", this->_info.type,
        save_types[this->_info.type].descr);
...
void BackupDevice::writeByte(u8 val) { fpMC->write_u8(val); }
void BackupDevice::writeWord(u16 val) { fpMC->write_16LE(val); }
void BackupDevice::writeLong(u32 val) { fpMC->write_32LE(val); }

void BackupDevice::seek(u32 pos) { fpMC->fseek(pos, SEEK_SET); }

void BackupDevice::flushBackup() { fpMC->fflush(); }

bool BackupDevice::saveBuffer(u8 *data, u32 size, bool _rewind,
                              bool _truncate) {
  if (_rewind) {
    fpMC->fseek(0, SEEK_SET);
    if (_truncate)
...
void BackupDevice::movie_mode() { reset(); }
...
void BackupDevice::reset() {
  ...
  // if the user has requested a manual choice for backup type, and we havent
  // imported a raw save file, then apply it now
  if (state == DETECTING &&
      CommonSettings.manualBackupType != MC_TYPE_AUTODETECT) {
    state = RUNNING;
  ...
  if (state == DETECTING) {
    if (!memcmp(gameInfo.header.gameCode, "ASMK", 4))
      addr_size = 1; // super mario 64 ds (KOR, which is different somehow)
    else if (!memcmp(gameInfo.header.gameCode, "ASM", 3))
      addr_size = 2; // super mario 64 ds
    else if (!memcmp(gameInfo.header.gameCode, "BDE", 3))
      addr_size = 2; // Dementium II
    else if (!memcmp(gameInfo.header.gameCode, "AL3", 3))
      addr_size = 1; // spongebob atlantis squarepantis.
    else if (!memcmp(gameInfo.header.gameCode, "AH5", 3))
      addr_size = 1; // over the hedge
    else if (!memcmp(gameInfo.header.gameCode, "AVH", 3))
      addr_size = 1; // over the hedge - Hammy Goes Nuts!
    else if (!memcmp(gameInfo.header.gameCode, "AQ3", 3))
      addr_size = 1; // spider-man 3
    else if (!memcmp(gameInfo.header.gameCode, "BPV", 3))
      addr_size = 2; // puzzler world (should be eeprom 64KBits)
...
// todo - this function is horrible. it's only needed due to our big
// disorganization between save types and sizes. the concept of a backup memory
// type "enum" (not actually an enum) was added after the original .dsv format
// the original .dsv format contains a 'type' which is kind of useless (flash vs
// eeprom) and then a memory size the memory size also happens to exactly
// identify a memory type. this function performs that mapping. we should fix
// this in a new revision of .dsv
...
void BackupDevice::detect() {
  ...
  if (state == DETECTING && data_autodetect.size() > 0) {
    ...
    case 1:
      addr_size =
          1; // choose 1 just to keep the busted savefile from growing too big
    ...
    case 3:
      ...
      // but unfortunately we select this case on accident sometimes when what
      // it meant to do was present the archaic 1+2 case (the archaic 1+2 case
      //is: specifying one address byte, and then reading the first two bytes,
      //instead of the first one byte, as most other games would do.) so, we're
      // gonna hack in checks for the games that are doing this
      addr_size = 2;
      break;
    ...
    case 4:
      ...
      break;
    default:
      // the archaic case: write the address and then some modulo-4 number of
      // bytes why modulo 4? who knows.
      addr_size = autodetect_size & 3;

      // SM64 (KOR) makes it here with autodetect_size=11 and nothing
      // interesting in the buffer we whitelisted it earlier though
...
u8 BackupDevice::data_command(u8 val, u8 PROCNUM) {
  ...
  if (com == BM_CMD_READLOW &&
      motionInitState == MOTION_INIT_STATE_RECEIVED_4_B && val == 0) {
    motionInitState = MOTION_INIT_STATE_IDLE;
  ...
  // if the game firmly believes we have motion support, then ignore all motion
  // commands, since theyre not emulated.
  ...
  case BM_CMD_READLOW:
    ...
    if (state == DETECTING) {
      if (com == BM_CMD_WRITELOW)
        printf("MC%c: Unexpected backup device initialization sequence using "
               "writes!\n",
               PROCNUM ? '7' : '9');
      ...
      if (addr_counter < addr_size) {
        ...
        // why does tomb raider underworld access 0x180 and go clear through to
        // 0x280? should this wrap around at 0 or at 0x100?
        // TODO - dont other backup memory types have similar wraparound issues?
        if (addr_size == 1) {
          addr &= 0x1FF;
          // since we changed the address unexpectedly (non-sequentially), be
          // sure to seek the FP
          fpMC->fseek(addr, SEEK_SET);
        }

        // if we're writing a byte at address 0, we need to ensure that we have
        // at least 1 byte in the file (IOW, ensure to size addr+1 == 0+1)
        ensure(addr + 1);
  ...
  case BM_CMD_IRDA:
    printf("MC%c: Unverified Backup Memory command: %02X FROM %08X\n",
           PROCNUM ? '7' : '9', com,
           PROCNUM ? NDS_ARM7.instruct_adr : NDS_ARM9.instruct_adr);
    val = 0xAA;
    break;
  ...
  default:
    if (com != 0) {
      printf(
          "MC%c: Unhandled Backup Memory command %02X, value %02X (PC:%08X)\n",
          PROCNUM ? '7' : '9', com, val,
          PROCNUM ? NDS_ARM7.instruct_adr : NDS_ARM9.instruct_adr);
      break;
    }
...
#ifdef _ENABLE_MOTION
    case 0xFE:
      if (motionInitState == MOTION_INIT_STATE_IDLE) {
        motionInitState = MOTION_INIT_STATE_FE;
        return 0;
      }
      break;
    case 0xFD:
      if (motionInitState == MOTION_INIT_STATE_FE) {
        motionInitState = MOTION_INIT_STATE_FD;
        return 0;
      }
      break;
    case 0xFB:
      if (motionInitState == MOTION_INIT_STATE_FD) {
        motionInitState = MOTION_INIT_STATE_FB;
        return 0;
      }
      break;
    case 0xF8:
    ...
    case BM_CMD_WRITEDISABLE:
      ...
      case MOTION_INIT_STATE_IDLE:
        motionInitState = MOTION_INIT_STATE_RECEIVED_4;
        break;
      case MOTION_INIT_STATE_RECEIVED_4:
        motionInitState = MOTION_INIT_STATE_RECEIVED_4_B;
        break;
      }
#endif
      write_enable = FALSE;
    ...
    case BM_CMD_READHIGH:
      // printf("XHI: %08X\n",addr);
      if (com == BM_CMD_WRITEHIGH)
        com = BM_CMD_WRITELOW;
      if (com == BM_CMD_READHIGH)
        com = BM_CMD_READLOW;
      addr_counter = 0;
      addr = 0;
      if (addr_size == 1) {
        //"write command that's only available on ST M95040-W that I know of"
        // this makes sense, since this device would only have a 256 bytes
        // address space with writelow and writehigh would allow access to the
        // upper 256 bytes but it was detected in pokemon diamond also during
        // the main save process
        addr = 0x1;
      }
      break;
...
// guarantees that the data buffer has room enough for the specified number of
// bytes
...
void BackupDevice::ensure(u32 addr, u8 val, EMUFILE *fpOut) {
  ...
  fp->fprintf("%s", kDesmumeSaveCookie); // this is what we'll use to recognize
                                         // the desmume format save

  fp->fflush();

  // this is a HORRIBLE IDEA.
  // leave the FP positioned to write the final byte
  // this is a HACK to make the basic read/write byte operation work when it
  // calls ensure(). IDEALLY, no assumptions about the file pointer can be made.
  // but someone (actually, not really) so very carefully profiled the save IO
  // code and discovered that not fseeking for every byte read/write was a great
  // optimization. so, now all this code is depending/assuming on the FP being
  // kept in a precise position, and I dont think its smart to change the main
  // user of this assumption to paper over this bug by making it fseek before
  // read/write, while leaving other unknown assuming clients intact
...
u32 BackupDevice::get_save_nogba_size(const char *fname) {
  ...
  if (fsrc) {
    ...
    if (fread(src, 1, sizeof(src), fsrc) != sizeof(src)) {
      fclose(fsrc);
      return 0xFFFFFFFF;
    }
...
u32 BackupDevice::get_save_nogba_size(u8 *data) {
  for (u8 i = 0; i < 0x1F; i++)
    if (data[i] != no_GBA_HEADER_ID[i])
      return 0xFFFFFFFF;
  if (data[0x1F] != 0x1A)
    return 0xFFFFFFFF;
  for (int i = 0; i < 0x4; i++)
    if (data[i + 0x40] != no_GBA_HEADER_SRAM_ID[i])
      return 0xFFFFFFFF;
...
u32 BackupDevice::pad_up_size(u32 startSize) {
  ...
  while ((ctr < saveSizes_count) && (size > saveSizes[ctr]))
    ctr++;
...
bool BackupDevice::import_duc(const char *filename, u32 force_size) {
  ...
  if (!memcmp(id16, "ARDS000000000001", 16))
    version = 1;

  // ID version 2
  fseek(file, 0xA1, SEEK_SET);
  fread(id3, 1, 3, file);
  if (!memcmp(id16, "\0\0\0\0", 4) && id3[2] == 0xC0)
    version = 2;
...
bool BackupDevice::import_dsv(const char *filename) {
  ...
  const bool isFileValid =
      BackupDevice::GetDSVFileInfo(theFile, &importFileFooter, &importFileSize);
  if (!isFileValid) {
    return result;
  }

  if ((this->addr_size != 0) && (this->addr_size != 0xFFFFFFFF) &&
      (this->addr_size != importFileFooter.info.addr_size)) {
    printf("BackupDevice: WARNING! Importing an address bus size that differs "
           "from what this game is currently using. (Importing \'%u\'; "
           "Expected \'%u\'.\n",
           (unsigned int)importFileFooter.info.addr_size,
           (unsigned int)addr_size);
  }

  if ((this->_info.padSize > 0) &&
      (this->_info.padSize != importFileFooter.info.padSize)) {
    printf("BackupDevice: NOTE - Importing a backup data size that differs "
           "from what this game is currently using. (Importing \'%u\'; "
           "Expected \'%u\'.\n",
           (unsigned int)importFileFooter.info.padSize,
           (unsigned int)this->_info.padSize);
  }
  ...
  const size_t backupDataReadByteCount =
      fread(backupData, 1, importFileFooter.info.padSize, theFile);
  fclose(theFile); // At this point, both backup data and footer should have
                   // been read, so we can close the import file now.

  if (backupDataReadByteCount != importFileFooter.info.padSize) {
    free(backupData);
    printf(
        "BackupDevice: DSV import failed! Could not read the backup data.\n");
    return result;
  }

  // Write out the entirety of the file data to the EMUFILE.
  // Note: If EMUFILE is an EMUFILE_FILE, know that we are doing a straight
  // overwrite here.
  ...
  const size_t newFileSize =
      this->_info.padSize + BackupDevice::GetDSVFooterSize();
...
bool BackupDevice::load_movie(EMUFILE *is) {
  ...
  int ok = readFooter();
  // TODO - in case we ever change the format again (and we should probably
  // entirely rewrite this if we do) we'd need to detect the old versions (that
  // is, returning -1 or -2 here or some other errors)
  ...
  // seems sloppy, the original intention was for this to basically be a
  // savestate, so that all fields are saved, but someone wrecked that
  state = RUNNING;
  addr_size = _info.addr_size;
...
void BackupDevice::forceManualBackupType() {
  addr_size = addr_size_for_old_save_size(
      save_types[CommonSettings.manualBackupType].size);
  state = RUNNING;
}

BackupDeviceFileInfo BackupDevice::GetFileInfo() { return this->_info; }

size_t BackupDevice::GetDSVFooterSize() {
  return (strlen(DESMUME_BACKUP_FOOTER_TXT) +
          sizeof(BackupDeviceFileSaveFooter));
}

bool BackupDevice::GetDSVFileInfo(FILE *inFileDSV,
  ...
  if (inFileSize < BackupDevice::GetDSVFooterSize()) {
    printf("BackupDevice: File validation failed! The file appears to be "
           "corrupted.\n");
    return result;
  }
  ...
  const size_t footerReadByteCount =
      fread(&inFileFooter, 1, sizeof(inFileFooter), inFileDSV);

  if (footerReadByteCount != sizeof(inFileFooter)) {
    printf("BackupDevice: File validation failed! Could not read the file "
           "footer.\n");
    return result;
  }

  if (strncmp(inFileFooter.cookie, kDesmumeSaveCookie,
              sizeof(inFileFooter.cookie)) != 0) {
    char inCookieTerminatedString[sizeof(inFileFooter.cookie) + 1];
    strncpy(inCookieTerminatedString, inFileFooter.cookie,
            sizeof(inFileFooter.cookie));
    inCookieTerminatedString[sizeof(inFileFooter.cookie)] = '\0';

    printf("BackupDevice: File validation failed! Incorrect cookie found. "
           "(Read \'%s\'; Expected \'%s\'.\n",
  ...
  if (inFileFooter.version != 0) {
    printf("BackupDevice: File validation failed! Incorrect version. (Read "
           "\'%u\'; Expected \'%u\'.\n",
  ...
  if (inFileFooter.info.padSize != backupDataSize) {
    printf("BackupDevice: File validation failed! Incorrect backup data size. "
           "(Read \'%u\'; Expected \'%u\'.\n",
           (unsigned int)inFileFooter.info.padSize,
           (unsigned int)backupDataSize);
    return result;
  }
