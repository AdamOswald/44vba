...
CommandLine::CommandLine()
    : is_cflash_configured(false), _load_to_memory(-1), _play_movie_file(0),
      _record_movie_file(0), _cflash_image(0), _cflash_path(0), _gbaslot_rom(0),
      _bios_arm9(NULL), _bios_arm7(NULL), _bios_swi(0), _fw_path(NULL),
      _fw_boot(0), _spu_sync_mode(-1), _spu_sync_method(-1), _spu_advanced(0),
      _num_cores(-1), _rigorous_timing(0), _advanced_timing(-1), _gamehacks(-1),
      _texture_deposterize(-1), _texture_smooth(-1), _slot1(NULL),
      _slot1_fat_dir(NULL), _slot1_fat_dir_type(false), _slot1_no8000prot(0)
#ifdef HAVE_JIT
      ,
      _cpu_mode(-1), _jit_size(-1)
#endif
      ,
      _console_type(NULL), _advanscene_import(NULL), load_slot(-1),
      arm9_gdb_port(0), arm7_gdb_port(0), start_paused(FALSE),
      autodetect_method(-1), render3d(COMMANDLINE_RENDER3D_DEFAULT),
      texture_upscale(-1), gpu_resolution_multiplier(-1),
      language(1) // english by default
      ,
      disable_sound(0), disable_limiter(0), windowed_fullscreen(0),
      _rtc_day(-1), _rtc_hour(-1) {
}
...
static const char *help_string =
    ...
    " --num-cores N              Override numcores detection and use this "
    "many" ENDL " --spu-synch                Use SPU synch (crackles; helps "
                "streams; default ON)" ENDL
    " --spu-method N             Select SPU synch method: 0:N, 1:Z, 2:P; "
    "default 0" ENDL " --3d-render [SW|AUTOGL|GL|OLDGL]" ENDL
    ...
    "                            Automatically upscales textures by the "
    "specified" ENDL "                            amount; 1:No scaling "
                     "(default), 2:2x upscaling," ENDL
    "                            4:4x upscaling" ENDL
    " --3d-texture-smoothing-enable" ENDL
    "                            Enables smooth texture sampling while "
    "rendering." ENDL
#ifdef HOST_WINDOWS
    " --gpu-resolution-multiplier N" ENDL
    "                            Increases the resolution of GPU rendering by "
    "this" ENDL "                            multipler; 1:256x192 (default), "
                "2:512x384," ENDL
    "                            3:768x576, 4:1024x768, 5:1280x960" ENDL
    " --windowed-fullscreen" ENDL "                            Launches in "
                                  "windowed fullscreen (same as alt+enter)" ENDL
#else
    ...
    " --rtc-hour H               Override RTC hour, 0=midnight, 23=an hour "
    "before" ENDL ENDL
    "Arguments affecting overall emulation parameters (`sync settings`): " ENDL
#ifdef HAVE_JIT
    " --jit-enable               Formerly --cpu-mode; default OFF" ENDL
    " --jit-size N               JIT block size 1-100; 1:accurate 100:fast "
    "(default)" ENDL
#endif
    " --advanced-timing          Use advanced bus-level timing; default ON" ENDL
    " --rigorous-timing          Use more realistic component timings; default "
    "OFF" ENDL
    " --gamehacks                Use game-specific hacks; default ON" ENDL
    " --spu-advanced             Enable advanced SPU capture functions "
    "(reverb)" ENDL " --backupmem-db             Use DB for autodetecting "
                    "backup memory type" ENDL ENDL
    ...
    " --bios-arm9 BIN_FILE       Uses the ARM9 BIOS provided at the specified "
    "path" ENDL " --bios-arm7 BIN_FILE       Uses the ARM7 BIOS provided at "
                "the specified path" ENDL
    " --firmware-path BIN_FILE   Uses the firmware provided at the specified "
    "path" ENDL " --firmware-boot 0|1        Boot from firmware" ENDL
    " --bios-swi                 Uses SWI from the provided bios files (else "
    "HLE)" ENDL " --lang N                   Firmware language (can affect "
                "game translations)" ENDL
    "                            0 = Japanese, 1 = English (default), 2 = "
    "French" ENDL
    "                            3 = German, 4 = Italian, 5 = Spanish" ENDL ENDL
    "Arguments affecting contents of SLOT-1:" ENDL
    " --slot1 [RETAIL|RETAILAUTO|R4|RETAILNAND|RETAILMCDROM|RETAILDEBUG]" ENDL
    "                            Device type to be used SLOT-1; default "
    "RETAILAUTO" ENDL " --preload-rom              precache ROM to RAM instead "
                      "of streaming from disk" ENDL
    " --slot1-fat-dir DIR        Directory to mount for SLOT-1 flash cards" ENDL
    " --slot1_no8000prot         Disables retail card copy protection <8000 "
    "feature" ENDL ENDL "Arguments affecting contents of SLOT-2:" ENDL
    " --cflash-image IMG_FILE    Mounts cflash in SLOT-2 with specified image "
    "file" ENDL " --cflash-path DIR          Mounts cflash in SLOT-2 with FS "
                "rooted at DIR" ENDL
    " --gbaslot-rom GBA_FILE     Mounts GBA specified rom in SLOT-2" ENDL ENDL
    "Commands taking place after ROM is loaded: (be sure to specify a "
    "ROM!)" ENDL
    ...
    " --record-movie DSM_FILE    begin recording a movie" ENDL ENDL
    "Arguments affecting video filters:" ENDL
    " --scanline-filter-a N      Fadeout intensity (N/16) (topleft) (default "
    "0)" ENDL " --scanline-filter-b N      Fadeout intensity (N/16) (topright) "
              "(default 2)" ENDL
    " --scanline-filter-c N      Fadeout intensity (N/16) (bottomleft) "
    "(default 2)" ENDL " --scanline-filter-d N      Fadeout intensity (N/16) "
                       "(bottomright) (default 4)" ENDL ENDL
#ifdef GDB_STUB
    "Arguments affecting debugging features:" ENDL
    " --arm9gdb PORTNUM          Enable the ARM9 GDB stub on the given "
    "port" ENDL " --arm7gdb PORTNUM          Enable the ARM7 GDB stub on the "
                "given port" ENDL ENDL
#endif
...
bool CommandLine::parse(int argc, char **argv) {
  ...
  for (;;) {
    ...
    static struct option long_options[] = {
        ...
        {"3d-texture-deposterize-enable", no_argument, &_texture_deposterize,
         1},
...
#ifdef HOST_WINDOWS
        {"gpu-resolution-multiplier", required_argument, NULL,
         OPT_GPU_RESOLUTION_MULTIPLIER},
    ...
    // user settings
    case OPT_NUMCORES:
      _num_cores = atoi(optarg);
      break;
    case OPT_SPU_METHOD:
      _spu_sync_method = atoi(optarg);
      break;
    case OPT_3D_RENDER:
      _render3d = optarg;
      break;
    case OPT_3D_TEXTURE_UPSCALE:
      texture_upscale = atoi(optarg);
      break;
    case OPT_GPU_RESOLUTION_MULTIPLIER:
      gpu_resolution_multiplier = atoi(optarg);
      break;

    // RTC settings
    case OPT_RTC_DAY:
      _rtc_day = atoi(optarg);
      break;
    case OPT_RTC_HOUR:
      _rtc_hour = atoi(optarg);
      break;

      // sync settings
#ifdef HAVE_JIT
    case OPT_JIT_SIZE:
      _jit_size = atoi(optarg);
      break;
#endif

    // system equipment
    case OPT_CONSOLE_TYPE:
      console_type = optarg;
      break;
    case OPT_ARM9:
      _bios_arm9 = strdup(optarg);
      break;
    case OPT_ARM7:
      _bios_arm7 = strdup(optarg);
      break;
    case OPT_FIRMPATH:
      _fw_path = strdup(optarg);
      break;
    case OPT_FIRMBOOT:
      _fw_boot = atoi(optarg);
      break;

    // slot-1 contents
    case OPT_SLOT1:
      slot1 = strtoupper(optarg);
      break;
    case OPT_SLOT1_FAT_DIR:
      slot1_fat_dir = optarg;
      break;

    // slot-2 contents
    case OPT_SLOT2_CFLASH_IMAGE:
      cflash_image = optarg;
      break;
    case OPT_SLOT2_CFLASH_DIR:
      _cflash_path = optarg;
      break;
    case OPT_SLOT2_GBAGAME:
      _gbaslot_rom = optarg;
      break;

    // commands
    case OPT_LOAD_SLOT:
      load_slot = atoi(optarg);
      break;
    case OPT_PLAY_MOVIE:
      play_movie_file = optarg;
      break;
    case OPT_RECORD_MOVIE:
      record_movie_file = optarg;
      break;

    // video filters
    case OPT_SCANLINES_A:
      _scanline_filter_a = atoi(optarg);
      break;
    case OPT_SCANLINES_B:
      _scanline_filter_b = atoi(optarg);
      break;
    case OPT_SCANLINES_C:
      _scanline_filter_c = atoi(optarg);
      break;
    case OPT_SCANLINES_D:
      _scanline_filter_d = atoi(optarg);
      break;

    // debugging
    case OPT_ARM9GDB:
      arm9_gdb_port = atoi(optarg);
      break;
    case OPT_ARM7GDB:
      arm7_gdb_port = atoi(optarg);
      break;

    // utilities
    case OPT_ADVANSCENE:
      CommonSettings.run_advanscene_import = optarg;
      break;
    case OPT_LANGUAGE:
  ...
  if (_load_to_memory != -1)
    CommonSettings.loadToMemory = (_load_to_memory == 1) ? true : false;
  // if(_num_cores != -1) CommonSettings.num_cores = _num_cores;
  if (_rigorous_timing)
    CommonSettings.rigorous_timing = true;
  if (_advanced_timing != -1)
    CommonSettings.advanced_timing = _advanced_timing == 1;
  if (_gamehacks != -1)
    CommonSettings.gamehacks.en = _gamehacks == 1;

#ifdef HAVE_JIT
  if (_cpu_mode != -1)
    CommonSettings.use_jit = (_cpu_mode == 1);
  if (_jit_size != -1) {
  ...
  if (console_type == "") {
  } else if (console_type == "FAT")
    CommonSettings.ConsoleType = NDS_CONSOLE_TYPE_FAT;
  else if (console_type == "LITE")
    CommonSettings.ConsoleType = NDS_CONSOLE_TYPE_LITE;
  else if (console_type == "IQUE")
    CommonSettings.ConsoleType = NDS_CONSOLE_TYPE_IQUE;
  else if (console_type == "DSI")
    CommonSettings.ConsoleType = NDS_CONSOLE_TYPE_DSI;
  ...
  if (_render3d == "NONE")
    render3d = COMMANDLINE_RENDER3D_NONE;
  else if (_render3d == "SW")
    render3d = COMMANDLINE_RENDER3D_SW;
  else if (_render3d == "OLDGL")
    render3d = COMMANDLINE_RENDER3D_OLDGL;
  else if (_render3d == "AUTOGL")
    render3d = COMMANDLINE_RENDER3D_AUTOGL;
  else if (_render3d == "GL")
    render3d = COMMANDLINE_RENDER3D_GL;

  if (_texture_deposterize != -1)
    CommonSettings.GFX3D_Renderer_TextureDeposterize =
        (_texture_deposterize == 1);
  if (_texture_smooth != -1)
    CommonSettings.GFX3D_Renderer_TextureSmoothing = (_texture_smooth == 1);
  ...
  if (_bios_arm9) {
    ...
    strcpy(CommonSettings.ARM9BIOS, _bios_arm9);
  }
  if (_bios_arm7) {
    CommonSettings.UseExtBIOS = true;
    strcpy(CommonSettings.ARM7BIOS, _bios_arm7);
  }
#ifndef HOST_WINDOWS
  if (_fw_path) {
    CommonSettings.UseExtFirmware = true;
    CommonSettings.UseExtFirmwareSettings = true;
    strcpy(CommonSettings.ExtFirmwarePath, _fw_path);
  }
#endif
  if (_fw_boot)
  ...
  if (_slot1_no8000prot)
    CommonSettings.RetailCardProtection8000 = false;
  if (_spu_sync_mode != -1)
    CommonSettings.SPU_sync_mode = _spu_sync_mode;
  if (_spu_sync_method != -1)
    CommonSettings.SPU_sync_method = _spu_sync_method;
  if (_spu_advanced)
...
bool CommandLine::validate() {
  ...
  if (load_slot < -1 || load_slot > 10) {
    printerror("I only know how to load from slots 0-10; -1 means 'do not load "
               "savegame' and is default\n");
    return false;
  }
  ...
  if ((_bios_arm9 && !_bios_arm7) || (_bios_arm7 && !_bios_arm9)) {
    printerror(
        "If either bios-arm7 or bios-arm9 are specified, both must be.\n");
    return false;
  }

  if (_bios_swi && (!_bios_arm7 || !_bios_arm9)) {
    printerror("If either bios-swi is used, bios-arm9 and bios-arm7 must be "
               "specified.\n");
  }

  if (_fw_boot && (!_fw_path)) {
    printerror(
        "If either firmware boot is used, firmware path must be specified.\n");
  }

  if ((_cflash_image && _gbaslot_rom) || (_cflash_path && _gbaslot_rom)) {
    printerror(
        "Cannot specify both cflash and gbaslot rom (both occupy SLOT-2)\n");
  }

  if (autodetect_method < -1 || autodetect_method > 1) {
    printerror(
        "Invalid autodetect save method (0 - internal, 1 - from database)\n");
  }

  if ((texture_upscale != -1) && (texture_upscale != 1) &&
      (texture_upscale != 2) && (texture_upscale != 4)) {
    printerror("Invalid texture upscaling value [1|2|4]. Ignoring command line "
               "setting.\n");
    texture_upscale = -1;
  }

  if ((gpu_resolution_multiplier != -1) &&
      ((gpu_resolution_multiplier < 1) || (gpu_resolution_multiplier > 5))) {
    printerror("Invalid GPU resolution multiplier [1..5]. Ignoring command "
               "line setting.\n");
...
void CommandLine::errorHelp(const char *binName) { printerror(help_string); }

void CommandLine::process_movieCommands() {
  if (play_movie_file != "") {
    FCEUI_LoadMovie(play_movie_file.c_str(), true, false, -1);
  } else if (record_movie_file != "") {
    FCEUI_SaveMovie(record_movie_file.c_str(), L"", START_BLANK, NULL,
                    FCEUI_MovieGetRTCDefault());
  }
}
