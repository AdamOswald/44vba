...
enum EDMABitWidth { EDMABitWidth_16 = 0, EDMABitWidth_32 = 1 };
...
// TODO
// n.b. this may be a bad idea, for complex registers like the dma control
// register. we need to know exactly what part was written to, instead of
// assuming all 32bits were written.
class TRegister_32 {
public:
...
public:
  ...
  void write16(u16 val) {
    mode = val & 3;
    // todo - do we clear the div0 flag here or is that strictly done by the
    // divider unit?
  }
...
public:
  ...
  // other conditions may be automatically triggered based on scanning
  // conditions
  BOOL triggered;
  ...
  DmaController()
      : enable(0), irq(0), repeatMode(0), _startmode(0), wordcount(0),
        startmode(EDMAMode_Immediate), bitWidth(EDMABitWidth_16),
        ...
        saddr(0), daddr(0), saddr_user(0), daddr_user(0), dmaCheck(FALSE),
        running(FALSE), paused(FALSE), triggered(FALSE), nextEvent(0),
        sad(&saddr_user), dad(&daddr_user) {
    sad.controller = this;
    dad.controller = this;
  ...
  public:
    // we pass in a pointer to the controller here so we can alert it if
    // anything changes
    DmaController *controller;
    u32 *const ptr;
    AddressRegister(u32 *_ptr) : ptr(_ptr) {}
    virtual u32 read32() { return *ptr; }
    virtual void write32(const u32 val) { *ptr = val; }
  };
  ...
  public:
    ...
    DmaController *controller;
    ControlRegister() {}
    virtual u32 read32() { return controller->read32(); }
    virtual void write32(const u32 val) { return controller->write32(val); }
  };
...
enum eCardMode {
  ...
  // an intermediate stage during the protocol bootup process. commands are KEY1
  // encrypted.
  eCardMode_KEY1,

  // an intermediate stage during the protocol bootup process. commands are KEY1
  // encrypted, while replies are KEY2 encrypted
  eCardMode_KEY2,

  // the final stage of the protocol bootup process. "main data load" mode.
  // commands are KEY2 encrypted, and replies are KEY2 encrypted. optionally,
  // according to some flag we havent designed yet but should probably go in
  // GCBUS_Controller, the whole KEY2 part can be bypassed (this is typical when
  //skipping the firmware boot process)
  eCardMode_NORMAL,
...
struct MMU_struct {
  ...
  // u8 MAIN_MEM[4*1024*1024]; //expanded from 4MB to 8MB to support debug
  // consoles u8 MAIN_MEM[8*1024*1024]; //expanded from 8MB to 16MB to support
  // dsi
  u8 MAIN_MEM[16 * 1024 * 1024]; // expanded from 8MB to 16MB to support dsi
  u8 ARM9_REG[0x1000000]; // this variable is evil and should be removed by
                          // correctly emulating all registers.
  u8 ARM9_BIOS[0x8000];
  ...
  // these are the user-controlled IF bits. some IF bits are generated as
  // necessary from hardware conditions
  u32 reg_IF_bits[2];
  // these flags are set occasionally to indicate that an irq should have
  // entered the pipeline, and processing will be deferred a tiny bit to help
  // emulate things
...
private:
  ...
  // we are going to go ahead and save these out in case we want to change the
  // way this is emulated in the future.. we may want to poke registers in here
  // at more convenient times and have the TSC dumbly pluck them out, rather
  // than generate the values on the fly
  u8 registers[0x80];
};

// this contains things which can't be memzeroed because they are smarter
// classes
...
FORCEINLINE void *MMU_gpu_map(const u32 vram_addr) {
  ...
  // but it returns a pointer to some zero memory in case of accesses to
  // unmapped memory. this correctly handles the case with tile accesses to
  // unmapped memory. it could also potentially go through a different LUT than
  // vram_arm9_map in case we discover that it needs to be set up with different
  // or no mirroring (I think it is a reasonable possibility that only the cpu
  //has the nutty mirroring rules)
  //
  // if this system isn't used, Fantasy Aquarium displays garbage in the first
  // ingame screen due to it storing 0x0F0F or somesuch in screen memory which
  // points to a ridiculously big tile which should contain all 0 pixels

  const u32 vram_page =
      vram_arm9_map[(vram_addr >> 14) & (VRAM_ARM9_PAGES - 1)];
  const u32 ofs = vram_addr & 0x3FFF;
  // blank pages are handled by the extra 16KB of blank memory at the end of
  // ARM9_LCD and the fact that blank pages are mapped to appear at that
  // location
...
template <int PROCNUM> FORCEINLINE u8 _MMU_read08(u32 addr) {
  return _MMU_read08<PROCNUM, MMU_AT_DATA>(addr);
}
template <int PROCNUM> FORCEINLINE u16 _MMU_read16(u32 addr) {
  return _MMU_read16<PROCNUM, MMU_AT_DATA>(addr);
}
template <int PROCNUM> FORCEINLINE u32 _MMU_read32(u32 addr) {
  return _MMU_read32<PROCNUM, MMU_AT_DATA>(addr);
}
template <int PROCNUM> FORCEINLINE void _MMU_write08(u32 addr, u8 val) {
  _MMU_write08<PROCNUM, MMU_AT_DATA>(addr, val);
}
template <int PROCNUM> FORCEINLINE void _MMU_write16(u32 addr, u16 val) {
  _MMU_write16<PROCNUM, MMU_AT_DATA>(addr, val);
}
template <int PROCNUM> FORCEINLINE void _MMU_write32(u32 addr, u32 val) {
  _MMU_write32<PROCNUM, MMU_AT_DATA>(addr, val);
}
...
FORCEINLINE void CheckMemoryDebugEvent(EDEBUG_EVENT event,
                                       const MMU_ACCESS_TYPE type,
                                       const u32 procnum, const u32 addr,
                                       const u32 size, const u32 val) {
...
FORCEINLINE u8 _MMU_read08(const int PROCNUM, const MMU_ACCESS_TYPE AT,
                           const u32 addr) {
...
FORCEINLINE u16 _MMU_read16(const int PROCNUM, const MMU_ACCESS_TYPE AT,
                            const u32 addr) {
  ...
  // special handling for execution from arm9, since we spend so much time in
  // there
  if (PROCNUM == ARMCPU_ARM9 && AT == MMU_AT_CODE) {
    if ((addr & 0x0F000000) == 0x02000000)
      return T1ReadWord_guaranteedAligned(MMU.MAIN_MEM,
                                          addr & _MMU_MAIN_MEM_MASK16);
...
FORCEINLINE u32 _MMU_read32(const int PROCNUM, const MMU_ACCESS_TYPE AT,
                            const u32 addr) {
  ...
  if (PROCNUM == ARMCPU_ARM9 && AT == MMU_AT_CODE) {
    if ((addr & 0x0F000000) == 0x02000000)
      return T1ReadLong_guaranteedAligned(MMU.MAIN_MEM,
                                          addr & _MMU_MAIN_MEM_MASK32);

    if (addr < 0x02000000)
      return T1ReadLong_guaranteedAligned(MMU.ARM9_ITCM, addr & 0x7FFC);

    // what happens when we execute from DTCM? nocash makes it look like we get
    // 0xFFFFFFFF but i can't seem to verify it historically, desmume would fall
    // through to its old memory map struct which would return unused memory (0)
    // it seems the hardware returns 0 or something benign because in actuality
    // 0xFFFFFFFF is an undefined opcode and we know our handling for that is
    // solid

    goto dunno;
  }

  // special handling for execution from arm7. try reading from main memory
  // first
  ...
  // for other arm9 cases, we have to check from dtcm first because it is
  // patched on top of the main memory range
  if (PROCNUM == ARMCPU_ARM9) {
    if ((addr & (~0x3FFF)) == MMU.DTCMRegion) {
      // Returns data from DTCM (ARM9 only)
...
FORCEINLINE void _MMU_write08(const int PROCNUM, const MMU_ACCESS_TYPE AT,
                              const u32 addr, u8 val) {
...
FORCEINLINE void _MMU_write16(const int PROCNUM, const MMU_ACCESS_TYPE AT,
                              const u32 addr, u16 val) {
...
FORCEINLINE void _MMU_write32(const int PROCNUM, const MMU_ACCESS_TYPE AT,
...
//	#define MMU_read16_acl(proc,adr,access)
//((access==CP15_ACCESS_EXECUTE)?_MMU_read16<proc,MMU_AT_CODE>(adr):_MMU_read16<proc,MMU_AT_DATA>(adr))
//	#define MMU_read32_acl(proc,adr,access)
//((access==CP15_ACCESS_EXECUTE)?_MMU_read32<proc,MMU_AT_CODE>(adr):_MMU_read32<proc,MMU_AT_DATA>(adr))
// #endif
...
FORCEINLINE u8 _MMU_read08(u32 addr) {
  return _MMU_read08(PROCNUM, AT, addr);
}

template <int PROCNUM, MMU_ACCESS_TYPE AT>
FORCEINLINE u16 _MMU_read16(u32 addr) {
  return _MMU_read16(PROCNUM, AT, addr);
}

template <int PROCNUM, MMU_ACCESS_TYPE AT>
FORCEINLINE u32 _MMU_read32(u32 addr) {
  return _MMU_read32(PROCNUM, AT, addr);
}
