...
#define CPU_FREEZE_IE_IF                                                       \
  0x02 // waiting for IE&IF to signal something (probably edge triggered on IRQ
       // too)
...
template <typename T> inline T UNSIGNED_OVERFLOW(T a, T b, T c) {
  return BIT31(((a) & (b)) | (((a) | (b)) & (~c)));
}

template <typename T> inline T UNSIGNED_UNDERFLOW(T a, T b, T c) {
  return BIT31(((~a) & (b)) | (((~a) | (b)) & (c)));
}

template <typename T> inline T SIGNED_OVERFLOW(T a, T b, T c) {
  return BIT31(((a) & (b) & (~c)) | ((~a) & (~(b)) & (c)));
}

template <typename T> inline T SIGNED_UNDERFLOW(T a, T b, T c) {
  return BIT31(((a) & (~(b)) & (~c)) | ((~a) & (b) & (c)));
}
...
inline bool BorrowFrom(s32 left, s32 right) { return ((u32)right > (u32)left); }
...
#define TEST_COND(cond, inst, CPSR)                                            \
  ((arm_cond_table[((CPSR.val >> 24) & 0xf0) | (cond)]) & (1 << (inst)))
...
typedef union {
  struct {
#ifdef MSB_FIRST
    u32 N : 1, Z : 1, C : 1, V : 1, Q : 1, RAZ : 19, I : 1, F : 1, T : 1,
        mode : 5;
#else
    u32 mode : 5, T : 1, F : 1, I : 1, RAZ : 19, Q : 1, V : 1, C : 1, Z : 1,
        N : 1;
#endif
  } bits;
  u32 val;
