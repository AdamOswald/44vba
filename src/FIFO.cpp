...
void IPC_FIFOsend(u8 proc, u32 val) {
  ...
  // LOG("IPC%s send FIFO 0x%08X size %03i (l 0x%X, tail %02i) (r 0x%X, tail
  // %02i)\n", 	proc?"7":"9", val, ipc_fifo[proc].size, cnt_l,
  //ipc_fifo[proc].tail, cnt_r, ipc_fifo[proc^1].tail);
...
u32 IPC_FIFOrecv(u8 proc) {
  ...
  // LOG("IPC%s recv FIFO 0x%08X size %03i (l 0x%X, tail %02i) (r 0x%X, tail
  // %02i)\n", 	proc?"7":"9", val, ipc_fifo[proc].size, cnt_l,
  //ipc_fifo[proc].tail, cnt_r, ipc_fifo[proc^1].tail);
...
void IPC_FIFOcnt(u8 proc, u16 val) {
  ...
  // IPCFIFOCNT_SENDIRQEN may have been set (and/or the fifo may have been
  // cleared) so we may need to trigger this irq (this approach is used by
  //libnds fifo system on occasion in fifoInternalSend, and began happening
  //frequently for value32 with r4326)
...
static void GXF_FIFO_handleEvents() {
  ...
  if (emptychange || lowchange)
    NDS_Reschedule();
}

static bool IsMatrixStackCommand(u8 cmd) { return cmd == 0x11 || cmd == 0x12; }

void GFX_FIFOsend(u8 cmd, u32 param) {
  // INFO("gxFIFO: send 0x%02X = 0x%08X (size %03i/0x%02X) gxstat 0x%08X\n",
  // cmd, param, gxFIFO.size, gxFIFO.size, gxstat); printf("fifo recv: %02X:
  ...
  //	//INFO("ERROR: gxFIFO is full (cmd 0x%02X = 0x%08X) (prev cmd 0x%02X =
  //0x%08X)\n", cmd, param, gxFIFO.cmd[255], gxFIFO.param[255]); 	return;
  //}
  ...
  gxFIFO.tail++;
  gxFIFO.size++;
  if (gxFIFO.tail > HACK_GXIFO_SIZE - 1)
    gxFIFO.tail = 0;

  // if a matrix op is entering the pipeline, do accounting for it
  //(this is tested by wild west, which will jam a few ops in the fifo and then
  //wait for the matrix stack to be un-busy so it can read back the current
  // matrix stack position). it is definitely only pushes and pops which set
  // this flag. seems like it would be less work in the HW to make a counter
  // than do cmps on all the command bytes, so maybe we're even doing it right.
  if (IsMatrixStackCommand(cmd))
    gxFIFO.matrix_stack_op_size++;

  // along the same lines:
  // american girls julie finds a way will put a bunch of stuff and then a box
  // test into the fifo and then immediately test the busy flag so we need to
  // set the busy flag here. does it expect the fifo to be running then? well,
  // it's definitely jammed -- making it unjammed at one point did fix this bug.
  // it's still not clear whether we're handling the immediate vs fifo commands
  // properly at all :( anyway, here we go, similar treatment. consider this a
  // hack.
  if (cmd == 0x70)
    MMU_new.gxstat.tb = 1; // just set the flag--youre insane if you queue more
                           // than one of these anyway
  if (cmd == 0x71)
    MMU_new.gxstat.tb = 1;
...
BOOL GFX_PIPErecv(u8 *cmd, u32 *param) {
  ...
  *param = gxFIFO.param[gxFIFO.head];
  ...
  gxFIFO.head++;
  gxFIFO.size--;
  if (gxFIFO.head > HACK_GXIFO_SIZE - 1)
    gxFIFO.head = 0;
...
void GFX_FIFOcnt(u32 val) {
  ////INFO("gxFIFO: write cnt 0x%08X (prev 0x%08X) FIFO size %03i PIPE size
  ...
  //{
  //	gfx3d_ClearStack();
  //	val &= 0xFFFF5FFF;		// clear reset (bit15) & stack level
  //(bit13)
  // }
...
void DISP_FIFOinit() { memset(&disp_fifo, 0, sizeof(DISP_FIFO)); }

void DISP_FIFOsend(u32 val) {
  // INFO("DISP_FIFO send value 0x%08X (head 0x%06X, tail 0x%06X)\n", val,
  // disp_fifo.head, disp_fifo.tail);
