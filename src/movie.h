...
enum EMOVIECMD { MOVIECMD_MIC = 1, MOVIECMD_RESET = 2, MOVIECMD_LID = 4 };
...
public:
  ...
  void toggleBit(int bit) { pad ^= mask(bit); }

  void setBit(int bit) { pad |= mask(bit); }

  void clearBit(int bit) { pad &= ~mask(bit); }
  ...
  bool checkBit(int bit) { return (pad & mask(bit)) != 0; }
...
public:
  ...
  int version;
  int emuVersion;
  // todo - somehow force mutual exclusion for poweron and reset (with an error
  // in the parser)
  ...
  public:
    bool containsKey(std::string key) { return find(key) != end(); }
...
extern EMOVIEMODE
    movieMode; // adelikat: main needs this for frame counter display
extern MovieData
    currMovieData; // adelikat: main needs this for frame counter display
...
FCEUI_LoadMovie(const char *fname, bool _read_only, bool tasedit,
                int _pauseframe); // returns NULL on success, errmsg on failure
