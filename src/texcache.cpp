...
// only dump this from ogl renderer. for now, softrasterizer creates things in
// an incompatible pixel format #define DEBUG_DUMP_TEXTURE
...
#define CONVERT(color)                                                         \
  ((TEXCACHEFORMAT == TexFormat_32bpp) ? (COLOR555TO8888_OPAQUE(color))        \
                                       : COLOR555TO6665_OPAQUE(color))

// This class represents a number of regions of memory which should be viewed as
// contiguous
class MemSpan {
public:
  static const int MAXSIZE = 17; // max size for textures: 1024*1024*2 bytes /
                                 // 128*1024 banks + 1 for wraparound

  MemSpan() : numItems(0), size(0) {}
  ...
  // the length shall be as specified in this MemSpan, unless you specify
  // otherwise
...
static MemSpan MemSpan_TexMem(u32 ofs, u32 len) {
  ...
  while (len) {
    ...
    if (ptr == MMU.blank_memory &&
        !GPU->GetEngineMain()->IsMasterBrightMaxOrMin()) {
...
static MemSpan MemSpan_TexPalette(u32 ofs, u32 len, bool silent) {
  ...
  while (len) {
    ...
    u32 slot =
        (ofs >> 14) & 7; // this masks to 8 slots, but there are really only 6
    if (slot > 5 && !silent) {
      PROGINFO("Texture palette overruns texture memory. Wrapping at palette "
               "slot 0.\n");
      slot -= 5;
    }
    ...
    if (ptr == MMU.blank_memory &&
        !GPU->GetEngineMain()->IsMasterBrightMaxOrMin() && !silent) {
      PROGINFO(
          "Tried to reference unmapped texture palette memory: 16k slot #%d\n",
          slot);
    }
    curr.ptr = ptr + curr.start;
  }
...
void TextureCache::Invalidate() {
  ...
  // TODO - we should handle this instead by setting dirty flags in the vram
  // memory mapping and noting whether palette memory was dirty. but this will
  // work for now
  ...
  for (TextureCacheMap::iterator it(this->_texCacheMap.begin());
       it != this->_texCacheMap.end(); ++it) {
    it->second->SetSuspectedInvalid();

    // when the palette changes, we assume all 4x4 textures are dirty.
    // this is because each 4x4 item doesnt carry along with it a copy of the
    // entire palette, for verification instead, we just use the one paletteDump
    // for verifying of all 4x4 textures; and if paletteDirty is set,
...
TextureStore *TextureCache::GetTexture(TEXIMAGE_PARAM texAttributes,
                                       u32 palAttributes) {
  TextureStore *theTexture = NULL;
  const TextureCacheKey key =
      TextureCache::GenerateKey(texAttributes, palAttributes);
...
TextureCacheKey TextureCache::GenerateKey(const TEXIMAGE_PARAM texAttributes,
  ...
  // Since the repeat, flip, and coordinate transformation modes are render
  // settings and not data settings, we can mask out those bits to help reduce
  // duplicate entries.
...
TextureStore::TextureStore(const TEXIMAGE_PARAM texAttributes,
                           const u32 palAttributes) {
  // for each texformat, multiplier from numtexels to numbytes (fixed
  // point 30.2)
  ...
  _packSize = (_sizeS * _sizeT * texSizes[_packFormat]) >>
              2; // shifted because the texSizes multiplier is fixed point

  if ((_packFormat == TEXMODE_I2) || (_packFormat == TEXMODE_I4) ||
      (_packFormat == TEXMODE_I8)) {
  ...
  _paletteAddress =
      (_packFormat == TEXMODE_I2) ? palAttributes << 3 : palAttributes << 4;
  _paletteSize = paletteSizeList[_packFormat] * sizeof(u16);

  if (_packFormat == TEXMODE_4X4) {
    const u32 indexBase =
        ((texAttributes.VRAMOffset & 0xC000) == 0x8000) ? 0x30000 : 0x20000;
    ...
    MemSpan currentPackedTexIndexMS =
        MemSpan_TexMem(_packIndexAddress, _packIndexSize);
  ...
  if (_paletteSize > 0) {
    MemSpan currentPaletteMS =
        MemSpan_TexPalette(_paletteAddress, _paletteSize, false);
...
u32 TextureStore::GetWidth() const { return this->_sizeS; }

u32 TextureStore::GetHeight() const { return this->_sizeT; }
...
u32 TextureStore::GetPackAddress() const { return this->_packAddress; }

u32 TextureStore::GetPackSize() const { return this->_packSize; }

u8 *TextureStore::GetPackData() { return this->_packData; }

u32 TextureStore::GetPaletteAddress() const { return this->_paletteAddress; }

u32 TextureStore::GetPaletteSize() const { return this->_paletteSize; }
...
u32 TextureStore::GetPackIndexSize() const { return this->_packIndexSize; }

u8 *TextureStore::GetPackIndexData() { return this->_packIndexData; }

void TextureStore::SetTextureData(const MemSpan &packedData,
                                  const MemSpan &packedIndexData) {
...
size_t TextureStore::GetUnpackSizeUsingFormat(
...
void TextureStore::Unpack(u32 *unpackBuffer) {
  // Whenever a 1-bit alpha or no-alpha texture is unpacked (this means any
  // texture format that is not A3I5 or A5I3), set all transparent pixels to 0
  // so that 3D renderers can assume that the transparent color is 0 during
  // texture sampling.

  switch (this->_packFormat) {
  case TEXMODE_A3I5:
    NDSTextureUnpackA3I5<TEXCACHEFORMAT>(this->_packSize, this->_packData,
                                         this->_paletteColorTable,
                                         unpackBuffer);
    break;

  case TEXMODE_I2:
    NDSTextureUnpackI2<TEXCACHEFORMAT>(
        this->_packSize, this->_packData, this->_paletteColorTable,
        this->_isPalZeroTransparent, unpackBuffer);
    break;
  ...
  case TEXMODE_4X4: {
    ...
    NDSTextureUnpack4x4<TEXCACHEFORMAT>(
        this->_packSizeFirstSlot, (u32 *)this->_packData,
        (u16 *)this->_packIndexData, this->_paletteAddress, this->_sizeS,
        this->_sizeT, unpackBuffer);
    break;
  }

  case TEXMODE_A5I3:
    NDSTextureUnpackA5I3<TEXCACHEFORMAT>(this->_packSize, this->_packData,
                                         this->_paletteColorTable,
                                         unpackBuffer);
    break;

  case TEXMODE_16BPP:
    NDSTextureUnpackDirect16Bit<TEXCACHEFORMAT>(
        this->_packSize, (u16 *)this->_packData, unpackBuffer);
    break;
...
void TextureStore::SetSuspectedInvalid() { this->_suspectedInvalid = true; }

bool TextureStore::IsAssumedInvalid() const { return this->_assumedInvalid; }

void TextureStore::SetAssumedInvalid() { this->_assumedInvalid = true; }

void TextureStore::SetLoadNeeded() { this->_isLoadNeeded = true; }

bool TextureStore::IsLoadNeeded() const { return this->_isLoadNeeded; }

TextureCacheKey TextureStore::GetCacheKey() const { return this->_cacheKey; }

size_t TextureStore::GetCacheSize() const { return this->_cacheSize; }
...
void TextureStore::Update() {
  MemSpan currentPaletteMS =
      MemSpan_TexPalette(this->_paletteAddress, this->_paletteSize, false);
  MemSpan currentPackedTexDataMS =
      MemSpan_TexMem(this->_packAddress, this->_packSize);
  MemSpan currentPackedTexIndexMS;

  if (this->_packFormat == TEXMODE_4X4) {
    currentPackedTexIndexMS =
        MemSpan_TexMem(this->_packIndexAddress, this->_packIndexSize);
  }
...
void TextureStore::VRAMCompareAndUpdate() {
...
#ifdef MSB_FIRST
  currentPaletteMS.dump16(this->_workingData + this->_packSize +
                          this->_packIndexSize);
#else
  currentPaletteMS.dump(this->_workingData + this->_packSize +
                        this->_packIndexSize);
#endif
  ...
  if (memcmp(this->_packData, this->_workingData, this->_packTotalSize)) {
    ...
    if (this->_packIndexSize == 0) {
      ...
      this->_paletteColorTable =
          (u16 *)(this->_packData + this->_packSize + this->_packIndexSize);
    }
...
template <TextureStoreUnpackFormat TEXCACHEFORMAT>
void NDSTextureUnpackI2(const size_t srcSize, const u8 *__restrict srcData,
                        const u16 *__restrict srcPal,
                        const bool isPalZeroTransparent,
  ...
  if (isPalZeroTransparent) {
    ...
    for (size_t i = 0; i < srcSize; i += 4, srcData += 4, dstBuffer += 16) {
      ...
      idx = _mm_or_si128(
          _mm_or_si128(
              _mm_or_si128(_mm_and_si128(idx, _mm_set1_epi32(0x00000003)),
                           _mm_and_si128(_mm_srli_epi32(idx, 2),
                                         _mm_set1_epi32(0x00000300))),
              _mm_and_si128(_mm_srli_epi32(idx, 4),
                            _mm_set1_epi32(0x00030000))),
          _mm_and_si128(_mm_srli_epi32(idx, 6), _mm_set1_epi32(0x03000000)));
      idx = _mm_slli_epi16(idx, 1);

      __m128i idx0 =
          _mm_add_epi8(_mm_unpacklo_epi8(idx, idx), _mm_set1_epi16(0x0100));
      __m128i idx1 =
      ...
      if (TEXCACHEFORMAT == TexFormat_15bpp) {
        ColorspaceConvert555To6665Opaque_SSE2<false>(
            palColor0, convertedColor[0], convertedColor[1]);
        ColorspaceConvert555To6665Opaque_SSE2<false>(
            palColor1, convertedColor[2], convertedColor[3]);
      } else {
        ColorspaceConvert555To8888Opaque_SSE2<false>(
            palColor0, convertedColor[0], convertedColor[1]);
        ColorspaceConvert555To8888Opaque_SSE2<false>(
            palColor1, convertedColor[2], convertedColor[3]);
      }
      ...
      convertedColor[0] =
          _mm_andnot_si128(_mm_unpacklo_epi16(idx0, idx0), convertedColor[0]);
      convertedColor[1] =
          _mm_andnot_si128(_mm_unpackhi_epi16(idx0, idx0), convertedColor[1]);
      convertedColor[2] =
          _mm_andnot_si128(_mm_unpacklo_epi16(idx1, idx1), convertedColor[2]);
      convertedColor[3] =
          _mm_andnot_si128(_mm_unpackhi_epi16(idx1, idx1), convertedColor[3]);
...
template <TextureStoreUnpackFormat TEXCACHEFORMAT>
void NDSTextureUnpackI4(const size_t srcSize, const u8 *__restrict srcData,
                        const u16 *__restrict srcPal,
                        const bool isPalZeroTransparent,
                        u32 *__restrict dstBuffer) {
#ifdef ENABLE_SSSE3
  ...
  if (isPalZeroTransparent) {
    ...
    for (size_t i = 0; i < srcSize; i += 8, srcData += 8, dstBuffer += 16) {
      ...
      idx = _mm_unpacklo_epi8(idx, idx);
      idx = _mm_or_si128(
          _mm_and_si128(idx, _mm_set1_epi16(0x000F)),
          _mm_and_si128(_mm_srli_epi16(idx, 4), _mm_set1_epi16(0x0F00)));
      idx = _mm_slli_epi16(idx, 1);
      ...
      const __m128i palMask = _mm_cmpeq_epi8(
          _mm_and_si128(idx, _mm_set1_epi8(0x10)), _mm_setzero_si128());
      ...
      const __m128i palColor0 = _mm_blendv_epi8(
          palColor0B, palColor0A, _mm_unpacklo_epi8(palMask, palMask));
      const __m128i palColor1 = _mm_blendv_epi8(
          palColor1B, palColor1A, _mm_unpackhi_epi8(palMask, palMask));
...
template <TextureStoreUnpackFormat TEXCACHEFORMAT>
void NDSTextureUnpackI8(const size_t srcSize, const u8 *__restrict srcData,
                        const u16 *__restrict srcPal,
                        const bool isPalZeroTransparent,
                        u32 *__restrict dstBuffer) {
  if (isPalZeroTransparent) {
...
template <TextureStoreUnpackFormat TEXCACHEFORMAT>
void NDSTextureUnpackA3I5(const size_t srcSize, const u8 *__restrict srcData,
                          const u16 *__restrict srcPal,
                          u32 *__restrict dstBuffer) {
  for (size_t i = 0; i < srcSize; i++, srcData++) {
    const u16 c = srcPal[*srcData & 0x1F] & 0x7FFF;
    const u8 alpha = *srcData >> 5;
    *dstBuffer++ = (TEXCACHEFORMAT == TexFormat_15bpp)
                       ? COLOR555TO6665(c, material_3bit_to_5bit[alpha])
                       : COLOR555TO8888(c, material_3bit_to_8bit[alpha]);
  }
}

template <TextureStoreUnpackFormat TEXCACHEFORMAT>
void NDSTextureUnpackA5I3(const size_t srcSize, const u8 *__restrict srcData,
                          const u16 *__restrict srcPal,
                          u32 *__restrict dstBuffer) {
#ifdef ENABLE_SSSE3
  ...
  for (size_t i = 0; i < srcSize; i += 16, srcData += 16, dstBuffer += 16) {
    ...
    const __m128i idx =
        _mm_slli_epi16(_mm_and_si128(bits, _mm_set1_epi8(0x07)), 1);
    const __m128i idx0 =
        _mm_add_epi8(_mm_unpacklo_epi8(idx, idx), _mm_set1_epi16(0x0100));
    const __m128i idx1 =
        _mm_add_epi8(_mm_unpackhi_epi8(idx, idx), _mm_set1_epi16(0x0100));
    ...
    if (TEXCACHEFORMAT == TexFormat_15bpp) {
      const __m128i alpha =
          _mm_srli_epi16(_mm_and_si128(bits, _mm_set1_epi8(0xF8)), 3);
      ...
      ColorspaceConvert555To6665_SSE2<false>(
          palColor0, alphaLo, convertedColor[0], convertedColor[1]);
      ColorspaceConvert555To6665_SSE2<false>(
          palColor1, alphaHi, convertedColor[2], convertedColor[3]);
    } else {
      const __m128i alpha = _mm_or_si128(
          _mm_and_si128(bits, _mm_set1_epi8(0xF8)),
          _mm_srli_epi16(_mm_and_si128(bits, _mm_set1_epi8(0xE0)), 5));
      const __m128i alphaLo = _mm_unpacklo_epi8(_mm_setzero_si128(), alpha);
      const __m128i alphaHi = _mm_unpackhi_epi8(_mm_setzero_si128(), alpha);

      ColorspaceConvert555To8888_SSE2<false>(
          palColor0, alphaLo, convertedColor[0], convertedColor[1]);
      ColorspaceConvert555To8888_SSE2<false>(
          palColor1, alphaHi, convertedColor[2], convertedColor[3]);
    }
  ...
  for (size_t i = 0; i < srcSize; i++, srcData++) {
    ...
    *dstBuffer++ = (TEXCACHEFORMAT == TexFormat_15bpp)
                       ? COLOR555TO6665(c, alpha)
                       : COLOR555TO8888(c, material_5bit_to_8bit[alpha]);
  }
#endif
}

#define PAL4X4(offset)                                                         \
  (LE_TO_LOCAL_16(                                                             \
       *(u16 *)(MMU.texInfo                                                    \
                    .texPalSlot[((palAddress + (offset)*2) >> 14) & 0x7] +     \
                ((palAddress + (offset)*2) & 0x3FFF))) &                       \
   0x7FFF)

template <TextureStoreUnpackFormat TEXCACHEFORMAT>
void NDSTextureUnpack4x4(const size_t srcSize, const u32 *__restrict srcData,
                         const u16 *__restrict srcIndex, const u32 palAddress,
    ...
    for (size_t x = 0; x < xTmpSize; x++, d++) {
      ...
      if (dead) {
        for (int sy = 0; sy < 4; sy++) {
          ...
          dstBuffer[currentPos] = dstBuffer[currentPos + 1] =
              dstBuffer[currentPos + 2] = dstBuffer[currentPos + 3] = 0;
        }
        continue;
      }
...
#ifdef MSB_FIRST
        tmp_col[2] =
            ((((tmp_col[0] & 0xFF000000) >> 1) +
              ((tmp_col[1] & 0xFF000000) >> 1)) &
             0xFF000000) |
...
#else
        tmp_col[2] =
            ((((tmp_col[0] & 0x00FF00FF) + (tmp_col[1] & 0x00FF00FF)) >> 1) &
             0x00FF00FF) |
            ((((tmp_col[0] & 0x0000FF00) + (tmp_col[1] & 0x0000FF00)) >> 1) &
             0x0000FF00) |
            0xFF000000;
      ...
      // TODO - this could be more precise for 32bpp mode (run it through the
...
template <TextureStoreUnpackFormat TEXCACHEFORMAT>
void NDSTextureUnpackDirect16Bit(const size_t srcSize,
                                 const u16 *__restrict srcData,
                                 u32 *__restrict dstBuffer) {
  ...
  for (; i < pixCountVec128; i += 8, srcData += 8, dstBuffer += 8) {
    ...
    const v128u16 alpha =
        _mm_cmpeq_epi16(_mm_srli_epi16(c, 15), _mm_set1_epi16(1));
    v128u32 convertedColor[2];

    if (TEXCACHEFORMAT == TexFormat_15bpp) {
      ColorspaceConvert555To6665Opaque_SSE2<false>(c, convertedColor[0],
                                                   convertedColor[1]);
    } else {
      ColorspaceConvert555To8888Opaque_SSE2<false>(c, convertedColor[0],
                                                   convertedColor[1]);
    }

    convertedColor[0] = _mm_blendv_epi8(_mm_setzero_si128(), convertedColor[0],
                                        _mm_unpacklo_epi16(alpha, alpha));
    convertedColor[1] = _mm_blendv_epi8(_mm_setzero_si128(), convertedColor[1],
                                        _mm_unpackhi_epi16(alpha, alpha));
