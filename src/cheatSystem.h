...
public:
  CHEATS() : currentGet(0) { memset(filename, 0, sizeof(filename)); }
  ~CHEATS() {}

  void init(char *path);
  BOOL add(u8 size, u32 address, u32 val, char *description, BOOL enabled);
  BOOL update(u8 size, u32 address, u32 val, char *description, BOOL enabled,
              u32 pos);
  ...
  static BOOL XXCodeFromString(CHEATS_LIST *cheatItem,
...
public:
  CHEATSEARCH()
      : statMem(0), mem(0), amount(0), lastRecord(0), _type(0), _size(0),
        _sign(0) {}
  ~CHEATSEARCH() { close(); }
...
enum CHEATS_DB_TYPE { CHEATS_DB_R4 = 0 };
...
public:
  CHEATSEXPORT()
      : fp(NULL), fsize(0), dataSize(0), encOffset(0), type(CHEATS_DB_R4),
        encrypted(false), numCheats(0), cheats(0), CRC(0), error(0) {
