...
struct SoundInterface_struct {
  ...
  void (*ClearBuffer)();
  void (*FetchSamples)(s16 *sampleBuffer, size_t sampleCount,
                       ESynchMode synchMode,
                       ISynchronizingAudioBuffer *theSynchronizer);
  size_t (*PostProcessSamples)(s16 *postProcessBuffer,
...
struct channel_struct {
  channel_struct()
      : num(0), vol(0), volumeDiv(0), hold(0), pan(0), waveduty(0), repeat(0),
        format(0), keyon(0), status(0), addr(0), timer(0), loopstart(0),
        length(0), totlength(0), double_totlength_shifted(0.0), sampcnt(0.0),
        sampinc(0.0), lastsampcnt(0), pcm16b(0), pcm16b_last(0), loop_pcm16b(0),
        index(0), loop_index(0), x(0), psgnoise_last(0) {}
  u32 num;
  u8 vol;
  u8 volumeDiv;
  u8 hold;
...
public:
  ...
  struct REGS {
    REGS()
        : mastervol(0), ctl_left(0), ctl_right(0), ctl_ch1bypass(0),
          ctl_ch3bypass(0), masteren(0), soundbias(0) {}
...
static FORCEINLINE u8 SPU_ReadByte(u32 addr) {
  return SPU_core->ReadByte(addr & 0x0FFF);
}
static FORCEINLINE u16 SPU_ReadWord(u32 addr) {
  return SPU_core->ReadWord(addr & 0x0FFF);
}
static FORCEINLINE u32 SPU_ReadLong(u32 addr) {
  return SPU_core->ReadLong(addr & 0x0FFF);
}
...
enum WAVMode { WAVMODE_ANY = -1, WAVMODE_CORE = 0, WAVMODE_USER = 1 };
