...
const RomBanner &GameInfo::getRomBanner() { return banner; }
...
bool GameInfo::ValidateHeader() {
  ...
  if (detectedRomType != ROMTYPE_HOMEBREW) {
    ...
    for (size_t i = 0; i < 4; i++) {
      ...
      if (c < 0 || (c > 0 && c < 32) || c == 127) {
        printf(
            "ROM Validation: Invalid character detected in ROM Game Code.\n");
        printf("                charIndex = %d, charValue = %d\n", (int)i, c);
        return isRomValid;
      }
    }
...
void GameInfo::populate() {
  ...
  if (isHomebrew()) {
    // we can't really make a serial for a homebrew game that hasnt set a game
    // code
  ...
  /*if(header.IconOff < romsize)
  {
      ...
      for (int i = 0; i < num; i++)
      {
              wcstombs(ROMfullName[i], (wchar_t
  *)(romdata+header.IconOff+0x240+(i*0x100)), 0x100); trim(ROMfullName[i]);
      }
  }*/
}
...
bool GameInfo::loadROM(std::string fname, u32 type) {
  ...
  if (res) {
#ifdef MSB_FIRST
    // endian swap necessary fields. It would be better if we made accessors for
    // these. I wonder if you could make a macro for a field accessor that would
    // take the bitsize and do the swap on the fly
    struct FieldSwap {
      const size_t offset;
      const size_t bytes;
    };
    ...
    if (cardSize < romsize) {
      msgbox->warn("The ROM header is invalid.\nThe device size has been "
                   "increased to allow for the provided file size.\n");
    ...
    // convert to an in-memory reader around a pre-read buffer if that's what's
    // requested
    if (loadToMemory) {
      reader->Seek(fROM, headerOffset, SEEK_SET);
    ...
    _isDSiEnhanced =
        ((readROM(0x180) == 0x8D898581U) && (readROM(0x184) == 0x8C888480U));
    if (hasRomBanner()) {
...
bool GameInfo::isDSiEnhanced() { return _isDSiEnhanced; }

bool GameInfo::isHomebrew() {
  return ((header.ARM9src < 0x4000) &&
          (T1ReadLong(header.logo, 0) != 0x51AEFF24) &&
          (T1ReadLong(header.logo, 4) != 0x699AA221));
}

static int rom_init_path(const char *filename, const char *physicalName,
  ...
  if (!strcasecmp(path.extension().c_str(), "zip") ||
    ...
    gameInfo.loadROM(physicalName ? physicalName : path.path,
                     type); // n.b. this does nothing if the file can't be found
                            // (i.e. if it was an extracted tempfile)...
...
int NDS_LoadROM(const char *filename, const char *physicalName,
                const char *logicalFilename) {
  ...
  // run crc over the whole buffer (chunk at a time, to avoid coding a streaming
  // crc
  ...
  gameInfo.chipID =
      0xC2; // The Manufacturer ID is defined by JEDEC (C2h = Macronix)
  if (!gameInfo.isHomebrew()) {
    gameInfo.chipID |= ((((128 << gameInfo.header.cardSize) / 1024) - 1)
                        << 8); // Chip size in megabytes minus 1
    ...
    // 6: Unknown
    // 7: ROM speed (Secure Area Block transfer mode (trasfer 8x200h or 1000h
    // bytes)
    // TODO:
  ...
  INFO("\nROM game code: %c%c%c%c\n", gameInfo.header.gameCode[0],
       gameInfo.header.gameCode[1], gameInfo.header.gameCode[2],
  ...
  const char *makerName =
      Database::MakerNameForMakerCode(gameInfo.header.makerCode, true);
  INFO("ROM developer: %s\n",
       ((gameInfo.header.makerCode == 0) && gameInfo.isHomebrew()) ? "Homebrew"
                                                                   : makerName);
  ...
  if (gameInfo.isHomebrew()) {
    /*
    //note: gameInfo.romdataForReader is safe here because we made sure to load
    the rom into memory for isHomebrew if (slot1_GetCurrentType() ==
    NDS_SLOT1_R4) DLDI::tryPatch((void*)gameInfo.romdataForReader,
    gameInfo.romsize, 1); else if (slot2_GetCurrentType() == NDS_SLOT2_CFLASH)
            DLDI::tryPatch((void*)gameInfo.romdataForReader, gameInfo.romsize,
    0);
            */
  }
...
void NDS_TriggerCardEjectIRQ() {
  ...
  NDS_makeIrq(ARMCPU_ARM9, IRQ_BIT_GC_IREQ_MC); // zero added on 11-aug-2013
                                                // with no proof of accuracy
}

class FrameSkipper {
public:
  void RequestSkip() { nextSkip = true; }
  ...
  void Advance() {
    ...
    const IOREG_DISPCAPCNT &DISPCAPCNT =
        mainEngine->GetIORegisterMap().DISPCAPCNT;
    ...
    if (capturing && consecutiveNonCaptures > 30) {
      ...
      SkipNext2DFrame = false;
      nextSkip = false;
    } else if ((lastDisplayTarget != mainEngine->GetTargetDisplayByID()) &&
               lastSkip && !skipped) {
    ...
    if (capturing)
      consecutiveNonCaptures = 0;
    else if (!(consecutiveNonCaptures >
               9000)) // arbitrary cap to avoid eventual wrap
  ...
  FORCEINLINE bool ShouldSkip2D() { return SkipCur2DFrame; }
  FORCEINLINE bool ShouldSkip3D() { return SkipCur3DFrame; }
  FrameSkipper() {
    nextSkip = false;
    skipped = false;
    lastSkip = false;
...
struct TSequenceItem {
  ...
  virtual bool load(EMUFILE &is) {
    if (is.read_64LE(timestamp) != 1)
      return false;
    if (is.read_32LE(param) != 1)
      return false;
    if (is.read_bool32(enabled) != 1)
      return false;
    return true;
  }

  FORCEINLINE bool isTriggered() { return enabled && nds_timer >= timestamp; }

  FORCEINLINE u64 next() { return timestamp; }
};
...
struct TSequenceItem_Timer : public TSequenceItem {
  ...
  FORCEINLINE void schedule() {
    enabled =
        MMU.timerON[procnum][num] && MMU.timerMODE[procnum][num] != 0xFFFF;
  }

  FORCEINLINE u64 next() { return nds.timerCycle[procnum][num]; }

  FORCEINLINE void exec() {
    IF_DEVELOPER(
        DEBUG_statistics.sequencerExecutionCounters[13 + procnum * 4 + num]++);
...
struct TSequenceItem_DMA : public TSequenceItem {
  ...
  FORCEINLINE bool isEnabled() { return controller->dmaCheck ? TRUE : FALSE; }

  FORCEINLINE u64 next() { return controller->nextEvent; }

  FORCEINLINE void exec() {
    IF_DEVELOPER(
        DEBUG_statistics.sequencerExecutionCounters[5 + procnum * 4 + chan]++);
    ...
    //			//disable the channel
    //			if(MMU.DMAStartTime[procnum][chan] != EDMAMode_GXFifo) {
    //				T1WriteLong(regs, 0xB8 + (0xC*chan), T1ReadLong(regs, 0xB8 +
    //(0xC*chan)) & 0x7FFFFFFF); 				MMU.DMACrt[procnum][chan] &= 0x7FFFFFFF;
    ////blehhh i hate this shit being mirrored in memory
    //			}
    //
...
struct TSequenceItem_divider : public TSequenceItem {
  ...
  FORCEINLINE u64 next() { return MMU.divCycles; }
...
struct TSequenceItem_sqrtunit : public TSequenceItem {
  ...
  FORCEINLINE u64 next() { return MMU.sqrtCycles; }
...
#undef SAVE
  ...
  bool load(EMUFILE &is, int version) {
    if (is.read_64LE(nds_timer) != 1)
      return false;
    if (is.read_64LE(nds_arm9_timer) != 1)
      return false;
    if (is.read_64LE(nds_arm7_timer) != 1)
      return false;
    if (!dispcnt.load(is))
      return false;
    if (!divider.load(is))
...
void NDS_RescheduleReadSlot1(int procnum, int size) {
  ...
  // if data is to be returned, the first word is read before it's available and
  // irqs and dmas fire
  if (size != 0)
    delay += 4;
...
static void execHardware_hblank() {
  ...
  // now, we try and give the game as much time as possible to finish doing its
  // work for the scanline, by drawing scanline N at the end of drawing time
  // (but before subsequent interrupt or hdma-driven events happen) don't try to
  // do this at the end of the scanline, because some games (sonic classics) may
  // use hblank IRQ to set scroll regs for the next scanline
  if (nds.VCount < 192) {
    if (nds.VCount == 0) {
  ...
  if (nds.VCount == 262) {
    ...
    // b. i think that 193 hblanks actually fire (one for the hblank in scanline
    // 262) this is demonstrated by NSMB splot-parallaxing clouds for some
    // reason the game will setup two hdma scroll register buffers to be run
    // consecutively, and unless we do this, the second buffer will be offset by
    // one scanline causing a glitch in the 0th scanline
    // triggerDma(EDMAMode_HBlank);

    // BUT! this was removed in order to make glitches in megaman zero
    // collection (mmz 4 1st level) work. and, it seems that it is no longer
    // necessary in nsmb. perhaps something else fixed it
  }
...
static void execHardware_hstart_vblankStart() {
  ...
  for (int i = 0; i < 2; i++) {
    if (MMU.reg_IF_pending[i] & (1 << IRQ_BIT_LCD_VBLANK)) {
      ...
      // for ARM7, cheats process when a vblank IRQ fires. necessary for AR
      // compatibility and to stop cheats from breaking game boot-ups. note that
      // how we process raw cheats is up to us. so we'll do it the same way we
      // used to, elsewhere
      if (i == 1 && cheats)
  ...
  nds.runCycleCollector[ARMCPU_ARM9][nds.idleFrameCounter] =
      1120380 - nds.idleCycles[0];
  nds.runCycleCollector[ARMCPU_ARM7][nds.idleFrameCounter] =
      1120380 - nds.idleCycles[1];
...
static void execHardware_hdraw() {
  // due to hacks in our selection of rendering time, we do not actually render
  // here as intended. consider changing this if there is some problem with
  // raster fx timing but check the documentation near the gpu rendering calls
...
static void execHardware_hstart_irq() {
  ...
  // this is necessary to fix "egokoro kyoushitsu" which idles waiting for
  // vcount=192, which never happens due to a long vblank irq 100% accurate
  // emulation would require the read of VCOUNT to be in the pipeline already
  // with the irq coming in behind it, thus allowing the vcount to register as
  // 192 occasionally (maybe about 1 out of 28 frames) the actual length of the
  // delay is in execHardware() where the events are scheduled
  sequencer.reschedule = true;
  if (nds.VCount == 192) {
...
static void execHardware_hstart() {
  ...
  // end of 3d vblank
  // this should be 214, but we are going to be generous for games with tight
  // timing they shouldnt be changing any textures at 262 but they might
  // accidentally still be at 214 so..
  if ((CommonSettings.rigorous_timing && nds.VCount == 214) ||
      (!CommonSettings.rigorous_timing && nds.VCount == 262)) {
  ...
  if (nds.VCount == 263) {
    ...
    if (!NDS_ARM9.freeze && nds.overclock < 2 &&
        CommonSettings.gamehacks.flags.overclock) {
      ...
      // overclock arm7 lock is always released here; if it wasn't actiev, this
      // benign
      NDS_ARM7.freeze &= ~CPU_FREEZE_OVERCLOCK_HACK;
    ...
    // this is important for the character select in Dragon Ball Kai - Ultimate
    // Butouden it seems if you allow the 3d to begin before the vblank, then it
    // will get interrupted and not complete. the game needs to pick up the
    // gxstat reg busy as clear after it finishes processing vblank. therefore,
    // this can't happen until sometime after vblank. devil survivor 2 will have
    // screens get stuck if this is on any other scanline. obviously 192 is the
    // right choice.
    gfx3d_VBlankSignal();
  ...
  if (nds.VCount < 192) {
    ...
    // it should be driven by a fifo (and generate just in time as the scanline
    // is displayed) but that isnt even possible until we have some sort of
    // sub-scanline timing. it may not be necessary.
    triggerDma(EDMAMode_MemDisplay);
  }
}

void NDS_Reschedule() {
  IF_DEVELOPER(if (!sequencer.reschedule)
...
u64 Sequencer::findNext() {
  ...
  if (divider.isEnabled())
    next = _fast_min(next, divider.next());
  if (sqrtunit.isEnabled())
    next = _fast_min(next, sqrtunit.next());
  if (gxfifo.enabled)
    next = _fast_min(next, gxfifo.next());
  if (readslot1.isEnabled())
    next = _fast_min(next, readslot1.next());
  if (wifi.enabled)
    next = _fast_min(next, wifi.next());

#define test(X, Y)                                                             \
  if (dma_##X##_##Y.isEnabled())                                               \
    next = _fast_min(next, dma_##X##_##Y.next());
  test(0, 0);
  test(0, 1);
  test(0, 2);
  test(0, 3);
...
#undef test
#define test(X, Y)                                                             \
  if (timer_##X##_##Y.enabled)                                                 \
    next = _fast_min(next, timer_##X##_##Y.next());
  test(0, 0);
  test(0, 1);
  test(0, 2);
  test(0, 3);
...
void Sequencer::execHardware() {
  if (dispcnt.isTriggered()) {
    ...
    case ESI_DISPCNT_HDraw:
      execHardware_hdraw();
      // duration of non-blanking period is ~1606 clocks (gbatek agrees) [but
      // says its different on arm7] im gonna call this 267 dots = 267*6=1602
    ...
    case ESI_DISPCNT_HBlank:
      ...
      // there are surely 355 dots per scanline, less 267 for non-blanking
      // period. the rest is hblank and then after that is hstart
...
FORCEINLINE void arm9log() {
  ...
  if (dolog) {
    ...
    if (NDS_ARM9.CPSR.bits.T)
      des_thumb_instructions_set[((NDS_ARM9.instruction) >> 6) & 1023](
          NDS_ARM9.instruct_adr, NDS_ARM9.instruction, dasmbuf);
    else
      des_arm_instructions_set[INDEX(NDS_ARM9.instruction)](
          NDS_ARM9.instruct_adr, NDS_ARM9.instruction, dasmbuf);
    ...
    fprintf(
        ...
        "R12:%08X R13:%08X R14:%08X R15:%08X| next %08X, N:%i Z:%i C:%i V:%i\n",
        currFrameCounter, nds.VCount, nds_timer, NDS_ARM9.R[0], NDS_ARM9.R[1],
        NDS_ARM9.R[2], NDS_ARM9.R[3], NDS_ARM9.R[4], NDS_ARM9.R[5],
        NDS_ARM9.R[6], NDS_ARM9.R[7], NDS_ARM9.R[8], NDS_ARM9.R[9],
        NDS_ARM9.R[10], NDS_ARM9.R[11], NDS_ARM9.R[12], NDS_ARM9.R[13],
        NDS_ARM9.R[14], NDS_ARM9.R[15], NDS_ARM9.next_instruction,
        NDS_ARM9.CPSR.bits.N, NDS_ARM9.CPSR.bits.Z, NDS_ARM9.CPSR.bits.C,
        NDS_ARM9.CPSR.bits.V);
#endif
    fprintf(fp_dis9, "%s %08X\t%08X \t%s\n",
            NDS_ARM9.CPSR.bits.T ? "THUMB" : "ARM", NDS_ARM9.instruct_adr,
            NDS_ARM9.instruction, dasmbuf);
...
#else
    printf("%05d:%03d %12lld 9:%08X %08X %-30s R00:%08X R01:%08X R02:%08X "
           "R03:%08X R04:%08X R05:%08X R06:%08X R07:%08X R08:%08X R09:%08X "
           "R10:%08X R11:%08X R12:%08X R13:%08X R14:%08X R15:%08X\n",
           currFrameCounter, nds.VCount, nds_timer, NDS_ARM9.instruct_adr,
           NDS_ARM9.instruction, dasmbuf, NDS_ARM9.R[0], NDS_ARM9.R[1],
           NDS_ARM9.R[2], NDS_ARM9.R[3], NDS_ARM9.R[4], NDS_ARM9.R[5],
           NDS_ARM9.R[6], NDS_ARM9.R[7], NDS_ARM9.R[8], NDS_ARM9.R[9],
           NDS_ARM9.R[10], NDS_ARM9.R[11], NDS_ARM9.R[12], NDS_ARM9.R[13],
           NDS_ARM9.R[14], NDS_ARM9.R[15]);
#endif
  }
#endif
}
...
FORCEINLINE void arm7log() {
  ...
  if (dolog) {
    ...
    if (NDS_ARM7.CPSR.bits.T)
      des_thumb_instructions_set[((NDS_ARM7.instruction) >> 6) & 1023](
          NDS_ARM7.instruct_adr, NDS_ARM7.instruction, dasmbuf);
    else
      des_arm_instructions_set[INDEX(NDS_ARM7.instruction)](
          NDS_ARM7.instruct_adr, NDS_ARM7.instruction, dasmbuf);
#ifdef LOG_TO_FILE
    if (!fp_dis7)
      return;
    ...
    fprintf(
        ...
        "R5:%08X R6:%08X R7:%08X R8:%08X R9:%08X\n\t\t;R10:%08X R11:%08X "
        "R12:%08X R13:%08X R14:%08X R15:%08X| next %08X, N:%i Z:%i C:%i V:%i\n",
        currFrameCounter, nds.VCount, nds_timer, NDS_ARM7.R[0], NDS_ARM7.R[1],
        NDS_ARM7.R[2], NDS_ARM7.R[3], NDS_ARM7.R[4], NDS_ARM7.R[5],
        NDS_ARM7.R[6], NDS_ARM7.R[7], NDS_ARM7.R[8], NDS_ARM7.R[9],
        NDS_ARM7.R[10], NDS_ARM7.R[11], NDS_ARM7.R[12], NDS_ARM7.R[13],
        NDS_ARM7.R[14], NDS_ARM7.R[15], NDS_ARM7.next_instruction,
        NDS_ARM7.CPSR.bits.N, NDS_ARM7.CPSR.bits.Z, NDS_ARM7.CPSR.bits.C,
        NDS_ARM7.CPSR.bits.V);
#endif
    fprintf(fp_dis7, "%s %08X\t%08X \t%s\n",
            NDS_ARM7.CPSR.bits.T ? "THUMB" : "ARM", NDS_ARM7.instruct_adr,
            NDS_ARM7.instruction, dasmbuf);
...
#else
    printf("%05d:%03d %12lld 7:%08X %08X %-30s R00:%08X R01:%08X R02:%08X "
           "R03:%08X R04:%08X R05:%08X R06:%08X R07:%08X R08:%08X R09:%08X "
           "R10:%08X R11:%08X R12:%08X R13:%08X R14:%08X R15:%08X\n",
           currFrameCounter, nds.VCount, nds_timer, NDS_ARM7.instruct_adr,
           NDS_ARM7.instruction, dasmbuf, NDS_ARM7.R[0], NDS_ARM7.R[1],
           NDS_ARM7.R[2], NDS_ARM7.R[3], NDS_ARM7.R[4], NDS_ARM7.R[5],
           NDS_ARM7.R[6], NDS_ARM7.R[7], NDS_ARM7.R[8], NDS_ARM7.R[9],
           NDS_ARM7.R[10], NDS_ARM7.R[11], NDS_ARM7.R[12], NDS_ARM7.R[13],
           NDS_ARM7.R[14], NDS_ARM7.R[15]);
#endif
  }
#endif
}
...
template <bool doarm9, bool doarm7>
#endif
static /*donotinline*/ std::pair<s32, s32>
armInnerLoop(const u64 nds_timer_base, const s32 s32next, s32 arm9, s32 arm7) {
  ...
  while (timer < s32next && !sequencer.reschedule && execute) {
    ...
    for (int i = 0; i < NDS_ARM9.breakPoints.size(); ++i) {
            if (NDS_ARM9.instruct_adr == NDS_ARM9.breakPoints[i] &&
    !NDS_ARM9.debugStep) { emu_paused = true; paused = true; execute = false;
                    // update debug display
                    PostMessageA(DisViewWnd[0], WM_COMMAND, IDC_DISASMSEEK,
    NDS_ARM9.instruct_adr); InvalidateRect(DisViewWnd[0], NULL, FALSE); return
    std::make_pair(arm9, arm7);
            }
    }
    for (int i = 0; i < NDS_ARM7.breakPoints.size(); ++i) {
            if (NDS_ARM7.instruct_adr == NDS_ARM7.breakPoints[i] &&
    !NDS_ARM7.debugStep) { emu_paused = true; paused = true; execute = false;
                    // update debug display
                    PostMessageA(DisViewWnd[1], WM_COMMAND, IDC_DISASMSEEK,
    NDS_ARM7.instruct_adr); InvalidateRect(DisViewWnd[1], NULL, FALSE); return
    std::make_pair(arm9, arm7);
            }
    }
    */
    ...
    if (doarm9 && (!doarm7 || arm9 <= timer)) {
      ...
      if (NDS_ARM9.debugStep) {
        ...
        execute = false;
        // PostMessageA(DisViewWnd[0], WM_COMMAND, IDC_DISASMSEEK,
        // NDS_ARM9.instruct_adr); InvalidateRect(DisViewWnd[0], NULL, FALSE);
      }
      if (NDS_ARM9.stepOverBreak == NDS_ARM9.instruct_adr &&
      ...
      if (NDS_ARM9.runToRetTmp != 0 &&
          NDS_ARM9.runToRetTmp == NDS_ARM9.instruct_adr) {
        NDS_ARM9.runToRetTmp = 0;
        NDS_ARM9.runToRet = true;
      }
    }
    if (doarm7 && (!doarm9 || arm7 <= timer)) {
      bool cpufreeze = !!(NDS_ARM7.freeze &
                          (CPU_FREEZE_WAIT_IRQ | CPU_FREEZE_OVERCLOCK_HACK));
...
#ifdef HAVE_JIT
          return armInnerLoop<doarm9, false, jit>(nds_timer_base, s32next, arm9,
                                                  arm7);
#else
          return armInnerLoop<doarm9, false>(nds_timer_base, s32next, arm9,
                                             arm7);
#endif
        }
      }
      if (NDS_ARM7.debugStep) {
        ...
        execute = false;
        // PostMessageA(DisViewWnd[1], WM_COMMAND, IDC_DISASMSEEK,
        // NDS_ARM7.instruct_adr); InvalidateRect(DisViewWnd[1], NULL, FALSE);
      }
      if (NDS_ARM7.stepOverBreak == NDS_ARM7.instruct_adr &&
      ...
      // aaand handle step to return
      if (NDS_ARM7.runToRetTmp != 0 &&
          NDS_ARM7.runToRetTmp == NDS_ARM7.instruct_adr) {
...
template <bool FORCE> void NDS_exec(s32 nb) {
#ifdef GDB_STUB
  gdbstub_mutex_lock();
#endif
  ...
  IF_DEVELOPER(for (int i = 0; i < 32; i++)
                   DEBUG_statistics.sequencerExecutionCounters[i] = 0);

  if (nds.sleeping) {
    // speculative code: if ANY irq happens, wake up the arm7.
    // I think the arm7 program analyzes the system and may decide not to wake
    // up if it is dissatisfied with the conditions
    ...
    for (;;) {
...
#ifdef DEVELOPER
      singleStep = false;
      //(gdb stub doesnt yet know how to trigger these immediately by calling
      //reschedule)
      ...
      // since there won't be anything for it to do (everything should be
      ...
      std::pair<s32, s32> arm9arm7 =
          CommonSettings.use_jit
              ? armInnerLoop<true, true, true>(nds_timer_base, s32next, arm9,
                                               arm7)
              : armInnerLoop<true, true, false>(nds_timer_base, s32next, arm9,
                                                arm7);
#else
      std::pair<s32, s32> arm9arm7 =
      ...
      //	printf("curious. please report: over by
      //%d\n",(int)(nds_timer-next));
#endif

      // if we were waiting for an irq, don't wait too long:
      // let's re-analyze it after this hardware event (this rolls back a big
      // burst of irq waiting which may have been interrupted by a resynch)
...
template <int PROCNUM> static void execHardware_interrupts_core() {
  ...
  if (masked && MMU.reg_IME[PROCNUM] && !ARMPROC.CPSR.bits.I) {
    // printf("Executing IRQ on procnum %d with IF = %08X and IE =
    // %08X\n",PROCNUM,IF,IE);
...
static void PrepareBiosARM7() {
  ...
  if (NDS_ARM7.BIOS_loaded) {
    ...
    // fake bios content, critical to normal operations, since we dont have a
    // real bios.

    T1WriteLong(MMU.ARM7_BIOS, 0x0000,
                0xEAFFFFFE); // B 00000000 (reset: infinite loop) (originally:
                             // 0xE25EF002 - SUBS PC, LR, #2
    T1WriteLong(
        MMU.ARM7_BIOS, 0x0004,
        0xEAFFFFFE); // B 00000004 (undefined instruction: infinite loop)
    T1WriteLong(MMU.ARM7_BIOS, 0x0008,
                0xEAFFFFFE); // B 00000280 (SWI: infinite loop [since we will be
                             // HLEing the SWI routines])
    T1WriteLong(MMU.ARM7_BIOS, 0x000C,
                0xEAFFFFFE); // B 0000000C (prefetch abort: infinite loop)
    T1WriteLong(MMU.ARM7_BIOS, 0x0010,
                0xEAFFFFFE); // B 00000010 (data abort: infinite loop)
    T1WriteLong(MMU.ARM7_BIOS, 0x0018,
                0xEA000000); // B 00000020 (IRQ: branch to handler)
    T1WriteLong(MMU.ARM7_BIOS, 0x001C,
                0xEAFFFFFE); // B 0000001C (FIQ vector: infinite loop)
    // IRQ handler
...
static void PrepareBiosARM9() {
  ...
  if (NDS_ARM9.BIOS_loaded) {
    ...
    // fake bios content, critical to normal operations, since we dont have a
    // real bios. it'd be cool if we could write this in some kind of assembly
    // language, inline or otherwise, without some bulky dependencies perhaps we
    // could build it with devkitarm? but thats bulky (offline) dependencies, to
    // be sure..

    // reminder: bios chains data abort to fast irq

    // exception vectors:
    T1WriteLong(MMU.ARM9_BIOS, 0x0000,
                0xEAFFFFFE); // (infinite loop for) Reset !!!
    // T1WriteLong(MMU.ARM9_BIOS, 0x0004, 0xEAFFFFFE);		// (infinite loop for)
    // Undefined instruction
    T1WriteLong(
        MMU.ARM9_BIOS, 0x0004,
        0xEA000004); // Undefined instruction -> Fast IRQ (just guessing)
    T1WriteLong(MMU.ARM9_BIOS, 0x0008, 0xEA00009C); // SWI -> ?????
    T1WriteLong(MMU.ARM9_BIOS, 0x000C,
                0xEAFFFFFE); // (infinite loop for) Prefetch Abort
    ...
    // copy the logo content into the bios - Pokemon Platinum uses this in Pal
    // Park trade it compares the logo from the arm9 bios to the logo in the GBA
    // header. NOTE: in the unlikely event that the valid header is missing from
    // the gameInfo, we'd be doing wrong here.
    //       however, its nice not to have the logo embedded here.
    //       TODO - take a CRC of the logo, check vs logoCRC16, and a hardcoded
    //       value, to make sure all is in order--report error if not
    ...
    T1WriteLong(MMU.ARM9_BIOS, 0x0278,
                0xEE190F11); // MRC CP15, 0, R0, CR9, CR1, 0
    ...
    // friendly reminder: to calculate an immediate offset: encoded =
    // (desired_address-cur_address-8)
    ...
    T1WriteLong(MMU.ARM9_BIOS, 0x02A4,
                0xE59FD000 | (0x2D4 - 0x2A4 - 8));  // LDR SP, [FFFF02D4]
    ...
    T1WriteLong(MMU.ARM9_BIOS, 0x02B4,
                0xEE11CF10); // MRC CP15, 0, R12, CR1, CR0, 0
    T1WriteLong(MMU.ARM9_BIOS, 0x02B8, 0xE92D5000); // STMDB SP!, {R12,LR}
    T1WriteLong(MMU.ARM9_BIOS, 0x02BC, 0xE3CCC001); // BIC R12, R12, #1

    T1WriteLong(MMU.ARM9_BIOS, 0x02C0,
                0xEE01CF10); // MCR CP15, 0, R12, CR1, CR0, 0
...
static void JumbleMemory() {
  // put random garbage in vram for homebrew games, to help mimic the situation
  // where libnds does not clear out junk which the card's launcher may or may
  // not have left behind analysis:
  // 1. retail games dont clear TCM, so why should we jumble it and expect
  // homebrew to clear it?
  // 2. some retail games _dont boot_ if main memory is jumbled. wha...?
  // 3. clearing this is not as useful as tracking uninitialized reads in dev+
  // builds
  // 4. the vram clearing causes lots of graphical corruptions in badly coded
  // homebrews. this reduces compatibility substantially conclusion: disable it
  // for now and bring it back as an option if(gameInfo.isHomebrew)
  //{
...
bool NDS_LegitBoot() {
  ...
  // we know that it takes certain jit parameters to successfully boot the
  // firmware. CRAZYMAX: is it safe to accept anything smaller than 12?
  CommonSettings.jit_max_block_size =
      std::min(CommonSettings.jit_max_block_size, 12U);
#endif

  // since firmware only boots encrypted roms, we have to make sure it's
  // encrypted first this has not been validated on big endian systems. it
  // almost positively doesn't work.
...
bool NDS_FakeBoot() {
  ...
  // since we're bypassing the code to decrypt the secure area, we need to make
  // sure its decrypted first this has not been validated on big endian systems.
  // it almost positively doesn't work.
  if (gameInfo.header.CRC16 != 0) {
    bool okRom =
        DecryptSecureArea((u8 *)&gameInfo.header, (u8 *)gameInfo.secureArea);
  ...
  // firmware loads the game card arm9 and arm7 programs as specified in rom
  // header
  {
    bool hasSecureArea =
        ((gameInfo.romType == ROM_NDS) && (gameInfo.header.CRC16 != 0));
    ...
    for (u32 i = 0; i < header->ARM9binSize; i += 4) {
      u32 tmp =
          (hasSecureArea && ((src >= 0x4000) && (src < 0x8000)))
              ? LE_TO_LOCAL_32(*(u32 *)(gameInfo.secureArea + (src - 0x4000)))
              : gameInfo.readROM(src);
  ...
  // bios does this (thats weird, though. shouldnt it get changed when the card
  // is swapped in the firmware menu? right now our firmware menu isnt detecting
  // any change to the card. are some games depending on it being written here?
  // please document. _MMU_write16<ARMCPU_ARM9>(0x027FF808,
  //T1ReadWord(MMU.CART_ROM, 0x15E));

  // firmware sets up a copy of the firmware user settings in memory.
  // TBD - this code is really clunky
  // it seems to be copying the MMU.fw.data data into RAM in the user memory
  // stash locations
  u8 temp_buffer[NDS_FW_USER_SETTINGS_MEM_BYTE_COUNT];
  if (copy_firmware_user_data(temp_buffer, MMU.fw.data._raw)) {
    for (int fw_index = 0; fw_index < NDS_FW_USER_SETTINGS_MEM_BYTE_COUNT;
         fw_index++)
      _MMU_write08<ARMCPU_ARM9>(0x027FFC80 + fw_index, temp_buffer[fw_index]);
  }

  // something copies the whole header to Main RAM 0x27FFE00 on startup.
  // (http://nocash.emubase.de/gbatek.htm#dscartridgeheader) once upon a time
  // this copied 0x90 more. this was thought to be wrong, and changed.
  if (nds.Is_DSI()) {
    // dsi needs this copied later in memory. there are probably a number of
    // things that  get copied to a later location in memory.. thats where the
    // NDS consoles tend to stash stuff.
  ...
  // the firmware will be booting to these entrypoint addresses via BX (well,
  // the arm9 at least; is unverified for the arm7)
  ...
  // firmware makes system think it's booted from card -- EXTREMELY IMPORTANT!!!
  // This is actually checked by some things. (which things?) Thanks to cReDiAr
  _MMU_write08<ARMCPU_ARM7>(
      0x02FFFC40,
      0x1); //<zero> removed redundant write to ARM9, this is going to shared
            //main memory. But one has to wonder why r3478 was made which
            //corrected a typo resulting in only ARMCPU_ARM7 getting used.
  ...
  // I think it is wrong to take gbatek's "SYS" and put it in USR--maybe USR
  // doesnt matter. i think SYS is all the misc modes. please verify by setting
  // nonsensical stack values for USR here
  NDS_ARM9.R[13] = NDS_ARM9.R13_usr;
  // n.b.: im not sure about all these, I dont know enough about arm9
  // svc/irq/etc modes and how theyre named in desmume to match them up
  // correctly. i just guessed.

  //--------------------------------
  // setup the homebrew argv
  // this is useful for nitrofs apps which are emulating themselves via cflash
  // struct __argv {
  //	int argvMagic;		//!< argv magic number, set to 0x5f617267 ('_arg')
  //if valid 	char *commandLine;	//!< base address of command line, set of null
  //terminated strings
  ...
  // its safe to put things in this position.. apparently nothing important is
  // here. however, some games could be checking them as an anti-desmume
  // measure, so we might have to control it with slot-1 settings to suggest
  // booting a homebrew app perhaps we could automatically boot homebrew to an
  // R4-like device.
  _MMU_write32<ARMCPU_ARM9>(0x02FFFE70, 0x5f617267);
  _MMU_write32<ARMCPU_ARM9>(0x02FFFE74,
                            kCommandline); //(commandline starts here)
...
void NDS_Reset() {
  ...
  /*
      ...
      if(!gameInfo.reader)
      {
              ...
              NDS_LoadROM(stash.filename.c_str(), stash.physicalName.c_str(),
     stash.logicalFilename.c_str());
  ...
  nds.freezeBus = 0;
  nds.power1.lcd = nds.power1.gpuMain = nds.power1.gfx3d_render =
      nds.power1.gfx3d_geometry = nds.power1.gpuSub = nds.power1.dispswap =
          1; // is this proper?
  nds.power_geometry = nds.power_render =
      TRUE; // whether this is proper follows from prior
  ...
  if (CommonSettings.UseExtFirmware && NDS_ARM7.BIOS_loaded &&
    ...
    // 1. we have the ARM7 and ARM9 bioses (its doubtful that our HLE bios
    // implement the necessary functions)
    // 2. firmware is available
    // 3. user has requested booting from firmware
    willBootFromFirmware =
        (CommonSettings.BootFromFirmware && didLoadExtFirmware);
  }

  // If we're doing a fake boot, then we must ensure that this value gets set
  // before any firmware settings are changed.
  if (!willBootFromFirmware) {
    // bios (or firmware) sets this default, which is generally not important
    // for retail games but some homebrews are depending on
  ...
  if (didLoadExtFirmware) {
    ...
    // If we didn't successfully load firmware from somewhere, then we need to
    // use our own internal firmware as a stand-in.
    NDS_InitDefaultFirmware(&MMU.fw.data);
  }

  // Load the firmware settings.
  if (CommonSettings.UseExtFirmwareSettings && didLoadExtFirmware) {
    // Partially clobber the loaded firmware with user settings from the .dfc
    // file.
    std::string extFWUserSettingsString =
        CFIRMWARE::GetUserSettingsFilePath(CommonSettings.ExtFirmwarePath);
    strncpy(CommonSettings.ExtFirmwareUserSettingsPath,
            extFWUserSettingsString.c_str(), MAX_PATH);
  ...
  // this needs to happen last, pretty much, since it establishes the correct
  // scheduling state based on all of the above initialization
  initSchedule();
...
static std::string MakeInputDisplayString(u16 pad, const std::string *Buttons,
                                          int count) {
  std::string s;
...
static std::string MakeInputDisplayString(u16 pad, u16 padExt) {
  const std::string Buttons[] = {"A", "B", "Sl", "St", "R",
                                 "L", "U", "D",  "Rs", "Ls"};
...
// the desmume host system will track the screen coordinate, but the hardware
// should be receiving the raw ADC values. so we'll need to use this to simulate
// the ADC values corresponding to the desired screen coords, based on the
// current TSC calibrations
u16 NDS_getADCTouchPosX(int scrX_lsl4) {
  scrX_lsl4 >>= 4;
  int rv =
      ((scrX_lsl4 - TSCal.scr.x1 + 1) * TSCal.adc.width) / TSCal.scr.width +
      TSCal.adc.x1;
...
u16 NDS_getADCTouchPosY(int scrY_lsl4) {
  scrY_lsl4 >>= 4;
  int rv =
      ((scrY_lsl4 - TSCal.scr.y1 + 1) * TSCal.adc.height) / TSCal.scr.height +
      TSCal.adc.y1;
  rv = min(0xFFF, max(0, rv));
  return (u16)(rv);
}

static UserInput rawUserInput =
    {}; // requested input, generally what the user is physically pressing
static UserInput intermediateUserInput =
    {}; // intermediate buffer for modifications (seperated from finalUserInput
        // for safety reasons)
static UserInput finalUserInput =
    {}; // what gets sent to the game and possibly recorded
bool validToProcessInput = false;

const UserInput &NDS_getRawUserInput() { return rawUserInput; }
UserInput &NDS_getProcessingUserInput() {
  assert(validToProcessInput);
  return intermediateUserInput;
}
bool NDS_isProcessingUserInput() { return validToProcessInput; }
const UserInput &NDS_getFinalUserInput() { return finalUserInput; }
...
// (userinput is kind of a misnomer, e.g. finalUserInput has to mirror nds.pad,
// nds.touchX, etc.)
static void saveUserInput(EMUFILE &os) {
  saveUserInput(os, finalUserInput);
  saveUserInput(
      os,
      intermediateUserInput); // saved in case a savestate is made during input
                              // processing (which Lua could do if nothing else)
  os.write_bool32(validToProcessInput);
  for (int i = 0; i < 14; i++)
    os.write_32LE(TurboTime.array[i]); // saved to make autofire more tolerable
                                       // to use with re-recording
}
static bool loadUserInput(EMUFILE &is, int version) {
  bool rv = true;
...
void NDS_setPad(bool R, bool L, bool D, bool U, bool T, bool S, bool B, bool A,
...
static void NDS_applyFinalInput() {
  ...
  u16 gbaKeys =
      ((input.buttons.A) ? 0 : (1 << 0)) | ((input.buttons.B) ? 0 : (1 << 1)) |
      ((input.buttons.T) ? 0 : (1 << 2)) | ((input.buttons.S) ? 0 : (1 << 3)) |
      ((input.buttons.R) ? 0 : (1 << 4)) | ((input.buttons.L) ? 0 : (1 << 5)) |
      ((input.buttons.U) ? 0 : (1 << 6)) | ((input.buttons.D) ? 0 : (1 << 7)) |
      ((input.buttons.E) ? 0 : (1 << 8)) | ((input.buttons.W) ? 0 : (1 << 9));
  ...
  u16 ndsKeysExt = ((input.buttons.X) ? 0 : (1 << 0)) |
                   ((input.buttons.Y) ? 0 : (1 << 1)) | (1 << 2) |
                   ((input.buttons.G) ? 0 : (1 << 3)) | // debug button
                   (1 << 4) | (1 << 5) | ((nds.isTouch) ? 0 : (1 << 6)) |
                   ((!LidClosed) ? 0 : (1 << 7));
...
NDSError NDS_GetLastError() { return _lastNDSError; }
...
void emu_halt(EmuHaltReasonCode reasonCode, NDSErrorTag errorTag) {
  ...
  // NDS_ARM9.R[15], NDS_ARM9.instruct_adr, NDS_ARM7.R[15],
...
// returns true if exmemcnt specifies satisfactory parameters for the device,
// which calls this function
bool ValidateSlot2Access(u32 procnum, u32 demandSRAMSpeed,
                         u32 demand1stROMSpeed, u32 demand2ndROMSpeed,
                         int clockbits) {
  ...
  u32 romSpeed1 =
      _rom1Speeds[(exmemcnt & EXMEMCNT_MASK_SLOT2_ROM_1ST_TIME) >> 2];
  u32 romSpeed2 =
      _rom2Speeds[(exmemcnt & EXMEMCNT_MASK_SLOT2_ROM_2ND_TIME) >> 4];
  ...
  // what we're interested in here is whether the rom/ram are too low -> too
  // fast. then accesses won't have enough time to work. i'm not sure if this
  // gives us enough flexibility, but it is good enough for now. should make the
  // arguments to this function bitmasks later if we need better.
...
void NDS_GetCPULoadAverage(u32 &outLoadAvgARM9, u32 &outLoadAvgARM7) {
  ...
  for (size_t i = 0; i < 16; i++) {
    // blend together a few frames to keep low-framerate games from having a
    // jittering load average (they will tend to work 100% for a frame and then
    //sleep for a while) 4 frames should handle even the slowest of games
    u32 sample =
        nds.runCycleCollector[ARMCPU_ARM9]
                             [(i + 0 + nds.idleFrameCounter) & 15] +
        nds.runCycleCollector[ARMCPU_ARM9]
                             [(i + 1 + nds.idleFrameCounter) & 15] +
        nds.runCycleCollector[ARMCPU_ARM9]
                             [(i + 2 + nds.idleFrameCounter) & 15] +
        nds.runCycleCollector[ARMCPU_ARM9][(i + 3 + nds.idleFrameCounter) & 15];
    sample /= 4;
    calcLoad = calcLoad / 8 + sample * 7 / 8;
  }
  outLoadAvgARM9 =
      std::min<u32>(100, std::max<u32>(0, (u32)(calcLoad * 100 / 1120380)));

  // calculate a 16 frame arm7 load average
  calcLoad = 0;
  for (size_t i = 0; i < 16; i++) {
    u32 sample =
        nds.runCycleCollector[ARMCPU_ARM7]
                             [(i + 0 + nds.idleFrameCounter) & 15] +
        nds.runCycleCollector[ARMCPU_ARM7]
                             [(i + 1 + nds.idleFrameCounter) & 15] +
        nds.runCycleCollector[ARMCPU_ARM7]
...
void TCommonSettings::GameHacks::clear() { memset(&flags, 0, sizeof(flags)); }
