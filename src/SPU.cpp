...
static inline s16 read16(u32 addr) {
  return (s16)_MMU_read16<ARMCPU_ARM7, MMU_AT_DEBUG>(addr);
}
static inline u8 read08(u32 addr) {
  return _MMU_read08<ARMCPU_ARM7, MMU_AT_DEBUG>(addr);
}
static inline s8 read_s8(u32 addr) {
  return (s8)_MMU_read08<ARMCPU_ARM7, MMU_AT_DEBUG>(addr);
}
...
// static ISynchronizingAudioBuffer* synchronizer =
// metaspu_construct(ESynchMethod_Z);
static ISynchronizingAudioBuffer *synchronizer =
    metaspu_construct(ESynchMethod_N);
...
double samples_per_hline = (48000 / 59.8261f) / 263.0f; /*59.8261f*/

static double samples = 0;

template <typename T> static FORCEINLINE T MinMax(T val, T min, T max) {
  if (val < min)
    return min;
  else if (val > max)
    return max;
...
SoundInterface_struct *SPU_SoundCore() { return SNDCore; }
...
int SPU_Init(int coreid, int buffersize) {
  ...
  for (unsigned int i = 0; i < COSINE_INTERPOLATION_RESOLUTION; i++)
    cos_lut[i] =
        (1.0 -
         cos(((double)i / (double)COSINE_INTERPOLATION_RESOLUTION) * M_PI)) *
        0.5;
  ...
  SPU_SetSynchMode(CommonSettings.SPU_sync_mode,
                   CommonSettings.SPU_sync_method);
...
void SPU_SetSynchMode(int mode, int method) {
  ...
  if (synchmethod != (ESynchMethod)method) {
    ...
    //  or maybe not, maybe the platform-specific code that calls this function
    //  can deal with it.
...
void SPU_Reset(void) {
  ...
  // After 0.9.6, lets experiment with removing it and just properly zapping the
  // spu instead
  //  Reset Registers
...
SPU_struct::SPU_struct(int buffersize)
    : bufpos(0), buflength(0), sndbuf(0), outbuf(0), bufsize(buffersize) {
...
static FORCEINLINE void adjust_channel_timer(channel_struct *chan) {
  chan->sampinc = (((double)ARM7_CLOCK) / (DESMUME_SAMPLE_RATE * 2)) /
                  (double)(0x10000 - chan->timer);
}
...
void SPU_struct::KeyOn(int channel) {
  ...
  // LOG("Channel %d key on: vol = %d, volumeDiv = %d, hold = %d, pan = %d,
  // waveduty = %d, repeat = %d, format = %d, source address = %07X," 		"timer =
  //%04X, loop start = %04X, length = %06X, MMU.ARM7_REG[0x501] = %02X\n",
  //channel, chan->vol, chan->volumeDiv, chan->hold, 		chan->pan, chan->waveduty,
  //chan->repeat, chan->format, chan->addr, chan->timer, chan->loopstart,
  //chan->length, T1ReadByte(MMU.ARM7_REG, 0x501));
  ...
  thischan.double_totlength_shifted =
      (double)(thischan.totlength << format_shift[thischan.format]);
...
u8 SPU_struct::ReadByte(u32 addr) {
  ...
  if ((addr & 0x0F00) == 0x0400) {
    ...
    case 0x2:
      return thischan.pan;
    case 0x3:
      return (thischan.waveduty | (thischan.repeat << 3) |
              (thischan.format << 5) |
              ((thischan.status == CHANSTAT_PLAY) ? 0x80 : 0));
    case 0x8:
      return thischan.timer >> 0;
    case 0x9:
      return thischan.timer >> 8;
    case 0xA:
      return thischan.loopstart >> 0;
    case 0xB:
      return thischan.loopstart >> 8;
    }
    return 0;
  }
  ...
  case 0x501:
    return (regs.ctl_left | (regs.ctl_right << 2) | (regs.ctl_ch1bypass << 4) |
            (regs.ctl_ch3bypass << 5) | (regs.masteren << 7));

  // SOUNDBIAS
  case 0x504:
    return regs.soundbias >> 0;
  case 0x505:
    return regs.soundbias >> 8;

  // SNDCAP0CNT/SNDCAP1CNT
  case 0x508:
  case 0x509: {
    u32 which = (addr - 0x508);
    return regs.cap[which].add | (regs.cap[which].source << 1) |
           (regs.cap[which].oneshot << 2) | (regs.cap[which].bits8 << 3) |
           (regs.cap[which].runtime.running << 7);
  }

  // SNDCAP0DAD
  case 0x510:
    return regs.cap[0].dad >> 0;
  case 0x511:
    return regs.cap[0].dad >> 8;
  case 0x512:
    return regs.cap[0].dad >> 16;
  case 0x513:
    return regs.cap[0].dad >> 24;

  // SNDCAP0LEN
  case 0x514:
    return regs.cap[0].len >> 0;
  case 0x515:
    return regs.cap[0].len >> 8;

  // SNDCAP1DAD
  case 0x518:
    return regs.cap[1].dad >> 0;
  case 0x519:
    return regs.cap[1].dad >> 8;
  case 0x51A:
    return regs.cap[1].dad >> 16;
  case 0x51B:
    return regs.cap[1].dad >> 24;

  // SNDCAP1LEN
  case 0x51C:
    return regs.cap[1].len >> 0;
  case 0x51D:
    return regs.cap[1].len >> 8;
  } // switch on address
...
u16 SPU_struct::ReadWord(u32 addr) {
  ...
  if ((addr & 0x0F00) == 0x0400) {
    ...
    switch (addr & 0xF) {
    case 0x0:
      return (thischan.vol | (thischan.volumeDiv << 8) | (thischan.hold << 15));
    case 0x2:
      return (thischan.pan | (thischan.waveduty << 8) |
              (thischan.repeat << 11) | (thischan.format << 13) |
              ((thischan.status == CHANSTAT_PLAY) ? (1 << 15) : 0));
    case 0x8:
      return thischan.timer;
    case 0xA:
      return thischan.loopstart;
  ...
  // SOUNDCNT
  case 0x500:
    return (regs.mastervol | (regs.ctl_left << 8) | (regs.ctl_right << 10) |
            (regs.ctl_ch1bypass << 12) | (regs.ctl_ch3bypass << 13) |
            (regs.masteren << 15));

  // SOUNDBIAS
  case 0x504:
    return regs.soundbias;

  // SNDCAP0CNT/SNDCAP1CNT
  case 0x508: {
    u8 val0 = regs.cap[0].add | (regs.cap[0].source << 1) |
              (regs.cap[0].oneshot << 2) | (regs.cap[0].bits8 << 3) |
              (regs.cap[0].runtime.running << 7);
    u8 val1 = regs.cap[1].add | (regs.cap[1].source << 1) |
              (regs.cap[1].oneshot << 2) | (regs.cap[1].bits8 << 3) |
...
u32 SPU_struct::ReadLong(u32 addr) {
  ...
  if ((addr & 0x0F00) == 0x0400) {
    ...
    case 0x0:
      return (thischan.vol | (thischan.volumeDiv << 8) | (thischan.hold << 15) |
              (thischan.pan << 16) | (thischan.waveduty << 24) |
              (thischan.repeat << 27) | (thischan.format << 29) |
              ((thischan.status == CHANSTAT_PLAY) ? (1 << 31) : 0));
    case 0x8:
      return (thischan.timer | (thischan.loopstart << 16));
    } // switch on individual channel regs
    return 0;
  }
  ...
  // SNDCAP0DAD
  case 0x510:
    return regs.cap[0].dad;

  // SNDCAP0LEN
  case 0x514:
    return (u32)regs.cap[0].len;

  // SNDCAP1DAD
  case 0x518:
    return regs.cap[1].dad;

  // SNDCAP1LEN
  case 0x51C:
    return (u32)regs.cap[1].len;
  } // switch on address

  return 0;
}

SPUFifo::SPUFifo() { reset(); }

void SPUFifo::reset() { head = tail = size = 0; }
...
void SPU_struct::WriteByte(u32 addr, u8 val) {
  ...
  if ((addr & 0x0F00) == 0x0400) {
    ...
    case 0x3:
      ...
      break;
    case 0x4:
      thischan.addr &= 0xFFFFFF00;
      thischan.addr |= (val & 0xFC);
      break;
    case 0x5:
      thischan.addr &= 0xFFFF00FF;
      thischan.addr |= (val << 8);
      break;
    case 0x6:
      thischan.addr &= 0xFF00FFFF;
      thischan.addr |= (val << 16);
      break;
    case 0x7:
      thischan.addr &= 0x00FFFFFF;
      thischan.addr |= ((val & 7) << 24);
      break; // only 27 bits of this register are used
    case 0x8:
      thischan.timer &= 0xFF00;
      thischan.timer |= (val << 0);
      adjust_channel_timer(&thischan);
      break;
    case 0x9:
      thischan.timer &= 0x00FF;
      thischan.timer |= (val << 8);
      adjust_channel_timer(&thischan);
      break;

    case 0xA:
      thischan.loopstart &= 0xFF00;
      thischan.loopstart |= (val << 0);
      break;
    case 0xB:
      thischan.loopstart &= 0x00FF;
      thischan.loopstart |= (val << 8);
      break;
    case 0xC:
      thischan.length &= 0xFFFFFF00;
      thischan.length |= (val << 0);
      break;
    case 0xD:
      thischan.length &= 0xFFFF00FF;
      thischan.length |= (val << 8);
      break;
    case 0xE:
      thischan.length &= 0xFF00FFFF;
      thischan.length |=
          ((val & 0x3F) << 16); // only 22 bits of this register are used
    case 0xF:
      break;
  ...
  case 0x501:
    ...
    // from r4925 - after changing 'masteren', we retrigger any sounds?
    // doubtful. maybe we STOP sounds here, but we don't enable them (this would
    // retrigger any previous sounds that had finished; glitched AC:WW)
  ...
  // SOUNDBIAS
  case 0x504:
    regs.soundbias &= 0xFF00;
    regs.soundbias |= (val << 0);
    break;
  case 0x505:
    regs.soundbias &= 0x00FF;
    regs.soundbias |= ((val & 3) << 8);
    break;
  ...
  // SNDCAP0DAD
  case 0x510:
    regs.cap[0].dad &= 0xFFFFFF00;
    regs.cap[0].dad |= (val & 0xFC);
    break;
  case 0x511:
    regs.cap[0].dad &= 0xFFFF00FF;
    regs.cap[0].dad |= (val << 8);
    break;
  case 0x512:
    regs.cap[0].dad &= 0xFF00FFFF;
    regs.cap[0].dad |= (val << 16);
    break;
  case 0x513:
    regs.cap[0].dad &= 0x00FFFFFF;
    regs.cap[0].dad |= ((val & 7) << 24);
    break;

  // SNDCAP0LEN
  case 0x514:
    regs.cap[0].len &= 0xFF00;
    regs.cap[0].len |= (val << 0);
    break;
  case 0x515:
    regs.cap[0].len &= 0x00FF;
    regs.cap[0].len |= (val << 8);
    break;

  // SNDCAP1DAD
  case 0x518:
    regs.cap[1].dad &= 0xFFFFFF00;
    regs.cap[1].dad |= (val & 0xFC);
    break;
  case 0x519:
    regs.cap[1].dad &= 0xFFFF00FF;
    regs.cap[1].dad |= (val << 8);
    break;
  case 0x51A:
    regs.cap[1].dad &= 0xFF00FFFF;
    regs.cap[1].dad |= (val << 16);
    break;
  case 0x51B:
    regs.cap[1].dad &= 0xFF000000;
    regs.cap[1].dad |= ((val & 7) << 24);
    break;

  // SNDCAP1LEN
  case 0x51C:
    regs.cap[1].len &= 0xFF00;
    regs.cap[1].len |= (val << 0);
    break;
  case 0x51D:
    regs.cap[1].len &= 0x00FF;
    regs.cap[1].len |= (val << 8);
    break;
  } // switch on address
}
...
void SPU_struct::WriteWord(u32 addr, u16 val) {
  ...
  if ((addr & 0x0F00) == 0x0400) {
    ...
    case 0x2:
      ...
      break;
    case 0x4:
      thischan.addr &= 0xFFFF0000;
      thischan.addr |= (val & 0xFFFC);
      break;
    case 0x6:
      thischan.addr &= 0x0000FFFF;
      thischan.addr |= ((val & 0x07FF) << 16);
      break;
    case 0x8:
      thischan.timer = val;
      adjust_channel_timer(&thischan);
    ...
    case 0xA:
      ...
      break;
    case 0xC:
      thischan.length &= 0xFFFF0000;
      thischan.length |= (val << 0);
      break;
    case 0xE:
      thischan.length &= 0x0000FFFF;
      thischan.length |= ((val & 0x003F) << 16);
      break;
    } // switch on individual channel regs
    return;
  }
  ...
  // SNDCAP0DAD
  case 0x510:
    regs.cap[0].dad &= 0xFFFF0000;
    regs.cap[0].dad |= (val & 0xFFFC);
    break;
  case 0x512:
    regs.cap[0].dad &= 0x0000FFFF;
    regs.cap[0].dad |= ((val & 0x07FF) << 16);
    break;
  ...
  // SNDCAP1DAD
  case 0x518:
    regs.cap[1].dad &= 0xFFFF0000;
    regs.cap[1].dad |= (val & 0xFFFC);
    break;
  case 0x51A:
    regs.cap[1].dad &= 0x0000FFFF;
    regs.cap[1].dad |= ((val & 0x07FF) << 16);
    break;
...
void SPU_struct::WriteLong(u32 addr, u32 val) {
  ...
  if ((addr & 0x0F00) == 0x0400) {
    ...
    case 0xC:
      thischan.length = (val & 0x003FFFFF);
  ...
  // SNDCAP0DAD
  case 0x510:
    regs.cap[0].dad = (val & 0x07FFFFFC);
    break;

  // SNDCAP0LEN
  case 0x514:
    regs.cap[0].len = (val & 0xFFFF);
    break;

  // SNDCAP1DAD
  case 0x518:
    regs.cap[1].dad = (val & 0x07FFFFFC);
    break;

  // SNDCAP1LEN
  case 0x51C:
    regs.cap[1].len = (val & 0xFFFF);
    break;
  } // switch on address
}
...
static FORCEINLINE s32 Interpolate(s32 a, s32 b, double ratio) {
  ...
  case SPUInterpolation_Cosine:
    ...
    return s32floor(
        (cos_lut[(unsigned int)(ratio *
                                (double)COSINE_INTERPOLATION_RESOLUTION)] *
         (sampleB - sampleA)) +
        sampleA);
    break;
...
template <SPUInterpolationMode INTERPOLATE_MODE>
static FORCEINLINE void Fetch16BitData(const channel_struct *const chan,
                                       s32 *data) {
  if (chan->sampcnt < 0) {
    *data = 0;
    return;
...
template <SPUInterpolationMode INTERPOLATE_MODE>
static FORCEINLINE void FetchADPCMData(channel_struct *const chan,
                                       s32 *const data) {
  if (chan->sampcnt < 8) {
    *data = 0;
    return;
  ...
  if (INTERPOLATE_MODE != SPUInterpolation_None)
    *data = Interpolate<INTERPOLATE_MODE>((s32)chan->pcm16b_last,
                                          (s32)chan->pcm16b, chan->sampcnt);
  else
  ...
  if (chan->sampcnt > chan->double_totlength_shifted) {
    ...
    if (chan->repeat == 1) {
      while (chan->sampcnt > chan->double_totlength_shifted)
        chan->sampcnt -=
            chan->double_totlength_shifted - (double)(chan->loopstart << shift);
...
static FORCEINLINE void TestForLoop2(SPU_struct *SPU, channel_struct *chan) {
  ...
  if (chan->sampcnt > chan->double_totlength_shifted) {
    ...
    if (chan->repeat == 1) {
      double step =
          (chan->double_totlength_shifted - (double)(chan->loopstart << 3));
...
FORCEINLINE static void SPU_Mix(SPU_struct *SPU, channel_struct *chan,
                                s32 data) {
  switch (CHANNELS) {
  case 0:
    MixL(SPU, chan, data);
...
FORCEINLINE static void ____SPU_ChanUpdate(SPU_struct *const SPU,
                                           channel_struct *const chan) {
...
FORCEINLINE static void ___SPU_ChanUpdate(const bool actuallyMix,
...
template <SPUInterpolationMode INTERPOLATE_MODE>
FORCEINLINE static void __SPU_ChanUpdate(const bool actuallyMix,
                                         SPU_struct *const SPU,
                                         channel_struct *const chan) {
  switch (chan->format) {
  case 0:
...
FORCEINLINE static void _SPU_ChanUpdate(const bool actuallyMix,
                                        SPU_struct *const SPU,
                                        channel_struct *const chan) {
...
// ENTERNEW
static void SPU_MixAudio_Advanced(bool actuallyMix, SPU_struct *SPU,
                                  int length) {
  // the advanced spu function correctly handles all sound control mixing
  // options, as well as capture this code is not entirely optimal, as it relies
  // on sort of manhandling the core mixing functions in order to get the
  // results it needs.
  ...
  for (int samp = 0; samp < length; samp++) {
    ...
    for (int i = 0; i < 16; i++) {
      ...
      if (chan->status == CHANSTAT_PLAY) {
        ...
        // clear the output buffer since this is where _SPU_ChanUpdate wants to
        // accumulate things
    ...
    case SPU_struct::REGS::LOM_CH1:
      sndout[0] = submix[1 * 2 + 0];
      break;
    case SPU_struct::REGS::LOM_CH3:
      sndout[0] = submix[3 * 2 + 0];
      break;
    case SPU_struct::REGS::LOM_CH1_PLUS_CH3:
      sndout[0] = submix[1 * 2 + 0] + submix[3 * 2 + 0];
      break;
    }
    ...
    case SPU_struct::REGS::ROM_CH1:
      sndout[1] = submix[1 * 2 + 1];
      break;
    case SPU_struct::REGS::ROM_CH3:
      sndout[1] = submix[3 * 2 + 1];
      break;
    case SPU_struct::REGS::ROM_CH1_PLUS_CH3:
      sndout[1] = submix[1 * 2 + 1] + submix[3 * 2 + 1];
      break;
    }
    ...
    for (int capchan = 0; capchan < 2; capchan++) {
      if (SPU->regs.cap[capchan].runtime.running) {
        ...
        for (u32 j = last; j < curr; j++) {
          ...
          // So, playback will always end up being what was captured a couple of
          // samples ago. This system counts on playback always having read
          // ahead 16 samples. In that case, playback will end up being what was
          // processed at one entire buffer length ago, since the 16 samples
          // would have read ahead before they got captured over

          // It's actually the source channels which should have a fifo, but we
          // are not going to take the hit in speed and complexity. Save it for
          // a future rewrite. Instead, what we do here is delay the capture by
          // 16 samples to create a similar effect. Subjectively, it seems to be
          // working.
          ...
          //(actually capture sample from fifo instead of most recently
          //generated)
          u32 multiplier;
...
static void SPU_MixAudio(bool actuallyMix, SPU_struct *SPU, int length) {
  ...
  // branch here so that slow computers don't have to take the advanced (slower)
  // codepath. it remainds to be seen exactly how much slower it is if it isnt
  // much slower then we should refactor everything to be simpler, once it is
  // working
  if (advanced && SPU == SPU_core) {
    ...
    for (int i = 0; i < 16; i++) {
      ...
      // Mix audio
      _SPU_ChanUpdate(!CommonSettings.spu_muteChannels[i] && actuallyMix, SPU,
                      chan);
    }

    // zero out capture buffers - effectively transform
    // no-advanced-spu-emulation to capturing-zeroes this is needed so when the
    // option is changed (or a state with a different setting is loaded) this
    // code is bulkier and slower than it might otherwise be to reduce the
    // chance of bugs IDEALLY the non-advanced codepath would be removed (while
    // the advanced codepath was optimized and improved) and this code would
    // disappear, to be replaced with code more capable of emitting zeroes at
    // the opportune time.
    for (int capchan = 0; capchan < 2; capchan++) {
      SPU_struct::REGS::CAP &cap = SPU->regs.cap[capchan];
  ...
  // in all likelihood, any game doing this probably master disabled the SPU
  // also so, optimization of this case is probably not necessary. later, we'll
...
// this will produce a variable number of samples, calculated to keep a 44100hz
// output in sync with the emulator framerate
...
void SPU_Emulate_core() {
  ...
  if (soundProcessor->FetchSamples != NULL) {
    soundProcessor->FetchSamples(SPU_core->outbuf, spu_core_samples, synchmode,
                                 synchronizer);
  } else {
    SPU_DefaultFetchSamples(SPU_core->outbuf, spu_core_samples, synchmode,
                            synchronizer);
  }
}
...
void SPU_Emulate_user(bool mix) {
  ...
  if (postProcessBufferSize < freeSampleCount * 2 * sizeof(s16)) {
    ...
    postProcessBuffer =
        (s16 *)realloc(postProcessBuffer, postProcessBufferSize);
  }

  // printf("free %d\n", freeSampleCount);

  if (soundProcessor->PostProcessSamples != NULL) {
    processedSampleCount = soundProcessor->PostProcessSamples(
        postProcessBuffer, freeSampleCount, synchmode, synchronizer);
  } else {
    processedSampleCount = SPU_DefaultPostProcessSamples(
        postProcessBuffer, freeSampleCount, synchmode, synchronizer);
  }
...
void SPU_DefaultFetchSamples(s16 *sampleBuffer, size_t sampleCount,
...
size_t
SPU_DefaultPostProcessSamples(s16 *postProcessBuffer,
                              size_t requestedSampleCount, ESynchMode synchMode,
                              ISynchronizingAudioBuffer *theSynchronizer) {
  size_t processedSampleCount = 0;
  ...
  case ESynchMode_DualSynchAsynch:
    if (SPU_user != NULL) {
      ...
      memcpy(postProcessBuffer, SPU_user->outbuf,
             requestedSampleCount * 2 * sizeof(s16));
      processedSampleCount = requestedSampleCount;
    }
    break;

  case ESynchMode_Synchronous:
    processedSampleCount = theSynchronizer->output_samples(
        postProcessBuffer, requestedSampleCount);
    break;
...
void SNDDummyFetchSamples(s16 *sampleBuffer, size_t sampleCount,
                          ESynchMode synchMode,
                          ISynchronizingAudioBuffer *theSynchronizer);
size_t SNDDummyPostProcessSamples(s16 *postProcessBuffer,
                                  size_t requestedSampleCount,
                                  ESynchMode synchMode,
                                  ISynchronizingAudioBuffer *theSynchronizer);

SoundInterface_struct SNDDummy = {
    SNDCORE_DUMMY,       "Dummy Sound Interface", SNDDummyInit,
    SNDDummyDeInit,      SNDDummyUpdateAudio,     SNDDummyGetAudioSpace,
    SNDDummyMuteAudio,   SNDDummyUnMuteAudio,     SNDDummySetVolume,
    SNDDummyClearBuffer, SNDDummyFetchSamples,    SNDDummyPostProcessSamples};
...
WavWriter::WavWriter() : spufp(NULL) {}
...
bool WavWriter::isRecording() const { return spufp != NULL; }

static WavWriter wavWriter;

void WAV_End() { wavWriter.close(); }
...
bool spu_loadstate(EMUFILE &is, int size) {
  // note! if we load a state created with advanced spu logic on a system
  // without it, there's a high likelihood of captured data existing. this would
  // get played back forever without being replaced by captured data. it's been
  // solved by capturing zeroes though even when advanced spu logic is disabled.
  ...
  for (int j = 0; j < 16; j++) {
    ...
    chan.double_totlength_shifted =
        (double)(chan.totlength << format_shift[chan.format]);
    // printf("%f\n",chan.double_totlength_shifted);
    if (version >= 2) {
  ...
  if (version >= 6)
    for (int i = 0; i < 2; i++)
      spu->regs.cap[i].runtime.fifo.load(is);
  else
    for (int i = 0; i < 2; i++)
      spu->regs.cap[i].runtime.fifo.reset();
