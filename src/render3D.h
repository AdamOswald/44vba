...
enum Render3DErrorCode { RENDER3DERROR_NOERR = 0 };
...
protected:
  ...
  void _Upscale(const u32 *__restrict src, u32 *__restrict dst);
...
protected:
  ...
  CACHE_ALIGN u32 clearImageDepthBuffer[GPU_FRAMEBUFFER_NATIVE_WIDTH *
                                        GPU_FRAMEBUFFER_NATIVE_HEIGHT];
  CACHE_ALIGN u8 clearImageFogBuffer[GPU_FRAMEBUFFER_NATIVE_WIDTH *
                                     GPU_FRAMEBUFFER_NATIVE_HEIGHT];
  ...
  FlushFramebuffer(const FragmentColor *__restrict srcFramebuffer,
...
public:
  ...
  virtual Render3DError
  Render(const GFX3D &engine); // Called when the renderer should do its job and
                               // render the current display lists.

  virtual Render3DError
  RenderFinish(); // Called whenever 3D rendering needs to finish. This function
                  // should block the calling thread
  // and only release the block when 3D rendering is finished. (Before reading
  // the 3D layer, be sure to always call this function.)

  virtual Render3DError
  RenderFlush(bool willFlushBuffer32,
              bool willFlushBuffer16); // Called whenever the emulator needs the
                                       // flushed results of the 3D renderer.
                                       // Before calling this,
  // the 3D renderer must be finished using RenderFinish() or confirmed already
  // finished using GetRenderNeedsFinish().
  ...
  virtual NDSColorFormat RequestColorFormat(
      NDSColorFormat colorFormat); // Called whenever the output framebuffer
                                   // color format changes. The framebuffer
  // output by the 3D renderer is expected to match the requested format. If the
  // internal color format of the 3D renderer doesn't natively match the
  // requested format, then a colorspace conversion will be required in order to
  // match. The only exception to this rule is if the requested output format is
  // RGBA5551. In this particular case, the 3D renderer is expected to output a
  // framebuffer in RGBA6665 color format. Again, if the internal color format
  // does not match this, then a colorspace conversion will be required for
  // RGBA6665.
...
template <size_t SIMDBYTES> class Render3D_SIMD : public Render3D {
public:
  Render3D_SIMD();
