...
Despite gbatek's specifications contrariwise, GCDATAIN is writeable. R4 uses
this for writes to the card. Despite gbatek's specifications contrariwise,
GCROMCTRL[30] can take a value of 1 to indicate that the GC bus transaction
should be writes. It is unclear what kicks GC bus transactions. It's possibly
bit 31 of GCROMCTRL, but normmatt thinks he might have had to hack around that
to make something work, and that it might just be any write to GCROMCTRL. Since
GCROMCTRL[26:24] can't represent 'data block size' of 1 or 2, it is assumed that
transactions always happen in 4 byte pieces.
...so, any 8/16bit accesses to GCDATAIN would transfer a whole 32bit unit and
then just return the requested portion
*/

// TODO - create a Slot1_TurboRom which we can select when booting from
// fakebios/nonfirmware and which would provide a useful service to people
// porting to tighter platforms by bypassing largely useless cruft
...
static void scanDir() {
  ...
  VFAT vfat;
  if (vfat.build(slot1_R4_path_type ? path.RomDirectory.c_str()
                                    : fatDir.c_str(),
                 16)) {
...
std::string slot1_GetFatDir() { return fatDir; }

EMUFILE *slot1_GetFatImage() { return fatImage; }
...
NDS_SLOT1_TYPE slot1_device_type =
    NDS_SLOT1_RETAIL_AUTO; // default for frontends that dont even configure
                           // this
NDS_SLOT1_TYPE slot1_selected_type = NDS_SLOT1_NONE;

void slot1_Init() {
  // due to sloppy initialization code in various untestable desmume ports, we
  // might try this more than once
...
bool slot1_Change(NDS_SLOT1_TYPE changeToType) {
  if ((changeToType == slot1_device_type) ||
      (changeToType == slot1_GetSelectedType()))
...
NDS_SLOT1_TYPE slot1_GetCurrentType() { return slot1_device_type; }
...
void slot1_Savestate(EMUFILE &os) { slot1_device->savestate(os); }
void slot1_Loadstate(EMUFILE &is) { slot1_device->loadstate(is); }
...
//			card.address = 0;
//			card.transfer_count = 6;
//			card.blocklen = card.command[6] | (card.command[5] << 8) |
//(card.command[4] << 16) | (card.command[3] << 24); 			NDS_makeInt(PROCNUM, 20);
//			break;

//		// READ_SINGLE_BLOCK
//		case 0x51:
//			card.address = card.command[6] | (card.command[5] << 8) |
//(card.command[4] << 16) | (card.command[3] << 24); 			card.transfer_count =
//(card.blocklen + 3) >> 2; 			NDS_makeInt(PROCNUM, 20); 			break;
//		}
//		break;
