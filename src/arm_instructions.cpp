...
// - Force User mode memory access for LDRx/STRx opcodes with bit24=0 and
// bit21=1
...
#define LSL_IMM u32 shift_op = cpu->R[REG_POS(i, 0)] << ((i >> 7) & 0x1F);
...
#define OP_ADC(a, b)                                                           \
  cpu->R[REG_POS(i, 12)] =                                                     \
      cpu->R[REG_POS(i, 16)] + shift_op + cpu->CPSR.bits.C;                    \
  if (REG_POS(i, 12) == 15) {                                                  \
    cpu->next_instruction = cpu->R[15];                                        \
    return b;                                                                  \
  }                                                                            \
...
#define OP_ADCS(a, b)                                                          \
  {                                                                            \
    ...
    cpu->CPSR.bits.Z = (cpu->R[REG_POS(i, 12)] == 0);                          \
    cpu->CPSR.bits.V =                                                         \
        BIT31((v ^ shift_op ^ -1) & (v ^ cpu->R[REG_POS(i, 12)]));             \
    return a;                                                                  \
  }
...
#define OP_RSC(a, b)                                                           \
  cpu->R[REG_POS(i, 12)] =                                                     \
      shift_op - cpu->R[REG_POS(i, 16)] + cpu->CPSR.bits.C - 1;                \
  if (REG_POS(i, 12) == 15) {                                                  \
    cpu->next_instruction = cpu->R[15];                                        \
    return b;                                                                  \
  }                                                                            \
...
#define OP_RSCS(a, b)                                                          \
  {                                                                            \
    ...
    cpu->CPSR.bits.Z = (cpu->R[REG_POS(i, 12)] == 0);                          \
    cpu->CPSR.bits.V =                                                         \
        BIT31((shift_op ^ v) & (shift_op ^ cpu->R[REG_POS(i, 12)]));           \
    return a;                                                                  \
  }
...
TEMPLATE static u32 FASTCALL OP_UMLAL(const u32 i) {
  ...
  u32 tmp = (u32)res; // low
  cpu->R[REG_POS(i, 16)] = (u32)(res >> 32) + cpu->R[REG_POS(i, 16)] +
                           CarryFrom(tmp, cpu->R[REG_POS(i, 12)]);
...
TEMPLATE static u32 FASTCALL OP_UMULL_S(const u32 i) {
  ...
  cpu->CPSR.bits.N = BIT31(cpu->R[REG_POS(i, 16)]);
  cpu->CPSR.bits.Z =
      (cpu->R[REG_POS(i, 16)] == 0) && (cpu->R[REG_POS(i, 12)] == 0);
...
TEMPLATE static u32 FASTCALL OP_SMLAL(const u32 i) {
  ...
  // LOG("%08X * %08X + %08X%08X\n", cpu->R[REG_POS(i,0)], cpu->R[REG_POS(i,8)],
  // cpu->R[REG_POS(i,16)], cpu->R[REG_POS(i,12)]);
...
#define OP_MSR_CPSR_(operand)                                                  \
  u32 byte_mask = (cpu->CPSR.bits.mode == USR)                                 \
...
TEMPLATE static u32 FASTCALL OP_CLZ(const u32 i) {
  ...
  pos = CLZ_TAB[Rm & 0xF] + CLZ_TAB[(Rm >> 4) & 0xF] +
        CLZ_TAB[(Rm >> 8) & 0xF] + CLZ_TAB[(Rm >> 12) & 0xF] +
        CLZ_TAB[(Rm >> 16) & 0xF] + CLZ_TAB[(Rm >> 20) & 0xF] +
        CLZ_TAB[(Rm >> 24) & 0xF] + CLZ_TAB[(Rm >> 28) & 0xF];
...
TEMPLATE static u32 FASTCALL OP_SMUL_B_B(const u32 i) {
  ...
  // INFO("SMUL_B_B\n");
  cpu->R[REG_POS(i, 16)] =
      (u32)(LWORD(cpu->R[REG_POS(i, 0)]) * LWORD(cpu->R[REG_POS(i, 8)]));
...
TEMPLATE static u32 FASTCALL OP_SMUL_B_T(const u32 i) {
  // INFO("SMUL_B_T\n");
  cpu->R[REG_POS(i, 16)] =
      (u32)(LWORD(cpu->R[REG_POS(i, 0)]) * HWORD(cpu->R[REG_POS(i, 8)]));
...
TEMPLATE static u32 FASTCALL OP_SMUL_T_B(const u32 i) {
  // INFO("SMUL_T_B\n");
  cpu->R[REG_POS(i, 16)] =
      (u32)(HWORD(cpu->R[REG_POS(i, 0)]) * LWORD(cpu->R[REG_POS(i, 8)]));
...
TEMPLATE static u32 FASTCALL OP_SMUL_T_T(const u32 i) {
  // INFO("SMUL_T_T\n");
  cpu->R[REG_POS(i, 16)] =
      (u32)(HWORD(cpu->R[REG_POS(i, 0)]) * HWORD(cpu->R[REG_POS(i, 8)]));
...
TEMPLATE static u32 FASTCALL OP_SMLA_B_B(const u32 i) {
  ...
  if (OverflowFromADD(cpu->R[REG_POS(i, 16)], tmp, cpu->R[REG_POS(i, 12)]))
    cpu->CPSR.bits.Q = 1;
  // INFO("SMLABB %08X * %08X + %08X = %08X\n", (s16)cpu->R[REG_POS(i,0)],
  // (s16)cpu->R[REG_POS(i,8)], cpu->R[REG_POS(i,12)],
  // (s32)cpu->R[REG_POS(i,16)]);
...
TEMPLATE static u32 FASTCALL OP_SMLA_B_T(const u32 i) {
  ...
  // cpu->R[REG_POS(i,0)], REG_POS(i,8), cpu->R[REG_POS(i,8)], REG_POS(i,12), a,
  // tmp + a);
...
TEMPLATE static u32 FASTCALL OP_SMLAL_B_B(const u32 i) {
  ...
  s64 product = (s64)(a * b);
  s64 dst64 =
      (s64)(cpu->R[REG_POS(i, 12)] + (((s64)cpu->R[REG_POS(i, 16)]) << 32));
...
TEMPLATE static u32 FASTCALL OP_SMULW_B(const u32 i) {
  s64 tmp =
      (s64)LWORD(cpu->R[REG_POS(i, 8)]) * (s64)((s32)cpu->R[REG_POS(i, 0)]);

  // INFO("SMULWB %08X * %08X = %08X\n", cpu->R[REG_POS(i,0)],
  // cpu->R[REG_POS(i,8)], ((tmp>>16)&0xFFFFFFFF));
...
TEMPLATE static u32 FASTCALL OP_SMULW_T(const u32 i) {
  s64 tmp =
      (s64)HWORD(cpu->R[REG_POS(i, 8)]) * (s64)((s32)cpu->R[REG_POS(i, 0)]);
...
TEMPLATE static u32 FASTCALL OP_SMLAW_T(const u32 i) {
  ...
  // cpu->R[REG_POS(i,8)], a, ((tmp>>16)&0xFFFFFFFF) + a);
...
TEMPLATE static u32 FASTCALL OP_LDMIA(const u32 i) {
  ...
  if (BIT15(i)) {
    ...
    // TODO
    // The general-purpose registers loaded can include the PC. If they do, the
    // word loaded for the PC is treated as an address and a branch occurs to
    // that address. In ARMv5 and above, bit[0] of the loaded value determines
    // whether execution continues after this branch in ARM state or in Thumb
    // state, as though a BX (loaded_value) instruction had been executed (but
    // see also The T and J bits on page A2-15 for operation on non-T variants
    // of ARMv5). In earlier versions of the architecture, bits[1:0] of the
    // loaded value are ignored and execution continues in ARM state, as though
    // the instruction MOV PC,(loaded_value) had been executed.
    //
...
//-----------------------------------------------------------------------------
//   STC
//   the NDS has no coproc that responses to a STC, no feedback is given to the
//   arm
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//   LDC
//   the NDS has no coproc that responses to a LDC, no feedback is given to the
//   arm
//-----------------------------------------------------------------------------
...
TEMPLATE static u32 FASTCALL OP_MCR(const u32 i) {
  ...
  if (cpnum != 15) {
    // emu_halt();
    // INFO("Stopped (OP_MCR) \n");
    INFO("ARM%c: MCR P%i, 0, R%i, C%i, C%i, %i, %i (don't allocated "
         "coprocessor)\n",
         PROCNUM ? '7' : '9', cpnum, REG_POS(i, 12), REG_POS(i, 16),
         REG_POS(i, 0), (i >> 21) & 0x7, (i >> 5) & 0x7);
    return 2;
  }

  cp15.moveARM2CP(cpu->R[REG_POS(i, 12)], REG_POS(i, 16), REG_POS(i, 0),
                  (i >> 21) & 0x7, (i >> 5) & 0x7);
...
TEMPLATE static u32 FASTCALL OP_MRC(const u32 i) {
  ...
  if (cpnum != 15) {
    // emu_halt();
    // INFO("Stopped (OP_MRC) \n");
    INFO("ARM%c: MRC P%i, 0, R%i, C%i, C%i, %i, %i (don't allocated "
         "coprocessor)\n",
         PROCNUM ? '7' : '9', cpnum, REG_POS(i, 12), REG_POS(i, 16),
         REG_POS(i, 0), (i >> 21) & 0x7, (i >> 5) & 0x7);
    return 2;
  ...
  u32 data = 0;
  cp15.moveCP2ARM(&data, REG_POS(i, 16), REG_POS(i, 0), (i >> 21) & 0x7,
                  (i >> 5) & 0x7);
  if (REG_POS(i, 12) == 15) {
  ...
  // cpu->coproc[cpnum]->moveCP2ARM(&cpu->R[REG_POS(i, 12)], REG_POS(i, 16),
  // REG_POS(i, 0), (i>>21)&7, (i>>5)&7);
  return 4;
}
...
TEMPLATE static u32 FASTCALL OP_SWI(const u32 i) {
  ...
  // ideas-style debug prints (execute this SWI with the null terminated string
  // address in R0)
  if (swinum == 0xFC) {
    IdeasLog(cpu);
    return 0;
  ...
  bool bypassBuiltinSWI = (cpu->intVector == 0x00000000 && PROCNUM == 0) ||
                          (cpu->intVector == 0xFFFF0000 && PROCNUM == 1);

  // printf("ARM%c SWI %02X\t; %s\n", PROCNUM?'7':'9', (swinum & 0x1F),
  // ARM_swi_names[PROCNUM][(swinum & 0x1F)]);
