...
// this class will also eventually try to take over the responsibility of using
// the args that it handles for example: preparing the emulator run by loading
// the rom, savestate, and/or movie in the correct pattern. it should also
// populate CommonSettings with its initial values EDIT: not really. combining
// this with what a frontend wants to do is complicated. you might design the
// API so that the frontend sets all those up, but I'm not sure I like that
// Really, this should be a passive structure that just collects the results
// provided by the shared command line processing, to be used later as
// appropriate (and the CommonSettings setup REMOVED or at least refactored into
//a separate method)
...
public:
  ...
  // print a little help message for cases when erroneous commandlines are
  // entered
