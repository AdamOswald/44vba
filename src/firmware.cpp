...
#define SETTINGS_SIZE                                                          \
  (sizeof(FWUserSettings) + WIFI_SETTINGS_SIZE + WIFI_AP_SETTINGS_SIZE)
...
u16 CFIRMWARE::_getBootCodeCRC16(const u8 *arm9Data, const u32 arm9Size,
                                 const u8 *arm7Data, const u32 arm7Size) {
  unsigned int i, j;
  u32 crc = 0xFFFF;
  const u16 val[8] = {0xC0C1, 0xC181, 0xC301, 0xC601,
...
bool CFIRMWARE::load(const char *firmwareFilePath) {
  ...
  this->_isLoaded = NDS_ReadFirmwareDataFromFile(
      firmwareFilePath, newFirmwareData, &fileSize, NULL, NULL);
  if (!this->_isLoaded) {
  ...
  this->_userDataAddr =
      LE_TO_LOCAL_16(newFirmwareData->header.userSettingsOffset) * 8;

  // fix bad dump of firmware? (wrong DS type)
  // fix mario kart touch screen calibration
  if ((newFirmwareData->header.key.unused != 0xFFFF) &&
      (newFirmwareData->header.key.consoleType == NDS_CONSOLE_TYPE_IQUE_LITE)) {
...
bool CFIRMWARE::unpack() {
  ...
  part1ram =
      (0x02800000 - (this->_header.part1_ram_boot9_addr << (2 + shift2)));
  part2addr = (this->_header.part2_rom_boot7_addr << (2 + shift3));
  part2ram =
      (0x03810000 - (this->_header.part2_ram_boot7_addr << (2 + shift4)));
  ...
  u16 crc16_mine =
      this->_getBootCodeCRC16(tmp_data9, arm9Size, tmp_data7, arm7Size);

  if (crc16_mine != this->_header.part12_boot_crc16) {
    INFO("Firmware: ERROR: the boot code CRC16 (0x%04X) doesn't match the "
         "value in the firmware header (0x%04X)",
         crc16_mine, this->_header.part12_boot_crc16);
    delete[] tmp_data9;
    delete[] tmp_data7;
    delete workingFirmwareData;
    return false;
  ...
  if (isPatched) {
    ...
    memcpy(&this->_header, &workingFirmwareData->_raw[patch_offset],
           sizeof(this->_header));
    ...
    arm9Size =
        this->_decompress(&workingFirmwareData->_raw[part1addr], tmp_data9);
    if (tmp_data9 == NULL) {
      delete workingFirmwareData;
      return false;
    }
    ...
    arm7Size =
        this->_decompress(&workingFirmwareData->_raw[part2addr], tmp_data7);
    if (tmp_data7 == NULL) {
      delete[] tmp_data9;
      delete workingFirmwareData;
      return false;
...
bool CFIRMWARE::loadSettings(const char *firmwareUserSettingsFilePath) {
  return NDS_ApplyFirmwareSettingsWithFile(&MMU.fw.data,
                                           firmwareUserSettingsFilePath);
}

bool CFIRMWARE::saveSettings(const char *firmwareUserSettingsFilePath) {
  if ((firmwareUserSettingsFilePath == NULL) ||
      (strlen(firmwareUserSettingsFilePath) == 0)) {
    return false;
  }
  ...
  if (userSettings1.updateCounter ==
      ((LE_TO_LOCAL_16(userSettings0.updateCounter) + 1) & 0x7F)) {
  ...
  if (fp) {
    ...
    if (usr) {
      ...
      memcpy(usr + DFC_ID_SIZE + sizeof(FWUserSettings), &MMU.fw.data.wifiInfo,
             WIFI_SETTINGS_SIZE);
      memcpy(usr + DFC_ID_SIZE + sizeof(FWUserSettings) + WIFI_SETTINGS_SIZE,
             &MMU.fw.data.wifiAP1, WIFI_AP_SETTINGS_SIZE);
...
std::string CFIRMWARE::GetUserSettingsFilePath(const char *firmwareFilePath) {
  ...
  std::string finalPath = configPath + DIRECTORY_DELIMITER_CHAR + fwFileName +
                          FILE_EXT_DELIMITER_CHAR + FW_CONFIG_FILE_EXT;

  return finalPath;
}

bool CFIRMWARE::loaded() { return this->_isLoaded; }
...
void *CFIRMWARE::getTouchCalibrate() {
  ...
  if (!this->_isLoaded || !CommonSettings.UseExtFirmware) {
    ...
    FWUserSettings &userSettings =
        (FWUserSettings &)MMU.fw.data._raw[this->_userDataAddr];
...
void NDS_GetDefaultFirmwareConfig(FirmwareConfig &outConfig) {
  ...
  outConfig.tscPixel_y1 =
      0x20 + 1; // either that or NDS_getADCTouchPosX/Y are wrong.
  // VALUES FROM NOCASH
...
bool NDS_ApplyFirmwareSettingsWithFile(NDSFirmwareData *outFirmware,
                                       const char *inFileName) {
  bool didReadExtFirmwareData = false;
  u8 *extFirmwareData = NULL;
  size_t fileSize = 0;

  if ((outFirmware == NULL) || (inFileName == NULL) ||
      (strlen(inFileName) == 0)) {
  ...
  if (fread(extFirmwareData, 1, DFC_ID_SIZE, fp) == DFC_ID_SIZE) {
    if (memcmp(extFirmwareData, DFC_ID_CODE, DFC_ID_SIZE) == 0) {
      const size_t readSettingsSize =
          fread(extFirmwareData, 1, SETTINGS_SIZE, fp);
  ...
  if (didReadExtFirmwareData) {
    // Need a temp struct here because WIFI_SETTINGS_SIZE is smaller than
    // sizeof(FWWifiInfo).
    FWWifiInfo wifiInfo = outFirmware->wifiInfo;
    memcpy(&wifiInfo, extFirmwareData + sizeof(FWUserSettings),
           WIFI_SETTINGS_SIZE);

    NDS_ApplyFirmwareSettings(
        outFirmware, NULL, (FWUserSettings *)extFirmwareData,
        (FWUserSettings *)extFirmwareData, &wifiInfo,
        (FWAccessPointSettings *)(extFirmwareData + sizeof(FWUserSettings) +
                                  WIFI_SETTINGS_SIZE),
        NULL, NULL);
...
void NDS_ApplyFirmwareSettingsWithConfig(NDSFirmwareData *outFirmware,
                                         const FirmwareConfig &inConfig) {
  if (outFirmware == NULL) {
    return;
  }
  ...
  userSettings0.crc16 =
      LOCAL_TO_LE_16((u16)calc_CRC16(0xFFFF, &userSettings0, 0x70));
  userSettings1.crc16 =
      LOCAL_TO_LE_16((u16)calc_CRC16(0xFFFF, &userSettings1, 0x70));
  ...
  // Wifi settings CRC16
  wifiInfo.crc16 =
      LOCAL_TO_LE_16((u16)calc_CRC16(0, &wifiInfo.length, wifiInfo.length));
  ...
  // WiFi profiles
  if (((*(u32 *)inConfig.ipv4Address_AP1 != 0) &&
       (*(u32 *)inConfig.ipv4Gateway_AP1 != 0) &&
       (inConfig.subnetMask_AP1 != 0)) ||
  ...
  if (((*(u32 *)inConfig.ipv4Address_AP2 != 0) &&
       (*(u32 *)inConfig.ipv4Gateway_AP2 != 0) &&
       (inConfig.subnetMask_AP2 != 0)) ||
  ...
  if (((*(u32 *)inConfig.ipv4Address_AP3 != 0) &&
       (*(u32 *)inConfig.ipv4Gateway_AP3 != 0) &&
       (inConfig.subnetMask_AP3 != 0)) ||
  ...
  NDS_ApplyFirmwareSettings(outFirmware, &headerKey, &userSettings0,
                            &userSettings1, &wifiInfo, &wifiAP1, &wifiAP2,
                            &wifiAP3);
}
...
void NDS_InitDefaultFirmware(NDSFirmwareData *outFirmware) {
  ...
  outFirmware->header.userSettingsOffset =
      LOCAL_TO_LE_16((u16)(offsetof(NDSFirmwareData, userSettings0) / 8));
  ...
  // Since we don't support DSi at the moment, we just fill this area with the
  // default NDS data of 0xFF.
  memset(outFirmware->userSettings0.nds.unused5, 0xFF,
         sizeof(outFirmware->userSettings0.nds.unused5));
  ...
  memcpy(outFirmware->wifiInfo.Type2.rfChannelValue24, FW_RFChannel,
         sizeof(FW_RFChannel));
  memcpy(outFirmware->wifiInfo.Type2.bbChannelValue8, FW_BBChannel,
         sizeof(FW_BBChannel));
  ...
  memset(outFirmware->wifiInfo.unused5, 0xFF,
         sizeof(outFirmware->wifiInfo.unused5));

  NDS_ApplyFirmwareSettingsWithConfig(outFirmware, defaultConfig);
}

bool NDS_ReadFirmwareDataFromFile(const char *fileName,
                                  NDSFirmwareData *outFirmware,
                                  size_t *outFileSize, int *outConsoleType,
                                  u8 *outMACAddr) {
  bool result = false;
  ...
  if ((readBytes != sizeof(u32)) ||
      ((identifierValue & 0x00FFFFFF) != 0x0043414D)) {
    fclose(fp);
    return result;
  }

  // Now that the firmware file has been validated, let's start reading the
  // individual pieces of data that the caller has requested.
  result = true;
  ...
  if (outFirmware != NULL) {
    ...
    if (readBytes == sizeof(NDSFirmwareData)) {
      ...
      printf("Ext. Firmware: Failed to read the firmware data. (%zu out of %zu "
             "bytes read.)\n",
    ...
    if (outConsoleType != NULL) {
      ...
      if (readBytes == sizeof(FW_HEADER_KEY)) {
        *outConsoleType = (int)fpKey.consoleType;
        ...
        printf("Ext. Firmware: Failed to read the console type. (%zu out of "
               "%zu bytes read.)\n",
    ...
    if (outMACAddr != NULL) {
      ...
      if (readBytes != 6) {
        printf("Ext. Firmware: Failed to read the MAC address. (%zu out of %zu "
               "bytes read.)\n",
...
// the original intention was for this code to have similarity to the AUXSPI
// backup memory devices. however, that got overgrown, and this stuff stayed
// pretty simple. perhaps it can be re-defined in terms of a simpler AUXSPI
// device after the AUXSPI devices are re-designed.
...
void fw_reset_com(fw_memory_chip *mc) {
  if (mc->com == FW_CMD_PAGEWRITE) {
    if (mc->isFirmware && CommonSettings.UseExtFirmware &&
        CommonSettings.UseExtFirmwareSettings && (extFirmwareObj != NULL)) {
...
u8 fw_transfer(fw_memory_chip *mc, u8 data) {
  if (mc->com == FW_CMD_READ ||
      mc->com == FW_CMD_PAGEWRITE) /* check if we are in a command that needs 3
                                      bytes address */
  {
    if (mc->addr_shift > 0) /* if we got a complete address */
    {
      mc->addr_shift--;
      mc->addr |=
          data << (mc->addr_shift * 8); /* argument is a byte of address */
    } else /* if we have received 3 bytes of address, proceed command */
    {
      ...
      case FW_CMD_PAGEWRITE:
        if (mc->addr < mc->size) {
          if ((mc->addr >= offsetof(NDSFirmwareData, wifiAP1.wfcUserID[0])) &&
              (mc->addr <= offsetof(NDSFirmwareData, wifiAP1.wfcUserID[5]))) {
            CommonSettings.fwConfig
                .WFCUserID[mc->addr - offsetof(NDSFirmwareData,
                                               wifiAP1.wfcUserID[0])] = data;
          }
    ...
    // but we chose to use an ST from martin's ds fat string so programs might
    // have a clue as to the firmware size: 20 40 12
    case 0:
      data = 0x20;
      mc->addr = 1;
      break;
    case 1:
      data = 0x40; // according to gbatek this is the device ID for the flash on
                   // someone's ds fat
      mc->addr = 2;
      break;
    case 2:
...
void mc_free(fw_memory_chip *mc) { mc_init(mc, MC_TYPE_AUTODETECT); }
