...
static void install_post_exec_fn(void *instance,
...
void armcpu_t::SetControlInterface(
    const armcpu_ctrl_iface *theControlInterface) {
  this->ctrl_iface = *theControlInterface;
}

armcpu_ctrl_iface *armcpu_t::GetControlInterface() { return &this->ctrl_iface; }

void armcpu_t::SetControlInterfaceData(void *theData) {
  this->ctrl_iface.data = theData;
}

void *armcpu_t::GetControlInterfaceData() { return this->ctrl_iface.data; }

void armcpu_t::SetCurrentMemoryInterface(
    armcpu_memory_iface *theMemoryInterface) {
...
void armcpu_t::SetBaseMemoryInterface(
    const armcpu_memory_iface *theMemInterface) {
...
void *armcpu_t::GetBaseMemoryInterfaceData() { return this->base_mem_if.data; }
...
// call this whenever CPSR is changed (other than CNVZQ or T flags); interrupts
// may need to be unleashed
void armcpu_t::changeCPSR() {
  // but all it does is give them a chance to unleash by forcing an immediate
  // reschedule
  // TODO - we could actually set CPSR through here and look for a change in the
  // I bit that would be a little optimization as well as a safety measure if we
  // prevented setting CPSR directly
  NDS_Reschedule();
}
...
void armcpu_init(armcpu_t *armcpu, u32 adr) {
  ...
  armcpu->LDTBit = (armcpu->proc_ID ==
                    0); // set ARMv5 style bit--different for each processor
  ...
  armcpu->R8_fiq = armcpu->R9_fiq = armcpu->R10_fiq = armcpu->R11_fiq =
      armcpu->R12_fiq = armcpu->R13_fiq = armcpu->R14_fiq = 0;

  armcpu->SPSR_svc.val = armcpu->SPSR_abt.val = armcpu->SPSR_und.val =
      armcpu->SPSR_irq.val = armcpu->SPSR_fiq.val = 0;
...
template <u32 PROCNUM> FORCEINLINE static u32 armcpu_prefetch() {
  ...
  if (armcpu->CPSR.bits.T == 0) {
    ...
    // #else
    curInstruction &=
        0xFFFFFFFC; // please don't change this to 0x0FFFFFFC -- the NDS will
                    // happily run on 0xF******* addresses all day long
    // please note that we must setup R[15] before reading the instruction since
    // there is a protection which prevents PC > 0x3FFF from reading the bios
    // region
  ...
  // #else
  curInstruction &=
      0xFFFFFFFE; // please don't change this to 0x0FFFFFFE -- the NDS will
                  // happily run on 0xF******* addresses all day long
  // please note that we must setup R[15] before reading the instruction since
  // there is a protection which prevents PC > 0x3FFF from reading the bios
  // region
...
#if 0 /* not used */
static BOOL FASTCALL test_EQ(Status_Reg CPSR) {
    return ( CPSR.bits.Z);
}
static BOOL FASTCALL test_NE(Status_Reg CPSR) {
    return (!CPSR.bits.Z);
}
static BOOL FASTCALL test_CS(Status_Reg CPSR) {
    return ( CPSR.bits.C);
}
static BOOL FASTCALL test_CC(Status_Reg CPSR) {
    return (!CPSR.bits.C);
}
static BOOL FASTCALL test_MI(Status_Reg CPSR) {
    return ( CPSR.bits.N);
}
static BOOL FASTCALL test_PL(Status_Reg CPSR) {
    return (!CPSR.bits.N);
}
static BOOL FASTCALL test_VS(Status_Reg CPSR) {
    return ( CPSR.bits.V);
}
static BOOL FASTCALL test_VC(Status_Reg CPSR) {
    return (!CPSR.bits.V);
}
static BOOL FASTCALL test_HI(Status_Reg CPSR) {
    return (CPSR.bits.C) && (!CPSR.bits.Z);
}
static BOOL FASTCALL test_LS(Status_Reg CPSR) {
    return (CPSR.bits.Z) || (!CPSR.bits.C);
}
static BOOL FASTCALL test_GE(Status_Reg CPSR) {
    return (CPSR.bits.N==CPSR.bits.V);
}
static BOOL FASTCALL test_LT(Status_Reg CPSR) {
    return (CPSR.bits.N!=CPSR.bits.V);
}
static BOOL FASTCALL test_GT(Status_Reg CPSR) {
    return (!CPSR.bits.Z) && (CPSR.bits.N==CPSR.bits.V);
}
static BOOL FASTCALL test_LE(Status_Reg CPSR) {
    return ( CPSR.bits.Z) || (CPSR.bits.N!=CPSR.bits.V);
}
static BOOL FASTCALL test_AL(Status_Reg CPSR) {
    return 1;
}
...
#define TEST_COND2(cond, CPSR) (cond < 15 && test_conditions[cond](CPSR))
#endif
...
void armcpu_exception(armcpu_t *cpu, u32 number) {
  ...
  case EXCEPTION_RESERVED_0x14:
    emu_halt(EMUHALT_REASON_ARM_RESERVED_0X14_EXCEPTION,
             (cpu->proc_ID == ARMCPU_ARM9) ? NDSErrorTag_ARM9
                                           : NDSErrorTag_ARM7);
    break;
  case EXCEPTION_IRQ:
    cpumode = IRQ;
...
u32 TRAPUNDEF(armcpu_t *cpu) {
  ...
  if (((cpu->intVector != 0) ^ (cpu->proc_ID == ARMCPU_ARM9))) {
    ...
    emu_halt(EMUHALT_REASON_ARM_UNDEFINED_INSTRUCTION_EXCEPTION,
             (cpu->proc_ID == ARMCPU_ARM9) ? NDSErrorTag_ARM9
                                           : NDSErrorTag_ARM7);
    return 4;
  }
...
// #ifdef DEVELOPER
  ...
  if (ARMPROC.CPSR.bits.T == 0) {
    if (CONDITION(ARMPROC.instruction) ==
            0x0E // fast path for unconditional instructions
        || (TEST_COND(CONDITION(ARMPROC.instruction), CODE(ARMPROC.instruction),
                      ARMPROC.CPSR)) // handles any condition
    ) {
#ifdef HAVE_LUA
      CallRegisteredLuaMemHook(
          ARMPROC.instruct_adr, 4, ARMPROC.instruction,
          LUAMEMHOOK_EXEC); // should report even if condition=false?
#endif
#ifdef TARGET_INTERFACE
      call_registered_interface_mem_hook(ARMPROC.instruct_adr, 4, HOOK_EXEC);
#endif
#ifdef DEVELOPER
      DEBUG_statistics.instructionHits[PROCNUM]
          .arm[INSTRUCTION_INDEX(ARMPROC.instruction)]++;
#endif
      cExecute =
          arm_instructions_set[PROCNUM][INSTRUCTION_INDEX(ARMPROC.instruction)](
              ARMPROC.instruction);
    } else
...
#ifdef HAVE_LUA
  CallRegisteredLuaMemHook(ARMPROC.instruct_adr, 2, ARMPROC.instruction,
                           LUAMEMHOOK_EXEC);
#endif
#ifdef TARGET_INTERFACE
...
#endif
  cExecute = thumb_instructions_set[PROCNUM][ARMPROC.instruction >> 6](
      ARMPROC.instruction);
...
template <int PROCNUM, bool jit> u32 armcpu_exec() {
  if (jit) {
    ...
    ArmOpCompiled f =
        (ArmOpCompiled)JIT_COMPILED_FUNC(ARMPROC.instruct_adr, PROCNUM);
...
const armcpu_ctrl_iface arm_default_ctrl_iface = {
    stall_cpu,   unstall_cpu,          read_cpu_reg,
    set_cpu_reg, install_post_exec_fn, remove_post_exec_fn,
    NULL};
