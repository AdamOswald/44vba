...
// this was originally declared in MMU.h but we suffered some organizational
// problems and had to remove it
enum MMU_ACCESS_TYPE {
  ...
  MMU_AT_DEBUG // used for emulator debugging functions (bypasses some debug
               // handling)
};

static INLINE u8 T1ReadByte(u8 *const mem, const u32 addr) { return mem[addr]; }

static INLINE u16 T1ReadWord_guaranteedAligned(void *const mem,
                                               const u32 addr) {
  assert((addr & 1) == 0);
#ifdef MSB_FIRST
...
static INLINE u32 T1ReadLong_guaranteedAligned(u8 *const mem, const u32 addr) {
  assert((addr & 3) == 0);
#ifdef MSB_FIRST
  return (mem[addr + 3] << 24 | mem[addr + 2] << 16 | mem[addr + 1] << 8 |
          mem[addr]);
#else
  return *(u32 *)(mem + addr);
#endif
...
static INLINE void HostWriteTwoWords(u8 *const mem, const u32 addr,
                                     const u32 val) {
#ifdef MSB_FIRST
