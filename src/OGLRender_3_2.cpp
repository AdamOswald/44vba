...
OGLEXT(PFNGLBINDFRAGDATALOCATIONINDEXEDPROC,
       glBindFragDataLocationIndexed) // Core in v3.3
...
OGLEXT(PFNGLFRAMEBUFFERRENDERBUFFERPROC,
       glFramebufferRenderbuffer)                             // Core in v3.0
OGLEXT(PFNGLFRAMEBUFFERTEXTURE2DPROC, glFramebufferTexture2D) // Core in v3.0
OGLEXT(PFNGLCHECKFRAMEBUFFERSTATUSPROC,
       glCheckFramebufferStatus)                            // Core in v3.0
...
OGLEXT(PFNGLRENDERBUFFERSTORAGEMULTISAMPLEPROC,
       glRenderbufferStorageMultisample)                        // Core in v3.0
...
OGLEXT(PFNGLGETACTIVEUNIFORMBLOCKIVPROC,
       glGetActiveUniformBlockiv) // Core in v3.1
...
void OGLLoadEntryPoints_3_2() {
  ...
  // FBO
  INITOGLEXT(PFNGLGENFRAMEBUFFERSPROC,
             glGenFramebuffers) // Promote to core version
  INITOGLEXT(PFNGLBINDFRAMEBUFFERPROC,
             glBindFramebuffer) // Promote to core version
  INITOGLEXT(PFNGLFRAMEBUFFERRENDERBUFFERPROC,
             glFramebufferRenderbuffer) // Promote to core version
  INITOGLEXT(PFNGLFRAMEBUFFERTEXTURE2DPROC,
             glFramebufferTexture2D) // Promote to core version
  INITOGLEXT(PFNGLCHECKFRAMEBUFFERSTATUSPROC,
             glCheckFramebufferStatus) // Promote to core version
  INITOGLEXT(PFNGLDELETEFRAMEBUFFERSPROC,
             glDeleteFramebuffers) // Promote to core version
  INITOGLEXT(PFNGLBLITFRAMEBUFFERPROC,
             glBlitFramebuffer) // Promote to core version
  INITOGLEXT(PFNGLGENRENDERBUFFERSPROC,
             glGenRenderbuffers) // Promote to core version
  INITOGLEXT(PFNGLBINDRENDERBUFFERPROC,
             glBindRenderbuffer) // Promote to core version
  INITOGLEXT(PFNGLRENDERBUFFERSTORAGEPROC,
             glRenderbufferStorage) // Promote to core version
  INITOGLEXT(PFNGLRENDERBUFFERSTORAGEMULTISAMPLEPROC,
             glRenderbufferStorageMultisample) // Promote to core version
  INITOGLEXT(PFNGLDELETERENDERBUFFERSPROC,
             glDeleteRenderbuffers) // Promote to core version
...
Render3DError OpenGLRenderer_3_2::InitExtensions() {
  ...
  // Load and create shaders. Return on any error, since v3.2 Core Profile makes
  // shaders mandatory.
  this->isShaderSupported = true;

  // OpenGL v3.2 Core Profile should have all the necessary features to be able
  // to flip and convert the framebuffer.
  this->willFlipOnlyFramebufferOnGPU = true;
  this->willFlipAndConvertFramebufferOnGPU = true;

  this->_isDualSourceBlendingSupported =
      this->IsExtensionPresent(&oglExtensionSet, "GL_ARB_blend_func_extended");
  this->_isSampleShadingSupported =
      this->IsExtensionPresent(&oglExtensionSet, "GL_ARB_sample_shading");
  this->_isConservativeDepthSupported =
      this->IsExtensionPresent(&oglExtensionSet, "GL_ARB_conservative_depth") &&
      IsOpenGLDriverVersionSupported(4, 0, 0);
  this->_isConservativeDepthAMDSupported =
      this->IsExtensionPresent(&oglExtensionSet, "GL_AMD_conservative_depth") &&
      IsOpenGLDriverVersionSupported(4, 0, 0);
  ...
  if (this->_isSampleShadingSupported) {
    error = this->CreateMSGeometryZeroDstAlphaProgram(
        GeometryZeroDstAlphaPixelMaskVtxShader_150,
        MSGeometryZeroDstAlphaPixelMaskFragShader_150);
    if (error == OGLERROR_NOERR) {
  ...
  error = this->InitPostprocessingPrograms(
      EdgeMarkVtxShader_150, EdgeMarkFragShader_150,
      FramebufferOutputVtxShader_150, FramebufferOutput6665FragShader_150,
      NULL);
  if (error != OGLERROR_NOERR) {
    glUseProgram(0);
    this->DestroyGeometryPrograms();
  ...
  // Load and create FBOs. Return on any error, since v3.2 Core Profile makes
  // FBOs mandatory.
  ...
  if (this->_deviceInfo.maxSamples >= 2) {
    ...
    // GFX3D_Renderer_MultisampleSize. However, if the client has this set to 0,
    // then set sampleSize to 2 in order to force the generation and the
...
Render3DError OpenGLRenderer_3_2::CreateFBOs() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, this->_framebufferWidth,
               this->_framebufferHeight, 0, GL_DEPTH_STENCIL,
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8,
               ...
               GL_DEPTH_STENCIL, GL_UNSIGNED_INT_24_8, tempClearImageBuffer);
  ...
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOROUT_ATTACHMENT_ID,
                         GL_TEXTURE_2D, OGLRef.texGColorID, 0);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_POLYID_ATTACHMENT_ID, GL_TEXTURE_2D,
                         OGLRef.texGPolyID, 0);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_FOGATTRIBUTES_ATTACHMENT_ID,
                         GL_TEXTURE_2D, OGLRef.texGFogAttrID, 0);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_WORKING_ATTACHMENT_ID,
...
Render3DError OpenGLRenderer_3_2::CreateMultisampledFBO(GLsizei numSamples) {
  ...
  if (this->willUsePerSampleZeroDstPass) {
    ...
    glTexParameteri(GL_TEXTURE_2D_MULTISAMPLE, GL_TEXTURE_WRAP_T,
                    GL_CLAMP_TO_EDGE);
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, numSamples, GL_RGBA,
    ...
    glRenderbufferStorageMultisample(GL_RENDERBUFFER, numSamples, GL_RGBA,
  ...
  if (this->willUsePerSampleZeroDstPass) {
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOROUT_ATTACHMENT_ID,
                           GL_TEXTURE_2D_MULTISAMPLE, OGLRef.texMSGColorID, 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_WORKING_ATTACHMENT_ID,
                           GL_TEXTURE_2D_MULTISAMPLE, OGLRef.texMSGWorkingID,
                           0);
  } else {
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOROUT_ATTACHMENT_ID,
                              GL_RENDERBUFFER, OGLRef.rboMSGColorID);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_WORKING_ATTACHMENT_ID,
                              GL_RENDERBUFFER, OGLRef.rboMSGWorkingID);
  }

  glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_POLYID_ATTACHMENT_ID,
                            GL_RENDERBUFFER, OGLRef.rboMSGPolyID);
  glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_FOGATTRIBUTES_ATTACHMENT_ID,
                            GL_RENDERBUFFER, OGLRef.rboMSGFogAttrID);
  glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT,
                            GL_RENDERBUFFER, OGLRef.rboMSGDepthStencilID);

  if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
    INFO("OpenGL: Failed to create multisampled FBO. Multisample antialiasing "
         "will be disabled.\n");
    this->DestroyMultisampledFBO();
...
void OpenGLRenderer_3_2::ResizeMultisampledFBOs(GLsizei numSamples) {
  ...
  if (this->willUsePerSampleZeroDstPass) {
    glActiveTexture(GL_TEXTURE0 + OGLTextureUnitID_GColor);
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, numSamples, GL_RGBA, w,
                            h, GL_TRUE);
    glActiveTexture(GL_TEXTURE0 + OGLTextureUnitID_FinalColor);
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, numSamples, GL_RGBA, w,
                            h, GL_TRUE);
    ...
    glBindRenderbuffer(GL_RENDERBUFFER, OGLRef.rboMSGColorID);
    glRenderbufferStorageMultisample(GL_RENDERBUFFER, numSamples, GL_RGBA, w,
                                     h);
    glBindRenderbuffer(GL_RENDERBUFFER, OGLRef.rboMSGWorkingID);
    glRenderbufferStorageMultisample(GL_RENDERBUFFER, numSamples, GL_RGBA, w,
                                     h);
...
Render3DError OpenGLRenderer_3_2::CreateGeometryPrograms() {
  ...
  if (OGLRef.uboRenderStatesID == 0) {
    ...
    glBufferData(GL_UNIFORM_BUFFER, sizeof(OGLRenderStates), NULL,
                 GL_DYNAMIC_DRAW);
    glBindBufferBase(GL_UNIFORM_BUFFER, OGLBindingPointID_RenderStates,
                     OGLRef.uboRenderStatesID);
  }
  ...
  if (this->_is64kUBOSupported) {
    if (OGLRef.uboPolyStatesID == 0) {
      ...
      glBufferData(GL_UNIFORM_BUFFER,
                   MAX_CLIPPED_POLY_COUNT_FOR_UBO * sizeof(OGLPolyStates), NULL,
                   GL_DYNAMIC_DRAW);
      glBindBufferBase(GL_UNIFORM_BUFFER, OGLBindingPointID_PolyStates,
                       OGLRef.uboPolyStatesID);
    }
  } else {
    if (OGLRef.tboPolyStatesID == 0) {
      ...
      glBufferData(GL_TEXTURE_BUFFER, POLYLIST_SIZE * sizeof(OGLPolyStates),
  ...
  if (this->_isConservativeDepthSupported ||
      this->_isConservativeDepthAMDSupported) {
  ...
  vtxShaderHeader << "\n";
  vtxShaderHeader << "#define IS_USING_UBO_POLY_STATES "
                  << ((OGLRef.uboPolyStatesID != 0) ? 1 : 0) << "\n";
  vtxShaderHeader << "\n";

  std::string vtxShaderCode =
      vtxShaderHeader.str() + std::string(GeometryVtxShader_150);
  ...
  if (this->_isConservativeDepthSupported ||
    ...
    // Prioritize using GL_AMD_conservative_depth over
    // GL_ARB_conservative_depth, since AMD drivers seem to have problems with
  ...
  fragShaderHeader << "#define IS_CONSERVATIVE_DEPTH_SUPPORTED "
                   << ((this->_isConservativeDepthSupported ||
                        this->_isConservativeDepthAMDSupported)
                           ? 1
                           : 0)
  ...
  for (size_t flagsValue = 0; flagsValue < 256;
    ...
    glBindFragDataLocation(OGLRef.programGeometryID[flagsValue], 0,
                           "outFragColor");
    ...
    if (programFlags.EnableEdgeMark) {
      glBindFragDataLocation(
          OGLRef.programGeometryID[flagsValue],
          GeometryAttachmentPolyID[programFlags.DrawBuffersMode], "outPolyID");
    }
    ...
    const GLuint uniformBlockRenderStates = glGetUniformBlockIndex(
        OGLRef.programGeometryID[flagsValue], "RenderStates");
    glUniformBlockBinding(OGLRef.programGeometryID[flagsValue],
                          uniformBlockRenderStates,
                          OGLBindingPointID_RenderStates);

    GLint uboSize = 0;
    glGetActiveUniformBlockiv(OGLRef.programGeometryID[flagsValue],
                              uniformBlockRenderStates,
    ...
    if (OGLRef.uboPolyStatesID != 0) {
      const GLuint uniformBlockPolyStates = glGetUniformBlockIndex(
          OGLRef.programGeometryID[flagsValue], "PolyStates");
      glUniformBlockBinding(OGLRef.programGeometryID[flagsValue],
                            uniformBlockPolyStates,
                            OGLBindingPointID_PolyStates);
    } else {
      const GLint uniformTexBufferPolyStates = glGetUniformLocation(
          OGLRef.programGeometryID[flagsValue], "PolyStates");
    ...
    OGLRef.uniformPolyStateIndex[flagsValue] =
        glGetUniformLocation(OGLRef.programGeometryID[flagsValue], "polyIndex");
...
Render3DError OpenGLRenderer_3_2::CreateGeometryZeroDstAlphaProgram(
  ...
  error =
      this->ShaderProgramCreate(OGLRef.vtxShaderGeometryZeroDstAlphaID,
                                OGLRef.fragShaderGeometryZeroDstAlphaID,
                                OGLRef.programGeometryZeroDstAlphaID,
                                vtxShaderCode.c_str(), fragShaderCode.c_str());
  if (error != OGLERROR_NOERR) {
...
Render3DError OpenGLRenderer_3_2::CreateMSGeometryZeroDstAlphaProgram(
    const char *vtxShaderCString, const char *fragShaderCString) {
  Render3DError error = OGLERROR_NOERR;
  OGLRenderRef &OGLRef = *this->ref;
  ...
  error =
      this->ShaderProgramCreate(OGLRef.vtxShaderMSGeometryZeroDstAlphaID,
                                OGLRef.fragShaderMSGeometryZeroDstAlphaID,
                                OGLRef.programMSGeometryZeroDstAlphaID,
                                vtxShaderCode.c_str(), fragShaderCode.c_str());
  if (error != OGLERROR_NOERR) {
    INFO("OpenGL: Failed to create the MULTISAMPLED GEOMETRY ZERO DST ALPHA "
         "shader program.\n");
    glUseProgram(0);
    this->DestroyMSGeometryZeroDstAlphaProgram();
    return error;
  ...
  glBindAttribLocation(OGLRef.programMSGeometryZeroDstAlphaID,
                       OGLVertexAttributeID_Position, "inPosition");

  glLinkProgram(OGLRef.programMSGeometryZeroDstAlphaID);
  if (!this->ValidateShaderProgramLink(
          OGLRef.programMSGeometryZeroDstAlphaID)) {
    INFO("OpenGL: Failed to link the MULTISAMPLED GEOMETRY ZERO DST ALPHA "
         "shader program.\n");
    glUseProgram(0);
    this->DestroyMSGeometryZeroDstAlphaProgram();
    return OGLERROR_SHADER_CREATE_ERROR;
  ...
  const GLint uniformTexGColor = glGetUniformLocation(
      OGLRef.programMSGeometryZeroDstAlphaID, "texInFragColor");
  glUniform1i(uniformTexGColor, OGLTextureUnitID_GColor);
...
void OpenGLRenderer_3_2::DestroyMSGeometryZeroDstAlphaProgram() {
  ...
  glDetachShader(OGLRef.programMSGeometryZeroDstAlphaID,
                 OGLRef.vtxShaderMSGeometryZeroDstAlphaID);
  glDetachShader(OGLRef.programMSGeometryZeroDstAlphaID,
                 OGLRef.fragShaderMSGeometryZeroDstAlphaID);
...
Render3DError
OpenGLRenderer_3_2::CreateEdgeMarkProgram(const char *vtxShaderCString,
  ...
  const GLuint uniformBlockRenderStates =
      glGetUniformBlockIndex(OGLRef.programEdgeMarkID, "RenderStates");
  glUniformBlockBinding(OGLRef.programEdgeMarkID, uniformBlockRenderStates,
                        OGLBindingPointID_RenderStates);
...
Render3DError
OpenGLRenderer_3_2::CreateFogProgram(const OGLFogProgramKey fogProgramKey,
  ...
  std::stringstream shaderHeader;
  shaderHeader << "#version 150\n";
  shaderHeader << "#define USE_DUAL_SOURCE_BLENDING "
               << ((this->_isDualSourceBlendingSupported) ? 1 : 0) << "\n";
  shaderHeader << "\n";
  ...
  std::string vtxShaderCode =
      shaderHeader.str() + std::string(vtxShaderCString);
  std::string fragShaderCode = shaderHeader.str() + fragDepthConstants.str() +
                               std::string(fragShaderCString);
  ...
  error = this->ShaderProgramCreate(
      OGLRef.vertexFogShaderID, shaderID.fragShader, shaderID.program,
      vtxShaderCode.c_str(), fragShaderCode.c_str());
  ...
  const GLuint uniformBlockRenderStates =
      glGetUniformBlockIndex(shaderID.program, "RenderStates");
  glUniformBlockBinding(shaderID.program, uniformBlockRenderStates,
                        OGLBindingPointID_RenderStates);
...
Render3DError OpenGLRenderer_3_2::CreateFramebufferOutput6665Program(
  ...
  glBindAttribLocation(OGLRef.programFramebufferRGBA6665OutputID[outColorIndex],
                       OGLVertexAttributeID_Position, "inPosition");
  glBindFragDataLocation(
      OGLRef.programFramebufferRGBA6665OutputID[outColorIndex], 0,
      "outFragColor6665");
...
Render3DError OpenGLRenderer_3_2::CreateFramebufferOutput8888Program(
    const size_t outColorIndex, const char *vtxShaderCString,
    const char *fragShaderCString) {
  OGLRenderRef &OGLRef = *this->ref;
...
void OpenGLRenderer_3_2::GetExtensionSet(
    std::set<std::string> *oglExtensionSet) {
...
Render3DError OpenGLRenderer_3_2::InitFinalRenderStates(
    const std::set<std::string> *oglExtensionSet) {
  Render3DError error =
      OpenGLRenderer_2_1::InitFinalRenderStates(oglExtensionSet);
  if (error != OGLERROR_NOERR) {
    return error;
  }

  if (this->_isDualSourceBlendingSupported) {
    INITOGLEXT(PFNGLBINDFRAGDATALOCATIONINDEXEDPROC,
...
Render3DError OpenGLRenderer_3_2::ZeroDstAlphaPass(
  ...
  const bool isRunningMSAAWithPerSampleShading =
      isRunningMSAA &&
      this->willUsePerSampleZeroDstPass; // Doing per-sample shading should be a
                                         // little more accurate than not doing
                                         // so.
  ...
  if (isRunningMSAA && !isRunningMSAAWithPerSampleShading) {
    ...
    glBlitFramebuffer(0, 0, this->_framebufferWidth, this->_framebufferHeight,
  ...
  glUseProgram((isRunningMSAAWithPerSampleShading)
                   ? OGLRef.programMSGeometryZeroDstAlphaID
                   : OGLRef.programGeometryZeroDstAlphaID);
...
void OpenGLRenderer_3_2::_ResolveWorkingBackFacing() {
  ...
  glBlitFramebuffer(0, 0, this->_framebufferWidth, this->_framebufferHeight, 0,
                    0, this->_framebufferWidth, this->_framebufferHeight,
                    GL_COLOR_BUFFER_BIT, GL_NEAREST);
...
Render3DError OpenGLRenderer_3_2::ReadBackPixels() {
  ...
  if (this->_outputFormat == NDSColorFormat_BGR666_Rev) {
    ...
    if (this->_lastTextureDrawTarget == OGLTextureUnitID_GColor) {
      glDrawBuffer(GL_WORKING_ATTACHMENT_ID);
      glReadBuffer(GL_COLOROUT_ATTACHMENT_ID);
      glBlitFramebuffer(0, this->_framebufferHeight, this->_framebufferWidth, 0,
                        0, 0, this->_framebufferWidth, this->_framebufferHeight,
                        GL_COLOR_BUFFER_BIT, GL_NEAREST);
      glReadBuffer(GL_WORKING_ATTACHMENT_ID);
    } else {
    ...
    // Read back the pixels in RGBA format, since an OpenGL 3.2 device should be
    // able to read back this format without a performance penalty.
    if (this->_mappedFramebuffer != NULL) {
      glUnmapBuffer(GL_PIXEL_PACK_BUFFER);
      this->_mappedFramebuffer = NULL;
    }
...
Render3DError OpenGLRenderer_3_2::BeginRender(const GFX3D &engine) {
  ...
  // Copy the vertex data to the GPU asynchronously due to the potentially large
  // upload size. This buffer write will need to be synchronized before we start
  // drawing.
  ...
  VERT *vtxPtr =
      (VERT *)glMapBufferRange(GL_ARRAY_BUFFER, 0, vtxBufferSize,
  ...
  if ((OGLRef.uboPolyStatesID != 0) &&
    ...
    // In practice, there shouldn't be any game scene with a clipped polygon
    // count that would exceed MAX_CLIPPED_POLY_COUNT_FOR_UBO. But if for some
    // reason there is, then we need to limit the polygon count here. Please
  ...
  // Replace the entire buffer as a hint to the driver to orphan the buffer and
  ...
  glBindBuffer(GL_UNIFORM_BUFFER, OGLRef.uboRenderStatesID);
  glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(this->_pendingRenderStates),
                  &this->_pendingRenderStates);

  OGLPolyStates *polyStates = this->_pendingPolyStates;

  if (OGLRef.uboPolyStatesID == 0) {
    // Some drivers seem to have problems with GL_TEXTURE_BUFFER used as the
    // target for glMapBufferRange() or glBufferSubData(), causing certain
    // polygons to intermittently flicker in certain games. Therefore, we'll use
    // glMapBuffer() in this case in order to prevent these glitches from
    // happening.
  ...
  for (size_t i = 0; i < this->_clippedPolyCount; i++) {
    ...
    polyStates[i].PolygonAlpha =
        (thePoly.isWireframe()) ? 0x1F : thePoly.attribute.Alpha;
    polyStates[i].IsWireframe = (thePoly.isWireframe()) ? 1 : 0;
    polyStates[i].EnableFog = (thePoly.attribute.Fog_Enable) ? 1 : 0;
    polyStates[i].SetNewDepthForTranslucent =
        (thePoly.attribute.TranslucentDepthWrite_Enable) ? 1 : 0;

    polyStates[i].EnableTexture =
        (this->_textureList[i]->IsSamplingEnabled()) ? 1 : 0;
    polyStates[i].TexSingleBitAlpha =
        (packFormat != TEXMODE_A3I5 && packFormat != TEXMODE_A5I3) ? 1 : 0;
    polyStates[i].TexSizeShiftS =
        thePoly.texParam
            .SizeShiftS; // Note that we are using the preshifted size of S
    polyStates[i].TexSizeShiftT =
        thePoly.texParam
            .SizeShiftT; // Note that we are using the preshifted size of T
  }

  if (OGLRef.uboPolyStatesID != 0) {
    // Replace the entire buffer as a hint to the driver to orphan the buffer
    // and avoid a synchronization cost.
    glBindBuffer(GL_UNIFORM_BUFFER, OGLRef.uboPolyStatesID);
    glBufferSubData(GL_UNIFORM_BUFFER, 0,
                    MAX_CLIPPED_POLY_COUNT_FOR_UBO * sizeof(OGLPolyStates),
                    this->_pendingPolyStates);
  } else {
  ...
  if (this->_enableFog) {
    ...
    if (it == this->_fogProgramMap.end()) {
      Render3DError error = this->CreateFogProgram(
          this->_fogProgramKey, FogVtxShader_150, FogFragShader_150);
      if (error != OGLERROR_NOERR) {
        return error;
      }
    }
...
Render3DError OpenGLRenderer_3_2::ClearUsingImage(
  ...
  if (this->_emulateDepthLEqualPolygonFacing) {
    ...
    glClearBufferfv(GL_COLOR,
                    GeometryAttachmentWorkingBuffer[this->_geometryProgramFlags
                                                        .DrawBuffersMode],
                    oglBackfacing);
  }
  ...
  if (this->_enableEdgeMark) {
    ...
    const GLfloat oglPolyID[4] = {(GLfloat)opaquePolyID / 63.0f, 0.0f, 0.0f,
                                  1.0f};
    glClearBufferfv(
        GL_COLOR,
        GeometryAttachmentPolyID[this->_geometryProgramFlags.DrawBuffersMode],
        oglPolyID);
  }
  ...
  if (this->_enableFog) {
    ...
    glReadBuffer(GL_FOGATTRIBUTES_ATTACHMENT_ID);
    glDrawBuffer(GL_FOGATTRIBUTES_ATTACHMENT_ID);
    glBlitFramebuffer(0, GPU_FRAMEBUFFER_NATIVE_HEIGHT,
                      GPU_FRAMEBUFFER_NATIVE_WIDTH, 0, 0, 0,
                      this->_framebufferWidth, this->_framebufferHeight,
                      GL_COLOR_BUFFER_BIT, GL_NEAREST);
  }

  // Blit the color and depth/stencil buffers. Do this last so that color
  // attachment 0 is set to the read FBO.
  glReadBuffer(GL_COLOROUT_ATTACHMENT_ID);
  glDrawBuffer(GL_COLOROUT_ATTACHMENT_ID);
  glBlitFramebuffer(
      0, GPU_FRAMEBUFFER_NATIVE_HEIGHT, GPU_FRAMEBUFFER_NATIVE_WIDTH, 0, 0, 0,
      this->_framebufferWidth, this->_framebufferHeight,
...
Render3DError OpenGLRenderer_3_2::ClearUsingValues(
  ...
  const GLfloat oglColor[4] = {divide6bitBy63_LUT[clearColor6665.r],
  ...
  glClearBufferfi(GL_DEPTH_STENCIL, 0,
                  (GLfloat)clearAttributes.depth / (GLfloat)0x00FFFFFF,
  ...
  if (this->_enableEdgeMark) {
    const GLfloat oglPolyID[4] = {(GLfloat)clearAttributes.opaquePolyID / 63.0f,
                                  0.0f, 0.0f, 1.0f};
    glClearBufferfv(
        GL_COLOR,
        GeometryAttachmentPolyID[this->_geometryProgramFlags.DrawBuffersMode],
  ...
  if (this->_enableFog) {
    const GLfloat oglFogAttr[4] = {(GLfloat)clearAttributes.isFogged, 0.0f,
                                   0.0f, 1.0f};
    glClearBufferfv(GL_COLOR,
                    GeometryAttachmentFogAttributes[this->_geometryProgramFlags
                                                        .DrawBuffersMode],
                    oglFogAttr);
  }
...
void OpenGLRenderer_3_2::SetPolygonIndex(const size_t index) {
  ...
  glUniform1i(
      this->ref->uniformPolyStateIndex[this->_geometryProgramFlags.value],
      index);
...
Render3DError OpenGLRenderer_3_2::SetupPolygon(const POLY &thePoly,
  ...
  glUniform1i(
      OGLRef.uniformPolyDepthOffsetMode[this->_geometryProgramFlags.value], 0);
...
Render3DError OpenGLRenderer_3_2::SetupTexture(const POLY &thePoly,
...
Render3DError OpenGLRenderer_3_2::SetFramebufferSize(size_t w, size_t h) {
  ...
  glBufferData(GL_PIXEL_PACK_BUFFER, newFramebufferColorSizeBytes, NULL,
               GL_STREAM_READ);
  ...
  this->_framebufferColor =
      NULL; // Don't need to make a client-side buffer since we will be reading
  ...
  this->CreateFramebufferOutput6665Program(1, FramebufferOutputVtxShader_150,
                                           FramebufferOutput6665FragShader_150);

  if (this->_isSampleShadingSupported) {
    Render3DError shaderError = this->CreateMSGeometryZeroDstAlphaProgram(
        GeometryZeroDstAlphaPixelMaskVtxShader_150,
        MSGeometryZeroDstAlphaPixelMaskFragShader_150);
...
Render3DError OpenGLRenderer_3_2::RenderPowerOff() {
  ...
  glReadPixels(0, 0, this->_framebufferWidth, this->_framebufferHeight, GL_BGRA,
               GL_UNSIGNED_BYTE, 0);
