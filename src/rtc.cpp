...
SFORMAT SF_RTC[] = {{"R000", 1, 1, &rtc.regStatus1},
...
static inline u8 toBCD(u8 x) { return ((x / 10) << 4) | (x % 10); }
...
DateTime rtcGetTime(void) {
  if (movieMode == MOVIEMODE_INACTIVE) {
    ...
    // when we could just assume that there are 60fps and base the seconds on
    // frameCounter/60 but, we were imagining that one day we might need more
    // precision
...
void rtcGetTimeAsString(char *buffer) {
  ...
  snprintf(buffer, 25, "%04d-%3s-%02d %s %02d:%02d:%02d", rtcTime.get_Year(),
           DateTime::GetNameOfMonth(rtcTime.get_Month()), rtcTime.get_Day(),
           wday[rtcTime.get_DayOfWeek() % 7], rtcTime.get_Hour(),
           rtcTime.get_Minute(), rtcTime.get_Second());
}
...
static void rtcRecv() {
  ...
  case 2: // date & time
  {
    ...
    // but it is so wrong, someone mustve thought they knew what they were
    // doing, so i am leaving it... rtc.data[3] = (tm.tm_wday + 6) & 7; if
    // (rtc.data[3] == 7) rtc.data[3] = 6;

    // do this instead (gbatek seems to say monday=0 but i don't think that is
    // right) 0=sunday is necessary to make animal crossing behave maybe it
    // means "custom assignment" can be specified by the game
...
static void rtcSend() {
  ...
  case 2: // date & time
    ...
    // rtc.data[0], rtc.data[1], rtc.data[2], rtc.data[3], rtc.data[4],
    // rtc.data[5], rtc.data[6]);
    break;
  case 3: // time
    // INFO("RTC: write time : %02X %02X %02X\n", rtc.data[0], rtc.data[1],
    // rtc.data[2]);
    break;
  case 4: // freq/alarm 1
    /*if (cmdBitsSize[0x04] == 8)
...
void rtcWrite(u16 val) {
  ...
  case 3: // write:
    if ((rtc._prevSCK) && (!rtc._SCK)) {
      if (rtc._SIO)
        rtc.data[rtc.bitsCount >> 3] |= (1 << (rtc.bitsCount & 0x07));
      rtc.bitsCount++;
