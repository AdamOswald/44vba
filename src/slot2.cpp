...
void slot2_Disconnect() { slot2_device->disconnect(); }
...
bool slot2_Change(NDS_SLOT2_TYPE changeToType) {
  ...
  if (slot2_device_type == changeToType) {
    return false;
  } else if (slot2_device_type != NDS_SLOT2_AUTO &&
             changeToType == NDS_SLOT2_AUTO) {
  ...
  } else //(slot2_device_type != NDS_SLOT2_AUTO && changeToType !=
         //NDS_SLOT2_AUTO)
  {
    if (slot2_device != NULL) {
      slot2_device->disconnect();
...
NDS_SLOT2_TYPE slot2_GetCurrentType() { return slot2_device_type; }
...
NDS_SLOT2_TYPE slot2_DetermineTypeByGameCode(const char *theGameCode) {
  ...
  static const Slot2AutoDeviceType gameCodeDeviceTypes[] = {
      ...
      {"Y6R",
       NDS_SLOT2_GUITARGRIP}, // Guitar Hero - On Tour - Modern Hits (Demo)
...
void slot2_Savestate(EMUFILE &os) { slot2_device->savestate(os); }

void slot2_Loadstate(EMUFILE &is) { slot2_device->loadstate(is); }
...
template <u8 PROCNUM> static bool skipSlot2Data() {
