...
//
// Note that all the registers are 18 bits in length. However, for the sake of
// compatibility, we pad out each registers to 32 bits.
...
} IOREG_W_TXBUF_LOCATION; // 0x4808080, 0x4808090, 0x4808094, 0x4808098,
                          // 0x48080A0, 0x48080A4, 0x48080A8
...
typedef struct {
  ...
  union {
    u8 MACADDR[6]; // 48-bit MAC address of the console. Should be initialized
                   // from firmware.
  ...
  union {
    u8 BSSID[6]; // 48-bit MAC address of the console. Should be initialized
                 // from firmware.
...
typedef struct {
  ...
  DesmumeFrameHeaderAttributes
      packetAttributes; // Attribute bits for describing the packet
...
// Given a connection of 2 megabits per second, we take ~4 microseconds to
// transfer a byte. This works out to needing ~8 microseconds to transfer a
// halfword.
...
typedef union {
  u8 rawFrameData[sizeof(TXPacketHeader) + MAX_PACKET_SIZE_80211 + sizeof(u16) +
                  sizeof(u16)];
...
typedef struct {
  ...
  u16 timeStamp;
  u16 txRate;
  u16 length; // Total length of header+body, in bytes (excluding the FCS
              // checksum).
  u8 rssiMax;
  u8 rssiMin;
} RXPacketHeader;
...
public:
  ...
  virtual void *open(const char *source, int snaplen, int flags,
                     int readtimeout, char *errbuf) = 0;
...
protected:
  ...
  const u8 *_RXPacketFilter(const u8 *rxBuffer, const size_t rxBytes,
  ...
  RXQueuedPacket
  _GenerateSoftAPMgmtResponseFrame(WifiFrameManagementSubtype mgmtFrameSubtype,
                                   u16 sequenceNumber, u64 timeStamp);
  RXQueuedPacket _GenerateSoftAPCtlACKFrame(
      const WifiDataFrameHeaderSTA2DS &inIEEE80211FrameHeader,
      const size_t sendPacketLength);

  bool _SoftAPTrySendPacket(const TXPacketHeader &txHeader,
                            const u8 *IEEE80211PacketData);
  bool _AdhocTrySendPacket(const TXPacketHeader &txHeader,
                           const u8 *IEEE80211PacketData);
...
public:
  ...
  void RXPacketRawToQueue(const RXRawPacketData &rawPacket);
  ...
  static size_t ConvertDataFrame80211To8023(const u8 *inIEEE80211Frame,
                                            const size_t txLength,
                                            u8 *outIEEE8023Frame);
  static size_t ConvertDataFrame8023To80211(const u8 *inIEEE8023Frame,
                                            const size_t txLength,
