...
// We're not exactly committing to OpenGL 3.2 Core Profile just yet, so redefine
// APPLE extensions as a temporary measure.
#if defined(GL_APPLE_vertex_array_object) &&                                   \
    !defined(GL_ARB_vertex_array_object)
...
#define OGLEXT(procPtr, func) procPtr func = NULL;
#define INITOGLEXT(procPtr, func)                                              \
  func = (procPtr)glXGetProcAddress((const GLubyte *)#func);
#define EXTERNOGLEXT(procPtr, func) extern procPtr func;
#endif
...
#define glBlitFramebufferEXT(srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1,  \
                             dstY1, mask, filter)
#endif
...
#define glRenderbufferStorageMultisampleEXT(target, samples, internalformat,   \
                                            width, height)
#endif
...
EXTERNOGLEXT(PFNGLBLENDEQUATIONSEPARATEPROC,
...
EXTERNOGLEXT(PFNGLENABLEVERTEXATTRIBARRAYPROC,
             glEnableVertexAttribArray) // Core in v2.0
EXTERNOGLEXT(PFNGLDISABLEVERTEXATTRIBARRAYPROC,
             glDisableVertexAttribArray) // Core in v2.0
EXTERNOGLEXT(PFNGLVERTEXATTRIBPOINTERPROC,
...
EXTERNOGLEXT(PFNGLRENDERBUFFERSTORAGEMULTISAMPLEEXTPROC,
...
// Generic vertex attributes
EXTERNOGLEXT(PFNGLBINDATTRIBLOCATIONPROC, glBindAttribLocation) // Core in v2.0
EXTERNOGLEXT(PFNGLBINDFRAGDATALOCATIONPROC,
             glBindFragDataLocation) // Core in v3.0
EXTERNOGLEXT(PFNGLBINDFRAGDATALOCATIONINDEXEDPROC,
...
EXTERNOGLEXT(PFNGLFRAMEBUFFERRENDERBUFFERPROC,
             glFramebufferRenderbuffer) // Core in v3.0
EXTERNOGLEXT(PFNGLFRAMEBUFFERTEXTURE2DPROC,
             glFramebufferTexture2D) // Core in v3.0
EXTERNOGLEXT(PFNGLCHECKFRAMEBUFFERSTATUSPROC,
...
EXTERNOGLEXT(PFNGLRENDERBUFFERSTORAGEPROC,
             glRenderbufferStorage) // Core in v3.0
EXTERNOGLEXT(PFNGLRENDERBUFFERSTORAGEMULTISAMPLEPROC,
             glRenderbufferStorageMultisample) // Core in v3.0
EXTERNOGLEXT(PFNGLDELETERENDERBUFFERSPROC,
             glDeleteRenderbuffers) // Core in v3.0
EXTERNOGLEXT(PFNGLTEXIMAGE2DMULTISAMPLEPROC,
             glTexImage2DMultisample) // Core in v3.2

// UBO
EXTERNOGLEXT(PFNGLGETUNIFORMBLOCKINDEXPROC,
             glGetUniformBlockIndex) // Core in v3.1
EXTERNOGLEXT(PFNGLUNIFORMBLOCKBINDINGPROC,
             glUniformBlockBinding)                     // Core in v3.1
EXTERNOGLEXT(PFNGLBINDBUFFERBASEPROC, glBindBufferBase) // Core in v3.0
EXTERNOGLEXT(PFNGLGETACTIVEUNIFORMBLOCKIVPROC,
...
struct OGLRenderRef {
  ...
  CACHE_ALIGN GLushort workingCIColorBuffer[GPU_FRAMEBUFFER_NATIVE_WIDTH *
                                            GPU_FRAMEBUFFER_NATIVE_HEIGHT];
  CACHE_ALIGN GLuint
      workingCIDepthStencilBuffer[2][GPU_FRAMEBUFFER_NATIVE_WIDTH *
                                     GPU_FRAMEBUFFER_NATIVE_HEIGHT];
  CACHE_ALIGN GLuint
      workingCIFogAttributesBuffer[2][GPU_FRAMEBUFFER_NATIVE_WIDTH *
                                      GPU_FRAMEBUFFER_NATIVE_HEIGHT];
...
extern bool (*oglrender_framebufferDidResizeCallback)(const bool isFBOSupported,
                                                      size_t w, size_t h);
...
private:
  Render3DError _FlushFramebufferFlipAndConvertOnCPU(
...
protected:
  ...
  Render3DError FlushFramebuffer(const FragmentColor *__restrict srcFramebuffer,
                                 FragmentColor *__restrict dstFramebufferMain,
                                 u16 *__restrict dstFramebuffer16);
  OpenGLTexture *GetLoadedTextureFromPolygon(const POLY &thePoly,
                                             bool enableTexturing);

  template <OGLPolyDrawMode DRAWMODE>
  size_t DrawPolygonsForIndexRange(const CPoly *clippedPolyList,
                                   const size_t clippedPolyCount,
                                   size_t firstIndex, size_t lastIndex,
                                   size_t &indexOffset,
                                   POLYGON_ATTR &lastPolyAttr);
  template <OGLPolyDrawMode DRAWMODE>
  Render3DError DrawAlphaTexturePolygon(
  ...
  template <OGLPolyDrawMode DRAWMODE>
  Render3DError
  DrawOtherPolygon(const GLenum polyPrimitive, const GLsizei vertIndexCount,
                   const GLushort *indexBufferPtr,
                   const bool performDepthEqualTest,
                   const bool enableAlphaDepthWrite, const u8 opaquePolyID,
  ...
  CreateGeometryZeroDstAlphaProgram(const char *vtxShaderCString,
                                    const char *fragShaderCString) = 0;
  virtual void DestroyGeometryZeroDstAlphaProgram() = 0;
  virtual Render3DError
  CreateEdgeMarkProgram(const char *vtxShaderCString,
                        const char *fragShaderCString) = 0;
  virtual void DestroyEdgeMarkProgram() = 0;
  virtual Render3DError CreateFogProgram(const OGLFogProgramKey fogProgramKey,
  ...
  virtual Render3DError
  CreateFramebufferOutput6665Program(const size_t outColorIndex,
                                     const char *vtxShaderCString,
                                     const char *fragShaderCString) = 0;
  virtual void DestroyFramebufferOutput6665Programs() = 0;
  virtual Render3DError
  CreateFramebufferOutput8888Program(const size_t outColorIndex,
                                     const char *vtxShaderCString,
                                     const char *fragShaderCString) = 0;
  virtual void DestroyFramebufferOutput8888Programs() = 0;

  virtual Render3DError
  InitFinalRenderStates(const std::set<std::string> *oglExtensionSet) = 0;
  virtual Render3DError InitPostprocessingPrograms(
      const char *edgeMarkVtxShader, const char *edgeMarkFragShader,
  ...
  virtual Render3DError UploadClearImage(const u16 *__restrict colorBuffer,
                                         const u32 *__restrict depthBuffer,
                                         const u8 *__restrict fogBuffer,
                                         const u8 opaquePolyID) = 0;
  ...
  virtual Render3DError SetupPolygon(const POLY &thePoly,
                                     ...
                                     bool willChangeStencilBuffer) = 0;
...
public:
  ...
  bool IsExtensionPresent(const std::set<std::string> *oglExtensionSet,
                          const std::string extensionName) const;
  Render3DError ShaderProgramCreate(GLuint &vtxShaderID, GLuint &fragShaderID,
                                    GLuint &programID,
                                    const char *vtxShaderCString,
  ...
  bool IsVersionSupported(unsigned int checkVersionMajor,
                          unsigned int checkVersionMinor,
...
protected:
  ...
  virtual Render3DError CreateEdgeMarkProgram(const char *vtxShaderCString,
                                              const char *fragShaderCString);
  ...
  virtual Render3DError
  InitFinalRenderStates(const std::set<std::string> *oglExtensionSet);
  virtual Render3DError
  InitPostprocessingPrograms(const char *edgeMarkVtxShader,
  ...
  virtual Render3DError ZeroDstAlphaPass(const CPoly *clippedPolyList,
                                         const size_t clippedPolyCount,
  ...
  virtual Render3DError ClearUsingImage(const u16 *__restrict colorBuffer,
                                        ...
                                        const u8 opaquePolyID);
  virtual Render3DError
  ClearUsingValues(const FragmentColor &clearColor6665,
...
public:
  ...
  virtual Render3DError RenderFlush(bool willFlushBuffer32,
