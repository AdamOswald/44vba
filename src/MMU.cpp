...
static u64 isqrt(u64 x) {
   ...
   * with that condition.)
   */
  squaredbit = (u64)((((u64)~0LL) >> 1) & ~(((u64)~0LL) >> 2));
...
void mmu_log_debug_ARM9(u32 adr, const char *fmt, ...) {
  ...
  // #if 1
  if (adr >= 0x4000000 && adr <= 0x400006E)
    return; // Display Engine A
  if (adr >= 0x40000B0 && adr <= 0x4000134)
    return; // DMA, Timers and Keypad
  if (adr >= 0x4000180 && adr <= 0x40001BC)
    return; // IPC/ROM
  if (adr >= 0x4000204 && adr <= 0x400024A)
    return; // Memory & IRQ control
  if (adr >= 0x4000280 && adr <= 0x4000306)
    return; // Maths
  if (adr >= 0x4000320 && adr <= 0x40006A3)
    return; // 3D dispaly engine
  if (adr >= 0x4001000 && adr <= 0x400106E)
    return; // Display Engine B
  if (adr >= 0x4100000 && adr <= 0x4100014)
    return; // IPC/ROM
            // #endif
  va_list list;
  char msg[512];
...
void mmu_log_debug_ARM7(u32 adr, const char *fmt, ...) {
  ...
  if (adr > 0x4808FFF)
    return;
#if 1
  if (adr >= 0x4000004 && adr < 0x4000180)
    return; // ARM7 I/O Map
  if (adr >= 0x4000180 && adr <= 0x40001C4)
    return; // IPC/ROM
  if (adr >= 0x4000204 && adr <= 0x400030C)
    return; // Memory and IRQ Control
  if (adr >= 0x4000400 && adr <= 0x400051E)
    return; // Sound Registers
  if (adr >= 0x4100000 && adr <= 0x4100014)
    return; // IPC/ROM
  if (adr >= 0x4800000 && adr <= 0x4808FFF)
    return; // WLAN Registers
#endif
  va_list list;
  char msg[512];
...
u8 *MMU_struct::MMU_MEM[2][256] = {
    ...
    {/* 0X*/ DUP16(MMU.ARM7_BIOS),
     /* 1X*/ DUP16(MMU.UNUSED_RAM),
     /* 2X*/ DUP16(MMU.MAIN_MEM),
     /* 3X*/ DUP8(MMU.SWIRAM), DUP8(MMU.ARM7_ERAM),
     /* 4X*/ DUP8(MMU.ARM7_REG), DUP8(MMU.ARM7_WIRAM),
...
u32 MMU_struct::MMU_MASK[2][256] = {
    ...
    {/* 0X*/ DUP16(0x00003FFF),
     /* 1X*/ DUP16(0x00000003),
     /* 2X*/ DUP16(0x003FFFFF),
     /* 3X*/ DUP8(0x00007FFF), DUP8(0x0000FFFF),
     /* 4X*/ DUP8(0x0000FFFF), DUP8(0x0000FFFF),
     /* 5X*/ DUP16(0x00000003),
     /* 6X*/ DUP16(0x00FFFFFF),
     /* 7X*/ DUP16(0x00000003),
     /* 8X*/ DUP16(0x00000003),
...
// in the range of 0x06000000 - 0x06800000 in 16KB pages (the ARM9 vram mappable
// area) this maps to 16KB pages in the LCDC buffer which is what will actually
// contain the data
u8 vram_arm9_map[VRAM_ARM9_PAGES];

// this chooses which banks are mapped in the 128K banks starting at 0x06000000
// in ARM7
u8 vram_arm7_map[2];

//----->
// consider these later, for better recordkeeping, instead of using the u8* in
// MMU

////for each 128KB texture slot, this maps to a 16KB starting page in the LCDC
///buffer
// #define VRAM_TEX_SLOTS 4
// u8 vram_tex_map[VRAM_TEX_SLOTS];
//
////for each 16KB tex palette slot, this maps to a 16KB starting page in the
///LCDC buffer
...
// this is to remind you that the LCDC mapping returns a strange value (not
// 0x06800000) as you would expect in order to play nicely with the MMU address
// and mask tables
...
// maps an ARM9 BG/OBJ or LCDC address into an LCDC address, and informs the
// caller of whether it isn't mapped
// TODO - in cases where this does some mapping work, we could bypass the logic
// at the end of the _read* and _write* routines this is a good optimization to
// consider NOTE - this whole approach is probably fundamentally wrong.
// according to dasShiny research, its possible to map multiple banks to the
// same addresses. something more sophisticated would be needed. however, it
// hasnt proven necessary yet for any known test case.
...
static FORCEINLINE u32 MMU_LCDmap(u32 addr, bool &unmapped, bool &restricted) {
  ...
  // handle SIWRAM and non-shared IWRAM in here too, since it is quite similar
  // to vram. in fact it is probably implemented with the same pieces of
  // hardware. its sort of like arm7 non-shared IWRAM is lowest priority, and
  // then SIWRAM goes on top. however, we implement it differently than vram in
  // emulator for historical reasons. instead of keeping a page map like we do
  // vram, we just have a list of all possible page maps (there are only 4 each
  // for arm9 and arm7)
  if (addr >= 0x03000000 && addr < 0x04000000) {
    // blocks 0,1,2,3 is arm7 non-shared IWRAM and blocks 4,5 is SIWRAM, and
    // block 8 is un-mapped zeroes
    int iwram_block_16k;
    ...
    case 1: // SIWRAM
      return ARM7_HACKY_SIWRAM_LOCATION + ((iwram_block_16k & 3) << 14) +
             iwram_offset;
    case 2: // zeroes
    CASE2:
      unmapped = true;
  ...
  // TODO - this is gross! this should be renovated if the vram mapping is ever
  // done in a more sophisticated way taking into account dasShiny research
  if (addr >= 0x068A4000)
    addr = 0x06800000 +
           //(addr%0xA4000); //yuck!! is this even how it mirrors? but we have
           //to keep from overrunning the buffer somehow
           (addr & 0x80000); // just as likely to be right (I have no clue how
                             // it should work) but faster.
...
std::string VramConfiguration::describe() {
  ...
  for (int i = 0; i < VRAM_BANK_COUNT; i++) {
    ret << (char)(i + 'A') << ": " << banks[i].ofs << " "
        << describePurpose(banks[i].purpose) << std::endl;
  }
  return ret.str();
}
...
template <VRAMBankID VRAMBANK> static inline void MMU_VRAMmapRefreshBank() {
  ...
  case VRAM_BANK_B:
    ...
    case 0: // LCDC
      ...
      if (VRAMBankCnt.OFS != 0)
        PROGINFO("Bank %i: MST %i OFS %i\n", VRAMBankCnt.MST_ABHI,
                 VRAMBankCnt.OFS);
      break;
    case 1: // ABG
    ...
    case 3: // texture
      ...
      MMU.texInfo.textureSlotAddr[VRAMBankCnt.OFS] =
          MMU_vram_physical(vram_bank_info[VRAMBANK].page_addr);
      break;
    default:
  ...
  case VRAM_BANK_D:
    ...
    case 2: // arm7
      ...
      if (VRAMBANK == 2)
        T1WriteByte(MMU.MMU_MEM[ARMCPU_ARM7][0x40], 0x240,
                    T1ReadByte(MMU.MMU_MEM[ARMCPU_ARM7][0x40], 0x240) | 1);
      if (VRAMBANK == 3)
        T1WriteByte(MMU.MMU_MEM[ARMCPU_ARM7][0x40], 0x240,
                    T1ReadByte(MMU.MMU_MEM[ARMCPU_ARM7][0x40], 0x240) | 2);
      // printf("DING!\n");
      switch (VRAMBankCnt.OFS) {
      case 0:
      case 1:
  ...
  case VRAM_BANK_E:
    ...
    case 3: // texture palette
      ...
      MMU.texInfo.texPalSlot[0] =
          MMU_vram_physical(vram_bank_info[VRAMBANK].page_addr);
      MMU.texInfo.texPalSlot[1] =
          MMU_vram_physical(vram_bank_info[VRAMBANK].page_addr + 1);
      MMU.texInfo.texPalSlot[2] =
          MMU_vram_physical(vram_bank_info[VRAMBANK].page_addr + 2);
      MMU.texInfo.texPalSlot[3] =
          MMU_vram_physical(vram_bank_info[VRAMBANK].page_addr + 3);
      break;
  ...
  case VRAM_BANK_G: {
    ...
    case 1: // ABG
      ...
      MMU_vram_arm9(
          VRAMBANK,
          VRAM_PAGE_ABG + pageofs +
              2); // unexpected mirroring (required by spyro eternal night)
      break;
    case 2: // AOBJ
      vramConfiguration.banks[VRAMBANK].purpose = VramConfiguration::AOBJ;
      MMU_vram_arm9(VRAMBANK, VRAM_PAGE_AOBJ + pageofs);
      MMU_vram_arm9(VRAMBANK, VRAM_PAGE_AOBJ + pageofs +
                                  2); // unexpected mirroring - I have no proof,
                                      // but it is inferred from the ABG above
      break;
    case 3: // texture palette
      vramConfiguration.banks[VRAMBANK].purpose = VramConfiguration::TEXPAL;
      MMU.texInfo.texPalSlot[pageofs] =
          MMU_vram_physical(vram_bank_info[VRAMBANK].page_addr);
      break;
    case 4: // A BG extended palette
      switch (VRAMBankCnt.OFS) {
      case 0:
      case 1:
        vramConfiguration.banks[VRAMBANK].purpose =
            VramConfiguration::ABGEXTPAL;
        MMU.ExtPal[0][VRAMBankCnt.OFS * 2] =
            MMU_vram_physical(vram_bank_info[VRAMBANK].page_addr);
        MMU.ExtPal[0][VRAMBankCnt.OFS * 2 + 1] =
            MMU.ExtPal[0][VRAMBankCnt.OFS * 2] + ADDRESS_STEP_8KB;
        break;
      default:
        vramConfiguration.banks[VRAMBANK].purpose = VramConfiguration::INVALID;
        PROGINFO(
            "Unsupported ofs setting %d for engine A bgextpal vram bank %c\n",
            VRAMBankCnt.OFS, 'A' + VRAMBANK);
        break;
      }
      break;
  ...
  case VRAM_BANK_I:
    ...
    case 1: // BBG
      ...
      MMU_vram_arm9(VRAMBANK,
                    VRAM_PAGE_BBG +
                        6); // required for ultimate mortal kombat pause screen
      MMU_vram_arm9(VRAMBANK, VRAM_PAGE_BBG +
                                  7); //(6,7) are mapped similarly to (2,3) [and
                                      //(6,7) are mapped similarly to H's (4)]
      break;
    case 2: // BOBJ
      vramConfiguration.banks[VRAMBANK].purpose = VramConfiguration::BOBJ;
      ...
      MMU_vram_arm9(
          VRAMBANK,
          VRAM_PAGE_BOBJ +
              1); // FF3 end scene (lens flare sprite) needs this as it renders
                  // a sprite off the end of the 16KB and back around
      break;
...
static inline void MMU_VRAMmapControl(u8 block, u8 VRAMBankCnt) {
  ...
  // first, save the texture info so we can check it for changes and trigger
  // purges of the texcache
  MMU_struct::TextureInfo oldTexInfo = MMU.texInfo;

  // unmap everything
  MMU_VRAM_unmap_all();

  // unmap VRAM_BANK_C and VRAM_BANK_D from arm7. theyll get mapped again in a
  // moment if necessary
  ...
  // We're probably still not mapping things 100% correctly, but this helped us
  // get closer: goblet of fire "care of magical creatures" maps I and D to BOBJ
  // (the I is an accident) and requires A to override it. This may create other
  // bugs....
  ...
  if (memcmp(&oldTexInfo, &MMU.texInfo, sizeof(MMU_struct::TextureInfo))) {
    // if(!nds.isIn3dVblank())
    //		PROGINFO("Changing texture or texture palette mappings outside of 3d
    //vblank\n");
  ...
  // in general, we find that it is not uncommon at all for games to
  // accidentally do this.
  //
  // being able to easily do these experiments was one of the primary
  // motivations for this remake of the vram mapping system
  ...
  // drill spirits EU - mission select (just for control purposes, as it doesnt
  // use H or I)
  //...
  // note that the "unexpected mirroring" items above may at some point rely on
  // being executed in a certain order. (sequentially A..I)

  const int types[] = {VRAM_PAGE_ABG, VRAM_PAGE_BBG, VRAM_PAGE_AOBJ,
                       VRAM_PAGE_BOBJ};
  const int sizes[] = {32, 8, 16, 8};
  for (int t = 0; t < 4; t++) {
    // the idea here is to pad out the mirrored space with copies of the
    // mappable area, without respect to what is mapped within that mappable
    // area. we hope that this is correct in all cases required for driller
    // spirits in mission select (mapping is simple A,B,C,D to each purpose)
    ...
    // it seems like these shouldnt pad out 128K banks (space beyond those
    // should have remained unmapped) int mirrorMask = -1; int type = types[t];
    ...
    //	{
    //		if(i==0) break; //can't mirror anything if theres nothing
    //mapped! 		if(mirrorMask == -1) 			mirrorMask = i-1; 		vram_arm9_map[page] =
    //vram_arm9_map[type+(i&mirrorMask)];
    //	}
    // }
  }
...
void MMU_Init(void) {
  ...
  // even though apps may change dtcm immediately upon startup, this is the
  // correct hardware starting value:
...
static void execdiv() {
  ...
  if (den == 0) {
    ...
    // the DIV0 flag in DIVCNT is set only if the full 64bit DIV_DENOM value is
    // zero, even in 32bit mode
  ...
  DIVLOG("DIV %08X%08X / %08X%08X = %08X%08X\r\n", (u32)(num >> 32), (u32)num,
         (u32)(den >> 32), (u32)den, (u32)(res >> 32), (u32)res);
...
void GC_Command::toCryptoBuffer(u32 buf[2]) {
  u8 temp[8] = {bytes[7], bytes[6], bytes[5], bytes[4],
...
template <int PROCNUM> void FASTCALL MMU_writeToGCControl(u32 val) {
  ...
  static const int blocksize_table[] = {0,      0x200,  0x400,  0x800,
                                        0x1000, 0x2000, 0x4000, 4};
  int blocksize = blocksize_table[blocksize_field];

  // store written value, without bit 31 and bit 23 set (those will be patched
  // in as operations proceed) T1WriteLong(MMU.MMU_MEM[PROCNUM][0x40], 0x1A4,
  // val & 0x7F7FFFFF);
  ...
  // the transfer size is determined by the specification here in GCROMCTRL, not
  // any logic private to the card.
...
template <int PROCNUM> u32 MMU_readFromGC() {
  GCBUS_Controller &card = MMU.dscard[PROCNUM];
...
template <int PROCNUM> void MMU_writeToGC(u32 val) {
  GCBUS_Controller &card = MMU.dscard[PROCNUM];
...
void FASTCALL MMU_writeToSPIData(u16 val) {
  ...
  {
    PM_SOUND_AMP = BIT(0),  /*!< \brief Power the sound hardware (needed to hear
                               stuff in GBA mode too) */
    PM_SOUND_MUTE = BIT(1), /*!< \brief   Mute the main speakers, headphone
                               output will still work. */
    PM_BACKLIGHT_BOTTOM =
        BIT(2), /*!< \brief   Enable the top backlight if set */
    PM_BACKLIGHT_TOP =
        BIT(3),             /*!< \brief   Enable the bottom backlight if set */
  ...
  u16 spicnt = T1ReadWord(MMU.MMU_MEM[ARMCPU_ARM7][(REG_SPICNT >> 20) & 0xff],
                          REG_SPICNT & 0xfff);
  ...
  case SPI_DEVICE_POWERMAN:
    if (!MMU.powerMan_CntRegWritten) {
      ...
      //(let's start with emulating a DS lite, since it is the more complex
      //case)
      if (MMU.powerMan_CntReg & 0x80) {
        ...
        // our totally pathetic register handling, only the one thing we've
        // wanted so far
  ...
  case SPI_DEVICE_TOUCHSCREEN: {
    ...
    case TSC_MEASURE_Y:
      // counter the number of adc touch coord reads and jitter it after a while
      // to simulate a shaky human hand or multiple reads this is actually
      // important for some games.. seemingly due to bugs.
      nds.adc_jitterctr++;
    ...
    {
      // used for pressure calculation - must be nonzero or else some softwares
      // will think the stylus is up. something is wrong in here and some of
      // these LSB dont make it back to libnds... whatever.
      u16 scratch;
  ...
  T1WriteWord(MMU.MMU_MEM[ARMCPU_ARM7][(REG_SPIDATA >> 20) & 0xff],
              REG_SPIDATA & 0xfff, val & 0x00FF);
}
// ======================================================================
// REG_SPIxxx END

// does some validation on the game's choice of IF value, correcting it if
// necessary
...
template <int PROCNUM> static void REG_IF_WriteByte(u32 addr, u8 val) {
  ...
  // ZERO 01-dec-2010 : I am no longer sure this approach is correct.. it proved
  // to be wrong for IPC fifo....... it seems as if IF bits should always be
  // cached (only the user can clear them)
...
template <int PROCNUM> u32 MMU_struct::gen_IF() {
  u32 IF = reg_IF_bits[PROCNUM];

  if (PROCNUM == ARMCPU_ARM9) {
    // according to gbatek, these flags are forced on until the condition is
    // removed. no proof of this though...
...
static void writereg_POWCNT1(const int size, const u32 adr, const u32 val) {
  ...
  // gbatek: "When disabled, corresponding Ports become Read-only, corresponding
  ...
  if (wasGeomEnabled && !isGeomEnabled) {
    ...
    // so, so bad. we need to model this with hardware-like operations instead
    // of c++ code
    GFX3D_State prior = gfx3d.state;
    reconstruct(&gfx3d.state);
    memcpy(gfx3d.state.u16ToonTable, prior.u16ToonTable,
           sizeof(prior.u16ToonTable));
    // dont think we should save this one: it's sent with 3d commands, not
    // random bonus immediate register writes like the toon table
    // memcpy(gfx3d.state.shininessTable, prior.shininessTable,
    // sizeof(prior.shininessTable));
  }
}

static INLINE void MMU_IPCSync(u8 proc, u32 val) {
  // INFO("IPC%s sync 0x%04X (0x%02X|%02X)\n", proc?"7":"9", val, val >> 8, val
  // & 0xFF);
  ...
  if (nds.ensataEmulation && proc == 1 && nds.ensataIpcSyncCounter < 9) {
    ...
    if (iteration == 8 - nds.ensataIpcSyncCounter)
      ...
      printf("ERROR: ENSATA IPC SYNC HACK FAILED; BAD THINGS MAY HAPPEN\n");

    // for some reason, the arm9 doesn't handshake when ensata is detected.
    // so we complete the protocol here, which is to mirror the values 8..0 back
    // to the arm7 as they are written by the arm7
    sync_r &= 0xF0FF;
    sync_r |= (iteration << 8);
    sync_l &= 0xFFF0;
    sync_l |= iteration;
...
static INLINE u16 read_timer(int proc, int timerIndex) {
  ...
  // for unchained timers, we do not keep the timer up to date. its value will
  // need to be calculated here
  s32 diff = (s32)(nds.timerCycle[proc][timerIndex] - nds_timer);
  assert(diff >= 0);
  if (diff < 0)
    printf("NEW EMULOOP BAD NEWS PLEASE REPORT: TIME READ DIFF < 0 (%d) (%d) "
           "(%d)\n",
  ...
  // whichever instruction setup this counter should advance nds_timer (I
  // think?) and the division should truncate down to 65535 immediately
  else if (units > 65536) {
...
static INLINE void write_timer(int proc, int timerIndex, u16 val) {
  ...
  nds.timerCycle[proc][timerIndex] =
      nds_timer + (remain << MMU.timerMODE[proc][timerIndex]);
...
u32 TGXSTAT::read32() {
  ...
  if (gxFIFO.size >= 255)
    ret |= BIT(24); // fifo full
  if (gxFIFO.size < 128)
    ret |= BIT(25); // fifo half
  if (gxFIFO.size == 0)
    ret |= BIT(26); // fifo empty
  // determine busy flag.
  ...
  // _hack_getMatrixStackLevel(1), _hack_getMatrixStackLevel(2));
  return ret;
}
...
void MMU_struct_new::write_dma(const int proc, const int size, const u32 _adr,
                               const u32 val) {
  // printf("%08lld -- write_dma: %d %d %08X
...
bool DmaController::loadstate(EMUFILE &f) {
  ...
  f.read_u8(enable);
  f.read_u8(irq);
  f.read_u8(repeatMode);
  f.read_u8(_startmode);
  f.read_u8(userEnable);
  f.read_32LE(wordcount);
  u8 temp;
  f.read_u8(temp);
  startmode = (EDMAMode)temp;
  f.read_u8(temp);
  bitWidth = (EDMABitWidth)temp;
  f.read_u8(temp);
  sar = (EDMASourceUpdate)temp;
  f.read_u8(temp);
  dar = (EDMADestinationUpdate)temp;
  f.read_32LE(saddr);
  f.read_32LE(daddr);
  f.read_32LE(dmaCheck);
  f.read_32LE(running);
  f.read_32LE(paused);
  f.read_32LE(triggered);
...
void DmaController::savestate(EMUFILE &f) {
  ...
  f.write_u8(enable);
  f.write_u8(irq);
  f.write_u8(repeatMode);
  f.write_u8(_startmode);
  ...
  f.write_32LE(saddr);
  f.write_32LE(daddr);
  f.write_32LE(dmaCheck);
  f.write_32LE(running);
  f.write_32LE(paused);
  f.write_32LE(triggered);
...
void DmaController::write32(const u32 val) {
  ...
  if (enable) {
    // address registers are reloaded from user's settings whenever dma is
    // enabled this is tested well by contra4 classic games, which use this to
    // hdma scroll registers specifically in the fit-screen mode.
    saddr = saddr_user;
    daddr = daddr_user;
  }

  // printf("dma %d,%d set to startmode %d with wordcount set to:
  ...
  // we should probably also only be latching these other regs in that case
  // too.. but for now just this one will do (otherwise the dma repeat stop
  // procedure (in this case the ff4 title menu load with gamecard dma) will
  // fail) if(!running) enable = userEnable;
  ...
  //(otherwise the dma repeat stop procedure (in this case the ff4 title menu
  //load with gamecard dma) will fail)
  bool doNotStart = false;
  if (startmode != EDMAMode_Immediate && startmode != EDMAMode_GXFifo &&
      wasEnable)
    doNotStart = true;
...
void DmaController::exec() {
  // this function runs when the DMA ends. the dma start actually queues this
  // event after some kind of guess as to how long the DMA should take

  // printf("ARM%c DMA%d execute, count %08X, mode %d%s\n", procnum?'7':'9',
  // chan, wordcount, startmode, running?" - RUNNING":""); we'll need to
  // unfreeze the arm9 bus now
  if (procnum == ARMCPU_ARM9)
  ...
  if (enable) {
    ...
    if (procnum == ARMCPU_ARM9)
      startmode = (EDMAMode)_startmode;
    else {
      // arm7 startmode analysis:
      static const EDMAMode lookup[] = {EDMAMode_Immediate, EDMAMode_VBlank,
                                        EDMAMode_Card, EDMAMode7_Wifi};
    ...
    if (triggered) {
      // if(procnum==0) printf("vc=%03d %08lld trig type %d dma#%d w/words %d at
      // src:%08X dst:%08X
      // gxf:%d",nds.VCount,nds_timer,startmode,chan,wordcount,saddr,daddr,gxFIFO.size);
      running = TRUE;
      paused = FALSE;
      if (procnum == ARMCPU_ARM9)
        doCopy<ARMCPU_ARM9>();
...
template <int PROCNUM> void DmaController::doCopy() {
  ...
  if (PROCNUM == ARMCPU_ARM9)
    if (todo == 0)
      todo = 0x200000; // according to gbatek.. we've verified this behaviour on
                       // the arm7
  if (startmode == EDMAMode_MemDisplay) {
    todo = 128; // this is a hack. maybe an alright one though. it should be 4
                // words at a time. this is a whole scanline
  ...
  // if these do not use MMU_AT_DMA and the corresponding code in the read/write
  // routines, then danny phantom title screen will be filled with a garbage
  // char which is made by dmaing from 0x00000000 to 0x06000000
  // TODO - these might be losing out a lot by not going through the templated
  // version anymore. we might make another function to do just the raw copy op
  // which can use them with checks outside the loop
  int time_elapsed = 0;
  if (sz == 4) {
    for (s32 i = (s32)todo; i > 0; i--) {
      time_elapsed +=
          _MMU_accesstime<PROCNUM, MMU_AT_DMA, 32, MMU_AD_READ, TRUE>(src,
                                                                      true);
      time_elapsed +=
          _MMU_accesstime<PROCNUM, MMU_AT_DMA, 32, MMU_AD_WRITE, TRUE>(dst,
                                                                       true);
    ...
    for (s32 i = (s32)todo; i > 0; i--) {
      time_elapsed +=
          _MMU_accesstime<PROCNUM, MMU_AT_DMA, 16, MMU_AD_READ, TRUE>(src,
                                                                      true);
      time_elapsed +=
          _MMU_accesstime<PROCNUM, MMU_AT_DMA, 16, MMU_AD_WRITE, TRUE>(dst,
                                                                       true);
  ...
  // printf("ARM%c dma of size %d from 0x%08X to 0x%08X took %d
  // cycles\n",PROCNUM==0?'9':'7',todo*sz,saddr,daddr,time_elapsed);

  // reschedule an event for the end of this dma, and figure out how much it
  // cost us
  doSchedule();
  ...
  saddr = src;
  if (dar !=
      EDMADestinationUpdate_IncrementReload) // but dont write back dst if we
                                             // were supposed to reload
    daddr = dst;
...
void DmaController::doStop() {
  // if(procnum==0) printf("%08lld stop type %d
...
static INLINE void write_auxspicnt(const int PROCNUM, const int size,
                                   const int adr, const int val) {
  ...
  // printf("MMU%c: cnt %04X, old %04X\n", PROCNUM?'7':'9', MMU.AUX_SPI_CNT,
  // oldCnt);
}

template <u8 PROCNUM> bool validateIORegsWrite(u32 addr, u8 size, u32 val) {
  if (PROCNUM == ARMCPU_ARM9) {
    ...
    // R case eng_3D_POS_RESULT + 0x00: case eng_3D_POS_RESULT + 0x04: case
    // eng_3D_POS_RESULT + 0x08: case eng_3D_POS_RESULT + 0x0C: R case
    // eng_3D_VEC_RESULT + 0x00: case eng_3D_VEC_RESULT + 0x04: R case
    // eng_3D_CLIPMTX_RESULT + 0x00: case eng_3D_CLIPMTX_RESULT + 0x04: case
    // eng_3D_CLIPMTX_RESULT + 0x08: case eng_3D_CLIPMTX_RESULT + 0x0C: R case
    // eng_3D_CLIPMTX_RESULT + 0x10: case eng_3D_CLIPMTX_RESULT + 0x14: case
    // eng_3D_CLIPMTX_RESULT + 0x18: case eng_3D_CLIPMTX_RESULT + 0x1C: R case
    // eng_3D_CLIPMTX_RESULT + 0x20: case eng_3D_CLIPMTX_RESULT + 0x24: case
    // eng_3D_CLIPMTX_RESULT + 0x28: case eng_3D_CLIPMTX_RESULT + 0x2C: R case
    // eng_3D_CLIPMTX_RESULT + 0x30: case eng_3D_CLIPMTX_RESULT + 0x34: case
    // eng_3D_CLIPMTX_RESULT + 0x38: case eng_3D_CLIPMTX_RESULT + 0x3C: R case
    // eng_3D_VECMTX_RESULT + 0x00: case eng_3D_VECMTX_RESULT + 0x04: case
    // eng_3D_VECMTX_RESULT + 0x08: case eng_3D_VECMTX_RESULT + 0x0C: R case
    // eng_3D_VECMTX_RESULT + 0x20:
...
bool validateIORegsRead(u32 addr, u8 size)
{
    if (PROCNUM == ARMCPU_ARM9)
    {
        ...
        case eng_3D_POS_RESULT + 0x00:
        case eng_3D_POS_RESULT + 0x04:
        case eng_3D_POS_RESULT + 0x08:
        case eng_3D_POS_RESULT + 0x0C:
        case eng_3D_VEC_RESULT + 0x00:
        case eng_3D_VEC_RESULT + 0x04:
        case eng_3D_CLIPMTX_RESULT + 0x00:
        case eng_3D_CLIPMTX_RESULT + 0x04:
        case eng_3D_CLIPMTX_RESULT + 0x08:
        case eng_3D_CLIPMTX_RESULT + 0x0C:
        case eng_3D_CLIPMTX_RESULT + 0x10:
        case eng_3D_CLIPMTX_RESULT + 0x14:
        case eng_3D_CLIPMTX_RESULT + 0x18:
        case eng_3D_CLIPMTX_RESULT + 0x1C:
        case eng_3D_CLIPMTX_RESULT + 0x20:
        case eng_3D_CLIPMTX_RESULT + 0x24:
        case eng_3D_CLIPMTX_RESULT + 0x28:
        case eng_3D_CLIPMTX_RESULT + 0x2C:
        case eng_3D_CLIPMTX_RESULT + 0x30:
        case eng_3D_CLIPMTX_RESULT + 0x34:
        case eng_3D_CLIPMTX_RESULT + 0x38:
        case eng_3D_CLIPMTX_RESULT + 0x3C:
        case eng_3D_VECMTX_RESULT + 0x00:
        case eng_3D_VECMTX_RESULT + 0x04:
        case eng_3D_VECMTX_RESULT + 0x08:
        case eng_3D_VECMTX_RESULT + 0x0C:
...
#define VALIDATE_IO_REGS_READ(PROC, SIZE)                                      \
  if (!validateIORegsRead<PROC>(adr, SIZE))                                    \
    return 0;
#else
#define VALIDATE_IO_REGS_READ(PROC, SIZE) ;
#endif
...
void FASTCALL _MMU_ARM9_write08(u32 adr, u8 val) {
  ...
  {
    if (!validateIORegsWrite<ARMCPU_ARM9>(adr, 8, val))
      return;

    // TODO: add pal reg
    if (nds.power1.gpuMain == 0)
      if ((adr >= 0x04000008) && (adr <= 0x0400005F))
        return;
    if (nds.power1.gpuSub == 0)
      if ((adr >= 0x04001008) && (adr <= 0x0400105F))
        return;
    if (nds.power1.gfx3d_geometry == 0)
      if ((adr >= 0x04000400) && (adr <= 0x040006FF))
        return;
    if (nds.power1.gfx3d_render == 0)
      if ((adr >= 0x04000320) && (adr <= 0x040003FF))
        return;
    ...
    case REG_DISPA_DISP3DCNT + 1:
      // TODO: We need to handle acknowledgement flags properly. But for now,
      // just drop the bits. Test case: The "Planet Rescue: Animal Emergency"
      // title screen will check these flags.
    ...
    case REG_AUXSPIDATA: {
      // if(val!=0) MMU.AUX_SPI_CMD = val & 0xFF; //zero 20-aug-2013 - this
      // seems pointless
      u8 spidata = slot1_device->auxspi_transaction(ARMCPU_ARM9, (u8)val);
      T1WriteWord(MMU.MMU_MEM[ARMCPU_ARM9][(REG_AUXSPIDATA >> 20) & 0xff],
                  REG_AUXSPIDATA & 0xfff, spidata);
    ...
    MMU.MMU_MEM[ARMCPU_ARM9][adr >> 20]
               [adr & MMU.MMU_MASK[ARMCPU_ARM9][adr >> 20]] = val;
    return;
  }
...
void FASTCALL _MMU_ARM9_write16(u32 adr, u16 val) {
  ...
  case 0x04: // I/O register
  {
    if (!validateIORegsWrite<ARMCPU_ARM9>(adr, 16, val))
      return;
    ...
    case REG_EXMEMCNT: {
      ...
      T1WriteWord(MMU.MMU_MEM[ARMCPU_ARM7][0x40], 0x204,
                  (val & 0xFF80) | (remote_proc & 0x7F));
      return;
    }
    ...
    case REG_IE + 2:
      NDS_Reschedule();
      MMU.reg_IE[ARMCPU_ARM9] =
          (MMU.reg_IE[ARMCPU_ARM9] & 0xFFFF) | (((u32)val) << 16);
      return;
    case REG_IF:
    ...
    case REG_GCROMCTRL:
      MMU_writeToGCControl<ARMCPU_ARM9>(
          (T1ReadLong(MMU.MMU_MEM[0][0x40], 0x1A4) & 0xFFFF0000) | val);
      return;
    case REG_GCROMCTRL + 2:
      MMU_writeToGCControl<ARMCPU_ARM9>(
          (T1ReadLong(MMU.MMU_MEM[0][0x40], 0x1A4) & 0xFFFF) |
          ((u32)val << 16));
      return;
    }

    T1WriteWord(MMU.MMU_MEM[ARMCPU_ARM9][adr >> 20],
                adr & MMU.MMU_MASK[ARMCPU_ARM9][adr >> 20], val);
    return;
  }
...
void FASTCALL _MMU_ARM9_write32(u32 adr, u32 val) {
  ...
  case 0x04: // I/O register
  {
    if (!validateIORegsWrite<ARMCPU_ARM9>(adr, 32, val))
      return;

    // this blocks writes to memories when the engines that control them are
    // powered down but is there any other way to write to the geometry engine?
    // (via FIFO?)
    //  TODO: add pal reg
    if (nds.power1.gpuMain == 0)
      if ((adr >= 0x04000008) && (adr <= 0x0400005F))
    ...
    // this looks ugly but should reduce load on register writes, they are done
    // as lookups by the compiler
    switch (adr >> 4) {
    ...
    case REG_SQRTCNT:
      MMU_new.sqrt.write16((u16)val);
      return;
    case REG_DIVCNT:
      MMU_new.div.write16((u16)val);
      return;

    case REG_POWCNT1:
      writereg_POWCNT1(32, adr, val);
      break;
    ...
    case 0x04FFF010:
      if (nds.ensataEmulation && nds.ensataHandshake == ENSATA_HANDSHAKE_ack &&
          val == 0x13579bdf)
    ...
    case eng_3D_CLIPMTX_RESULT:
      if (nds.ensataEmulation && nds.ensataHandshake == ENSATA_HANDSHAKE_none &&
          val == 0x2468ace0) {
    ...
    T1WriteLong(MMU.MMU_MEM[ARMCPU_ARM9][adr >> 20],
                adr & MMU.MMU_MASK[ARMCPU_ARM9][adr >> 20], val);
    return;
  }
...
u8 FASTCALL _MMU_ARM9_read08(u32 adr) {
  ...
  if ((adr >> 24) == 4) {
    ...
    // sqrtcnt isnt big enough for these to exist. but they'd probably return 0
    // so its ok
    case REG_SQRTCNT + 2:
    ...
    // divcnt isnt big enough for these to exist. but they'd probably return 0
    // so its ok
    case REG_DIVCNT + 2:
    ...
    case REG_TM0CNTL + 1:
      return _MMU_ARM9_read16(adr - 1) >> 8;
    case REG_TM0CNTL + 2:
      return _MMU_ARM9_read16(adr);
    case REG_TM0CNTL + 3:
      return _MMU_ARM9_read16(adr - 1) >> 8;
...
u16 FASTCALL _MMU_ARM9_read16(u32 adr) {
  ...
  mmu_log_debug_ARM9(
      adr, "(read16) 0x%04X",
      T1ReadWord_guaranteedAligned(MMU.MMU_MEM[ARMCPU_ARM9][adr >> 20],
  ...
  if ((adr >> 24) == 4) {
    ...
    // sqrtcnt isnt big enough for this to exist. but it'd probably return 0 so
    // its ok
    case REG_SQRTCNT + 2:
    ...
    // divcnt isnt big enough for this to exist. but it'd probably return 0 so
    // its ok
    case REG_DIVCNT + 2:
  ...
  return T1ReadWord_guaranteedAligned(MMU.MMU_MEM[ARMCPU_ARM9][adr >> 20],
                                      adr &
                                          MMU.MMU_MASK[ARMCPU_ARM9][adr >> 20]);
}
...
u32 FASTCALL _MMU_ARM9_read32(u32 adr) {
  ...
  mmu_log_debug_ARM9(
      adr, "(read32) 0x%08X",
      T1ReadLong_guaranteedAligned(MMU.MMU_MEM[ARMCPU_ARM9][adr >> 20],
                                   adr & MMU.MMU_MASK[ARMCPU_ARM9][adr >> 20]));
  ...
  if ((adr >> 24) == 4) {
    ...
    // these aren't readable.
    // note: ratatouille stage 3 begins testing this.. it will write a 256, then
    // read it, and if it reads back a 256, the 3d display will be scrolled
    // invisibly. it needs to read a 0 to cause an unscrolled 3d display.
    case REG_DISPA_BG0HOFS:
    case REG_DISPA_BG1HOFS:
    case REG_DISPA_BG2HOFS:
    case REG_DISPA_BG3HOFS:
    ...
    // Dolphin Island Underwater Adventures uses this amidst seemingly
    // reasonable divs so we're going to emulate it. well, it's pretty
    // reasonable to read them as 32bits though, isnt it?
    case REG_DIVCNT:
      return MMU_new.div.read16();
    case REG_SQRTCNT:
    ...
    case eng_3D_CLIPMTX_RESULT + 60: {
      // LOG("4000640h..67Fh - CLIPMTX_RESULT - Read Current Clip Coordinates
      // Matrix (R)");
    ...
    case eng_3D_VECMTX_RESULT + 32: {
      // LOG("4000680h..6A3h - VECMTX_RESULT - Read Current Directional Vector
      // Matrix (R)");
    ...
    case eng_3D_RAM_COUNT: {
      ...
      // LOG ("read32 - RAM_COUNT -> 0x%X", ((u32
      // *)(MMU.MMU_MEM[ARMCPU_ARM9][(adr>>20)&0xFF]))[(adr&MMU.MMU_MASK[ARMCPU_ARM9][(adr>>20)&0xFF])>>2]);
    }
    ...
    return T1ReadLong_guaranteedAligned(
        MMU.MMU_MEM[ARMCPU_ARM9][adr >> 20],
        adr & MMU.MMU_MASK[ARMCPU_ARM9][adr >> 20]);
  }
  ...
  // Removed the &0xFF as they are implicit with the adr&0x0FFFFFFF [zeromus,
  // inspired by shash]
  return T1ReadLong_guaranteedAligned(MMU.MMU_MEM[ARMCPU_ARM9][adr >> 20],
                                      adr &
                                          MMU.MMU_MASK[ARMCPU_ARM9][adr >> 20]);
}
//==================================================================================================
...
void FASTCALL _MMU_ARM7_write08(u32 adr, u8 val) {
  ...
  if ((adr >> 24) == 4) {
    ...
    case REG_POSTFLG:
      ...
      if (CommonSettings.UseExtFirmware && CommonSettings.BootFromFirmware &&
          extFirmwareObj->loaded() && val == 1)
    ...
    case REG_HALTCNT: {
      ...
      // val, MMU.reg_IME[1], MMU.reg_IE[1], MMU.reg_IF_bits[1],
      // NDS_ARM7.instruct_adr);
      switch (val) {
    ...
    case REG_AUXSPIDATA: {
      ...
      T1WriteWord(MMU.MMU_MEM[ARMCPU_ARM7][(REG_AUXSPIDATA >> 20) & 0xff],
                  REG_AUXSPIDATA & 0xfff, spidata);
      MMU.AUX_SPI_CNT &= ~0x80; // remove busy flag
      return;
    }
    ...
    case REG_SPIDATA:
      ...
      // when load firmware header into RAM at 0x027FF830
      MMU_writeToSPIData(val);
      return;
    }
    MMU.MMU_MEM[ARMCPU_ARM7][adr >> 20]
               [adr & MMU.MMU_MASK[ARMCPU_ARM7][adr >> 20]] = val;
    return;
  }
...
void FASTCALL _MMU_ARM7_write16(u32 adr, u16 val) {
  ...
  if ((adr & 0xFFFF0000) == 0x04800000) {
    ...
    T1WriteWord(MMU.MMU_MEM[ARMCPU_ARM7][0x48],
                adr & MMU.MMU_MASK[ARMCPU_ARM7][0x48], val);
    return;
  }

  // Address is an IO register
  if ((adr >> 24) == 4) {
    if (!validateIORegsWrite<ARMCPU_ARM7>(adr, 16, val))
      return;
    ...
    case REG_SPICNT: {
      ...
      T1WriteWord(MMU.MMU_MEM[ARMCPU_ARM7][(REG_SPICNT >> 20) & 0xff],
                  REG_SPICNT & 0xfff, val);
    }
      return;
    ...
    case REG_SPIDATA:
      ...
      /* NOTICE: Perhaps we have to use gbatek-like reg names instead of
       * libnds-like ones ...*/
    ...
    case REG_IE + 2:
      ...
      MMU.reg_IE[ARMCPU_ARM7] =
          (MMU.reg_IE[ARMCPU_ARM7] & 0xFFFF) | (((u32)val) << 16);
      return;
    ...
    case REG_GCROMCTRL:
      MMU_writeToGCControl<ARMCPU_ARM7>(
          (T1ReadLong(MMU.MMU_MEM[1][0x40], 0x1A4) & 0xFFFF0000) | val);
      return;
    case REG_GCROMCTRL + 2:
      MMU_writeToGCControl<ARMCPU_ARM7>(
          (T1ReadLong(MMU.MMU_MEM[1][0x40], 0x1A4) & 0xFFFF) |
          ((u32)val << 16));
      return;
    }

    T1WriteWord(MMU.MMU_MEM[ARMCPU_ARM7][adr >> 20],
                adr & MMU.MMU_MASK[ARMCPU_ARM7][adr >> 20], val);
    return;
  }
...
void FASTCALL _MMU_ARM7_write32(u32 adr, u32 val) {
  ...
  if ((adr & 0xFFFF0000) == 0x04800000) {
    ...
    T1WriteLong(MMU.MMU_MEM[ARMCPU_ARM7][0x48],
                adr & MMU.MMU_MASK[ARMCPU_ARM7][0x48], val);
    return;
  }
  ...
  // Address is an IO register
  if ((adr >> 24) == 4) {
    if (!validateIORegsWrite<ARMCPU_ARM7>(adr, 32, val))
      return;
    ...
    T1WriteLong(MMU.MMU_MEM[ARMCPU_ARM7][adr >> 20],
                adr & MMU.MMU_MASK[ARMCPU_ARM7][adr >> 20], val);
    return;
  }
...
u8 FASTCALL _MMU_ARM7_read08(u32 adr) {
  ...
  mmu_log_debug_ARM7(
      adr, "(read08) 0x%02X",
      MMU.MMU_MEM[ARMCPU_ARM7][(adr >> 20) & 0xFF]
  ...
  if (adr < 0x4000) {
    ...
    // How accurate is this? our instruct_adr may not be exactly what the
    // hardware uses (may use something +/- 0x08 or so) This may be inaccurate
    // at the very edge cases.
    if (NDS_ARM7.instruct_adr > 0x3FFF)
      return 0xFF;
  }
  ...
  if ((adr >> 24) == 4) {
    ...
    case REG_TM0CNTL + 1:
      return _MMU_ARM7_read16(adr - 1) >> 8;
    case REG_TM0CNTL + 2:
      return _MMU_ARM7_read16(adr);
    case REG_TM0CNTL + 3:
      return _MMU_ARM7_read16(adr - 1) >> 8;
    ...
    return MMU.MMU_MEM[ARMCPU_ARM7][adr >> 20]
                      [adr & MMU.MMU_MASK[ARMCPU_ARM7][adr >> 20]];
  }
...
u16 FASTCALL _MMU_ARM7_read16(u32 adr) {
  ...
  if ((adr >> 24) == 4) {
    ...
    case REG_VRAMSTAT:
      // make sure WRAMSTAT is stashed and then fallthrough to return the value
      // from memory. i know, gross.
    ...
    case REG_KEYINPUT:
      ...
      // since the arm7 polls this (and EXTKEYIN) every frame, we shouldnt count
      // this as an input check LagFrameFlag=0;
      break;
    }
    return T1ReadWord_guaranteedAligned(
        MMU.MMU_MEM[ARMCPU_ARM7][adr >> 20],
  ...
  // Removed the &0xFF as they are implicit with the adr&0x0FFFFFFF
  return T1ReadWord_guaranteedAligned(MMU.MMU_MEM[ARMCPU_ARM7][adr >> 20],
                                      adr &
                                          MMU.MMU_MASK[ARMCPU_ARM7][adr >> 20]);
}
//================================================= MMU ARM7 read 32
u32 FASTCALL _MMU_ARM7_read32(u32 adr) {
  ...
  if ((adr >> 24) == 4) {
    ...
    case REG_VRAMSTAT:
      // make sure WRAMSTAT is stashed and then fallthrough return the value
      // from memory. i know, gross.
      T1WriteByte(MMU.MMU_MEM[ARMCPU_ARM7][0x40], 0x241, MMU.WRAMCNT);
      break;
    }
  ...
  //  Removed the &0xFF as they are implicit with the adr&0x0FFFFFFF [zeromus,
  //  inspired by shash]
  return T1ReadLong_guaranteedAligned(MMU.MMU_MEM[ARMCPU_ARM7][adr >> 20],
                                      adr &
                                          MMU.MMU_MASK[ARMCPU_ARM7][adr >> 20]);
}
 ...
 */
const armcpu_memory_iface arm9_base_memory_iface = {arm9_prefetch32,
                                                    arm9_prefetch16,
...
const armcpu_memory_iface arm7_base_memory_iface = {arm7_prefetch32,
                                                    arm7_prefetch16,
...
const armcpu_memory_iface arm9_direct_memory_iface = {
    ...
    arm9_read8,  arm9_read16,  arm9_read32,

    arm9_write8, arm9_write16, arm9_write32,
...
//               profile_memory_accesses[arm9][access_type][mem_profile].masked_value);*/
//       address_found = 1;
//       profile_memory_accesses[arm9][access_type][mem_profile].num_accesses +=
//       1;
//     }
//   }
//
