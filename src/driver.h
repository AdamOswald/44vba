...
// each platform needs to implement this, although it doesnt need to implement
// any functions
class BaseDriver {
public:
  BaseDriver();
  ...
  } // -1 == indefinitely, 0 == don't pospone, 500 == don't draw for 0.5 seconds
  ...
  virtual eStepMainLoopResult EMU_StepMainLoop(bool allowSleep, bool allowPause,
                                               int frameSkip, bool disableUser,
                                               bool disableCore) {
  ...
  enum eDebug_IOReg { EDEBUG_IOREG_DMA };
