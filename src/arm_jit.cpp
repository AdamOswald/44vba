...
#define printJIT(buf, val)                                                     \
  {                                                                            \
    ...
    prn->setPrototype(kX86FuncConvDefault,                                     \
                      FuncBuilder3<void, void *, void *, u32>());              \
    prn->setArgument(0, io);                                                   \
    prn->setArgument(1, txt);                                                  \
    prn->setArgument(2, data);                                                 \
    X86CompilerFuncCall *prn_flush = c.call((uintptr_t)fflush);                \
    prn_flush->setPrototype(kX86FuncConvDefault,                               \
                            FuncBuilder1<void, void *>());                     \
...
static uintptr_t *JIT_MEM[2][32] = {
    ...
    {/* 0X*/ DUP2(JIT.ARM7_BIOS),
     /* 1X*/ DUP2(NULL),
     /* 2X*/ DUP2(JIT.MAIN_MEM),
     /* 3X*/ JIT.SWIRAM, JIT.ARM7_ERAM,
     /* 4X*/ NULL, JIT.ARM7_WIRAM,
     /* 5X*/ DUP2(NULL),
...
static u32 JIT_MASK[2][32] = {
    ...
    {/* 0X*/ DUP2(0x00003FFF),
     /* 1X*/ DUP2(0x00000000),
     /* 2X*/ DUP2(0x003FFFFF),
     /* 3X*/ 0x00007FFF, 0x0000FFFF,
     /* 4X*/ 0x00000000, 0x0000FFFF,
     /* 5X*/ DUP2(0x00000000),
     /* 6X*/ 0x0003FFFF, 0x00000000,
     /* 7X*/ DUP2(0x00000000),
     /* 8X*/ DUP2(0x00000000),
     /* 9X*/ DUP2(0x00000000),
     /* AX*/ DUP2(0x00000000),
...
static void init_jit_mem() {
  ...
  for (int proc = 0; proc < 2; proc++)
    for (int i = 0; i < 0x4000; i++)
      JIT.JIT_MEM[proc][i] =
          JIT_MEM[proc][i >> 9] + (((i << 14) & JIT_MASK[proc][i >> 9]) >> 1);
}
...
// On x86_64, allocate jitted code from a static buffer to ensure that it's
// within 2GB of .text Allows call instructions to use pcrel offsets, as opposed
// to slower indirect calls. Reduces memory needed for function pointers.
...
struct ASMJIT_API StaticCodeGenerator : public Context {
  StaticCodeGenerator() {
    ...
    int err = mprotect(scratchpad - align, sizeof(scratchpad) + align,
  ...
  uint32_t generate(void **dest, Assembler *assembler) {
    ...
    if (size > (uintptr_t)(scratchpad + sizeof(scratchpad) - scratchptr)) {
      ...
      arm_jit_reset(1);
      // If arm_jit_reset didn't involve recompiling op_cmp, we could keep the
      // current function.
      *dest = NULL;
      return kErrorOk;
    }
...
#define reg_pos_ptr(x)                                                         \
  dword_ptr(bb_cpu, offsetof(armcpu_t, R) + 4 * REG_POS(i, (x)))
#define reg_pos_ptrL(x)                                                        \
  word_ptr(bb_cpu, offsetof(armcpu_t, R) + 4 * REG_POS(i, (x)))
#define reg_pos_ptrH(x)                                                        \
  word_ptr(bb_cpu, offsetof(armcpu_t, R) + 4 * REG_POS(i, (x)) + 2)
#define reg_pos_ptrB(x)                                                        \
  byte_ptr(bb_cpu, offsetof(armcpu_t, R) + 4 * REG_POS(i, (x)))
#define reg_pos_thumb(x)                                                       \
  dword_ptr(bb_cpu, offsetof(armcpu_t, R) + 4 * ((i >> (x)) & 0x7))
#define reg_pos_thumbB(x)                                                      \
  byte_ptr(bb_cpu, offsetof(armcpu_t, R) + 4 * ((i >> (x)) & 0x7))
...
#define profiler_counter_arm(opcode)                                           \
  qword_ptr(bb_profiler, offsetof(JIT_PROFILER, arm_count) +                   \
                             (INSTRUCTION_INDEX(opcode) * sizeof(u64)))
#define profiler_counter_thumb(opcode)                                         \
  qword_ptr(bb_profiler, offsetof(JIT_PROFILER, thumb_count) +                 \
                             ((opcode >> 6) * sizeof(u64)))
...
#define ROR_IMM                                                                \
  ...
  if (!imm) {                                                                  \
    ...
    c.ror(rhs, imm);                                                           \
  u32 rhs_first =                                                              \
      imm ? ROR(cpu->R[REG_POS(i, 0)], imm)                                    \
          : ((u32)cpu->CPSR.bits.C << 31) | (cpu->R[REG_POS(i, 0)] >> 1);
...
//==================================================================== common
//funcs
static void emit_MMU_aluMemCycles(int alu_cycles, GpVar mem_cycles,
                                  int population) {
...
// TODO: SPSR: if(cpu->CPSR.bits.mode == USR || cpu->CPSR.bits.mode == SYS)
// return 1;
...
enum {
  ...
  MEMTYPE_SWIRAM = 4,
  MEMTYPE_OTHER =
      5, // memory that is known to not be MAIN, DTCM, ERAM, or SWIRAM
};
...
#define T(op)                                                                  \
  op<0, 0>, op<0, 1>, op<0, 2>, NULL, NULL, op<1, 0>, op<1, 1>, NULL,          \
      op<1, 3>, op<1, 4>
...
#define OP_LDR_(mem_op, arg, sign_op, writeback)                               \
  ...
  X86CompilerFuncCall *ctx =                                                   \
      c.call((void *)mem_op##_tab[PROCNUM][classify_adr(adr_first, 0)]);       \
...
#define OP_STR_(mem_op, arg, sign_op, writeback)                               \
  ...
  u32 adr_first = sign_op(cpu->R[REG_POS(i, 16)], rhs_first);                  \
  X86CompilerFuncCall *ctx =                                                   \
      c.call((void *)mem_op##_tab[PROCNUM][classify_adr(adr_first, 1)]);       \
  ctx->setPrototype(ASMJIT_CALL_CONV, FuncBuilder2<u32, u32, u32>());          \
  ctx->setArgument(0, adr);                                                    \
...
template <int PROCNUM, u8 Rnum> static u32 FASTCALL OP_LDRD_REG(u32 adr) {
  cpu->R[Rnum] = READ32(cpu->mem_if->data, adr);

  // For even-numbered registers, we'll do a double-word load. Otherwise, we'll
  // just do a single-word load.
  if ((Rnum & 0x01) == 0) {
    cpu->R[Rnum + 1] = READ32(cpu->mem_if->data, adr + 4);
    return (MMU_memAccessCycles<PROCNUM, 32, MMU_AD_READ>(adr) +
            MMU_memAccessCycles<PROCNUM, 32, MMU_AD_READ>(adr + 4));
  }

  return MMU_memAccessCycles<PROCNUM, 32, MMU_AD_READ>(adr);
}
template <int PROCNUM, u8 Rnum> static u32 FASTCALL OP_STRD_REG(u32 adr) {
  WRITE32(cpu->mem_if->data, adr, cpu->R[Rnum]);

  // For even-numbered registers, we'll do a double-word store. Otherwise, we'll
  // just do a single-word store.
  if ((Rnum & 0x01) == 0) {
    WRITE32(cpu->mem_if->data, adr + 4, cpu->R[Rnum + 1]);
    return (MMU_memAccessCycles<PROCNUM, 32, MMU_AD_WRITE>(adr) +
            MMU_memAccessCycles<PROCNUM, 32, MMU_AD_WRITE>(adr + 4));
  }

  return MMU_memAccessCycles<PROCNUM, 32, MMU_AD_WRITE>(adr);
}
#define T(op, proc)                                                            \
  op<proc, 0>, op<proc, 1>, op<proc, 2>, op<proc, 3>, op<proc, 4>,             \
      op<proc, 5>, op<proc, 6>, op<proc, 7>, op<proc, 8>, op<proc, 9>,         \
      op<proc, 10>, op<proc, 11>, op<proc, 12>, op<proc, 13>, op<proc, 14>,    \
      op<proc, 15>
static const LDRD_STRD_REG op_ldrd_tab[2][16] = {{T(OP_LDRD_REG, 0)},
                                                 {T(OP_LDRD_REG, 1)}};
static const LDRD_STRD_REG op_strd_tab[2][16] = {{T(OP_STRD_REG, 0)},
                                                 {T(OP_STRD_REG, 1)}};
#undef T
...
static int OP_LDRD_STRD_POST_INDEX(const u32 i) {
  ...
  X86CompilerFuncCall *ctx =
      c.call((void *)(BIT5(i) ? op_strd_tab[PROCNUM][Rd_num]
                              : op_ldrd_tab[PROCNUM][Rd_num]));
...
//   SWP/SWPB
//-----------------------------------------------------------------------------
template <int PROCNUM> static u32 FASTCALL op_swp(u32 adr, u32 *Rd, u32 Rs) {
  u32 tmp = ROR(READ32(cpu->mem_if->data, adr), (adr & 3) << 3);
  WRITE32(cpu->mem_if->data, adr, Rs);
  *Rd = tmp;
  return (MMU_memAccessCycles<PROCNUM, 32, MMU_AD_READ>(adr) +
          MMU_memAccessCycles<PROCNUM, 32, MMU_AD_WRITE>(adr));
}
template <int PROCNUM> static u32 FASTCALL op_swpb(u32 adr, u32 *Rd, u32 Rs) {
  u32 tmp = READ8(cpu->mem_if->data, adr);
  WRITE8(cpu->mem_if->data, adr, Rs);
  *Rd = tmp;
  return (MMU_memAccessCycles<PROCNUM, 8, MMU_AD_READ>(adr) +
          MMU_memAccessCycles<PROCNUM, 8, MMU_AD_WRITE>(adr));
}

typedef u32 FASTCALL (*OP_SWP_SWPB)(u32, u32 *, u32);
static const OP_SWP_SWPB op_swp_tab[2][2] = {{op_swp<0>, op_swp<1>},
                                             {op_swpb<0>, op_swpb<1>}};
...
#ifdef ASMJIT_X64
// generic needs to spill regs and main doesn't; if it's inlined gcc isn't smart
// enough to keep the spills out of the common case.
#define LDM_INLINE NOINLINE
#else
// spills either way, and we might as well save codesize by not having separate
// functions
...
static LDM_INLINE FASTCALL u32 OP_LDM_STM_generic(u32 adr, u64 regs, int n) {
  ...
  do {
    ...
    cycles += MMU_memAccessCycles < PROCNUM, 32,
        store ? MMU_AD_WRITE : MMU_AD_READ > (adr);
    adr += 4 * dir;
    regs >>= 4;
  } while (--n > 0);
  return cycles;
...
static LDM_INLINE FASTCALL u32 OP_LDM_STM_other(u32 adr, u64 regs, int n) {
  ...
  cycles = n * MMU_memAccessCycles < PROCNUM, 32,
  store ? MMU_AD_WRITE : MMU_AD_READ > (adr);
#endif
  do {
    if (PROCNUM == ARMCPU_ARM9)
...
static FORCEINLINE FASTCALL u32 OP_LDM_STM_main(u32 adr, u64 regs, int n,
                                                u8 *ptr, u32 cycles) {
#ifdef ENABLE_ADVANCED_TIMING
  cycles = 0;
#endif
...
static u32 FASTCALL OP_LDM_STM(u32 adr, u64 regs, int n) {
  ...
  if ((adr ^ (adr + (dir > 0 ? (n - 1) * 4 : -15 * 4))) &
      ~0x3FFF) // a little conservative, but we don't want to run too many
               // comparisons
  {
    // the memory region spans a page boundary, so we can't factor the address
    // translation out of the loop
    return OP_LDM_STM_generic<PROCNUM, store, dir>(adr, regs, n);
  } else if (PROCNUM == ARMCPU_ARM9 && (adr & ~0x3FFF) == MMU.DTCMRegion) {
    // don't special-case DTCM cycles, even though that would be both faster and
    // more accurate, because that wouldn't match the non-jitted version with
    // !ACCOUNT_FOR_DATA_TCM_SPEED
  ...
  } else if (PROCNUM == ARMCPU_ARM7 && !store &&
             (adr & 0xFF800000) == 0x03800000) {
    ptr = MMU.ARM7_ERAM + (adr & 0xFFFC);
    cycles = n;
  } else if (PROCNUM == ARMCPU_ARM7 && !store &&
             (adr & 0xFF800000) == 0x03000000) {
...
static void call_ldm_stm(GpVar adr, u32 bitmask, bool store, int dir) {
  if (bitmask) {
    ...
    X86CompilerFuncCall *ctx =
        c.call((void *)op_ldm_stm_tab[PROCNUM][store][dir > 0]);
    ctx->setPrototype(ASMJIT_CALL_CONV,
                      FuncBuilder3<u32, u32, uint64_t, int>());
    ctx->setArgument(0, adr);
    ...
    X86CompilerFuncCall *ctx =
        c.call((void *)op_ldm_stm_tab[PROCNUM][store][dir > 0]);
    ctx->setPrototype(ASMJIT_CALL_CONV,
                      FuncBuilder4<u32, u32, u32, u32, int>());
    ctx->setArgument(0, adr);
...
static int op_ldm_stm2(u32 i, bool store, int dir, bool before,
                       bool writeback) {
  u32 bitmask = i & 0xFFFF;
  u32 pop = popregcount(bitmask);
  bool bit15 = BIT15(i);
  ...
  // (store?"STM":"LDM"), REG_POS(i, 16), cpu->R[REG_POS(i, 16)], bitmask);
  ...
  if (!bit15 || store) {
    ...
    // printf("ERROR1\n"); return 1; } oldmode = armcpu_switchMode(cpu, SYS);
    c.mov(oldmode, SYS);
    X86CompilerFuncCall *ctx = c.call((void *)armcpu_switchMode);
...
#define precalc(num)                                                           \
  {                                                                            \
    u32 mask = 0,                                                              \
        set =                                                                  \
            0xFFFFFFFF; /* (x & 0) == 0xFF..FF is allways false (disabled) */  \
    ...
    { /* reason for this define: naming includes var */                        \
      ...
      if (CP15_SIZEIDENTIFIER(cp15.protectBaseSize[num]) ==                    \
          0x1F) { /* for the 4GB region, u32 suffers wraparound */             \
        mask = 0;                                                              \
...
static int OP_MCR(const u32 i) {
  ...
  if (cpnum != 15) {
    // TODO - exception?
    printf("JIT: MCR P%i, 0, R%i, C%i, C%i, %i, %i (don't allocated "
           "coprocessor)\n",
           cpnum, REG_POS(i, 12), REG_POS(i, 16), REG_POS(i, 0),
           (i >> 21) & 0x7, (i >> 5) & 0x7);
    return 2;
  }
  if (REG_POS(i, 12) == 15) {
  ...
  case 1:
    if ((opcode1 == 0) && (opcode2 == 0) && (CRm == 0)) {
      GpVar tmp = c.newGpVar(kX86VarTypeGpd);
      // On the NDS bit0,2,7,12..19 are R/W, Bit3..6 are always set, all other
...
static int OP_LDR_IMM_OFF(const u32 i) {
  LDR_THUMB(
      LDR,
      ((i >> 4) &
       0x7C)); // FIXME: tempValue = (tempValue>>adr) | (tempValue<<(32-adr));
}
...
static int OP_STR_SPREL(const u32 i) {
  ...
  X86CompilerFuncCall *ctx =
      c.call((void *)STR_tab[PROCNUM][classify_adr(adr_first, 1)]);
  ctx->setPrototype(ASMJIT_CALL_CONV, FuncBuilder2<Void, u32, u32>());
  ctx->setArgument(0, addr);
  ctx->setArgument(1, data);
  ctx->setReturn(bb_cycles);
...
static int OP_LDR_SPREL(const u32 i) {
  ...
  X86CompilerFuncCall *ctx =
      c.call((void *)LDR_tab[PROCNUM][classify_adr(adr_first, 0)]);
  ctx->setPrototype(ASMJIT_CALL_CONV, FuncBuilder2<Void, u32, u32 *>());
  ctx->setArgument(0, addr);
  ctx->setArgument(1, data);
  ctx->setReturn(bb_cycles);
...
static int op_ldm_stm_thumb(u32 i, bool store) {
  ...
  // ARM_REF:	If the base register <Rn> is specified in <registers>, the final
  // value of <Rn> is the loaded value
...
static int op_push_pop(u32 i, bool store, bool pc_lr) {
  ...
  emit_MMU_aluMemCycles(store ? (pc_lr ? 4 : 3) : (pc_lr ? 5 : 2), bb_cycles,
                        pop);
  return 1;
}
...
template <int PROCNUM, int thumb> static u32 FASTCALL OP_DECODE() {
  u32 cycles;
  ...
  if (thumb) {
    ...
    if (CONDITION(opcode) == 0xE ||
        TEST_COND(CONDITION(opcode), CODE(opcode), cpu->CPSR))
...
static const ArmOpCompiled op_decode[2][2] = {OP_DECODE<0, 0>, OP_DECODE<0, 1>,
                                              OP_DECODE<1, 0>, OP_DECODE<1, 1>};
...
static bool instr_is_branch(u32 opcode) {
  ...
  if (bb_thumb) {
    ...
    return (x & BRANCH_ALWAYS) ||
           ...
           (x & BRANCH_SWI) || (x & JIT_BYPASS);
  } else
    return (x & BRANCH_ALWAYS) ||
           ((x & BRANCH_POS12) && REG_POS(opcode, 12) == 15) ||
           ((x & BRANCH_LDM) && BIT15(opcode)) || (x & BRANCH_SWI) ||
           (x & JIT_BYPASS);
}
...
static bool instr_uses_r15(u32 opcode) {
  ...
  if (bb_thumb)
    return ((x & SRCREG_POS0) && ((opcode & 7) | ((opcode >> 4) & 8)) == 15) ||
           ((x & SRCREG_POS3) && REG_POS(opcode, 3) == 15) || (x & JIT_BYPASS);
  else
    return ((x & SRCREG_POS0) && REG_POS(opcode, 0) == 15) ||
           ...
           ((x & SRCREG_STM) && BIT15(opcode)) || (x & JIT_BYPASS);
}
...
static int instr_cycles(u32 opcode) {
  ...
  if (instr_is_branch(opcode) &&
      !(instr_attributes(opcode) & (BRANCH_ALWAYS | BRANCH_LDM)))
    c += 2;
  return c;
}
...
static const char *disassemble(u32 opcode) {
  ...
  static const char *conds[16] = {"EQ", "NE", "CS", "CC", "MI", "PL",
                                  "VS", "VC", "HI", "LS", "GE", "LT",
                                  "GT", "LE", "AL", "NV"};
...
static void sync_r15(u32 opcode, bool is_last, bool force) {
  if (instr_does_prefetch(opcode)) {
    ...
    if (force || (instr_attributes(opcode) & JIT_BYPASS) ||
        (instr_attributes(opcode) & BRANCH_SWI) ||
        (is_last && !instr_is_branch(opcode))) {
      JIT_COMMENT("sync_r15: next_instruction %08Xh - %s%s%s%s",
                  bb_next_instruction, force ? " FORCE" : "",
...
static void emit_armop_call(u32 opcode) {
  ArmOpCompiler fc = bb_thumb
                         ? thumb_instruction_compilers[opcode >> 6]
                         : arm_instruction_compilers[INSTRUCTION_INDEX(opcode)];
  if (fc && fc(opcode))
    return;
  ...
  c.mov(arg, opcode);
  OpFunc f = bb_thumb
                 ? thumb_instructions_set[PROCNUM][opcode >> 6]
...
template <int PROCNUM> static u32 compile_basicblock() {
#if LOG_JIT
  ...
  fprintf(stderr, "adr %08Xh %s%c\n", start_adr,
          ARMPROC.CPSR.bits.T ? "THUMB" : "ARM", PROCNUM ? '7' : '9');
#endif
  ...
  for (u32 i = 0, bEndBlock = 0; bEndBlock == 0; i++) {
    ...
    if (bb_thumb)
      ...
      des_arm_instructions_set[INSTRUCTION_INDEX(opcode)](bb_adr, opcode,
                                                          dasmbuf);
    ...
    bEndBlock = instr_is_branch(opcode) ||
                (i >= (CommonSettings.jit_max_block_size - 1));
    ...
    if (instr_is_conditional(opcode)) {
      ...
      if (cycles == 0) {
        ...
        c.lea(bb_total_cycles,
              ptr(bb_total_cycles.r64(), bb_cycles.r64(), kScaleNone, -1));
      } else if (cycles > 1) {
      ...
      if (cycles == 0) {
        JIT_COMMENT("variable cycles");
        c.lea(bb_total_cycles,
              ptr(bb_total_cycles.r64(), bb_cycles.r64(), kScaleNone));
      }
    }
  ...
  if (!instr_does_prefetch(opcode)) {
    JIT_COMMENT("!instr_does_prefetch: copy next_instruction (%08X) to "
                "instruct_adr (%08X)",
  ...
  c.add(dword_ptr(bb_profiler_entry, offsetof(PROFILER_ENTRY, cycles)),
        bb_total_cycles);
  ...
  if (c.getError()) {
    fprintf(stderr, "JIT error at %s%c-%08X: %s\n", bb_thumb ? "THUMB" : "ARM",
            PROCNUM ? '7' : '9', start_adr, getErrorString(c.getError()));
...
template <int PROCNUM> u32 arm_jit_compile() {
  *PROCNUM_ptr = PROCNUM;
  ...
  // prevent endless recompilation of self-modifying code, which would be a
  // memleak since we only free code all at once. also allows us to clear
  // compiled_funcs[] while leaving it sparsely allocated, if the OS does memory
  // overcommit.
  u32 adr = cpu->instruct_adr;
...
#define JITFREE(x)                                                             \
  for (int iii = 0; iii < ARRAY_SIZE(x); iii++)                                \
    if (x[iii])                                                                \
      AsmJit::MemoryManager::getGlobal()->free((void *)x[iii]);                \
  memset(x, 0, sizeof(x));
...
void arm_jit_close() {
  ...
  for (u8 proc = 0; proc < 2; proc++) {
    ...
    for (u16 i = 0; i < 4096; i++) {
      u16 t = 0;
      if (profiler_counter[proc].arm_count[i] == 0)
        continue;
    ...
    for (u16 i = 0; i < 1024; i++) {
      u16 t = 0;
      if (profiler_counter[proc].thumb_count[i] == 0)
        continue;
    ...
    std::qsort(arm_info, last[0], sizeof(PROFILER_COUNTER_INFO),
               (int (*)(const void *, const void *))pcmp);
    std::qsort(thumb_info, last[1], sizeof(PROFILER_COUNTER_INFO),
               (int (*)(const void *, const void *))pcmp);
