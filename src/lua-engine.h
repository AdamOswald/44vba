...
void OpenLuaContext(int uid, void (*print)(int uid, const char *str) = 0,
                    void (*onstart)(int uid) = 0,
                    void (*onstop)(int uid, bool statusOK) = 0);
...
void CallRegisteredLuaMemHook(unsigned int address, int size,
                              unsigned int value, LuaMemHookType hookType);
...
struct LuaSaveData {
  ...
  void
  SaveRecord(int uid,
             unsigned int key); // saves Lua stack into a record and pops it
  void LoadRecord(int uid, unsigned int key, unsigned int itemsToLoad)
      const; // pushes a record's data onto the Lua stack
  void
  SaveRecordPartial(int uid, unsigned int key,
                    int idx); // saves part of the Lua stack (at the given
                              // index) into a record and does NOT pop anything

  void
  ExportRecords(void *file) const; // writes all records to an already-open file
...
private:
  ...
  // (because the memory the destructor deletes isn't refcounted and shouldn't
  // need to be copied) so pass LuaSaveDatas by reference and this should never
  // get called
  LuaSaveData(const LuaSaveData &copy) {}
};
void CallRegisteredLuaSaveFunctions(int savestateNumber, LuaSaveData &saveData);
void CallRegisteredLuaLoadFunctions(int savestateNumber,
                                    const LuaSaveData &saveData);
...
// QUICKLY determining whether a memory address range has a hook associated with
// it, with a bias toward fast rejection because the majority of addresses will
// not be hooked. (it must not use any part of Lua or perform any per-script
// operations,
//  otherwise it would definitely be too slow.)
// calculating the regions when a hook is added/removed may be slow,
// but this is an intentional tradeoff to obtain a high speed of checking during
// later execution
struct TieredRegion {
  template <unsigned int maxGap> struct Region {
    struct Island {
      unsigned int start;
      unsigned int end;
      FORCEINLINE bool Contains(unsigned int address, int size) const {
        return address < end && address + size > start;
      }
    };
  ...
  FORCEINLINE int NotEmpty() { return broad.islands.size(); }
  ...
  FORCEINLINE bool Contains(unsigned int address, int size) {
    return broad.islands[0].Contains(address, size) &&
           mid.Contains(address, size) && narrow.Contains(address, size);
  }
};
extern TieredRegion hookedRegions[LUAMEMHOOK_COUNT];

void CallRegisteredLuaMemHook_LuaMatch(unsigned int address, int size,
                                       unsigned int value,
                                       LuaMemHookType hookType);

FORCEINLINE void CallRegisteredLuaMemHook(unsigned int address, int size,
                                          unsigned int value,
                                          LuaMemHookType hookType) {
  // performance critical! (called VERY frequently)
  // I suggest timing a large number of calls to this function in Release if you
  // change anything in here, before and after, because even the most innocent
  // change can make it become 30% to 400% slower. a good amount to test is:
  // 100000000 calls with no hook set, and another 100000000 with a hook set.
  // (on my system that consistently took 200 ms total in the former case and
  // 350 ms total in the latter case)
  if (hookedRegions[hookType].NotEmpty()) {
    ...
    if (hookedRegions[hookType].Contains(address, size))
      CallRegisteredLuaMemHook_LuaMatch(
          address, size, value,
          hookType); // something has hooked this specific address
  }
}
