...
//
// the shape rasterizers contained herein are based on code supplied by Chris
// Hecker from http://chrishecker.com/Miscellaneous_Technical_Articles
...
// Shadow test cases
//  * SM64 -- Standing near signs and blocks. Note: shadows-behind-trees looks
//  bad in that game.
//  * MKDS -- Shadows under cart in kart selector (the shadows in mario kart are
//  complicated concave shapes)
//  * MKDS -- no junk beneath platform in kart selector / no shadow beneath
//  grate floor in bowser stage
//  * MKDS -- 150cc mirror cup turns all the models inside out
//  * The Wizard of Oz: Beyond the Yellow Brick Road -- shadows under Dorothy
//  and Toto
...
template <typename T> T _min(T a, T b, T c, T d) {
  return min(_min(a, b, d), c);
}
template <typename T> T _max(T a, T b, T c, T d) {
  return max(_max(a, b, d), c);
}
...
//{
//	const double _double2fixmagic = 68719476736.0*1.5;     //2^36 * 1.5,
//(52-_shiftamt=36) uses limited precisicion to floor 	const int _shiftamt = 16;
...
static FORCEINLINE void FloorDivMod(long Numerator, long Denominator,
                                    long &Floor, long &Mod, bool &failure) {
  ...
  // since I see them acting poppy in a way that doesnt happen in the HW.. so
  // alas it is also incorrect. This particular incorrectness is not likely ever
  // to get fixed!

  // assert(Denominator > 0);

  // but we have to bail out since our handling for these cases currently steps
  // scanlines the wrong way and goes totally nuts (freezes)
  if (Denominator <= 0)
    failure = true;
...
struct edge_fx_fl {
  ...
  struct Interpolant {
    ...
    FORCEINLINE void initialize(float top, float bottom, float dx, float dy,
                                long inXStep, float XPrestep, float YPrestep) {
      dx = 0;
      dy *= (bottom - top);
...
FORCEINLINE edge_fx_fl::edge_fx_fl(int Top, int Bottom, VERT **verts,
  ...
  if (Height != 0 || Width != 0) {
    ...
    if (dN != 0) {
      long InitialNumerator = (long)(dM * 16 * Y - dM * verts[Top]->y +
                                     dN * verts[Top]->x - 1 + dN * 16);
    ...
    invw.initialize(1 / verts[Top]->w, 1 / verts[Bottom]->w, dx, dy, XStep,
                    XPrestep, YPrestep);
    u.initialize(verts[Top]->u, verts[Bottom]->u, dx, dy, XStep, XPrestep,
                 YPrestep);
    v.initialize(verts[Top]->v, verts[Bottom]->v, dx, dy, XStep, XPrestep,
                 YPrestep);
    z.initialize(verts[Top]->z, verts[Bottom]->z, dx, dy, XStep, XPrestep,
                 YPrestep);
    for (int i = 0; i < 3; i++)
      color[i].initialize(verts[Top]->fcolor[i], verts[Bottom]->fcolor[i], dx,
                          dy, XStep, XPrestep, YPrestep);
  } else {
...
static FORCEINLINE void alphaBlend(FragmentColor &dst,
                                   const FragmentColor src) {
  if (src.a == 0) {
    return;
  }
...
Render3DError RasterizerUnit<RENDERER>::_SetupTexture(const POLY &thePoly,
                                                      size_t polyRenderIndex) {
  SoftRasterizerTexture *theTexture =
      (SoftRasterizerTexture *)this->_softRender
          ->GetTextureByPolygonRenderIndex(polyRenderIndex);
...
template <bool RENDERER>
FORCEINLINE FragmentColor RasterizerUnit<RENDERER>::_sample(const float u,
                                                            const float v) {
  // finally, we can use floor here. but, it is slower than we want.
  // the best solution is probably to wait until the pipeline is full of fixed
  // point
  const float fu = u * (float)this->_currentTexture->GetRenderWidth() /
                   (float)this->_currentTexture->GetWidth();
  const float fv = v * (float)this->_currentTexture->GetRenderHeight() /
                   (float)this->_currentTexture->GetHeight();
  s32 iu = 0;
  s32 iv = 0;
  ...
  this->_currentTexture->GetRenderSamplerCoordinates(this->_textureWrapMode, iu,
                                                     iv);

  FragmentColor color;
  color.color =
      textureData[(iv << this->_currentTexture->GetRenderWidthShift()) + iu];
...
FORCEINLINE float RasterizerUnit<RENDERER>::_round_s(double val) {
  if (val > 0.0) {
    return floorf(val * 256.0f + 0.5f) /
           256.0f; // this value(256.0) is good result.(I think)
  } else {
...
FORCEINLINE void
RasterizerUnit<RENDERER>::_shade(const PolygonMode polygonMode,
                                 const FragmentColor src, FragmentColor &dst,
  ...
  static const FragmentColor colorWhite =
      MakeFragmentColor(0x3F, 0x3F, 0x3F, 0x1F);
  const FragmentColor mainTexColor =
      (this->_currentTexture->IsSamplingEnabled())
          ? this->_sample(texCoordU, texCoordV)
          : colorWhite;
  ...
  case POLYGON_MODE_MODULATE:
    ...
    dst.a = modulate_table[GFX3D_5TO6_LOOKUP(mainTexColor.a)]
                          [GFX3D_5TO6_LOOKUP(src.a)] >>
            1;
    // dst.a = 28;
    // #ifdef _MSC_VER
  ...
  case POLYGON_MODE_TOONHIGHLIGHT: {
    const FragmentColor toonColor =
        this->_softRender->toonColor32LUT[src.r >> 1];

    if (gfx3d.renderState.shading == PolygonShadingMode_Highlight) {
      // Tested in the "Shadows of Almia" logo in the Pokemon Ranger: Shadows of
      // Almia title screen. Also tested in Advance Wars: Dual Strike and
      // Advance Wars: Days of Ruin when tiles highlight during unit selection.
      dst.r = modulate_table[mainTexColor.r][src.r];
...
FORCEINLINE void RasterizerUnit<RENDERER>::_pixel(
    const POLYGON_ATTR polyAttr, const bool isTranslucent,
    const size_t fragmentIndex, FragmentColor &dstColor, float r, float g,
    float b, float invu, float invv, float z, float w) {
  FragmentColor newDstColor32;
  FragmentColor shaderOutput;
  bool isOpaquePixel;

  u32 &dstAttributeDepth =
      this->_softRender->_framebufferAttributes->depth[fragmentIndex];
  u8 &dstAttributeOpaquePolyID =
      this->_softRender->_framebufferAttributes->opaquePolyID[fragmentIndex];
  u8 &dstAttributeTranslucentPolyID = this->_softRender->_framebufferAttributes
                                          ->translucentPolyID[fragmentIndex];
  u8 &dstAttributeStencil =
      this->_softRender->_framebufferAttributes->stencil[fragmentIndex];
  u8 &dstAttributeIsFogged =
      this->_softRender->_framebufferAttributes->isFogged[fragmentIndex];
  u8 &dstAttributeIsTranslucentPoly = this->_softRender->_framebufferAttributes
                                          ->isTranslucentPoly[fragmentIndex];
  u8 &dstAttributePolyFacing =
      this->_softRender->_framebufferAttributes->polyFacing[fragmentIndex];

  // not sure about the w-buffer depth value: this value was chosen to make the
  // skybox, castle window decals, and water level render correctly in SM64
  //
  ...
  // Note that an IEEE-754 single-precision float uses a 23-bit significand.
  // Therefore, we will multiply the Z-depth by a 22-bit significand for safety.
  const u32 newDepth = (gfx3d.renderState.wbuffer)
                           ? u32floor(w * 4096.0f)
                           : u32floor(z * 4194303.0f) << 2;

  // run the depth test
  bool depthFail = false;

  if (polyAttr.DepthEqualTest_Enable) {
    // The EQUAL depth test is used if the polygon requests it. Note that the
    // NDS doesn't perform a true EQUAL test -- there is a set tolerance to it
    // that makes it easier for pixels to pass the depth test.
    const u32 minDepth = (u32)max<s32>(
        0x00000000, (s32)dstAttributeDepth - DEPTH_EQUALS_TEST_TOLERANCE);
    const u32 maxDepth =
        min<u32>(0x00FFFFFF, dstAttributeDepth + DEPTH_EQUALS_TEST_TOLERANCE);

    if (newDepth < minDepth || newDepth > maxDepth) {
      depthFail = true;
    }
  } else if ((ISFRONTFACING && (dstAttributePolyFacing == PolyFacing_Back)) &&
             (dstColor.a == 0x1F)) {
    // The LEQUAL test is used in the special case where an incoming
    // front-facing polygon's pixel is to be drawn on top of a back-facing
    // polygon's opaque pixel.
    //
    // Test case: The Customize status screen in Sands of Destruction requires
    // this type of depth test in order to correctly show the animating
    // characters.
    if (newDepth > dstAttributeDepth) {
      depthFail = true;
    }
  } else {
    // The LESS depth test is the default type of depth test for all other
    // conditions.
    if (newDepth >= dstAttributeDepth) {
      depthFail = true;
    }
  ...
  if (ISSHADOWPOLYGON) {
    if (polyAttr.PolygonID == 0) {
      // shadow mask polygons only affect the stencil buffer, and then only when
      // they fail depth test if we made it here, the shadow mask polygon
      // fragment needs to be trashed
      return;
    } else {
  ...
  // pixel shader
  this->_shade<ISSHADOWPOLYGON>((PolygonMode)polyAttr.Mode, newDstColor32,
                                shaderOutput, invu * w, invv * w);

  // handle alpha test
  if (shaderOutput.a == 0 ||
      (this->_softRender->currentRenderState->enableAlphaTest &&
       shaderOutput.a < this->_softRender->currentRenderState->alphaTestRef)) {
    return;
  }
  ...
  if (isOpaquePixel) {
    ...
    // if this isnt correct, then complex shape cart shadows in mario kart don't
    // work right
...
template <bool RENDERER>
template <bool ISFRONTFACING, bool ISSHADOWPOLYGON, bool USELINEHACK>
FORCEINLINE void RasterizerUnit<RENDERER>::_drawscanline(
    const POLYGON_ATTR polyAttr, const bool isTranslucent,
    FragmentColor *dstColor, const size_t framebufferWidth,
    const size_t framebufferHeight, edge_fx_fl *pLeft, edge_fx_fl *pRight) {
  ...
  CACHE_ALIGN float coord[4] = {pLeft->u.curr, pLeft->v.curr, pLeft->z.curr,
                                pLeft->invw.curr};

  CACHE_ALIGN float color[4] = {pLeft->color[0].curr, pLeft->color[1].curr,
                                pLeft->color[2].curr,
  ...
  const CACHE_ALIGN float coord_dx[4] = {(pRight->u.curr - coord[0]) * invWidth,
                                         (pRight->v.curr - coord[1]) * invWidth,
                                         (pRight->z.curr - coord[2]) * invWidth,
                                         (pRight->invw.curr - coord[3]) *
                                             invWidth};

  const CACHE_ALIGN float color_dx[4] = {
      (pRight->color[0].curr - color[0]) * invWidth,
      (pRight->color[1].curr - color[1]) * invWidth,
      (pRight->color[2].curr - color[2]) * invWidth, 0.0f * invWidth};

  size_t adr = (pLeft->Y * framebufferWidth) + XStart;

  // CONSIDER: in case some other math is wrong (shouldve been clipped OK), we
  // might go out of bounds here. better check the Y value.
  ...
  if (x + width > framebufferWidth) {
    if (RENDERER && !USELINEHACK &&
        framebufferWidth == GPU_FRAMEBUFFER_NATIVE_WIDTH) {
      printf("rasterizer rendering at x=%d! oops!\n", x + width - 1);
      return;
    }
  ...
  while (width-- > 0) {
    this->_pixel<ISFRONTFACING, ISSHADOWPOLYGON>(
        polyAttr, isTranslucent, adr, dstColor[adr], color[0], color[1],
        color[2], coord[0], coord[1], coord[2], 1.0f / coord[3]);
    adr++;
    x++;
...
template <bool RENDERER>
template <bool ISFRONTFACING, bool ISSHADOWPOLYGON>
FORCEINLINE void RasterizerUnit<RENDERER>::_pixel_SSE2(
    const POLYGON_ATTR polyAttr, const bool isTranslucent,
    const size_t fragmentIndex, FragmentColor &dstColor,
    const __m128 &srcColorf, float invu, float invv, float z, float w) {
  FragmentColor newDstColor32;
  FragmentColor shaderOutput;
  bool isOpaquePixel;
  ...
  __m128 newColorf =
      _mm_add_ps(_mm_mul_ps(srcColorf, perspective), _mm_set1_ps(0.5f));
...
template <bool RENDERER>
template <bool ISFRONTFACING, bool ISSHADOWPOLYGON, bool USELINEHACK>
FORCEINLINE void RasterizerUnit<RENDERER>::_drawscanline_SSE2(
    const POLYGON_ATTR polyAttr, const bool isTranslucent,
    FragmentColor *dstColor, const size_t framebufferWidth,
    const size_t framebufferHeight, edge_fx_fl *pLeft, edge_fx_fl *pRight) {
  const int XStart = pLeft->X;
  ...
  // these are the starting values, taken from the left edge
  __m128 coord = _mm_setr_ps(pLeft->u.curr, pLeft->v.curr, pLeft->z.curr,
                             pLeft->invw.curr);

  __m128 color =
      _mm_setr_ps(pLeft->color[0].curr, pLeft->color[1].curr,
                  pLeft->color[2].curr, (float)polyAttr.Alpha / 31.0f);

  // our dx values are taken from the steps up until the right edge
  const __m128 invWidth = _mm_set1_ps(1.0f / (float)width);
  const __m128 coord_dx =
      _mm_mul_ps(_mm_setr_ps(pRight->u.curr - pLeft->u.curr,
                             pRight->v.curr - pLeft->v.curr,
                             pRight->z.curr - pLeft->z.curr,
                             pRight->invw.curr - pLeft->invw.curr),
                 invWidth);
  const __m128 color_dx = _mm_mul_ps(
      _mm_setr_ps(pRight->color[0].curr - pLeft->color[0].curr,
                  pRight->color[1].curr - pLeft->color[1].curr,
                  pRight->color[2].curr - pLeft->color[2].curr, 0.0f),
      invWidth);
  ...
  while (width-- > 0) {
    ...
    this->_pixel_SSE2<ISFRONTFACING, ISSHADOWPOLYGON>(
        polyAttr, isTranslucent, adr, dstColor[adr], color, coord_s[0],
        coord_s[1], coord_s[2], 1.0f / coord_s[3]);
    adr++;
    x++;
...
template <bool RENDERER>
template <bool SLI, bool ISFRONTFACING, bool ISSHADOWPOLYGON, bool USELINEHACK>
void RasterizerUnit<RENDERER>::_runscanlines(
    const POLYGON_ATTR polyAttr, const bool isTranslucent,
    FragmentColor *dstColor, const size_t framebufferWidth,
    const size_t framebufferHeight, const bool isHorizontal, edge_fx_fl *left,
    edge_fx_fl *right) {
  ...
  if (USELINEHACK && (left->Height == 0) && (right->Height == 0) &&
      (left->Y < framebufferHeight) && (left->Y >= 0)) {
    const bool draw = (!SLI || ((left->Y >= this->_SLI_startLine) &&
                                (left->Y < this->_SLI_endLine)));
    if (draw) {
#ifdef ENABLE_SSE2
      this->_drawscanline_SSE2<ISFRONTFACING, ISSHADOWPOLYGON, USELINEHACK>(
          polyAttr, isTranslucent, dstColor, framebufferWidth,
          framebufferHeight, left, right);
#else
      this->_drawscanline<ISFRONTFACING, ISSHADOWPOLYGON, USELINEHACK>(
          polyAttr, isTranslucent, dstColor, framebufferWidth,
          framebufferHeight, left, right);
#endif
    }
...
#define ROTSWAP(X)                                                             \
  if (TYPE > X)                                                                \
    swap(this->_verts[X - 1], this->_verts[X]);
  ROTSWAP(1);
  ROTSWAP(2);
  ROTSWAP(3);
  ROTSWAP(4);
...
// rotate verts until vert0.y is minimum, and then vert0.x is minimum in case of
// ties this is a necessary precondition for our shape engine
template <bool RENDERER>
template <bool ISFRONTFACING, int TYPE>
...
#define CHECKY(X)                                                              \
  if (TYPE > X)                                                                \
    if (this->_verts[0]->y > this->_verts[X]->y)                               \
      goto doswap;
    CHECKY(1);
    CHECKY(2);
    CHECKY(3);
  ...
  while (this->_verts[0]->y == this->_verts[1]->y &&
         this->_verts[0]->x > this->_verts[1]->x) {
    this->_rot_verts<TYPE>();
...
// I didnt reference anything for this algorithm but it seems like I've seen it
// somewhere before. Maybe it is like crow's algorithm
template <bool RENDERER>
template <bool SLI, bool ISFRONTFACING, bool ISSHADOWPOLYGON, bool USELINEHACK>
void RasterizerUnit<RENDERER>::_shape_engine(const POLYGON_ATTR polyAttr,
                                             const bool isTranslucent,
                                             FragmentColor *dstColor,
                                             const size_t framebufferWidth,
                                             const size_t framebufferHeight,
                                             int type) {
  bool failure = false;

  switch (type) {
  case 3:
    this->_sort_verts<ISFRONTFACING, 3>();
    break;
  case 4:
    this->_sort_verts<ISFRONTFACING, 4>();
    break;
  case 5:
    this->_sort_verts<ISFRONTFACING, 5>();
    break;
  case 6:
    this->_sort_verts<ISFRONTFACING, 6>();
    break;
  case 7:
    this->_sort_verts<ISFRONTFACING, 7>();
    break;
  case 8:
    this->_sort_verts<ISFRONTFACING, 8>();
    break;
  case 9:
    this->_sort_verts<ISFRONTFACING, 9>();
    break;
  case 10:
    this->_sort_verts<ISFRONTFACING, 10>();
    break;
  default:
  ...
  // we are going to step around the polygon in both directions starting from
  // vert 0. right edges will be stepped over clockwise and left edges stepped
  // over counterclockwise. these variables track that stepping, but in order to
  // facilitate wrapping we start extra high for the counter we're decrementing.
  int lv = type, rv = 0;

  edge_fx_fl left, right;
  bool step_left = true, step_right = true;
  for (;;) {
    // generate new edges if necessary. we must avoid regenerating edges when
    // they are incomplete so that they can be continued on down the shape
    assert(rv != type);
    int _lv = (lv == type) ? 0 : lv; // make sure that we ask for vert 0 when
                                     // the variable contains the starting value
    if (step_left)
      left = edge_fx_fl(_lv, lv - 1, (VERT **)&this->_verts, failure);
    if (step_right)
      right = edge_fx_fl(rv, rv + 1, (VERT **)&this->_verts, failure);
    ...
    this->_runscanlines<SLI, ISFRONTFACING, ISSHADOWPOLYGON, USELINEHACK>(
        polyAttr, isTranslucent, dstColor, framebufferWidth, framebufferHeight,
    ...
    // this is our completion condition: when our stepped edges meet in the
    // middle
    if (lv <= rv + 1)
      break;
  }
...
template <bool RENDERER>
void RasterizerUnit<RENDERER>::SetRenderer(
    SoftRasterizerRenderer *theRenderer) {
...
FORCEINLINE void RasterizerUnit<RENDERER>::Render() {
  ...
  for (size_t i = 0; i < polyCount; i++) {
    if (!RENDERER)
      _debug_thisPoly = (i == this->_softRender->_debug_drawClippedUserPoly);
    if (!this->_softRender->isPolyVisible[i])
      continue;
    this->_polynum = i;
    ...
    if (!this->_softRender->isPolyBackFacing[i]) {
      if (polyAttr.Mode == POLYGON_MODE_SHADOW) {
        if (useLineHack) {
          this->_shape_engine<SLI, true, true, true>(polyAttr, isTranslucent,
                                                     dstColor, dstWidth,
                                                     dstHeight, vertCount);
        } else {
          this->_shape_engine<SLI, true, true, false>(polyAttr, isTranslucent,
                                                      dstColor, dstWidth,
                                                      dstHeight, vertCount);
        }
      } else {
        if (useLineHack) {
          this->_shape_engine<SLI, true, false, true>(polyAttr, isTranslucent,
                                                      dstColor, dstWidth,
                                                      dstHeight, vertCount);
        } else {
          this->_shape_engine<SLI, true, false, false>(polyAttr, isTranslucent,
                                                       dstColor, dstWidth,
                                                       dstHeight, vertCount);
        }
      }
      ...
      if (polyAttr.Mode == POLYGON_MODE_SHADOW) {
        if (useLineHack) {
          this->_shape_engine<SLI, false, true, true>(polyAttr, isTranslucent,
                                                      dstColor, dstWidth,
                                                      dstHeight, vertCount);
        } else {
          this->_shape_engine<SLI, false, true, false>(polyAttr, isTranslucent,
                                                       dstColor, dstWidth,
                                                       dstHeight, vertCount);
        }
      } else {
        if (useLineHack) {
          this->_shape_engine<SLI, false, false, true>(polyAttr, isTranslucent,
                                                       dstColor, dstWidth,
                                                       dstHeight, vertCount);
        } else {
          this->_shape_engine<SLI, false, false, false>(polyAttr, isTranslucent,
                                                        dstColor, dstWidth,
                                                        dstHeight, vertCount);
        }
      }
...
template <bool USELINEHACK> void *_HACK_Viewer_ExecUnit(void *arg) {
...
template <bool USELINEHACK> void *SoftRasterizer_RunRasterizerUnit(void *arg) {
...
static void *SoftRasterizer_RunRenderEdgeMarkAndFog(void *arg) {
  SoftRasterizerPostProcessParams *params =
      (SoftRasterizerPostProcessParams *)arg;
...
#if defined(ENABLE_AVX)
    SoftRasterizerRenderer_AVX *oldRenderer =
        (SoftRasterizerRenderer_AVX *)CurrentRenderer;
#elif defined(ENABLE_SSE2)
    SoftRasterizerRenderer_SSE2 *oldRenderer =
        (SoftRasterizerRenderer_SSE2 *)CurrentRenderer;
#elif defined(ENABLE_ALTIVEC)
    SoftRasterizerRenderer_Altivec *oldRenderer =
        (SoftRasterizerRenderer_Altivec *)CurrentRenderer;
#else
    SoftRasterizerRenderer *oldRenderer =
        (SoftRasterizerRenderer *)CurrentRenderer;
#endif
...
SoftRasterizerTexture::SoftRasterizerTexture(TEXIMAGE_PARAM texAttributes,
                                             u32 palAttributes)
...
FORCEINLINE void SoftRasterizerTexture::_clamp(s32 &val, const int size,
                                               const s32 sizemask) const {
  if (val < 0)
    val = 0;
  if (val > sizemask)
    val = sizemask;
...
FORCEINLINE void SoftRasterizerTexture::_repeat(s32 &val, const int size,
                                                const s32 sizemask) const {
  val &= sizemask;
}
...
FORCEINLINE void SoftRasterizerTexture::_flip(s32 &val, const int size,
                                              const s32 sizemask) const {
  val &= ((size << 1) - 1);
  if (val >= size)
    val = (size << 1) - val - 1;
...
void SoftRasterizerTexture::Load() {
  if (this->_scalingFactor == 1 && !this->_useDeposterize) {
    ...
    ColorspaceConvertBuffer8888To6665<false, false>(
...
u32 *SoftRasterizerTexture::GetUnpackData() { return this->_unpackData; }

u32 *SoftRasterizerTexture::GetRenderData() { return this->_renderData; }

s32 SoftRasterizerTexture::GetRenderWidth() const { return this->_renderWidth; }
...
FORCEINLINE void
SoftRasterizerTexture::GetRenderSamplerCoordinates(const u8 wrapMode, s32 &iu,
                                                   s32 &iv) const {
  switch (wrapMode) {
  // flip none
  case 0x0:
...
void SoftRasterizerTexture::SetUseDeposterize(bool willDeposterize) {
  ...
  if ((this->_deposterizeDstSurface.Surface == NULL) && willDeposterize) {
    this->_deposterizeDstSurface.Surface =
        (unsigned char *)malloc_alignedCacheLine(this->_cacheSize * 2);
    this->_deposterizeDstSurface.workingSurface[0] =
        this->_deposterizeDstSurface.Surface + this->_cacheSize;
  } else if ((this->_deposterizeDstSurface.Surface != NULL) &&
             !willDeposterize) {
    free_aligned(this->_deposterizeDstSurface.Surface);
...
void SoftRasterizerTexture::SetScalingFactor(size_t scalingFactor) {
  ...
  if (this->_renderWidth != newWidth || this->_renderHeight != newHeight) {
    ...
    this->_customBuffer =
        (u32 *)malloc_alignedCacheLine(newWidth * newHeight * sizeof(u32));
...
GPU3DInterface gpu3DRasterize = {"SoftRasterizer", SoftRasterizerRendererCreate,
...
SoftRasterizerRenderer::SoftRasterizerRenderer() {
  ...
  _clippedPolyList =
      (CPoly *)malloc_alignedCacheLine(POLYLIST_SIZE * 2 * sizeof(CPoly));
  ...
  _enableHighPrecisionColorInterpolation =
      CommonSettings.GFX3D_HighResolutionInterpolateColor;
  ...
  if (true) //(_threadCount < 2)
  {
    ...
    _rasterizerUnit[0].SetSLI(_threadPostprocessParam[0].startLine,
                              _threadPostprocessParam[0].endLine, false);
    ...
    for (size_t i = 0; i < _threadCount; i++) {
      ...
      _threadPostprocessParam[i].endLine = (i < _threadCount - 1)
                                               ? (i + 1) * _customLinesPerThread
                                               : _framebufferHeight;
      ...
      _threadClearParam[i].endPixel = (i < _threadCount - 1)
                                          ? (i + 1) * _customPixelsPerThread
                                          : _framebufferPixCount;

      _rasterizerUnit[i].SetSLI(_threadPostprocessParam[i].startLine,
                                _threadPostprocessParam[i].endLine, false);
  ...
  if (_threadCount == 0) {
    printf("SoftRasterizer: Running directly on the emulation thread. "
           "(Multithreading disabled.)\n");
  } else {
    printf("SoftRasterizer: Running using %d additional %s. (Multithreading "
           "enabled.)\n",
...
ClipperMode SoftRasterizerRenderer::GetPreferredPolygonClippingMode() const {
  return (this->_enableHighPrecisionColorInterpolation)
             ? ClipperMode_FullColorInterpolate
             : ClipperMode_Full;
}

void SoftRasterizerRenderer::_TransformVertices() {
  const float wScalar =
      (float)this->_framebufferWidth / (float)GPU_FRAMEBUFFER_NATIVE_WIDTH;
  const float hScalar =
      (float)this->_framebufferHeight / (float)GPU_FRAMEBUFFER_NATIVE_HEIGHT;
  ...
  for (size_t i = 0; i < this->_clippedPolyCount; i++) {
    ...
    for (size_t j = 0; j < poly.type; j++) {
      ...
      // If both of these questions answer to yes, then how does the NDS handle
      // a NaN? For now, simply prevent w from being zero.
      //
      // Test case: Dance scenes in Princess Debut can generate undefined
      // vertices when the -ffast-math option (relaxed IEEE754 compliance) is
      // used.
      ...
      // CONSIDER: do we need to guarantee that these are in bounds? perhaps
      // not. vert.coord[0] = max(0.0f,min(1.0f,vert.coord[0])); vert.coord[1] =
      // max(0.0f,min(1.0f,vert.coord[1])); vert.coord[2] =
      // max(0.0f,min(1.0f,vert.coord[2]));
      ...
      // at some point our shape engine needs these to be converted to "fixed
      // point" which is currently just a float
...
void SoftRasterizerRenderer::_GetPolygonStates() {
  ...
  for (size_t i = 0; i < this->_clippedPolyCount; i++) {
    ...
    // this should be moved to gfx3d, but first we need to redo the way the
    // lists are built because it is too convoluted right now. (must we throw
    //out verts if a poly gets backface culled? if not, then it might be easier)

    // an older approach
    //(not good enough for quads and other shapes)
    // float ab[2], ac[2]; Vector2Copy(ab, verts[1].coord); Vector2Copy(ac,
    // verts[2].coord); Vector2Subtract(ab, verts[0].coord); Vector2Subtract(ac,
    // verts[0].coord); float cross = Vector2Cross(ab, ac); polyAttr.backfacing
    // = (cross>0);

    // a better approach
    //  we have to support somewhat non-convex polygons (see NSMB world map 1st
    //  screen). this version should handle those cases better.
    ...
    this->isPolyVisible[i] =
        visibleFunction[this->isPolyBackFacing[i]][cullingMode];
  }
}
...
void SoftRasterizerRenderer::GetAndLoadAllTextures() {
  for (size_t i = 0; i < this->_clippedPolyCount; i++) {
    ...
    // this SHOULD be read-only, although some day the texcache may collect
    // statistics or something and then it won't be safe.
...
Render3DError
SoftRasterizerRenderer::ApplyRenderingSettings(const GFX3D_State &renderState) {
  this->_enableHighPrecisionColorInterpolation =
      CommonSettings.GFX3D_HighResolutionInterpolateColor;
...
Render3DError SoftRasterizerRenderer::BeginRender(const GFX3D &engine) {
  ...
  memcpy(this->_clippedPolyList, engine.clippedPolyList,
         this->_clippedPolyCount * sizeof(CPoly));
  ...
  ColorspaceConvertBuffer555To6665Opaque<false, false>(
      engine.renderState.u16ToonTable, (u32 *)this->toonColor32LUT, 32);

  if (this->_enableEdgeMark) {
    this->_UpdateEdgeMarkColorTable(
        this->currentRenderState->edgeMarkColorTable);
  }
...
Render3DError SoftRasterizerRenderer::RenderGeometry() {
  ...
  if (this->_threadCount > 0) {
    if (this->_enableLineHack) {
      for (size_t i = 0; i < this->_threadCount; i++) {
        this->_task[i].execute(&SoftRasterizer_RunRasterizerUnit<true>,
                               &this->_rasterizerUnit[i]);
      }
    } else {
      for (size_t i = 0; i < this->_threadCount; i++) {
        this->_task[i].execute(&SoftRasterizer_RunRasterizerUnit<false>,
                               &this->_rasterizerUnit[i]);
      }
    }
    ...
    texCache.Evict(); // Since we're finishing geometry rendering here and now,
                      // also check the texture cache now.
  }

  //	printf("rendered %d of %d polys after backface
  //culling\n",gfx3d.polylist->count-culled,gfx3d.polylist->count);

  return RENDER3DERROR_NOERR;
}

void SoftRasterizerRenderer::_UpdateEdgeMarkColorTable(
    const u16 *edgeMarkColorTable) {
  // TODO: need to test and find out whether these get grabbed at flush time, or
  // at render time we can do this by rendering a 3d frame and then freezing the
  // system, but only changing the edge mark colors
  for (size_t i = 0; i < 8; i++) {
    this->_edgeMarkTable[i].color = COLOR555TO6665(
        edgeMarkColorTable[i] & 0x7FFF,
        (this->currentRenderState->enableAntialiasing) ? 0x10 : 0x1F);

    // zero 20-jun-2013 - this doesnt make any sense. at least, it should be
    // related to the 0x8000 bit. if this is undocumented behaviour, lets write
    // about which scenario proves it here, or which scenario is requiring this
    // code.
...
void SoftRasterizerRenderer::_UpdateFogTable(const u8 *fogDensityTable) {
  ...
  // except much faster. (because it's not a 2d loop and isn't so branchy
  // either) maybe it's fast enough to not need to be cached, now.
  const int increment = ((1 << 10) >> this->currentRenderState->fogShift);
  const int incrementDivShift = 10 - this->currentRenderState->fogShift;
  u32 fogOffset =
      min<u32>(max<u32>(this->currentRenderState->fogOffset, 0), 32768);
  u32 iMin = min<u32>(32768, ((1 + 1) << incrementDivShift) + fogOffset + 1 -
                                 increment);
  u32 iMax = min<u32>(32768, ((32 + 1) << incrementDivShift) + fogOffset + 1 -
                                 increment);
  assert(iMin <= iMax);
  ...
  for (u32 i = iMin; i < iMax; i++) {
    ...
    u32 diff = value - i;
    assert(j >= 1 && j < 32);
    fogFactor = ((diff * (fogDensityTable[j - 1]) +
                  (increment - diff) * (fogDensityTable[j])) >>
                 incrementDivShift);
...
Render3DError SoftRasterizerRenderer::RenderEdgeMarkingAndFog(
    const SoftRasterizerPostProcessParams &param) {
  for (size_t i = param.startLine * this->_framebufferWidth,
              y = param.startLine;
       y < param.endLine; y++) {
    for (size_t x = 0; x < this->_framebufferWidth; x++, i++) {
      ...
      if (param.enableEdgeMarking) {
        ...
        // - the edges are completely sharp/opaque on the very brief title
        // screen intro,
        // - the level-start intro gets a pseudo-antialiasing effect around the
        // silhouette,
        // - the character edges in-level are clearly transparent, and also show
        // well through shield powerups.

        if (!this->_edgeMarkDisabled[polyID >> 3] &&
            this->_framebufferAttributes->isTranslucentPoly[i] == 0) {
          const bool isEdgeMarkingClearValues =
              ((polyID != this->_clearAttributes.opaquePolyID) &&
               (depth < this->_clearAttributes.depth));

          const bool right =
              (x >= this->_framebufferWidth - 1)
                  ? isEdgeMarkingClearValues
                  : ((polyID !=
                      this->_framebufferAttributes->opaquePolyID[i + 1]) &&
                     (depth >= this->_framebufferAttributes->depth[i + 1]));
          const bool down =
              (y >= this->_framebufferHeight - 1)
                  ? isEdgeMarkingClearValues
                  : ((polyID !=
                      this->_framebufferAttributes
                          ->opaquePolyID[i + this->_framebufferWidth]) &&
                     (depth >= this->_framebufferAttributes
                                   ->depth[i + this->_framebufferWidth]));
          const bool left =
              (x < 1) ? isEdgeMarkingClearValues
                      : ((polyID !=
                          this->_framebufferAttributes->opaquePolyID[i - 1]) &&
                         (depth >= this->_framebufferAttributes->depth[i - 1]));
          const bool up =
              (y < 1) ? isEdgeMarkingClearValues
                      : ((polyID !=
                          this->_framebufferAttributes
                              ->opaquePolyID[i - this->_framebufferWidth]) &&
                         (depth >= this->_framebufferAttributes
          ...
          FragmentColor edgeMarkColor =
              this->_edgeMarkTable
                  [this->_framebufferAttributes->opaquePolyID[i] >> 3];

          if (right) {
            if (x < this->_framebufferWidth - 1) {
              edgeMarkColor =
                  this->_edgeMarkTable
                      [this->_framebufferAttributes->opaquePolyID[i + 1] >> 3];
            }
          } else if (down) {
      ...
      if (param.enableFog) {
        ...
        fogColor.color = COLOR555TO6665(param.fogColor & 0x7FFF,
                                        (param.fogColor >> 16) & 0x1F);

        const size_t fogIndex = depth >> 9;
        assert(fogIndex < 32768);
        const u8 fog = (this->_framebufferAttributes->isFogged[i] != 0)
                           ? this->_fogTable[fogIndex]
                           : 0;
...
SoftRasterizerRenderer::GetLoadedTextureFromPolygon(const POLY &thePoly,
  ...
  if (theTexture == NULL) {
    theTexture =
        new SoftRasterizerTexture(thePoly.texParam, thePoly.texPalette);
...
Render3DError SoftRasterizerRenderer::ClearUsingImage(
    const u16 *__restrict colorBuffer, const u32 *__restrict depthBuffer,
    const u8 *__restrict fogBuffer, const u8 opaquePolyID) {
  const size_t xRatio =
      (size_t)((GPU_FRAMEBUFFER_NATIVE_WIDTH << 16) / this->_framebufferWidth) +
      1;
  const size_t yRatio = (size_t)((GPU_FRAMEBUFFER_NATIVE_HEIGHT << 16) /
                                 this->_framebufferHeight) +
                        1;

  for (size_t y = 0, iw = 0; y < this->_framebufferHeight; y++) {
    const size_t readLine =
        (size_t)(((y * yRatio) >> 16) * GPU_FRAMEBUFFER_NATIVE_WIDTH);

    for (size_t x = 0; x < this->_framebufferWidth; x++, iw++) {
      const size_t ir = readLine + ((x * xRatio) >> 16);

      this->_framebufferColor[iw].color = COLOR555TO6665(
          colorBuffer[ir] & 0x7FFF, (colorBuffer[ir] >> 15) * 0x1F);
      ...
      this->_framebufferAttributes->translucentPolyID[iw] =
          kUnsetTranslucentPolyID;
...
void SoftRasterizerRenderer::ClearUsingValues_Execute(const size_t startPixel,
...
Render3DError SoftRasterizerRenderer::ClearUsingValues(
  ...
  if (doMultithreadedClear) {
    for (size_t threadIndex = 0; threadIndex < this->_threadCount;
         threadIndex++) {
      this->_task[threadIndex].execute(&SoftRasterizer_RunClearUsingValues,
                                       &this->_threadClearParam[threadIndex]);
    }
  } else {
...
Render3DError SoftRasterizerRenderer::EndRender() {
  ...
  if (!this->_renderGeometryNeedsFinish) {
    if (this->_enableEdgeMark || this->_enableFog) {
      this->_threadPostprocessParam[0].enableEdgeMarking =
          this->_enableEdgeMark;
      this->_threadPostprocessParam[0].enableFog = this->_enableFog;
      this->_threadPostprocessParam[0].fogColor =
          this->currentRenderState->fogColor;
      this->_threadPostprocessParam[0].fogAlphaOnly =
          this->currentRenderState->enableFogAlphaOnly;
...
Render3DError SoftRasterizerRenderer::RenderFinish() {
  ...
  if (this->_renderGeometryNeedsFinish) {
    ...
    // Now that geometry rendering is finished on all threads, check the texture
    // cache.
    texCache.Evict();
    ...
    if (this->_enableEdgeMark || this->_enableFog) {
      for (size_t i = 0; i < this->_threadCount; i++) {
        this->_threadPostprocessParam[i].enableEdgeMarking =
            this->_enableEdgeMark;
        this->_threadPostprocessParam[i].enableFog = this->_enableFog;
        this->_threadPostprocessParam[i].fogColor =
            this->currentRenderState->fogColor;
        this->_threadPostprocessParam[i].fogAlphaOnly =
            this->currentRenderState->enableFogAlphaOnly;

        this->_task[i].execute(&SoftRasterizer_RunRenderEdgeMarkAndFog,
                               &this->_threadPostprocessParam[i]);
      }
...
Render3DError SoftRasterizerRenderer::RenderFlush(bool willFlushBuffer32,
  ...
  FragmentColor *framebufferMain =
      (willFlushBuffer32 && (this->_outputFormat == NDSColorFormat_BGR888_Rev))
...
Render3DError SoftRasterizerRenderer::SetFramebufferSize(size_t w, size_t h) {
  ...
  const size_t pixCount = (this->_framebufferSIMDPixCount > 0)
                              ? this->_framebufferSIMDPixCount
  ...
  if (this->_threadCount == 0) {
    ...
    this->_rasterizerUnit[0].SetSLI(this->_threadPostprocessParam[0].startLine,
                                    this->_threadPostprocessParam[0].endLine,
                                    false);
  } else {
    ...
    for (size_t i = 0; i < this->_threadCount; i++) {
      this->_threadPostprocessParam[i].startLine =
          i * this->_customLinesPerThread;
      this->_threadPostprocessParam[i].endLine =
          (i < this->_threadCount - 1) ? (i + 1) * this->_customLinesPerThread
                                       : h;

      this->_threadClearParam[i].startPixel = i * this->_customPixelsPerThread;
      this->_threadClearParam[i].endPixel =
          (i < this->_threadCount - 1) ? (i + 1) * this->_customPixelsPerThread
                                       : pixCount;

      this->_rasterizerUnit[i].SetSLI(
          this->_threadPostprocessParam[i].startLine,
          this->_threadPostprocessParam[i].endLine, false);
    }
  }
...
SoftRasterizer_SIMD<SIMDBYTES>::SoftRasterizer_SIMD() {
  if (_threadCount == 0) {
    ...
    const size_t pixelsPerThread =
        ((_framebufferSIMDPixCount / SIMDBYTES) / _threadCount) * SIMDBYTES;
    ...
    for (size_t i = 0; i < _threadCount; i++) {
      ...
      _threadClearParam[i].endPixel = (i < _threadCount - 1)
                                          ? (i + 1) * pixelsPerThread
                                          : _framebufferSIMDPixCount;
    }
  }
}

template <size_t SIMDBYTES>
Render3DError SoftRasterizer_SIMD<SIMDBYTES>::ClearUsingValues(
    const FragmentColor &clearColor6665,
    const FragmentAttributes &clearAttributes) {
...
#pragma LOOPVECTORIZE_DISABLE
  for (size_t i = this->_framebufferSIMDPixCount;
       i < this->_framebufferPixCount; i++) {
...
template <size_t SIMDBYTES>
Render3DError SoftRasterizer_SIMD<SIMDBYTES>::SetFramebufferSize(size_t w,
                                                                 size_t h) {
  ...
  if (this->_threadCount == 0) {
    ...
    const size_t pixelsPerThread =
        ((pixCount / SIMDBYTES) / this->_threadCount) * SIMDBYTES;
    ...
    for (size_t i = 0; i < this->_threadCount; i++) {
      ...
      this->_threadClearParam[i].endPixel =
          (i < this->_threadCount - 1) ? (i + 1) * pixelsPerThread : pixCount;
...
void SoftRasterizerRenderer_AVX::LoadClearValues(
    const FragmentColor &clearColor6665,
    const FragmentAttributes &clearAttributes) {
  ...
  this->_clearAttrOpaquePolyID_v256u8 =
      _mm256_set1_epi8(clearAttributes.opaquePolyID);
  this->_clearAttrTranslucentPolyID_v256u8 =
      _mm256_set1_epi8(clearAttributes.translucentPolyID);
  ...
  this->_clearAttrIsTranslucentPoly_v256u8 =
      _mm256_set1_epi8(clearAttributes.isTranslucentPoly);
  this->_clearAttrPolyFacing_v256u8 =
      _mm256_set1_epi8(clearAttributes.polyFacing);
}

void SoftRasterizerRenderer_AVX::ClearUsingValues_Execute(
    const size_t startPixel, const size_t endPixel) {
  for (size_t i = startPixel; i < endPixel; i += 32) {
    _mm256_stream_si256((v256u32 *)(this->_framebufferColor + i + 0),
                        this->_clearColor_v256u32);
    _mm256_stream_si256((v256u32 *)(this->_framebufferColor + i + 8),
                        this->_clearColor_v256u32);
    _mm256_stream_si256((v256u32 *)(this->_framebufferColor + i + 16),
                        this->_clearColor_v256u32);
    _mm256_stream_si256((v256u32 *)(this->_framebufferColor + i + 24),
                        this->_clearColor_v256u32);

    _mm256_stream_si256(
        (v256u32 *)(this->_framebufferAttributes->depth + i + 0),
        this->_clearDepth_v256u32);
    _mm256_stream_si256(
        (v256u32 *)(this->_framebufferAttributes->depth + i + 8),
        this->_clearDepth_v256u32);
    _mm256_stream_si256(
        (v256u32 *)(this->_framebufferAttributes->depth + i + 16),
        this->_clearDepth_v256u32);
    _mm256_stream_si256(
        (v256u32 *)(this->_framebufferAttributes->depth + i + 24),
        this->_clearDepth_v256u32);

    _mm256_stream_si256(
        (v256u8 *)(this->_framebufferAttributes->opaquePolyID + i),
        this->_clearAttrOpaquePolyID_v256u8);
    _mm256_stream_si256(
        (v256u8 *)(this->_framebufferAttributes->translucentPolyID + i),
        this->_clearAttrTranslucentPolyID_v256u8);
    _mm256_stream_si256((v256u8 *)(this->_framebufferAttributes->stencil + i),
                        this->_clearAttrStencil_v256u8);
    _mm256_stream_si256((v256u8 *)(this->_framebufferAttributes->isFogged + i),
                        this->_clearAttrIsFogged_v256u8);
    _mm256_stream_si256(
        (v256u8 *)(this->_framebufferAttributes->isTranslucentPoly + i),
        this->_clearAttrIsTranslucentPoly_v256u8);
    _mm256_stream_si256(
        (v256u8 *)(this->_framebufferAttributes->polyFacing + i),
        this->_clearAttrPolyFacing_v256u8);
  }
}
...
void SoftRasterizerRenderer_SSE2::LoadClearValues(
    const FragmentColor &clearColor6665,
    const FragmentAttributes &clearAttributes) {
  ...
  this->_clearAttrOpaquePolyID_v128u8 =
      _mm_set1_epi8(clearAttributes.opaquePolyID);
  this->_clearAttrTranslucentPolyID_v128u8 =
      _mm_set1_epi8(clearAttributes.translucentPolyID);
  ...
  this->_clearAttrIsTranslucentPoly_v128u8 =
      _mm_set1_epi8(clearAttributes.isTranslucentPoly);
  this->_clearAttrPolyFacing_v128u8 = _mm_set1_epi8(clearAttributes.polyFacing);
}

void SoftRasterizerRenderer_SSE2::ClearUsingValues_Execute(
    const size_t startPixel, const size_t endPixel) {
  for (size_t i = startPixel; i < endPixel; i += 16) {
    _mm_stream_si128((v128u32 *)(this->_framebufferColor + i + 0),
                     this->_clearColor_v128u32);
    _mm_stream_si128((v128u32 *)(this->_framebufferColor + i + 4),
                     this->_clearColor_v128u32);
    _mm_stream_si128((v128u32 *)(this->_framebufferColor + i + 8),
                     this->_clearColor_v128u32);
    _mm_stream_si128((v128u32 *)(this->_framebufferColor + i + 12),
                     this->_clearColor_v128u32);

    _mm_stream_si128((v128u32 *)(this->_framebufferAttributes->depth + i + 0),
                     this->_clearDepth_v128u32);
    _mm_stream_si128((v128u32 *)(this->_framebufferAttributes->depth + i + 4),
                     this->_clearDepth_v128u32);
    _mm_stream_si128((v128u32 *)(this->_framebufferAttributes->depth + i + 8),
                     this->_clearDepth_v128u32);
    _mm_stream_si128((v128u32 *)(this->_framebufferAttributes->depth + i + 12),
                     this->_clearDepth_v128u32);

    _mm_stream_si128((v128u8 *)(this->_framebufferAttributes->opaquePolyID + i),
                     this->_clearAttrOpaquePolyID_v128u8);
    _mm_stream_si128(
        (v128u8 *)(this->_framebufferAttributes->translucentPolyID + i),
        this->_clearAttrTranslucentPolyID_v128u8);
    _mm_stream_si128((v128u8 *)(this->_framebufferAttributes->stencil + i),
                     this->_clearAttrStencil_v128u8);
    _mm_stream_si128((v128u8 *)(this->_framebufferAttributes->isFogged + i),
                     this->_clearAttrIsFogged_v128u8);
    _mm_stream_si128(
        (v128u8 *)(this->_framebufferAttributes->isTranslucentPoly + i),
        this->_clearAttrIsTranslucentPoly_v128u8);
    _mm_stream_si128((v128u8 *)(this->_framebufferAttributes->polyFacing + i),
                     this->_clearAttrPolyFacing_v128u8);
  }
}

#elif defined(ENABLE_ALTIVEC)

void SoftRasterizerRenderer_Altivec::LoadClearValues(
    const FragmentColor &clearColor6665,
    const FragmentAttributes &clearAttributes) {
  this->_clearColor_v128u32 =
      (v128u32){clearColor6665.color, clearColor6665.color,
                clearColor6665.color, clearColor6665.color};
  this->_clearDepth_v128u32 =
      (v128u32){clearAttributes.depth, clearAttributes.depth,
                clearAttributes.depth, clearAttributes.depth};

  this->_clearAttrOpaquePolyID_v128u8 =
      (v128u8){clearAttributes.opaquePolyID, clearAttributes.opaquePolyID,
               clearAttributes.opaquePolyID, clearAttributes.opaquePolyID,
               clearAttributes.opaquePolyID, clearAttributes.opaquePolyID,
               clearAttributes.opaquePolyID, clearAttributes.opaquePolyID,
               clearAttributes.opaquePolyID, clearAttributes.opaquePolyID,
               clearAttributes.opaquePolyID, clearAttributes.opaquePolyID,
  ...
  this->_clearAttrTranslucentPolyID_v128u8 = (v128u8){
      clearAttributes.translucentPolyID, clearAttributes.translucentPolyID,
      clearAttributes.translucentPolyID, clearAttributes.translucentPolyID,
      clearAttributes.translucentPolyID, clearAttributes.translucentPolyID,
      clearAttributes.translucentPolyID, clearAttributes.translucentPolyID,
      clearAttributes.translucentPolyID, clearAttributes.translucentPolyID,
  ...
  this->_clearAttrStencil_v128u8 = (v128u8){
      clearAttributes.stencil, clearAttributes.stencil, clearAttributes.stencil,
      clearAttributes.stencil, clearAttributes.stencil, clearAttributes.stencil,
      clearAttributes.stencil, clearAttributes.stencil, clearAttributes.stencil,
      clearAttributes.stencil, clearAttributes.stencil, clearAttributes.stencil,
      clearAttributes.stencil, clearAttributes.stencil, clearAttributes.stencil,
      clearAttributes.stencil};

  this->_clearAttrIsFogged_v128u8 =
      (v128u8){clearAttributes.isFogged, clearAttributes.isFogged,
               clearAttributes.isFogged, clearAttributes.isFogged,
               clearAttributes.isFogged, clearAttributes.isFogged,
               clearAttributes.isFogged, clearAttributes.isFogged,
               clearAttributes.isFogged, clearAttributes.isFogged,
               clearAttributes.isFogged, clearAttributes.isFogged,
  ...
  this->_clearAttrIsTranslucentPoly_v128u8 = (v128u8){
      clearAttributes.isTranslucentPoly, clearAttributes.isTranslucentPoly,
      clearAttributes.isTranslucentPoly, clearAttributes.isTranslucentPoly,
      clearAttributes.isTranslucentPoly, clearAttributes.isTranslucentPoly,
      clearAttributes.isTranslucentPoly, clearAttributes.isTranslucentPoly,
      clearAttributes.isTranslucentPoly, clearAttributes.isTranslucentPoly,
  ...
  this->_clearAttrPolyFacing_v128u8 =
      (v128u8){clearAttributes.polyFacing, clearAttributes.polyFacing,
               clearAttributes.polyFacing, clearAttributes.polyFacing,
               clearAttributes.polyFacing, clearAttributes.polyFacing,
               clearAttributes.polyFacing, clearAttributes.polyFacing,
               clearAttributes.polyFacing, clearAttributes.polyFacing,
               clearAttributes.polyFacing, clearAttributes.polyFacing,
...
void SoftRasterizerRenderer_Altivec::ClearUsingValues_Execute(
    const size_t startPixel, const size_t endPixel) {
  for (size_t i = startPixel; i < endPixel; i += 16) {
    ...
    vec_st(this->_clearDepth_v128u32, (i * 4) + 0,
           this->_framebufferAttributes->depth);
    vec_st(this->_clearDepth_v128u32, (i * 4) + 16,
           this->_framebufferAttributes->depth);
    vec_st(this->_clearDepth_v128u32, (i * 4) + 32,
           this->_framebufferAttributes->depth);
    vec_st(this->_clearDepth_v128u32, (i * 4) + 48,
           this->_framebufferAttributes->depth);

    vec_st(this->_clearAttrOpaquePolyID_v128u8, i,
           this->_framebufferAttributes->opaquePolyID);
    vec_st(this->_clearAttrTranslucentPolyID_v128u8, i,
           this->_framebufferAttributes->translucentPolyID);
    vec_st(this->_clearAttrStencil_v128u8, i,
           this->_framebufferAttributes->stencil);
    vec_st(this->_clearAttrIsFogged_v128u8, i,
           this->_framebufferAttributes->isFogged);
    vec_st(this->_clearAttrIsTranslucentPoly_v128u8, i,
           this->_framebufferAttributes->isTranslucentPoly);
    vec_st(this->_clearAttrPolyFacing_v128u8, i,
