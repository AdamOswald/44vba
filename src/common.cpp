...
static std::map<void *, void *>
    _alignedPtrList; // Key: Aligned pointer / Value: Original pointer
...
void msgFakeInfo(const char *fmt, ...) { MSG_PRINT; }
...
void msgFakeError(const char *fmt, ...) { MSG_PRINT; }

void msgFakeWarn(const char *fmt, ...) { MSG_PRINT; }
...
void *malloc_aligned(size_t length, size_t alignment) {
  const uintptr_t ptrOffset =
      alignment; // This value must be a power of 2, or this function will fail.
  ...
  void *alignedPtr =
      (void *)(((uintptr_t)originalPtr + ptrOffset) & ptrOffsetMask);
