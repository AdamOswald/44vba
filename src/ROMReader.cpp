...
ROMReader_struct STDROMReader = {ROMREADER_STD,    "Standard ROM Reader",
                                 STDROMReaderInit, STDROMReaderDeInit,
                                 STDROMReaderSize, STDROMReaderSeek,
                                 STDROMReaderRead, STDROMReaderWrite};
...
void STDROMReaderDeInit(void *file) { delete ((STDROMReaderData *)file); }

u32 STDROMReaderSize(void *file) { return romLen; }
...
ROMReader_struct GZIPROMReader = {ROMREADER_GZIP,    "Gzip ROM Reader",
                                  GZIPROMReaderInit, GZIPROMReaderDeInit,
                                  GZIPROMReaderSize, GZIPROMReaderSeek,
                                  GZIPROMReaderRead, GZIPROMReaderWrite};

void *GZIPROMReaderInit(const char *filename) {
  return (void *)gzopen(filename, "rb");
}

void GZIPROMReaderDeInit(void *file) { gzclose((gzFile)file); }
...
ROMReader_struct ZIPROMReader = {
    ROMREADER_ZIP,    "Zip ROM Reader", ZIPROMReaderInit, ZIPROMReaderDeInit,
    ZIPROMReaderSize, ZIPROMReaderSeek, ZIPROMReaderRead, ZIPROMReaderWrite};
...
void ZIPROMReaderDeInit(void *file) { zzip_close((ZZIP_FILE *)file); }
...
u32 MemROMReaderSize(void *) { return (u32)mem.len; }
...
int MemROMReaderRead(void *file, void *buffer, u32 size) {
  ...
  if (todo <= 0)
    ...
    *(u8 *)buffer = ((u8 *)mem.buf)[mem.pos];
  else
    memcpy(buffer, (u8 *)mem.buf + mem.pos, todo);
  mem.pos += todo;
  return todo;
}
...
int MemROMReaderWrite(void *file, void *buffer, u32 size) {
  ...
  if (todo == 1)
    *(u8 *)buffer = ((u8 *)mem.buf)[mem.pos];
    ...
    memcpy((u8 *)mem.buf + mem.pos, buffer, todo);
  mem.pos += todo;
  return todo;
}

static ROMReader_struct MemROMReader = {
    ROMREADER_MEM,    "Memory ROM Reader", MemROMReaderInit, MemROMReaderDeInit,
    MemROMReaderSize, MemROMReaderSeek,    MemROMReaderRead, MemROMReaderWrite,
};
