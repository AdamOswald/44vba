...
int lastSaveState =
    0; // Keeps track of last savestate used for quick save/load functions

// void*v is actually a void** which will be indirected before reading
// since this isnt supported right now, it is declared in here to make things
// compile
...
// a savestate chunk loader can set this if it wants to permit a silent failure
// (for compatibility)
static bool SAV_silent_fail_flag;

SFORMAT SF_NDS_INFO[] = {{"GINF", 1, sizeof(gameInfo.header), &gameInfo.header},
...
SFORMAT SF_ARM7[] = {{"7INS", 4, 1, &NDS_ARM7.instruction},
...
SFORMAT SF_ARM9[] = {{"9INS", 4, 1, &NDS_ARM9.instruction},
...
SFORMAT SF_NDS[] = {{"_WCY", 4, 1, &nds.wifiCycle},
...
SFORMAT SF_MMU[] = {{"M7BI", 1, sizeof(MMU.ARM7_BIOS), MMU.ARM7_BIOS},
                    ...
                    //{ "MASC", 1, 2,       &MMU.AUX_SPI_CMD}, //zero
                    //20-aug-2013 - this seems pointless
...
SFORMAT SF_MOVIE[] = {
    {"FRAC", 4, 1, &currFrameCounter}, {"LAGC", 4, 1, &TotalLagFrames}, {0}};

SFORMAT SF_WIFI[] = {{"W000", 4, 1, &legacyWifiSF.powerOn},
...
SFORMAT reserveChunks[] = {{"RESV", 1, 1, &reserveVal}, {0}};
...
static bool mmu_loadstate(EMUFILE &is, int size) {
  ...
  if (version == 0 || version == 1) {
    ...
    if (version == 0) {
      ...
      if (is.read_32LE(bupmem_size) != 1)
        return false;
      // if(bupmem_size != MMU.bupmem.size) return false; //mismatch between
      // current initialized and saved size
  ...
  // to prevent old savestates from confusing IF bits, mask out ones which had
  // been stored but should have been generated
...
void scan_savestates() {
  ...
  for (int i = 0; i < NB_STATES; i++) {
    ...
    if (strlen(filename) + strlen(".dst") +
            strlen("-2147483648") /* = biggest string for i */
        > MAX_PATH)
      return;
...
void savestate_slot(int num) {
  ...
  if (strlen(filename) + strlen(".dsx") +
          strlen("-2147483648") /* = biggest string for num */
      > MAX_PATH)
    return;
...
void loadstate_slot(int num) {
  ...
  // save the state before we load the state, to give people a path for recovery
  // in case they hit the wrong key
#ifdef HOST_WINDOWS
...
#endif
  {
    if (movieMode == MOVIEMODE_INACTIVE) {
      ...
      for (;;) {
        ...
        printf(
            "Creating backup of current state prior to loadstate as path: %s\n",
            fname.c_str());
        break;
      }
    }
...
// note: guessSF is so we don't have to do a linear search through the SFORMAT
// array every time in the (most common) case that we already know where the
// next entry is.
static const SFORMAT *CheckS(const SFORMAT *guessSF, const SFORMAT *firstSF,
                             u32 size, u32 count, char *desc) {
...
static void savestate_WriteChunk(EMUFILE &os, int type,
                                 void (*saveproc)(EMUFILE &os)) {
  u32 pos1 = os.ftell();
  ...
  // for reference in case the new one above starts misbehaving somehow:
      ...
      // - this is retarded. why not write placeholders for size and then write
...
bool savestate_save(EMUFILE &outstream, int compressionLevel) {
  ...
  EMUFILE_MEMORY ms;
  EMUFILE &os = (compressionLevel != Z_NO_COMPRESSION) ? (EMUFILE &)ms
                                                       : (EMUFILE &)outstream;
  ...
  outstream.write_32LE(
      comprlen); // compressed length (-1 if it is not compressed)
...
static void wifi_savestate(EMUFILE &os) { wifiHandler->SaveState(os); }
...
static bool ReadStateChunks(EMUFILE &is, s32 totalsize) {
  ...
  NDS_header header;
  SFORMAT SF_INFO[] = {
      {"GINF", 1, sizeof(header), &header}, {"GRSZ", 1, 4, &romsize},
      {"DVMJ", 1, 1, &version_major},       {"DVMI", 1, 1, &version_minor},
      {"DSBD", 1, 1, &version_build},       {"GREV", 1, 4, &svn_rev},
      {"GTIM", 1, 8, &save_time},           {0}};
  ...
  while (totalsize > 0) {
    ...
    switch (t) {
    case 1:
      if (!ReadStateChunk(is, SF_ARM9, size))
        ret = false;
      break;
    case 2:
      if (!ReadStateChunk(is, SF_ARM7, size))
        ret = false;
      break;
    case 3:
      if (!cp15_loadstate(is, size))
        ret = false;
      break;
    case 4:
      if (!ReadStateChunk(is, SF_MEM, size))
        ret = false;
      break;
    case 5:
      if (!ReadStateChunk(is, SF_NDS, size))
        ret = false;
      break;
    case 51:
      if (!nds_loadstate(is, size))
        ret = false;
      break;
    case 60:
      if (!ReadStateChunk(is, SF_MMU, size))
        ret = false;
      break;
    case 61:
      if (!mmu_loadstate(is, size))
        ret = false;
      break;
    case 7:
      if (!gpu_loadstate(is, size))
        ret = false;
      break;
    case 8:
      if (!spu_loadstate(is, size))
        ret = false;
      break;
    case 81:
      if (!mic_loadstate(is, size))
        ret = false;
      break;
    case 90:
      if (!ReadStateChunk(is, SF_GFX3D, size))
        ret = false;
      break;
    case 91:
      if (!gfx3d_loadstate(is, size))
        ret = false;
      break;
    case 100:
      if (!ReadStateChunk(is, SF_MOVIE, size))
        ret = false;
      break;
    case 101:
      if (!mov_loadstate(is, size))
        ret = false;
      break;
    case 111:
      if (!wifiHandler->LoadState(is, size))
        ret = false;
      break;
    case 120:
      if (!ReadStateChunk(is, SF_RTC, size))
        ret = false;
      break;
    case 130:
      if (!ReadStateChunk(is, SF_INFO, size))
        ret = false;
      else
        haveInfo = true;
      break;
    case 140:
      if (!s_slot1_loadstate(is, size))
        ret = false;
      break;
    case 150:
      if (!s_slot2_loadstate(is, size))
        ret = false;
      break;
    // reserved for future versions
    case 160:
    case 170:
    case 180:
      if (!ReadStateChunk(is, reserveChunks, size))
        ret = false;
      break;
      // ============================
  ...
  if (chunkError) {
    msgbox->warn("There was an error loading the savestate. Your game session "
                 "is probably corrupt now.");
  } else {
  ...
  if (haveInfo) {
    ...
    if (save_time) {
      ...
      printf("\tSave created: %04d-%.3s-%02d %s %02d:%02d:%02d\n",
             tm.get_Year(), DateTime::GetNameOfMonth(tm.get_Month()),
             tm.get_Day(), wday[tm.get_DayOfWeek() % 7], tm.get_Hour(),
             tm.get_Minute(), tm.get_Second());
    }
    ...
    // printf("\tGame title: %s\n", buf);
    // printf("\tGame code: %c%c%c%c\n", header.gameCode[0], header.gameCode[1],
    // header.gameCode[2], header.gameCode[3]); const char *makerName =
    // Database::MakerNameForMakerCode(header.makerCode); makerName = makerName
    // ? makerName : "Unknown"; printf("\tMaker code: %c%c (0x%04X) - %s\n",
    // header.makerCode & 0xFF, header.makerCode >> 8, header.makerCode,
    // makerName); printf("\tDevice capacity: %dMb (real size %dMb)\n", ((128 *
    // 1024) << header.cardSize) / (1024 * 1024), romsize / (1024 * 1024));
    // printf("\tCRC16: %04Xh\n", header.CRC16);
    // printf("\tHeader CRC16: %04Xh\n", header.headerCRC16);

    if (gameInfo.romsize != romsize ||
        memcmp(&gameInfo.header, &header, sizeof(header)) != 0)
      msgbox->warn("The savestate you are loading does not match the ROM you "
...
static void loadstate() {
  ...
  for (int i = 0; i < 0xA; i++)
    _MMU_write08<ARMCPU_ARM9>(0x04000240 + i,
                              _MMU_read08<ARMCPU_ARM9>(0x04000240 + i));
  ...
  // zero 27-jul-09 : was formerly up to 7F but that wrote to dispfifo which is
  // dumb (one of nitsuja's desynch bugs [that he found, not caused]) so then i
  // brought it down to 66 but this resulted in a conceptual bug with affine
  // start registers, which shouldnt get regenerated so then i just made this
  // exhaustive list
  ...
  //_MMU_write16<ARMCPU_ARM9>(i, _MMU_read16<ARMCPU_ARM9>(i));
  static const u8 mainRegenAddr[] = {0x00, 0x02, 0x08, 0x0a, 0x0c, 0x0e, 0x40,
                                     0x42, 0x44, 0x46, 0x48, 0x4a, 0x4c, 0x50,
                                     0x52, 0x54, 0x64, 0x66, 0x6c};
  static const u8 subRegenAddr[] = {0x00, 0x02, 0x08, 0x0a, 0x0c, 0x0e,
                                    0x40, 0x42, 0x44, 0x46, 0x48, 0x4a,
                                    0x4c, 0x50, 0x52, 0x54, 0x6c};
  for (u32 i = 0; i < ARRAY_SIZE(mainRegenAddr); i++)
    _MMU_write16<ARMCPU_ARM9>(
        REG_BASE_DISPA + mainRegenAddr[i],
        _MMU_read16<ARMCPU_ARM9>(REG_BASE_DISPA + mainRegenAddr[i]));
  for (u32 i = 0; i < ARRAY_SIZE(subRegenAddr); i++)
    _MMU_write16<ARMCPU_ARM9>(
        REG_BASE_DISPB + subRegenAddr[i],
        _MMU_read16<ARMCPU_ARM9>(REG_BASE_DISPB + subRegenAddr[i]));
  // no need to restore 0x60 since control and MMU.ARM9_REG are both in the
  // savestates, and restoring it could mess up the ack bits anyway
...
bool savestate_load(EMUFILE &is) {
  ...
  if (!is.read_32LE(ssversion))
    return false;
  if (!is.read_32LE(_DESMUME_version))
    return false;
  if (!is.read_32LE(len))
    return false;
  if (!is.read_32LE(comprlen))
    return false;

  if (ssversion != SAVESTATE_VERSION)
    return false;
  ...
  if (comprlen != 0xFFFFFFFF) {
    ...
    int error =
        uncompress((uint8 *)&buf[0], &uncomprlen, (uint8 *)&cbuf[0], comprlen);
  ...
  // the full reset wipes more things, so we can make sure that they are being
  // restored correctly
  ...
  if (!x && !SAV_silent_fail_flag) {
    msgbox->error(
        ...
        "no savestate backup system, your current game session is wrecked");
    return false;
  }
