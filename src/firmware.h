...
// the count of bytes (user settings) copied from the firmware into memory (for
// easy access) during the firmware boot process
...
private:
  ...
  u16 _getBootCodeCRC16(const u8 *arm9Data, const u32 arm9Size,
...
void NDS_ApplyFirmwareSettings(
    NDSFirmwareData *outFirmware, const FW_HEADER_KEY *headerKey,
    const FWUserSettings *userSettings0, const FWUserSettings *userSettings1,
    const FWWifiInfo *wifiInfo, const FWAccessPointSettings *wifiAP1,
    const FWAccessPointSettings *wifiAP2, const FWAccessPointSettings *wifiAP3);
...
struct fw_memory_chip {
  ...
  u8 addr_shift; // shift for address (since addresses are transfered by 3 bytes
                 // units)
...
void mc_init(fw_memory_chip *mc,
             int type); /* reset and init values for memory struct */
...
void mc_load_file(fw_memory_chip *mc,
                  const char *filename); /* load save file and setup fp */
