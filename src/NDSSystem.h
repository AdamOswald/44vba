...
struct NDS_header {
  ...
  u8 autostart;  // 01F - Autostart (Bit2: Skip "Press Button" after Health and
                 // Safety)
  ...
  u32 ARM7exe;     // 034 - ARM7 entry_address (2000000h..23BFE00h, or
                   // 37F8000h..3807E00h)
  u32 ARM7cpy;     // 038 - ARM7 ram_address   (2000000h..23BFE00h, or
                   // 37F8000h..3807E00h)
  u32 ARM7binSize; // 03C - ARM7 size          (max 3BFE00h, or FE00h)
                   // (3839.5KB, 63.5KB)
  ...
  u32 normalCmd; // 060 - Port 40001A4h setting for normal commands (usually
                 // 00586000h)
  u32 Key1Cmd;   // 064 - Port 40001A4h setting for KEY1 commands   (usually
                 // 001808F8h)

  u32 IconOff; // 068 - Icon_title_offset (0=None) (8000h and up)
  u16 CRC16;   // 06C - Secure Area Checksum, CRC-16 of [ [20h]..7FFFh] -
             // Calculations with this algorithm use 0xffff as the initial value
  ...
  u8 infoResevedRegion[8]; // 078 - ROM Information Reserved Region (must be set
                           // to 0x00)
  u32 endROMoffset; // 080 - Total Used ROM size (remaining/unused bytes usually
                    // FFh-padded)
  ...
  u8 reserved2[48]; // 090 - Must be set to 0x00 - "PASS" is contained within
                    // here?
  u8 logo[156];     // 0C0 - Nintendo Logo (compressed bitmap, same as in GBA
                    // Headers)
  u16 logoCRC16;   // 15C - Nintendo Logo Checksum, CRC-16 of [0C0h-15Bh], fixed
                   // CF56h
...
struct NDSSystem {
  ...
  // the DSI returns calibrated touch coords from its TSC (?), so we need to
  ...
  s32 cpuloopIterationCount; // counts the number of times during a frame that a
                             // reschedule happened

  // console type must be copied in when the system boots. it can't be changed
  // on the fly.
  int ConsoleType;
...
struct RomBanner {
  ...
  u8 bitmap[0x200]; // Icon Bitmap  (32x32 pix) (4x4 tiles, each 4x8 bytes, 4bit
                    // depth)
  u16 palette[0x10]; // Icon Palette (16 colors, 16bit, range 0000h-7FFFh)
                     // (Color 0 is transparent, so the 1st palette entry is
                     // ignored)
  enum { NUM_TITLES = 6 };
  union {
    struct {
...
struct GameInfo {
  ...
  GameInfo()
      : fROM(NULL), romdataForReader(NULL), crc(0), chipID(0x00000FC2),
        romsize(0), cardSize(0), mask(0), romType(ROM_NDS), headerOffset(0),
...
void NDS_setPad(bool right, bool left, bool down, bool up, bool select,
                bool start, bool B, bool A, bool Y, bool X, bool leftShoulder,
                bool rightShoulder, bool debug, bool lid);
void NDS_setMic(bool pressed);

// get physical user input
// not including the results of autofire/etc.
// the effects of calls to "set physical user input" functions will be
// immediately reflected here though.
...
int NDS_LoadROM(const char *filename, const char *physicalFilename = 0,
                const char *logicalFilename = 0);
...
extern struct TCommonSettings {
  TCommonSettings()
      : GFX3D_HighResolutionInterpolateColor(true), GFX3D_EdgeMark(true),
        GFX3D_Fog(true), GFX3D_Texture(true), GFX3D_LineHack(true),
        ...
        GFX3D_Renderer_TextureSmoothing(false), GFX3D_TXTHack(false),
        ...
        jit_max_block_size(12), loadToMemory(false), UseExtBIOS(false),
        SWIFromBIOS(false), PatchSWI3(false), UseExtFirmware(false),
        UseExtFirmwareSettings(false), RetailCardProtection8000(true),
        BootFromFirmware(false), DebugConsole(false), EnsataEmulation(false),
        cheatsDisable(false), rigorous_timing(false), advanced_timing(false),
        micMode(InternalNoise), spuInterpolationMode(2), manualBackupType(0),
        autodetectBackupMethod(0), spu_captureMuted(false), spu_advanced(false),
        StylusPressure(50), ConsoleType(NDS_CONSOLE_TYPE_FAT),
        backupSave(false), SPU_sync_mode(1), SPU_sync_method(0),
    ...
    for (int g = 0; g < 2; g++)
      for (int x = 0; x < 5; x++)
        dispLayers[g][x] = true;
#ifdef HAVE_JIT
    // zero 06-sep-2012 - shouldnt be defaulting this to true for now, since the
    // jit is buggy. id rather have people discover a bonus speedhack than
    // discover new bugs in a new version
    use_jit = false;
#else
    use_jit = false;
#endif
  ...
  // this is a temporary hack until we straighten out the flushing logic and/or
  // gxfifo int gfx3d_flushMode;

  int autodetectBackupMethod;
  // this is the user's choice of manual backup type, for cases when the
  // autodetection can't be trusted
  ...
  struct _Hud {
    _Hud()
        : ShowInputDisplay(false), ShowGraphicalInputDisplay(false),
          FpsDisplay(false), FrameCounterDisplay(false),
          ShowLagFrameCounter(false), ShowMicrophone(false), ShowRTC(false) {}
    bool ShowInputDisplay, ShowGraphicalInputDisplay, FpsDisplay,
        FrameCounterDisplay, ShowLagFrameCounter, ShowMicrophone, ShowRTC;
  } hud;
