...
using namespace std;
bool freshMovie =
    false; // True when a movie loads, false when movie is altered.  Used to
           // determine if a movie has been altered since opening
...
// Loading a movie calls NDS_Reset, which calls UnloadMovieEmulationSettings.
// Don't unload settings on that call.
...
bool MovieRecord::Compare(MovieRecord &compareRec) {
  ...
  // Check Stylus
  if (this->touch.padding != compareRec.touch.padding)
    return false;

  // Check comamnds
  // if new commands are ever recordable, they need to be added here if we go
  // with this method
  if (this->command_reset() != compareRec.command_reset())
    return false;
  if (this->command_microphone() != compareRec.command_microphone())
    return false;
  if (this->command_lid() != compareRec.command_lid())
    return false;

  return true;
}

const char MovieRecord::mnemonics[13] = {'R', 'L', 'D', 'U', 'T', 'S', 'B',
                                         'A', 'Y', 'X', 'W', 'E', 'G'};
...
void MovieRecord::dump(EMUFILE &fp) {
  ...
  fp.fputc('|');
  dumpPad(fp, pad);
  putdec<u8, 3, true>(fp, touch.x);
  fp.fputc(' ');
  putdec<u8, 3, true>(fp, touch.y);
  fp.fputc(' ');
...
MovieData::MovieData(bool fromCurrentSettings)
    : version(MOVIE_VERSION), emuVersion(EMU_DESMUME_VERSION_NUMERIC()),
...
void MovieData::installRomChecksum(std::string &key, std::string &val) {
  // TODO: The current implementation of reading the checksum doesn't work
  // correctly, and can cause crashes when the MovieData object is deallocated.
  // (This is caused by StringToBytes() overrunning romChecksum into romSerial,
  // making romSerial undefined.) Set romChecksum to some dummy value for now to
  // prevent crashing. This is okay, since romChecksum isn't actually used in
  // practice at this time. - rogerman, 2012/08/24
...
void MovieData::installComment(std::string &key, std::string &val) {
  comments.push_back(mbstowcs(val));
}
void MovieData::installSram(std::string &key, std::string &val) {
...
int MovieData::dump(EMUFILE &fp, bool binary) {
  ...
  fp.fprintf("useExtBios %d\n",
             CommonSettings.UseExtBIOS
                 ? 1
                 : 0); // TODO: include bios file data, not just a flag saying
                       // something was used
  ...
  fp.fprintf("useExtFirmware %d\n",
             CommonSettings.UseExtFirmware
                 ? 1
                 : 0); // TODO: include firmware file data, not just a flag
                       // saying something was used

  if (CommonSettings.UseExtFirmware) {
    fp.fprintf("bootFromFirmware %d\n",
               CommonSettings.BootFromFirmware ? 1 : 0);
  } else {
    std::wstring wnick((wchar_t *)CommonSettings.fwConfig.nickname,
                       CommonSettings.fwConfig.nicknameLength);
    std::string nick = wcstombs(wnick);

    std::wstring wmessage((wchar_t *)CommonSettings.fwConfig.message,
                          CommonSettings.fwConfig.messageLength);
  ...
  for (int i = 0; i < 256; i++) {
    ...
    if (micSamples.size() > i) {
      ...
      fp.fprintf(
          "%s %s\n", tmp,
          BytesToString(&micSamples[i][0], micSamples[i].size()).c_str());
    }
  }
...
bool LoadFM2(MovieData &movieData, EMUFILE &fp, int size,
             bool stopAfterHeader) {
  int endOfMovie;
  if (size == INT_MAX)
    endOfMovie = fp.size();
  else
  ...
  // just in case readUntilNotWhitespace read past the limit set by size
  // parameter
...
static void LoadSettingsFromMovie(MovieData movieData) {
  ...
  if (!CommonSettings.UseExtFirmware) {
    if (movieData.firmNickname != "") {
      CommonSettings.fwConfig.nicknameLength =
          movieData.firmNickname.length() > MAX_FW_NICKNAME_LENGTH
    ...
    if (movieData.firmMessage != "") {
      CommonSettings.fwConfig.messageLength =
          movieData.firmMessage.length() > MAX_FW_MESSAGE_LENGTH
              ? MAX_FW_MESSAGE_LENGTH
              : movieData.firmMessage.length();
      for (int i = 0; i < CommonSettings.fwConfig.messageLength; i++)
...
bool AreMovieEmulationSettingsActive() { return (bool)oldSettings; }
// begin playing an existing movie
const char *_CDECL_ FCEUI_LoadMovie(const char *fname, bool _read_only,
                                    bool tasedit, int _pauseframe) {
  // if(!tasedit && !FCEU_IsValidUI(FCEUI_PLAYMOVIE))
  //	return;

  // FIXME: name==null indicates to BROWSE to retrieve fname from user (or stdin
  // if that's impossible), when called from movie_play
  assert(fname);
  if (!fname)
    return "LoadMovie doesn't support browsing yet";

  // mbg 6/10/08 - we used to call StopMovie here, but that cleared
  // curMovieFilename and gave us crashes...
  if (movieMode == MOVIEMODE_PLAY || movieMode == MOVIEMODE_FINISHED)
    StopPlayback();
  else if (movieMode == MOVIEMODE_RECORD)
    StopRecording();
  ...
  if (currMovieData.savestate) {
    // SS file name should be the same as the movie file name, except for
    // extension
    std::string ssName = fname;
    ssName.erase(ssName.length() - 3, 3);
    ssName.append("dst");
    if (!savestate_load(ssName.c_str()))
      return "Could not load movie's savestate. There should be a .dst file "
             "with the same name as the movie, in the same folder.";
  } else {
    firstReset = true;
    NDS_Reset();
    firstReset = false;
...
//	//TODO - support dumping directly without compressing to save a buffer
//copy
////TODO
///*	memorystream ms;
//
...
void FCEUI_SaveMovie(const char *fname, std::wstring author,
                     START_FROM startFrom, std::string sramfname,
                     const DateTime &rtcstart) {
  ...
  if (author != L"")
    currMovieData.comments.push_back(L"author " + author);
  ...
  // we are going to go ahead and dump the header. from now on we will only be
  // appending frames
...
void FCEUMOV_HandlePlayback() {
  if (movieMode == MOVIEMODE_PLAY) {
    ...
    // if we are on the last frame, then pause the emulator if the player
    // requested it
...
void FCEUMOV_HandleRecording() {
  if (movieMode == MOVIEMODE_RECORD) {
    ...
    // assert(nds.touchX == input.touch.touchX && nds.touchY ==
    // input.touch.touchY); assert((mr.touch.x << 4) == nds.touchX &&
    // (mr.touch.y << 4) == nds.touchY);
...
bool CheckTimelines(MovieData &stateMovie, MovieData &currMovie, int &errorFr) {
  ...
  // First check, make sure we are checking is for a post-movie savestate, we
  // just want to adjust the length for now, we will handle this situation later
  // in another function
  if (currFrameCounter <= stateMovie.getNumRecords())
    length = currFrameCounter; // Note: currFrameCounter corresponds to the
                               // framecounter in the savestate
  else if (currFrameCounter >
           currMovie
               .getNumRecords()) // Now that we know the length of the records
                                 // of the savestate we plan to load, let's
                                 // match the length against the movie
    length =
        currMovie.getNumRecords(); // If length < currMovie records then this is
                                   // a "future" event from the current movie,
                                   // againt we will handle this situation
                                   // later, we just want to find the right
...
bool mov_loadstate(EMUFILE &fp, int size) {
  ...
  if (!movie_readonly && autoMovieBackup &&
      freshMovie) // If auto-backup is on, movie has not been altered this
                  // session and the movie is in read+write mode
  {
    FCEUI_MakeBackupMovie(false); // Backup the movie before the contents get
                                  // altered, but do not display messages
  }

  // a little rule: cant load states in read+write mode with a movie from an
  // archive. so we are going to switch it to readonly mode in that case
  //	if(!movie_readonly
  //		//*&& FCEU_isFileInArchive(curMovieFilename)*/
  //		) {
  //		FCEU_PrintError("Cannot loadstate in Read+Write with movie from
  //archive. Movie is now Read-Only."); 		movie_readonly = true;
  //	}
  ...
  if (!LoadFM2(tempMovieData, fp, size, false)) {
    ...
    /*	extern bool FCEU_state_loading_old_format;
            if(FCEU_state_loading_old_format) {
                    if(movieMode == MOVIEMODE_PLAY || movieMode ==
       MOVIEMODE_RECORD) { FCEUI_StopMovie(); FCEU_PrintError("You have tried to
       use an old savestate while playing a movie. This is unsupported (since
       the old savestate has old-format movie data in it which can't be
  ...
  * Check that GUID of movie and savestate-movie must match or else error
        o on error: a message informing that the savestate doesn't belong to
  this movie. This is a GUID mismatch error. Give user a choice to load it
  anyway.
              + failstate: if use declines, loadstate attempt canceled, movie
  can resume as if not attempted if user has backup savstates enabled else stop
  movie
  * Check that movie and savestate-movie are in same timeline. If not then this
  is a wrong timeline error. o on error: a message informing that the savestate
  doesn't belong to this movie
              + failstate: loadstate attempt canceled, movie can resume as if
  not attempted if user has backup savestates enabled else stop movie
  * Check that savestate-movie is not greater than movie. If not then this is a
  future event error and is not allowed in read-only o on error: message
  informing that the savestate is from a frame after the last frame of the movie
              + failstate - loadstate attempt cancelled, movie can resume if
  user has backup savesattes enabled, else stop movie
  * Check that savestate framcount <= savestate movie length. If not this is a
  post-movie savestate o on post-movie: See post-movie event section.
  ...
  * Check that GUID of movie and savestate-movie must match or else error
  ...
  anyway.
              + failstate: if use declines, loadstate attempt canceled, movie
  can resume as if not attempted (stop movie if resume fails)canceled, movie can
  resume if backup savestates enabled else stopmovie
  * Check that savestate framcount <= savestate movie length. If not this is a
  post-movie savestate o on post-movie: See post-movie event section.
  * savestate passed all error checks and will now be loaded in its entirety and
  replace movie (note: there will be no truncation yet)
  * current framecount will be set to savestate framecount
  * on the next frame of input, movie will be truncated to framecount
        o (note: savestate-movie can be a future event of the movie timeline, or
  a completely new timeline and it is still allowed)
  * Rerecord count of movie will be incremented
  * Movie is now in record mode

  Post-movie savestate event

  * Whan a savestate is loaded and is determined that the savestate-movie length
  is less than the savestate framecount then it is a post-movie savestate. These
  occur when a savestate was made during Movie Finished mode.
  * If read+write, the entire savestate movie will be loaded and replace current
  movie.
  ...
  */
  ...
  if (movieMode != MOVIEMODE_INACTIVE) {
    ...
    if (tempMovieData.guid != currMovieData.guid) {
      // mbg 8/18/08 - this code  can be used to turn the error message into an
      // OK/CANCEL
#if defined(WIN32_FRONTEND)
      std::string msg =
          "There is a mismatch between savestate's movie and current "
          "movie.\ncurrent: " +
          currMovieData.guid.toString() +
          "\nsavestate: " + tempMovieData.guid.toString() +
          "\n\nThis means that you have loaded a savestate belonging to a "
          "different movie than the one you are playing now.\n\nContinue "
          "loading this savestate anyway?";
      int result = MessageBox(MainWindow->getHWnd(), msg.c_str(),
                              "Error loading savestate", MB_OKCANCEL);
      if (result == IDCANCEL)
        return false;
#else
      FCEU_PrintError("Mismatch between savestate's movie and current "
                      "movie.\ncurrent: %s\nsavestate: %s\n",
                      currMovieData.guid.toString().c_str(),
                      tempMovieData.guid.toString().c_str());
      return false;
#endif
    }
    ...
    if (currFrameCounter > (int)currMovieData.records.size()) {
      ...
      // except it permits switching to play/record by loading an earlier
      // savestate. (and we continue to store the finished movie in savestates
      // made while finished)
      driver->SetLineColor(255, 0,
                           0); // let's make the text red too to hopefully catch
                               // the user's attention a bit.
      FinishPlayback();
      driver->SetLineColor(255, 255, 255);

      // FCEU_PrintError("Savestate is from a frame (%d) after the final frame
      // in the movie (%d). This is not permitted.", currFrameCounter,
      // currMovieData.records.size()-1); return false;
    } else if (movie_readonly) {
      //-------------------------------------------------------------
      // this code would reload the movie from disk. allegedly it is helpful to
      // hexers, but it is way too slow with dsm format. so it is only here as a
      // reminder, and in case someone wants to play with it
      //-------------------------------------------------------------
      //{
      //	fstream fs (curMovieFilename);
      //	if(!LoadFM2(tempMovieData, &fs, INT_MAX, false))
      //	{
      //		FCEU_PrintError("Failed to reload DSM after loading
      //savestate");
      //	}
      //	fs.close();
      //	currMovieData = tempMovieData;
...
static void FCEUMOV_PreLoad(void) { load_successful = 0; }
...
void LoadFM2_binarychunk(MovieData &movieData, EMUFILE &fp, int size) {
  ...
  // the amount todo is the min of the limiting size we received and the
  // remaining contents of the file
  int todo = std::min(size, flen);

  int numRecords = todo / recordsize;
  // printf("LOADED MOVIE: %d records; currFrameCounter:
...
void FCEUI_MakeBackupMovie(bool dispMessage) {
  ...
  bool overflow =
      false; // Used for special situation when backup numbering exceeds limit

  currentFn = curMovieFilename; // Get current moviefilename
  backupFn = curMovieFilename;  // Make backup filename the same as current
                               // moviefilename
  ...
  for (unsigned int backNum = 0; backNum < 999;
       backNum++) // 999 = arbituary limit to backup files
  {
    ...
    if (!exist)
      ...
      if (backNum ==
          999) // If 999 exists, we have overflowed, let's handle that
      {
  ...
  EMUFILE_FILE outf = EMUFILE_FILE(
      backupFn.c_str(),
      "wb"); // FCEUD_UTF8_fstream(backupFn, "wb");	//open/create file
  md.dump(outf, false); // dump movie data

  // TODO, decide if fstream successfully opened the file and print error
  // message if it doesn't

  if (dispMessage) // If we should inform the user
  {
    //		if (overflow)
    //			FCEUI_DispMessage("Backup overflow, overwriting
    //%s",backupFn.c_str()); //Inform user of overflow 		else
    //			FCEUI_DispMessage("%s created",backupFn.c_str()); //Inform user of
    //backup filename
  }
}

void BinaryDataFromString(std::string &inStringData,
                          std::vector<u8> *outBinaryData) {
  int len = Base64StringToBytesLength(inStringData);
  if (len == -1)
    len = HexStringToBytesLength(inStringData); // wasn't base64, try hex
  if (len >= 1) {
    outBinaryData->resize(len);
    StringToBytes(inStringData, &outBinaryData->front(),
                  len); // decodes either base64 or hex
  }
}
