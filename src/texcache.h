...
// changed by zeromus on 15-dec. I couldnt find any games that were getting
// anywhere NEAR 64 metal slug burns through sprites so fast, it can test it
// pretty quickly though #define TEXCACHE_DEFAULT_THRESHOLD (16*1024*1024)

// rogerman, 2016-11-02: Increase this to 32MB for games that use many large
// textures, such as Umihara Kawase Shun, which can cache over 20MB in the first
// level.
...
typedef std::map<TextureCacheKey, TextureStore *>
...
public:
  ...
  void SetTextureData(const MemSpan &packedData,
  ...
  size_t
  GetUnpackSizeUsingFormat(const TextureStoreUnpackFormat texCacheFormat) const;
  template <TextureStoreUnpackFormat TEXCACHEFORMAT>
  void Unpack(u32 *unpackBuffer);
