...
public:
  ...
  void vprintf(const char *format, va_list l, const char *filename,
               unsigned int line);
  ...
  static void log(unsigned int channel, const char *file, unsigned int line,
                  const char *format, ...);
  static void log(unsigned int channel, const char *file, unsigned int line,
                  std::ostream &os);
  static void log(unsigned int channel, const char *file, unsigned int line,
...
#if defined(DEBUG) || defined(GPUDEBUG) || defined(DIVDEBUG) ||                \
    defined(SQRTDEBUG) || defined(DMADEBUG) || defined(DEVELOPER)
...
#define INFOC(channel, ...)                                                    \
  Logger::log(channel, __FILE__, __LINE__, __VA_ARGS__)
...
enum EDEBUG_EVENT {
  ...
  DEBUG_EVENT_EXECUTE =
      3, // prefetch on arm9 or arm7, triggered after the read event
  DEBUG_EVENT_ACL_EXCEPTION = 4, // acl exception on arm9
  DEBUG_EVENT_CACHE_MISS = 5,    // cache miss on arm9
};

enum EDEBUG_NOTIFY { DEBUG_NOTIFY_READ_BEYOND_END_OF_CART, DEBUG_NOTIFY_MAX };
