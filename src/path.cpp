...
static const char InvalidPathChars[] = {'\x22',
                                        '\x3C',
                                        '\x3E',
                                        '\x7C',
                                        '\x00',
...
// but it is sort of windows-specific. Does it work in linux? Maybe we'll have
// to make it smarter
...
bool Path::IsPathRooted(const std::string &path) {
  ...
  return (
      delimiters.find(path[0]) != std::string::npos ||
...
void PathInfo::LoadModulePath() {
  ...
  while (p >= pathToModule && *p != DIRECTORY_DELIMITER_CHAR)
    p--;
  if (++p >= pathToModule)
    *p = 0;
...
void PathInfo::GetDefaultPath(char *pathToDefault, const char *key,
                              int maxCount) {
#ifdef HOST_WINDOWS
  std::string temp =
      (std::string) "." + DIRECTORY_DELIMITER_CHAR + pathToDefault;
...
#ifdef HOST_WINDOWS
  GetPrivateProfileString(SECTION, FORMATKEY, "%f_%s_%r", screenshotFormat,
                          MAX_FORMAT, IniName);
  savelastromvisit =
      GetPrivateProfileBool(SECTION, LASTVISITKEY, true, IniName);
  currentimageformat = (ImageFormat)GetPrivateProfileInt(
      SECTION, DEFAULTFORMATKEY, PNG, IniName);
  r4Format = (R4Format)GetPrivateProfileInt(SECTION, R4FORMATKEY, R4_CHEAT_DAT,
                                            IniName);
...
#endif
  /*
  needsSaving		= GetPrivateProfileInt(SECTION, NEEDSSAVINGKEY, TRUE,
  IniName); if(needsSaving)
  {
  needsSaving = FALSE;
  WritePathSettings();
  }*/
...
void PathInfo::SwitchPath(Action action, KnownPath path, char *buffer) {
  ...
  if (action == GET) {
    ...
    else if (thePath[len] != DIRECTORY_DELIMITER_CHAR)
    ...
    if (std::string(ALL_DIRECTORY_DELIMITER_STRING).find(buffer[len]) !=
        std::string::npos)
      buffer[len] = '\0';
...
std::string PathInfo::extension() { return Path::GetFileExt(path); }

std::string PathInfo::noextension() {
  std::string romNameWithPath = Path::GetFileDirectoryPath(path) +
                                DIRECTORY_DELIMITER_CHAR +
                                Path::GetFileNameWithoutExt(RomName);
...
void PathInfo::formatname(char *output) {
  ...
  for (char *p = screenshotFormat, *end = p + sizeof(screenshotFormat); p < end;
       p++) {
    if (*p != '%') {
      file.append(1, *p);
    } else {
...
PathInfo::ImageFormat PathInfo::imageformat() { return currentimageformat; }
...
const char *PathInfo::GetRomName() { return RomName.c_str(); }
