...
public:
  ...
  // transfers a byte to the slot-1 device via auxspi, and returns the incoming
  // byte cpu is provided for diagnostic purposes only.. the slot-1 device
  // wouldn't know which CPU it is.
...
enum NDS_SLOT1_TYPE {
  ...
  NDS_SLOT1_RETAIL_MCROM, // 0x01 - a standard MC (eeprom, flash, fram) -bearing
                          // retail card. Also supports motion, for now, because
                          // that's the way we originally coded it
