...
// this file is split from MMU.h for the purpose of avoiding ridiculous
// recompile times when changing it, because practically everything includes
// MMU.h.
#ifndef MMUTIMING_H
#define MMUTIMING_H
...
enum MMU_ACCESS_DIRECTION { MMU_AD_READ, MMU_AD_WRITE };
...
// this doesn't really get used unless ENABLE_CACHE_CONTROLLER_EMULATION is
// defined.
template <int SIZESHIFT, int ASSOCIATIVESHIFT, int BLOCKSIZESHIFT>
class CacheController {
public:
  template <MMU_ACCESS_DIRECTION DIR> FORCEINLINE bool Cached(u32 addr) {
...
private:
  ...
  enum {
    BLOCKMASK = ((u32)~0U >> (32 - TAGSHIFT)) & (u32)(~0U << BLOCKSIZESHIFT)
  };
...
template <int PROCNUM, MMU_ACCESS_TYPE AT, int READSIZE,
          MMU_ACCESS_DIRECTION DIRECTION, bool TIMING>
...
public:
  ...
  FORCEINLINE u32 Fetch(u32 address) {
    ...
    u32 time = _MMU_accesstime<PROCNUM, AT, READSIZE, DIRECTION, TIMING>(
        address,
  ...
  void Reset() { m_lastAddress = ~0; }
  FetchAccessUnit() { this->Reset(); }

  void savestate(EMUFILE &os, int version) { os.write_32LE(m_lastAddress); }
  bool loadstate(EMUFILE &is, int version) {
...
struct MMU_struct_timing {
  ...
  CacheController<13, 2, 5>
      arm9codeCache; // 8192 bytes, 4-way associative, 32-byte blocks
  CacheController<12, 2, 5>
      arm9dataCache; // 4096 bytes, 4-way associative, 32-byte blocks
  ...
  FORCEINLINE FetchAccessUnit<PROCNUM, MMU_AT_CODE> &armCodeFetch();
  template <int PROCNUM>
  FORCEINLINE FetchAccessUnit<PROCNUM, MMU_AT_DATA> &armDataFetch();
};
template <>
FORCEINLINE FetchAccessUnit<0, MMU_AT_CODE> &
MMU_struct_timing::armCodeFetch<0>() {
  return this->arm9codeFetch;
}
template <>
FORCEINLINE FetchAccessUnit<1, MMU_AT_CODE> &
MMU_struct_timing::armCodeFetch<1>() {
  return this->arm7codeFetch;
}
template <>
FORCEINLINE FetchAccessUnit<0, MMU_AT_DATA> &
MMU_struct_timing::armDataFetch<0>() {
  return this->arm9dataFetch;
}
template <>
FORCEINLINE FetchAccessUnit<1, MMU_AT_DATA> &
MMU_struct_timing::armDataFetch<1>() {
  return this->arm7dataFetch;
}
...
FORCEINLINE u32 _MMU_accesstime(u32 addr, bool sequential) {
  ...
  static const int M32 =
      (PROCNUM == ARMCPU_ARM9) ? 2 : 1; // access through 32-bit bus
  static const int M16 =
      M32 * ((READSIZE > 16) ? 2 : 1); // access through 16-bit bus
  ...
  // ITCM, ITCM, MAIN, SWI, REG, VMEM, LCD, OAM,  ROM,  ROM,  RAM,   U,  U,  U,
  // U, BIOS
#define X                                                                      \
  MC, MC, M16, M32, M32, M16, M16, M32, MSLW, MSLW, MSLW, M32, M32, M32, M32,  \
      M32,
      // duplicate it 16 times (this was somehow faster than using a mask of
      // 0xF)
...
#undef X
  ...
  if (TIMING && !sequential) {
    // if(c != MC || PROCNUM==ARMCPU_ARM7) // check not needed anymore because
    // ITCM/DTCM return earlier
...
FORCEINLINE u32 MMU_memAccessCycles(u32 addr) {
  if (TIMING)
    return MMU_timing.armDataFetch<PROCNUM>()
        .template Fetch<READSIZE, DIRECTION, true>((addr) &
                                                   (~((READSIZE >> 3) - 1)));
  else
    return MMU_timing.armDataFetch<PROCNUM>()
        .template Fetch<READSIZE, DIRECTION, false>((addr) &
                                                    (~((READSIZE >> 3) - 1)));
}
...
FORCEINLINE u32 MMU_codeFetchCycles(u32 addr) {
  if (USE_TIMING())
    return MMU_timing.armCodeFetch<PROCNUM>()
        .template Fetch<READSIZE, MMU_AD_READ, true>((addr) &
                                                     (~((READSIZE >> 3) - 1)));
  else
    return MMU_timing.armCodeFetch<PROCNUM>()
        .template Fetch<READSIZE, MMU_AD_READ, false>((addr) &
                                                      (~((READSIZE >> 3) - 1)));
}
...
FORCEINLINE u32 MMU_fetchExecuteCycles(u32 executeCycles, u32 fetchCycles) {
  ...
  if (USE_TIMING() && allow) {
    // execute and fetch are different stages of the pipeline for both arm7 and
    // arm9. again, we approximate the pipeline throughput using max.
