...
// Note: Technically, the shift count of palignr can be any value of [0-255].
// But practically speaking, the shift count should be a value of [0-15]. If we
// assume that the value range will always be [0-15], we can then substitute the
// palignr instruction with an SSE2 equivalent.
#if defined(ENABLE_SSE2) && !defined(ENABLE_SSSE3)
#define _mm_alignr_epi8(a, b, immShiftCount)                                   \
  _mm_or_si128(_mm_slli_si128(a, 16 - (immShiftCount)),                        \
               _mm_srli_si128(b, (immShiftCount)))
#endif

// Note: The SSE4.1 version of pblendvb only requires that the MSBs of the 8-bit
// mask vector are set in order to pass the b byte through. However, our SSE2
// substitute of pblendvb requires that all of the bits of the 8-bit mask vector
// are set. So when using this intrinsic in practice, just set/clear all mask
// bits together, and it should work fine for both SSE4.1 and SSE2.
#if defined(ENABLE_SSE2) && !defined(ENABLE_SSE4_1)
#define _mm_blendv_epi8(a, b, fullmask)                                        \
  _mm_or_si128(_mm_and_si128((fullmask), (b)),                                 \
               _mm_andnot_si128((fullmask), (a)))
#endif
...
typedef void (*PixelLookupFunc)(const s32 auxX, const s32 auxY, const int lg,
                                const u32 map, const u32 tile,
                                const u16 *__restrict pal, u8 &outIndex,
                                u16 &outColor);

enum PaletteMode { PaletteMode_16x16 = 0, PaletteMode_1x256 = 1 };
...
typedef union {
  ...
  struct {
    ...
    u8 OBJ_HBlank_process : 1; //    23: A+B; OBJ processed during HBlank (GBA
                               //    bit5)

    u8 CharacBase_Block : 3;    // 24-26: A  ; Character Base (64K step)
    u8 ScreenBase_Block : 3;    // 27-29: A  ; Screen Base (64K step)
    u8 ExBGxPalette_Enable : 1; //    30: A+B; 0=Disable, 1=Enable BG extended
                                //    Palette
    u8 ExOBJPalette_Enable : 1; //    31: A+B; 0=Disable, 1=Enable OBJ extended
                                //    Palette
#else
    u8 ForceBlank : 1;       //     7: A+B;
    u8 OBJ_BMP_mapping : 1;  //     6: A+B; 0=2D (128KB), 1=1D (128..256KB)
...
typedef union {
  ...
  struct {
    u16 VBlankFlag : 1;   //     0: Set at V-Blank; 0=Not in V-Blank, 1=V-Blank
                          //     occurred
    u16 HBlankFlag : 1;   //     1: Set at H-Blank; 0=Not in H-Blank, 1=H-Blank
                          //     occurred
    u16 VCounterFlag : 1; //     2: Set when this register's VCount matches the
                          //     currently rendered scanline, interacts with
                          //     VCOUNT (0x4000006); 0=Unmatched, 1=Matched
    u16 VBlankIRQ_Enable : 1;   //     3: Send an IRQ when VBlankFlag is set;
                                //     0=Disable, 1=Enable
    u16 HBlankIRQ_Enable : 1;   //     4: Send an IRQ when HBlankFlag is set;
                                //     0=Disable, 1=Enable
    u16 VCounterIRQ_Enable : 1; //     5: Send an IRQ when VCounterFlag is set;
                                //     0=Disable, 1=Enable
    u16 LCDInitReady : 1; //     6: Report that the LCD initialization is ready,
                          //     DSi only; 0=Unready 1=Ready
    u16 VCount : 9;       //  7-15: Current scanline, interacts with VCOUNT
                          //  (0x4000006)
  };
...
typedef union {
  ...
  struct {
    u16 CurrentScanline : 9; //  0- 8: The scanline currently being rendered;
                             //  0...262
...
typedef union {
  ...
  struct {
#ifndef MSB_FIRST
    u8 Priority : 2; //  0- 1: Rendering priority; 0...3, where 0 is highest
                     //  priority and 3 is lowest priority
    u8 CharacBase_Block : 4; //  2- 5: individual character base offset (n*16KB)
    u8 Mosaic : 1;           //     6: Mosaic render: 0=Disable, 1=Enable
    u8 PaletteMode : 1; //     7: Color/palette mode; 0=16 palettes of 16 colors
                        //     each, 1=Single palette of 256 colors

    u8 ScreenBase_Block : 5; //  8-12: individual screen base offset (text
                             //  n*2KB, BMP n*16KB)
...
} IOREG_BGnCNT; // 0x400x008, 0x400x00A, 0x400x00C, 0x400x00E: BGn layer control
                // (Engine A+B)
...
} IOREG_BGnHOFS; // 0x400x010, 0x400x014, 0x400x018, 0x400x01C: BGn horizontal
                 // offset (Engine A+B)

typedef IOREG_BGnHOFS
    IOREG_BGnVOFS; // 0x400x012, 0x400x016, 0x400x01A, 0x400x01E: BGn vertical
                   // offset (Engine A+B)

typedef IOREG_BGnHOFS
    IOREG_BG0HOFS; // 0x400x010: BG0 horizontal offset (Engine A+B)
typedef IOREG_BGnVOFS
    IOREG_BG0VOFS; // 0x400x012: BG0 vertical offset (Engine A+B)
typedef IOREG_BGnHOFS
    IOREG_BG1HOFS; // 0x400x014: BG1 horizontal offset (Engine A+B)
typedef IOREG_BGnVOFS
    IOREG_BG1VOFS; // 0x400x016: BG1 vertical offset (Engine A+B)
typedef IOREG_BGnHOFS
    IOREG_BG2HOFS; // 0x400x018: BG2 horizontal offset (Engine A+B)
typedef IOREG_BGnVOFS
    IOREG_BG2VOFS; // 0x400x01A: BG2 vertical offset (Engine A+B)
typedef IOREG_BGnHOFS
    IOREG_BG3HOFS; // 0x400x01C: BG3 horizontal offset (Engine A+B)
typedef IOREG_BGnVOFS
    IOREG_BG3VOFS; // 0x400x01E: BG3 vertical offset (Engine A+B)

typedef struct {
  IOREG_BGnHOFS BGnHOFS;
  IOREG_BGnVOFS BGnVOFS;
} IOREG_BGnOFS; // 0x400x010, 0x400x014, 0x400x018, 0x400x01C: BGn horizontal
                // offset (Engine A+B)
...
} IOREG_BGnPA; // 0x400x020, 0x400x030: BGn rotation/scaling parameter A (Engine
               // A+B)
typedef IOREG_BGnPA IOREG_BGnPB; // 0x400x022, 0x400x032: BGn rotation/scaling
                                 // parameter B (Engine A+B)
typedef IOREG_BGnPA IOREG_BGnPC; // 0x400x024, 0x400x034: BGn rotation/scaling
                                 // parameter C (Engine A+B)
typedef IOREG_BGnPA IOREG_BGnPD; // 0x400x026, 0x400x036: BGn rotation/scaling
                                 // parameter D (Engine A+B)
...
typedef IOREG_BGnX
    IOREG_BGnY; // 0x400x02C, 0x400x03C: BGn Y-coordinate (Engine A+B)

typedef IOREG_BGnPA
    IOREG_BG2PA; // 0x400x020: BG2 rotation/scaling parameter A (Engine A+B)
typedef IOREG_BGnPB
    IOREG_BG2PB; // 0x400x022: BG2 rotation/scaling parameter B (Engine A+B)
typedef IOREG_BGnPC
    IOREG_BG2PC; // 0x400x024: BG2 rotation/scaling parameter C (Engine A+B)
typedef IOREG_BGnPD
    IOREG_BG2PD; // 0x400x026: BG2 rotation/scaling parameter D (Engine A+B)
...
typedef IOREG_BGnPA
    IOREG_BG3PA; // 0x400x030: BG3 rotation/scaling parameter A (Engine A+B)
typedef IOREG_BGnPB
    IOREG_BG3PB; // 0x400x032: BG3 rotation/scaling parameter B (Engine A+B)
typedef IOREG_BGnPC
    IOREG_BG3PC; // 0x400x034: BG3 rotation/scaling parameter C (Engine A+B)
typedef IOREG_BGnPD
    IOREG_BG3PD; // 0x400x036: BG3 rotation/scaling parameter D (Engine A+B)
...
typedef struct {
  IOREG_BGnPA BGnPA;  // 0x400x020, 0x400x030: BGn rotation/scaling parameter A
                      // (Engine A+B)
  IOREG_BGnPB BGnPB;  // 0x400x022, 0x400x032: BGn rotation/scaling parameter B
                      // (Engine A+B)
  IOREG_BGnPC BGnPC;  // 0x400x024, 0x400x034: BGn rotation/scaling parameter C
                      // (Engine A+B)
  IOREG_BGnPD BGnPD;  // 0x400x026, 0x400x036: BGn rotation/scaling parameter D
                      // (Engine A+B)
...
typedef IOREG_WIN0H
    IOREG_WIN1H; // 0x400x042: Horizontal coordinates of Window 1 (Engine A+B)
...
typedef IOREG_WIN0V
    IOREG_WIN1V; // 0x400x046: Vertical coordinates of Window 1 (Engine A+B)
...
typedef IOREG_WIN0IN
    IOREG_WIN1IN; // 0x400x049: Control of inside of Window 1 (medium priority)
typedef IOREG_WIN0IN
    IOREG_WINOUT; // 0x400x04A: Control of outside of all windows
typedef IOREG_WIN0IN IOREG_WINOBJ; // 0x400x04B: Control of inside of Window OBJ
                                   // (lowest priority)
...
typedef union {
  ...
  struct {
#ifndef MSB_FIRST
    u16 BG0_Target1 : 1; //     0: Select layer BG0 for 1st target; 0=Disable,
                         //     1=Enable
    u16 BG1_Target1 : 1; //     1: Select layer BG1 for 1st target; 0=Disable,
                         //     1=Enable
    u16 BG2_Target1 : 1; //     2: Select layer BG2 for 1st target; 0=Disable,
                         //     1=Enable
    u16 BG3_Target1 : 1; //     3: Select layer BG3 for 1st target; 0=Disable,
                         //     1=Enable
    u16 OBJ_Target1 : 1; //     4: Select layer OBJ for 1st target; 0=Disable,
                         //     1=Enable
    u16 Backdrop_Target1 : 1; //     5: Select backdrop for 1st target;
                              //     0=Disable, 1=Enable
    u16 ColorEffect : 2;      //  6- 7: Color effect mode;
    //        0=Disable
    //        1=Alpha blend 1st and 2nd target, interacts with BLDALPHA
    //        (0x400x052) 2=Increase brightness, interacts with BLDY (0x400x054)
    //        3=Decrease brightness, interacts with BLDY (0x400x054)

    u16 BG0_Target2 : 1; //     8: Select layer BG0 for 2nd target; 0=Disable,
                         //     1=Enable
    u16 BG1_Target2 : 1; //     9: Select layer BG1 for 2nd target; 0=Disable,
                         //     1=Enable
    u16 BG2_Target2 : 1; //    10: Select layer BG2 for 2nd target; 0=Disable,
                         //    1=Enable
    u16 BG3_Target2 : 1; //    11: Select layer BG3 for 2nd target; 0=Disable,
                         //    1=Enable
    u16 OBJ_Target2 : 1; //    12: Select layer OBJ for 2nd target; 0=Disable,
                         //    1=Enable
    u16 Backdrop_Target2 : 1; //    13: Select backdrop for 2nd target;
                              //    0=Disable, 1=Enable
...
typedef union {
  ...
  struct {
#ifndef MSB_FIRST
    u16 EVA : 5; //  0- 4: Blending coefficient for 1st target; 0...31 (clamped
                 //  to 16)
    u16 : 3;     //  5- 7: Unused bits

    u16 EVB : 5; //  8-12: Blending coefficient for 2nd target; 0...31 (clamped
                 //  to 16)
...
} IOREG_BLDALPHA; // 0x400x052: Color effects selection, interacts with BLDCNT
                  // (0x400x050) (Engine A+B)
...
typedef union {
  ...
  struct {
#ifndef MSB_FIRST
    u16 EVY : 5; //  0- 4: Blending coefficient for increase/decrease
                 //  brightness; 0...31 (clamped to 16)
    u16 : 3;     //  5- 7: Unused bits
#else
    u16 : 3;     //  5- 7: Unused bits
    u16 EVY : 5; //  0- 4: Blending coefficient for increase/decrease
...
} IOREG_BLDY; // 0x400x054: Color effects selection, interacts with BLDCNT
              // (0x400x050) (Engine A+B)
...
typedef union {
  ...
  struct {
    ...
    u8 PolygonShading : 1;      //     1: Polygon shading mode, interacts with
                                //     POLYGON_ATTR (0x40004A4); 0=Toon Shading,
                                //     1=Highlight Shading
    u8 EnableAlphaTest : 1;     //     2: Perform alpha test, interacts with
                                //     ALPHA_TEST_REF (0x4000340); 0=Disable,
                                //     1=Enable
    u8 EnableAlphaBlending : 1; //     3: Perform alpha blending, interacts with
                                //     POLYGON_ATTR (0x40004A4); 0=Disable,
                                //     1=Enable
    u8 EnableAntiAliasing : 1; //     4: Render polygon edges with antialiasing;
                               //     0=Disable, 1=Enable
    u8 EnableEdgeMarking : 1;  //     5: Perform polygon edge marking, interacts
                               //     with EDGE_COLOR (0x4000330); 0=Disable,
                               //     1=Enable
    u8 FogOnlyAlpha : 1;       //     6: Apply fog to the alpha channel only,
                         //     interacts with FOG_COLOR (0x4000358) / FOG_TABLE
                         //     (0x4000360); 0=Color+Alpha, 1=Alpha
    u8 EnableFog : 1; //     7: Perform fog rendering, interacts with FOG_COLOR
                      //     (0x4000358) / FOG_OFFSET (0x400035C) / FOG_TABLE
                      //     (0x4000360);
    //        0=Disable, 1=Enable

    u8 FogShiftSHR : 4; //  8-11: SHR-Divider, interacts with FOG_OFFSET
                        //  (0x400035C); 0...10
    u8 AckColorBufferUnderflow : 1; //    12: Color Buffer RDLINES Underflow;
                                    //    0=None, 1=Underflow/Acknowledge
    u8 AckVertexRAMOverflow : 1; //    13: Polygon/Vertex RAM Overflow; 0=None,
                                 //    1=Overflow/Acknowledge
    u8 RearPlaneMode : 1; //    14: Use clear image, interacts with CLEAR_COLOR
                          //    (0x4000350) / CLEAR_DEPTH (0x4000354) /
                          //    CLRIMAGE_OFFSET (0x4000356);
    //        0=Blank, 1=Bitmap
    u8 : 1; //    15: Unused bits
...
typedef union {
  ...
  struct {
    ...
    u32 VRAMWriteBlock : 2;  // 16-17: VRAM write target block; 0=Block A,
                             // 1=Block B, 2=Block C, 3=Block D
    u32 VRAMWriteOffset : 2; // 18-19: VRAM write target offset; 0=0KB, 1=32KB,
                             // 2=64KB, 3=96KB
    u32 CaptureSize : 2;     // 20-21: Display capture dimensions; 0=128x128,
                             // 1=256x64, 2=256x128, 3=256x192
    u32 : 2;                 // 22-23: Unused bits

    u32 SrcA : 1; //    24: SrcA target; 0=Current framebuffer, 1=3D render
                  //    buffer
    ...
    u32 VRAMReadOffset : 2; // 26-27: VRAM read target offset; 0=0KB, 1=32KB,
                            // 2=64KB, 3=96KB
    u32 : 1;                //    28: Unused bit
    u32 CaptureSrc : 2;     // 29-30: Select capture target; 0=SrcA, 1=SrcB,
                            // 2=SrcA+SrcB blend, 3=SrcA+SrcB blend
    u32 CaptureEnable : 1;  //    31: Display capture status; 0=Disable/Ready
                            //    1=Enable/Busy
#else
    u32 : 3;     //  5- 7: Unused bits
    u32 EVA : 5; //  0- 4: Blending coefficient for SrcA; 0...31 (clamped to 16)
...
typedef u32
    IOREG_DISP_MMEM_FIFO; // 0x4000068: Main memory FIFO data (Engine A only)
...
typedef union {
  ...
  struct {
#ifndef MSB_FIRST
    u32 Intensity : 5; //  0- 4: Brightness coefficient for increase/decrease
                       //  brightness; 0...31 (clamped to 16)
    u32 : 3;           //  5- 7: Unused bits

    u32 : 6;      //  8-13: Unused bits
    u32 Mode : 2; // 14-15: Brightness mode; 0=Disable, 1=Increase, 2=Decrease,
                  // 3=Reserved
...
typedef union {
  ...
  struct {
    ...
    u8 TouchBacklight_Enable : 1; //  2: Touch display backlight; 0=Disable,
                                  //  1=Enable
    u8 MainBacklight_Enable : 1;  //  3: Main display backlight; 0=Disable,
                                  //  1=Enable
    u8 PowerLEDBlink_Enable : 1;  //  4: Power LED blink control; 0=Disable
                                  //  (Power LED remains steadily ON), 1=Enable
    u8 PowerLEDBlinkSpeed : 1;    //  5: Power LED blink speed when enabled;
                                  //  0=Slow, 1=Fast
    u8 SystemPowerState : 1; //  6: NDS system power state; 0=System is powered
                             //  ON, 1=System is powered OFF
    ...
    u8 PowerLEDBlinkSpeed : 1;    //  5: Power LED blink speed when enabled;
                                  //  0=Slow, 1=Fast
    u8 PowerLEDBlink_Enable : 1;  //  4: Power LED blink control; 0=Disable (LED
                                  //  is steady ON), 1=Enable
    u8 MainBacklight_Enable : 1;  //  3: Main display backlight; 0=Disable,
                                  //  1=Enable
    u8 TouchBacklight_Enable : 1; //  2: Touch display backlight; 0=Disable,
                                  //  1=Enable
...
typedef union {
  ...
  struct {
    ...
    //      2=High
    //      3=Maximum
    u8 ForceMaxBrightnessWithExtPower_Enable : 1; //   2: Forces maximum
                                                  //   brightness if external
                                                  //   power is present,
                                                  //   interacts with Bit 3;
                                                  //   0=Disable, 1=Enable
    u8 ExternalPowerState : 1; //   3: External power state; 0=External power is
                               //   not present, 1=External power is present
...
enum GPUEngineID { GPUEngineID_Main = 0, GPUEngineID_Sub = 1 };
...
typedef union {
  ...
  struct {
    ...
    union {
      ...
      struct {
        u16 Y : 8;        //  0- 7: Sprite Y-coordinate location within the
                          //  framebuffer; 0...255
        u16 RotScale : 1; //     8: Perform rotation/scaling; 0=Disable,
                          //     1=Enable
        u16 Disable : 1;  //     9: OBJ disable flag, only if Bit8 is cleared;
                          //     0=Perform render, 1=Do not perform render
        u16 Mode : 2;     // 10-11: OBJ mode; 0=Normal, 1=Transparent, 2=Window,
                          // 3=Bitmap
        u16 Mosaic : 1;   //    12: Mosaic render: 0=Disable, 1=Enable
        u16 PaletteMode : 1; //    13: Color/palette select; 0=16 palettes of 16
                             //    colors each, 1=Single palette of 256 colors
        u16 Shape : 2; // 14-15: OBJ shape; 0=Square, 1=Horizontal, 2=Vertical,
                       // 3=Prohibited
      };
      ...
      struct {
        u16 : 8;
        u16 : 1;
        u16 DoubleSize : 1; //     9: Perform double-size render, only if Bit8
                            //     is set; 0=Disable, 1=Enable
        u16 : 6;
      };
    };

    s16 X : 9; // 16-24: Sprite X-coordinate location within the framebuffer;
               // 0...511
    u16 RotScaleIndex : 3; // 25-27: Rotation/scaling parameter selection;
                           // 0...31
    ...
    u16 Priority : 2; // 42-43: Rendering priority; 0...3, where 0 is highest
                      // priority and 3 is lowest priority
    ...
    // 16-31: Whenever this is used, you will need to explicitly convert
    // endianness.
    u16 Size : 2; // 30-31: OBJ size, interacts with Bit 14-15
    //
    //        Size| Square | Horizontal | Vertical
    ...
    // 32-47: Whenever this is used, you will need to explicitly convert
    // endianness.
    u16 PaletteIndex : 4; // 44-47: Palette index; 0...15
    u16 Priority : 2;   // 42-43: Rendering priority; 0...3, where 0 is highest
                        // priority and 3 is lowest priority
    ...
    u16 attr3 : 16; // 48-63: Whenever this is used, you will need to explicitly
                    // convert endianness.
  };
} OAMAttributes;
...
enum NDSDisplayID { NDSDisplayID_Main = 0, NDSDisplayID_Touch = 1 };
...
typedef struct {
  // User-requested settings. These fields will always remain constant until
  // changed.
  ...
  bool isCustomSizeRequested; // Reports that the call to
                              // GPUSubsystem::SetFramebufferSize() resulted in
                              // a custom rendering size.
  ...
  size_t framebufferPageSize; // The size of a single framebuffer page, which
                              // includes the native and custom buffers of both
  // displays, measured in bytes.

  // Changed by calling GPUSubsystem::SetColorFormat() or
  // GPUSubsystem::SetFramebufferSize().
  size_t framebufferPageCount; // The number of framebuffer pages that were
                               // requested by the client.
  void *masterFramebufferHead; // Pointer to the head of the master framebuffer
                               // memory block that encompasses all buffers.

  // Changed by calling GPUEngineBase::SetEnableState().
  bool isDisplayEnabled[2]; // Reports that a particular display has been
                            // enabled or disabled by the user.

  // Frame render state information. These fields will change per frame,
  // depending on how each display was rendered.
  u8 bufferIndex; // Index of a specific framebuffer page for the GPU emulation
                  // to write data into.
  // Indexing starts at 0, and must be less than framebufferPageCount.
  // A specific index can be chosen at the DidFrameBegin event.
  size_t sequenceNumber; // A unique number assigned to each frame that
                         // increments for each DidFrameEnd event. Never resets.
  ...
  // If GPUSubsystem::GetWillAutoResolveToCustomBuffer() would return true, or
  // if GPUSubsystem::ResolveDisplayToCustomFramebuffer() is called, then this
  // buffer is used as the target buffer for resolving any native-sized renders.
  ...
  size_t renderedWidth[2];  // The display rendered at this width, measured in
                            // pixels.
  size_t renderedHeight[2]; // The display rendered at this height, measured in
                            // pixels.
  void *renderedBuffer[2];  // The display rendered to this buffer.

  GPUEngineID engineID[2]; // ID of the engine sourcing the display.

  bool didPerformCustomRender[2]; // Reports that the display actually rendered
                                  // at a custom size for this frame.
  //    true  - The display performed a custom-sized render.
  //    false - The display performed a native-sized render.

  bool masterBrightnessDiffersPerLine[2]; // Reports if the master brightness
                                          // values may differ per scanline. If
                                          // true, then it will
  // need to be applied on a per-scanline basis. Otherwise, it can be applied to
  // the entire framebuffer. For most NDS games, this field will be false.
  u8 masterBrightnessMode[2]
                         [GPU_FRAMEBUFFER_NATIVE_HEIGHT]; // The master
                                                          // brightness mode of
                                                          // each display line.
  u8 masterBrightnessIntensity
      [2][GPU_FRAMEBUFFER_NATIVE_HEIGHT]; // The master brightness intensity of
                                          // each display line.
  ...
  // Postprocessing information. These fields report the status of each
  // postprocessing step. Typically, these fields should be modified whenever
  // GPUSubsystem::PostprocessDisplay() is called.
  bool needConvertColorFormat[2];    // Reports if the display still needs to
                                     // convert its color format from RGB666 to
                                     // RGB888.
  bool needApplyMasterBrightness[2]; // Reports if the display still needs to
...
protected:
  ...
  CACHE_ALIGN u8
      _sprAlpha[GPU_FRAMEBUFFER_NATIVE_HEIGHT][GPU_FRAMEBUFFER_NATIVE_WIDTH];
  CACHE_ALIGN u8
      _sprType[GPU_FRAMEBUFFER_NATIVE_HEIGHT][GPU_FRAMEBUFFER_NATIVE_WIDTH];
  CACHE_ALIGN u8
      _sprPrio[GPU_FRAMEBUFFER_NATIVE_HEIGHT][GPU_FRAMEBUFFER_NATIVE_WIDTH];
  CACHE_ALIGN u8
      _sprWin[GPU_FRAMEBUFFER_NATIVE_HEIGHT][GPU_FRAMEBUFFER_NATIVE_WIDTH];
  ...
  CACHE_ALIGN FragmentColor
      _internalRenderLineTargetNative[GPU_FRAMEBUFFER_NATIVE_WIDTH];
  CACHE_ALIGN u8 _renderLineLayerIDNative[GPU_FRAMEBUFFER_NATIVE_HEIGHT]
  ...
  FragmentColor
      _asyncClearBackdropColor32; // Do not modify this variable directly.
  bool _asyncClearUseInternalCustomBuffer; // Do not modify this variable
                                           // directly.
  ...
  void _TransitionLineNativeToCustom(GPUEngineCompositorInfo &compInfo);

  void _MosaicSpriteLinePixel(GPUEngineCompositorInfo &compInfo, const size_t x,
                              u16 *__restrict dst, u8 *__restrict dst_alpha,
                              u8 *__restrict typeTab, u8 *__restrict prioTab);
  void _MosaicSpriteLine(GPUEngineCompositorInfo &compInfo, u16 *__restrict dst,
                         u8 *__restrict dst_alpha, u8 *__restrict typeTab,
  ...
  void _RenderPixelIterate_Final(GPUEngineCompositorInfo &compInfo,
                                 const IOREG_BGnParameter &param, const u32 map,
                                 const u32 tile, const u16 *__restrict pal);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool MOSAIC, bool WILLPERFORMWINDOWTEST, bool WILLDEFERCOMPOSITING,
            PixelLookupFunc GetPixelFunc, bool WRAP>
  void _RenderPixelIterate_ApplyWrap(GPUEngineCompositorInfo &compInfo,
  ...
  void _RenderPixelIterate(GPUEngineCompositorInfo &compInfo,
                           const IOREG_BGnParameter &param, const u32 map,
                           const u32 tile, const u16 *__restrict pal);

  TILEENTRY _GetTileEntry(const u32 tileMapAddress, const u16 xOffset,
                          const u16 layerWidthMask);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool MOSAIC, bool WILLPERFORMWINDOWTEST>
  FORCEINLINE void _CompositePixelImmediate(GPUEngineCompositorInfo &compInfo,
                                            const size_t srcX, u16 srcColor16,
                                            bool opaque);
  template <bool MOSAIC>
  void _PrecompositeNativeToCustomLineBG(GPUEngineCompositorInfo &compInfo);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool WILLPERFORMWINDOWTEST>
  void
  _CompositeNativeLineOBJ(GPUEngineCompositorInfo &compInfo,
                          const u16 *__restrict srcColorNative16,
                          const FragmentColor *__restrict srcColorNative32);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            GPULayerType LAYERTYPE, bool WILLPERFORMWINDOWTEST>
  void _CompositeLineDeferred(GPUEngineCompositorInfo &compInfo,
                              const u16 *__restrict srcColorCustom16,
                              const u8 *__restrict srcIndexCustom);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            GPULayerType LAYERTYPE, bool WILLPERFORMWINDOWTEST>
  void _CompositeVRAMLineDeferred(GPUEngineCompositorInfo &compInfo,
                                  const void *__restrict vramColorPtr);

  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool MOSAIC, bool WILLPERFORMWINDOWTEST, bool WILLDEFERCOMPOSITING>
  void _RenderLine_BGText(GPUEngineCompositorInfo &compInfo, const u16 XBG,
                          const u16 YBG);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool MOSAIC, bool WILLPERFORMWINDOWTEST, bool WILLDEFERCOMPOSITING>
  void _RenderLine_BGAffine(GPUEngineCompositorInfo &compInfo,
                            const IOREG_BGnParameter &param);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool MOSAIC, bool WILLPERFORMWINDOWTEST, bool WILLDEFERCOMPOSITING>
  void _RenderLine_BGExtended(GPUEngineCompositorInfo &compInfo,
                              const IOREG_BGnParameter &param,
                              bool &outUseCustomVRAM);

  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool MOSAIC, bool WILLPERFORMWINDOWTEST, bool WILLDEFERCOMPOSITING>
  void _LineText(GPUEngineCompositorInfo &compInfo);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool MOSAIC, bool WILLPERFORMWINDOWTEST, bool WILLDEFERCOMPOSITING>
  void _LineRot(GPUEngineCompositorInfo &compInfo);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool MOSAIC, bool WILLPERFORMWINDOWTEST, bool WILLDEFERCOMPOSITING>
  void _LineExtRot(GPUEngineCompositorInfo &compInfo, bool &outUseCustomVRAM);
  ...
  void _RenderLine_Layers(GPUEngineCompositorInfo &compInfo);
  ...
  bool _IsWindowInsideVerticalRange(GPUEngineCompositorInfo &compInfo);
  void _PerformWindowTesting(GPUEngineCompositorInfo &compInfo);

  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool MOSAIC, bool WILLPERFORMWINDOWTEST, bool WILLDEFERCOMPOSITING>
  FORCEINLINE void _RenderLine_LayerBG_Final(GPUEngineCompositorInfo &compInfo);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool MOSAIC, bool WILLPERFORMWINDOWTEST>
  FORCEINLINE void
  _RenderLine_LayerBG_ApplyMosaic(GPUEngineCompositorInfo &compInfo);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool WILLPERFORMWINDOWTEST>
  ...
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool WILLPERFORMWINDOWTEST>
  void _RenderLine_LayerOBJ(GPUEngineCompositorInfo &compInfo,
                            itemsForPriority_t *__restrict item);

  template <NDSColorFormat OUTPUTFORMAT, bool ISDEBUGRENDER>
  FORCEINLINE void _PixelCopy(GPUEngineCompositorInfo &compInfo,
                              const u16 srcColor16);
  template <NDSColorFormat OUTPUTFORMAT, bool ISDEBUGRENDER>
  FORCEINLINE void _PixelCopy(GPUEngineCompositorInfo &compInfo,
                              const FragmentColor srcColor32);
  template <NDSColorFormat OUTPUTFORMAT>
  FORCEINLINE void _PixelBrightnessUp(GPUEngineCompositorInfo &compInfo,
                                      const u16 srcColor16);
  template <NDSColorFormat OUTPUTFORMAT>
  FORCEINLINE void _PixelBrightnessUp(GPUEngineCompositorInfo &compInfo,
                                      const FragmentColor srcColor32);
  template <NDSColorFormat OUTPUTFORMAT>
  FORCEINLINE void _PixelBrightnessDown(GPUEngineCompositorInfo &compInfo,
                                        const u16 srcColor16);
  template <NDSColorFormat OUTPUTFORMAT>
  FORCEINLINE void _PixelBrightnessDown(GPUEngineCompositorInfo &compInfo,
                                        const FragmentColor srcColor32);
  ...
  FORCEINLINE void
  _PixelUnknownEffect(GPUEngineCompositorInfo &compInfo, const u16 srcColor16,
                      const bool enableColorEffect, const u8 spriteAlpha,
                      const OBJMode spriteMode);
  template <NDSColorFormat OUTPUTFORMAT, GPULayerType LAYERTYPE>
  FORCEINLINE void _PixelUnknownEffect(GPUEngineCompositorInfo &compInfo,
                                       const FragmentColor srcColor32,
                                       const bool enableColorEffect,
                                       const u8 spriteAlpha,
                                       const OBJMode spriteMode);

  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            GPULayerType LAYERTYPE>
  FORCEINLINE void _PixelComposite(GPUEngineCompositorInfo &compInfo,
                                   const u16 srcColor16,
                                   const bool enableColorEffect,
                                   const u8 spriteAlpha, const u8 spriteMode);
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            GPULayerType LAYERTYPE>
  FORCEINLINE void _PixelComposite(GPUEngineCompositorInfo &compInfo,
  ...
  FORCEINLINE u16 _ColorEffectBlend(const u16 colA, const u16 colB,
                                    const u16 blendEVA, const u16 blendEVB);
  FORCEINLINE u16 _ColorEffectBlend(const u16 colA, const u16 colB,
                                    const TBlendTable *blendTable);
  template <NDSColorFormat COLORFORMAT>
  FORCEINLINE FragmentColor _ColorEffectBlend(const FragmentColor colA,
                                              const FragmentColor colB,
                                              const u16 blendEVA,
                                              const u16 blendEVB);

  FORCEINLINE u16 _ColorEffectBlend3D(const FragmentColor colA, const u16 colB);
  template <NDSColorFormat COLORFORMATB>
  FORCEINLINE FragmentColor _ColorEffectBlend3D(const FragmentColor colA,
                                                const FragmentColor colB);
  ...
  _ColorEffectIncreaseBrightness(const FragmentColor col, const u16 blendEVY);

  FORCEINLINE u16 _ColorEffectDecreaseBrightness(const u16 col,
                                                 const u16 blendEVY);
  FORCEINLINE FragmentColor
  _ColorEffectDecreaseBrightness(const FragmentColor col, const u16 blendEVY);

#ifdef ENABLE_SSE2
  template <NDSColorFormat COLORFORMAT, bool USECONSTANTBLENDVALUESHINT>
  FORCEINLINE __m128i _ColorEffectBlend(const __m128i &colA,
                                        const __m128i &colB,
                                        const __m128i &blendEVA,
                                        const __m128i &blendEVB);
  template <NDSColorFormat COLORFORMATB>
  FORCEINLINE __m128i _ColorEffectBlend3D(const __m128i &colA_Lo,
                                          const __m128i &colA_Hi,
                                          const __m128i &colB);
  template <NDSColorFormat COLORFORMAT>
  FORCEINLINE __m128i _ColorEffectIncreaseBrightness(const __m128i &col,
                                                     const __m128i &blendEVY);
  template <NDSColorFormat COLORFORMAT>
  FORCEINLINE __m128i _ColorEffectDecreaseBrightness(const __m128i &col,
                                                     const __m128i &blendEVY);
  template <bool WILLDEFERCOMPOSITING>
  FORCEINLINE void _RenderPixel_CheckWindows16_SSE2(
      GPUEngineCompositorInfo &compInfo, const size_t dstX,
      __m128i &didPassWindowTest, __m128i &enableColorEffect) const;

  template <NDSColorFormat OUTPUTFORMAT, bool ISDEBUGRENDER>
  FORCEINLINE void
  _PixelCopy16_SSE2(GPUEngineCompositorInfo &compInfo, const __m128i &src3,
                    const __m128i &src2, const __m128i &src1,
                    const __m128i &src0, __m128i &dst3, __m128i &dst2,
                    __m128i &dst1, __m128i &dst0, __m128i &dstLayerID);
  template <NDSColorFormat OUTPUTFORMAT, bool ISDEBUGRENDER>
  FORCEINLINE void
  _PixelCopyWithMask16_SSE2(GPUEngineCompositorInfo &compInfo,
                            const __m128i &passMask8, const __m128i &src3,
                            const __m128i &src2, const __m128i &src1,
                            const __m128i &src0, __m128i &dst3, __m128i &dst2,
                            __m128i &dst1, __m128i &dst0, __m128i &dstLayerID);
  template <NDSColorFormat OUTPUTFORMAT>
  FORCEINLINE void
  _PixelBrightnessUp16_SSE2(GPUEngineCompositorInfo &compInfo,
                            const __m128i &src3, const __m128i &src2,
                            const __m128i &src1, const __m128i &src0,
                            __m128i &dst3, __m128i &dst2, __m128i &dst1,
                            __m128i &dst0, __m128i &dstLayerID);
  template <NDSColorFormat OUTPUTFORMAT>
  FORCEINLINE void _PixelBrightnessUpWithMask16_SSE2(
  ...
  template <NDSColorFormat OUTPUTFORMAT>
  FORCEINLINE void
  _PixelBrightnessDown16_SSE2(GPUEngineCompositorInfo &compInfo,
                              const __m128i &src3, const __m128i &src2,
                              const __m128i &src1, const __m128i &src0,
                              __m128i &dst3, __m128i &dst2, __m128i &dst1,
                              __m128i &dst0, __m128i &dstLayerID);
  template <NDSColorFormat OUTPUTFORMAT>
  FORCEINLINE void _PixelBrightnessDownWithMask16_SSE2(
      GPUEngineCompositorInfo &compInfo, const __m128i &passMask8,
      const __m128i &src3, const __m128i &src2, const __m128i &src1,
      const __m128i &src0, __m128i &dst3, __m128i &dst2, __m128i &dst1,
      __m128i &dst0, __m128i &dstLayerID);
  ...
  template <NDSColorFormat OUTPUTFORMAT, GPULayerType LAYERTYPE>
  FORCEINLINE void _PixelUnknownEffectWithMask16_SSE2(
      GPUEngineCompositorInfo &compInfo, const __m128i &passMask8,
      const __m128i &src3, const __m128i &src2, const __m128i &src1,
  ...
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            GPULayerType LAYERTYPE, bool WILLPERFORMWINDOWTEST>
  FORCEINLINE void _PixelComposite16_SSE2(
      ...
      const u8 *__restrict sprAlphaPtr, const u8 *__restrict sprModePtr);
#endif

  template <bool ISDEBUGRENDER, bool ISOBJMODEBITMAP>
  FORCEINLINE void
  _RenderSpriteUpdatePixel(GPUEngineCompositorInfo &compInfo, size_t frameX,
                           const u16 *__restrict srcPalette, const u8 palIndex,
                           const OBJMode objMode, const u8 prio,
                           const u8 spriteNum, u16 *__restrict dst,
                           u8 *__restrict dst_alpha, u8 *__restrict typeTab,
                           u8 *__restrict prioTab);
  template <bool ISDEBUGRENDER>
  void _RenderSpriteBMP(GPUEngineCompositorInfo &compInfo, const u32 objAddress,
                        const size_t length, size_t frameX, size_t spriteX,
                        const s32 readXStep, const u8 spriteAlpha,
                        const OBJMode objMode, const u8 prio,
                        const u8 spriteNum, u16 *__restrict dst,
                        u8 *__restrict dst_alpha, u8 *__restrict typeTab,
                        u8 *__restrict prioTab);
  template <bool ISDEBUGRENDER>
  void
  _RenderSprite256(GPUEngineCompositorInfo &compInfo, const u32 objAddress,
                   const size_t length, size_t frameX, size_t spriteX,
                   const s32 readXStep, const u16 *__restrict palColorBuffer,
                   const OBJMode objMode, const u8 prio, const u8 spriteNum,
                   u16 *__restrict dst, u8 *__restrict dst_alpha,
                   u8 *__restrict typeTab, u8 *__restrict prioTab);
  template <bool ISDEBUGRENDER>
  void _RenderSprite16(GPUEngineCompositorInfo &compInfo, const u32 objAddress,
                       const size_t length, size_t frameX, size_t spriteX,
                       const s32 readXStep,
                       ...
                       u16 *__restrict dst, u8 *__restrict dst_alpha,
                       u8 *__restrict typeTab, u8 *__restrict prioTab);
  void _RenderSpriteWin(const u8 *src, const bool col256, const size_t lg,
                        size_t sprX, size_t x, const s32 xdir);
  bool _ComputeSpriteVars(GPUEngineCompositorInfo &compInfo,
                          const OAMAttributes &spriteInfo, SpriteSize &sprSize,
                          s32 &sprX, s32 &sprY, s32 &x, s32 &y, s32 &lg,
                          s32 &xdir);

  u32 _SpriteAddressBMP(GPUEngineCompositorInfo &compInfo,
                        const OAMAttributes &spriteInfo,
                        const SpriteSize sprSize, const s32 y);

  template <bool ISDEBUGRENDER>
  void _SpriteRender(GPUEngineCompositorInfo &compInfo, u16 *__restrict dst,
                     u8 *__restrict dst_alpha, u8 *__restrict typeTab,
                     u8 *__restrict prioTab);
  template <SpriteRenderMode MODE, bool ISDEBUGRENDER>
  void _SpriteRenderPerform(GPUEngineCompositorInfo &compInfo,
                            u16 *__restrict dst, u8 *__restrict dst_alpha,
                            u8 *__restrict typeTab, u8 *__restrict prioTab);
...
public:
  ...
  void GetMasterBrightnessAtLineZero(GPUMasterBrightMode &outMode,
                                     u8 &outIntensity);
  ...
  template <NDSColorFormat OUTPUTFORMAT>
  void RenderLineClearAsyncStart(bool willClearInternalCustomBuffer,
                                 s32 startLineIndex, u16 clearColor16,
  ...
  template <NDSColorFormat OUTPUTFORMAT>
  void ApplyMasterBrightness(const NDSDisplayInfo &displayInfo);
  template <NDSColorFormat OUTPUTFORMAT, bool ISFULLINTENSITYHINT>
  void ApplyMasterBrightness(void *dst, const size_t pixCount,
...
protected:
  ...
  CACHE_ALIGN u16 _VRAMNativeBlockCaptureCopy[GPU_FRAMEBUFFER_NATIVE_WIDTH *
                                              GPU_VRAM_BLOCK_LINES * 4];
  ...
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool MOSAIC, bool WILLPERFORMWINDOWTEST, bool WILLDEFERCOMPOSITING>
  void _LineLarge8bpp(GPUEngineCompositorInfo &compInfo);

  template <NDSColorFormat OUTPUTFORMAT, size_t CAPTURELENGTH>
  void _RenderLine_DisplayCaptureCustom(
      ...
      const void *srcAPtr, const void *srcBPtr,
      void *dstCustomPtr); // Do not use restrict pointers, since srcB and dst
                           // can be the same

  template <NDSColorFormat OUTPUTFORMAT, size_t CAPTURELENGTH>
  void _RenderLine_DisplayCapture(const GPUEngineCompositorInfo &compInfo);
  ...
  void _RenderLine_DispCapture_Copy(
      const GPUEngineLineInfo &lineInfo, const void *src, void *dst,
      const size_t captureLengthExt); // Do not use restrict pointers, since src
                                      // and dst can be the same

  u16 _RenderLine_DispCapture_BlendFunc(const u16 srcA, const u16 srcB,
                                        const u8 blendEVA, const u8 blendEVB);
  template <NDSColorFormat COLORFORMAT>
  FragmentColor _RenderLine_DispCapture_BlendFunc(const FragmentColor srcA,
                                                  const FragmentColor srcB,
                                                  const u8 blendEVA,
                                                  const u8 blendEVB);

#ifdef ENABLE_SSE2
  template <NDSColorFormat COLORFORMAT>
  __m128i _RenderLine_DispCapture_BlendFunc_SSE2(const __m128i &srcA,
                                                 const __m128i &srcB,
                                                 const __m128i &blendEVA,
                                                 const __m128i &blendEVB);
#endif
  ...
  void _RenderLine_DispCapture_BlendToCustomDstBuffer(
      ...
      const u8 blendEVB,
      const size_t length); // Do not use restrict pointers, since srcB and dst
                            // can be the same

  template <NDSColorFormat OUTPUTFORMAT, size_t CAPTURELENGTH,
            bool CAPTUREFROMNATIVESRCA, bool CAPTUREFROMNATIVESRCB,
            bool CAPTURETONATIVEDST>
  void _RenderLine_DispCapture_Blend(
      const GPUEngineLineInfo &lineInfo, const void *srcA, const void *srcB,
      void *dst,
      const size_t captureLengthExt); // Do not use restrict pointers, since
                                      // srcB and dst can be the same

  template <NDSColorFormat OUTPUTFORMAT>
  void _HandleDisplayModeVRAM(const GPUEngineLineInfo &lineInfo);
  template <NDSColorFormat OUTPUTFORMAT>
...
public:
  ...
  template <GPUCompositorMode COMPOSITORMODE, NDSColorFormat OUTPUTFORMAT,
            bool WILLPERFORMWINDOWTEST>
  void RenderLine_Layer3D(GPUEngineCompositorInfo &compInfo);
};
...
public:
  virtual void DidFrameBegin(const size_t line, const bool isFrameSkipRequested,
                             const size_t pageCount,
                             u8 &selectedBufferIndexInOut) = 0;
  virtual void DidFrameEnd(bool isFrameSkipped,
                           const NDSDisplayInfo &latestDisplayInfo) = 0;
...
public:
  ...
  virtual void DidFrameEnd(bool isFrameSkipped,
                           const NDSDisplayInfo &latestDisplayInfo){};
...
private:
  ...
  u32 _videoFrameIndex;    // Increments whenever a video frame is completed.
                           // Resets every 60 video frames.
  u32 _render3DFrameCount; // The current 3D rendering frame count, saved to
                           // this variable once every 60 video frames.
  ...
  u8 *_DownscaleAndConvertForSavestate(const NDSDisplayID displayID,
                                       void *__restrict intermediateBuffer);
...
public:
  ...
  const NDSDisplayInfo &
  GetDisplayInfo(); // Frontends need to call this whenever they need to read
                    // the video buffers from the emulator core
  ...
  template <NDSColorFormat COLORFORMAT>
  void *GetCustomVRAMAddressUsingMappedAddress(const u32 addr,
                                               const size_t offset);
  ...
  // By default, the displays will automatically perform certain postprocessing
  // steps on the CPU before the DidFrameEnd event.
  //
  // To turn off this behavior, call SetWillPostprocessDisplays() and pass a
  // value of "false". This can be useful if the client wants to perform these
  // postprocessing steps itself, for example, if a client performs them on
  // another thread or on the GPU.
  //
  // If automatic postprocessing is turned off, clients can still manually
  // perform the postprocessing steps on the CPU by calling
  // PostprocessDisplay().
  //
  ...
  void PostprocessDisplay(const NDSDisplayID displayID,
                          NDSDisplayInfo &mutableInfo);

  // Normally, the GPUs will automatically resolve their native buffers to the
  // master custom framebuffer at the end of V-blank so that all rendered
  // graphics are contained within a single common buffer. This is necessary for
  // when someone wants to read the NDS framebuffers, but the reader can only
  // read a single buffer at a time. Certain functions, such as taking
  // screenshots, as well as many frontends running the NDS video displays,
  // require that they read from only a single buffer.
  //
  // However, if SetWillAutoResolveToCustomBuffer() is passed "false", then the
  // frontend becomes responsible for calling GetDisplayInfo() and reading the
  // native and custom buffers properly for each display. If a single buffer is
  // still needed for certain cases, then the frontend must manually call
  // ResolveDisplayToCustomFramebuffer() for each display before reading the
  ...
  void ResolveDisplayToCustomFramebuffer(const NDSDisplayID displayID,
                                         NDSDisplayInfo &mutableInfo);
...
public:
  ...
  const NDSDisplayInfo &
  GetFetchDisplayInfoForBufferIndex(const u8 bufferIndex) const;
