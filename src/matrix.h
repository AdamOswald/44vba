...
template <MatrixMode MODE> struct MatrixStack {
  static const size_t size =
      ((MODE == MATRIXMODE_PROJECTION) || (MODE == MATRIXMODE_TEXTURE)) ? 1
                                                                        : 32;
...
// this isnt as fast as it could be if we used a visual c++ intrinsic, but those
// appear not to be universally available
...
#ifdef ENABLE_SSE2
  return _mm_cvtss_si32(_mm_add_ss(_mm_set_ss(-0.5f),
                                   _mm_add_ss(_mm_set_ss(f), _mm_set_ss(f)))) >>
         1;
#else
  return (s32)floorf(f);
#endif
}
FORCEINLINE s32 s32floor(double d) { return s32floor((float)d); }
...
static void memset_u16_fast(void *dst, const u16 val) {
  ...
  const v512u16 val_vec512 = _mm512_set1_epi16(val);
  MACRODO_N(ELEMENTCOUNT / (sizeof(v512u16) / sizeof(u16)),
            _mm512_store_si512(dst_vec512 + (X), val_vec512));
}
...
static void memset_u32_fast(void *dst, const u32 val) {
  ...
  const v512u32 val_vec512 = _mm512_set1_epi32(val);
  MACRODO_N(ELEMENTCOUNT / (sizeof(v512u32) / sizeof(u32)),
            _mm512_store_si512(dst_vec512 + (X), val_vec512));
}
...
static void memset_u16_fast(void *dst, const u16 val) {
  ...
  const v256u16 val_vec256 = _mm256_set1_epi16(val);
  MACRODO_N(ELEMENTCOUNT / (sizeof(v256u16) / sizeof(u16)),
            _mm256_store_si256(dst_vec256 + (X), val_vec256));
}
...
static void memset_u32_fast(void *dst, const u32 val) {
  ...
  const v256u32 val_vec256 = _mm256_set1_epi32(val);
  MACRODO_N(ELEMENTCOUNT / (sizeof(v256u32) / sizeof(u32)),
            _mm256_store_si256(dst_vec256 + (X), val_vec256));
}
...
static void memset_u16_fast(void *dst, const u16 val) {
  ...
  const v128u16 val_vec128 = _mm_set1_epi16(val);
  MACRODO_N(ELEMENTCOUNT / (sizeof(v128u16) / sizeof(u16)),
            _mm_store_si128(dst_vec128 + (X), val_vec128));
}
...
static void memset_u32_fast(void *dst, const u32 val) {
  ...
  const v128u32 val_vec128 = _mm_set1_epi32(val);
  MACRODO_N(ELEMENTCOUNT / (sizeof(v128u32) / sizeof(u32)),
            _mm_store_si128(dst_vec128 + (X), val_vec128));
}
...
static void memset_u16_fast(void *dst, const u16 val) {
  ...
  const v128u16 val_vec128 = (v128u16){val, val, val, val, val, val, val, val};
  MACRODO_N(ELEMENTCOUNT / (sizeof(v128u16) / sizeof(u16)),
            vec_st(val_vec128, 0, dst_vec128 + (X)));
}
...
static void memset_u16(void *dst, const u16 val, const size_t elementCount) {
  ...
  u64 *dst_u64 = (u64 *)dst;
  const u64 val_u64 =
      ((u64)val << 48) | ((u64)val << 32) | ((u64)val << 16) | (u64)val;
...
static void memset_u16_fast(void *dst, const u16 val) {
  ...
  const u64 val_u64 =
      ((u64)val << 48) | ((u64)val << 32) | ((u64)val << 16) | (u64)val;
  MACRODO_N(ELEMENTCOUNT / (sizeof(val_u64) / sizeof(val)),
            (dst_u64[(X)] = val_u64));
#else
  for (size_t i = 0; i < ELEMENTCOUNT; i++)
    ((u16 *)dst)[i] = val;
