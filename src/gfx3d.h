...
extern CACHE_ALIGN MatrixStack<MATRIXMODE_POSITION_VECTOR>
...
typedef union {
  ...
  struct {
    ...
    u8 TranslucentDepthWrite_Enable : 1; //    11: Translucent depth write;
                                         //    0=Keep 1=Replace
    u8 FarPlaneIntersect_Enable : 1; //    12: Far-plane intersecting polygons;
                                     //    0=Hide, 1=Render/clipped
...
typedef union {
  ...
  struct {
    ...
    u16 MirroredRepeatS_Enable : 1; //    18: Mirrored repeat for S-coordinate,
                                    //    interacts with bit 16; 0=Disable
                                    //    1=Enable
    u16 MirroredRepeatT_Enable : 1; //    19: Mirrored repeat for T-coordinate,
                                    //    interacts with bit 17; 0=Disable
                                    //    1=Enable
    u16 SizeShiftS : 3;   // 20-22: Texel size shift for S-coordinate; 0...7,
                          // where the actual texel size is (8 << N)
    u16 SizeShiftT : 3;   // 23-25: Texel size shift for T-coordinate; 0...7,
                          // where the actual texel size is (8 << N)
    u16 PackedFormat : 3; // 26-28: Packed texture format;
    //        0=None
    //        1=A3I5, 5-bit indexed color (32-color palette) with 3-bit alpha
    //        (0...7, where 0=Fully Transparent and 7=Opaque) 2=I2, 2-bit
    //        indexed color (4-color palette) 3=I4, 4-bit indexed color
    //        (16-color palette) 4=I8, 8-bit indexed color (256-color palette)
    //        5=4x4-texel compressed
    //        6=A5I3, 3-bit indexed color (8-color palette) with 5-bit alpha
    //        (0...31, where 0=Fully Transparent and 31=Opaque) 7=Direct 16-bit
    //        color
    u16 KeyColor0_Enable : 1;      //    29: Use palette color 0 as transparent;
                                   //    0=Displayed 1=Transparent
    u16 TexCoordTransformMode : 2; // 30-31: Texture coordinate transformation
                                   // mode;
                                   //        0=No transformation
                                   //        1=TexCoord source
                                   //        2=Normal source
...
#ifndef MSB_FIRST
    u16 : 16;
    u16 TextureWrapMode : 4; // 16-19: Texture wrap mode for repeat and mirrored
                             // repeat
    u16 : 12;
#else
    u16 : 12;
...
struct POLY {
  ...
  void setVertIndexes(int a, int b, int c, int d = -1) {
    ...
    if (d != -1) {
      vertIndexes[3] = d;
      type = POLYGON_TYPE_QUAD;
    } else
      type = POLYGON_TYPE_TRIANGLE;
  }

  bool isWireframe() const { return (this->attribute.Alpha == 0); }

  bool isOpaque() const { return (this->attribute.Alpha == 31); }
  ...
  bool isTranslucent() const {
    ...
    const NDSTextureFormat texFormat =
        (NDSTextureFormat)this->texParam.PackedFormat;
    ...
    if ((texFormat == TEXMODE_A3I5 || texFormat == TEXMODE_A5I3) &&
        (mode != POLYGON_MODE_DECAL && mode != POLYGON_MODE_SHADOW)) {
      return true;
    }
...
// This struct is padded in such a way so that each component can be accessed
// with a 16-byte alignment.
struct VERT {
  union {
    float coord[4];
    struct {
  ...
  union {
    ...
    struct {
      float rf, gf, bf,
          af; // The alpha value is unused and only exists for padding purposes.
    };
  };
  ...
  union {
    ...
    struct {
      u8 r, g, b,
          a; // The alpha value is unused and only exists for padding purposes.
    };
  };
...
enum ClipperMode {
  ClipperMode_DetermineClipOnly =
      0, // Retains only the pointer to the original polygon info. All other
         // information in CPoly is considered undefined.
  ClipperMode_Full = 1, // Retains all of the modified polygon's info in CPoly,
                        // including the clipped vertex info.
  ClipperMode_FullColorInterpolate =
      2 // Same as ClipperMode_Full, but the vertex color attribute is better
        // interpolated.
};
...
protected:
  ...
  CPoly *_clippedPolyList; // The output of clipping operations goes into here.
...
public:
  ...
  bool ClipPoly(const u16 polyIndex, const POLY &poly,
                const VERT **verts); // the entry point for poly clipping
};

// used to communicate state to the renderer
struct GFX3D_State {
  GFX3D_State()
      : enableTexturing(true), enableAlphaTest(true), enableAlphaBlending(true),
        enableAntialiasing(false), enableEdgeMarking(false),
        enableClearImage(false), enableFog(false), enableFogAlphaOnly(false),
        shading(PolygonShadingMode_Toon), alphaTestRef(0),
        activeFlushCommand(0), pendingFlushCommand(0),
        clearDepth(DS_DEPTH15TO24(0x7FFF)), clearColor(0), fogColor(0),
        fogOffset(0), fogShift(0), invalidateToon(true) {
  ...
  BOOL enableTexturing, enableAlphaTest, enableAlphaBlending,
      enableAntialiasing, enableEdgeMarking, enableClearImage, enableFog,
      enableFogAlphaOnly;
...
struct GFX3D {
  GFX3D() : polylist(NULL), vertList(NULL), render3DFrameCount(0) {}
  ...
  size_t vertListCount;
  u32 render3DFrameCount; // Increments when gfx3d_doFlush() is called. Resets
                          // every 60 video frames.
};
extern GFX3D gfx3d;
