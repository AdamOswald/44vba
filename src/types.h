...
// along with this program/software. If not, see <http://www.gnu.org/licenses/>,
...
//
// For our case, Tier-0 will be the baseline AVX-512 tier that includes the
// basic Foundation and Conflict Detection extensions, which should be supported
// on all AVX-512 CPUs. Higher tiers include more extensions, where each higher
// tier also assumes support for all lower tiers.
//
// For typical use cases in DeSmuME, the most practical AVX-512 tier will be
// Tier-1.
#if defined(__AVX512F__) && defined(__AVX512CD__)
#define ENABLE_AVX512_0
#endif

#if defined(ENABLE_AVX512_0) && defined(__AVX512BW__) &&                       \
    defined(__AVX512DQ__) && !defined(FORCE_AVX512_0)
#define ENABLE_AVX512_1
#endif

#if defined(ENABLE_AVX512_1) && defined(__AVX512IFMA__) &&                     \
    defined(__AVX512VBMI__)
#define ENABLE_AVX512_2
#endif

#if defined(ENABLE_AVX512_2) && defined(__AVX512VNNI__) &&                     \
...
//------------alignment macros-------------
// dont apply these to types without further testing. it only works portably
// here on declarations of variables cant we find a pattern other people use
// more successfully?
#if _MSC_VER >= 9999 // Was 1900. The way we use DS_ALIGN doesn't jive with how
                     // alignas() wants itself to be used, so just use
                     // __declspec(align(X)) for now to avoid problems.
...
// #define atomic_add_32(V,M)
// InterlockedAddNoFence((volatile LONG *)(V),(LONG)(M))			//
// Requires Windows 8
...
// #define atomic_inc_32(V)
// InterlockedIncrementNoFence((volatile LONG *)(V))				//
// Requires Windows 8
...
// #define atomic_dec_32(V)
// InterlockedDecrementNoFence((volatile LONG *)(V))				//
// Requires Windows 8
...
// #define atomic_or_32(V,M)
// InterlockedOrNoFence((volatile LONG *)(V),(LONG)(M))			//
// Requires Windows 8
#define atomic_or_32(V, M) _InterlockedOr((volatile LONG *)(V), (LONG)(M))
#define atomic_or_barrier32(V, M)                                              \
  _InterlockedOr((volatile LONG *)(V), (LONG)(M))
// #define atomic_and_32(V,M)
// InterlockedAndNoFence((volatile LONG *)(V),(LONG)(M))			//
// Requires Windows 8
#define atomic_and_32(V, M) _InterlockedAnd((volatile LONG *)(V), (LONG)(M))
#define atomic_and_barrier32(V, M)                                             \
  _InterlockedAnd((volatile LONG *)(V), (LONG)(M))
// #define atomic_xor_32(V,M)
// InterlockedXorNoFence((volatile LONG *)(V),(LONG)(M))			//
// Requires Windows 8
#define atomic_xor_32(V, M) _InterlockedXor((volatile LONG *)(V), (LONG)(M))
#define atomic_xor_barrier32(V, M)                                             \
  _InterlockedXor((volatile LONG *)(V), (LONG)(M))

inline bool atomic_test_and_set_32(volatile s32 *V, s32 M) {
  return (_interlockedbittestandset((volatile LONG *)V, (LONG)M)) ? true
                                                                  : false;
}
...
inline bool atomic_test_and_clear_32(volatile s32 *V, s32 M) {
  return (_interlockedbittestandreset((volatile LONG *)V, (LONG)M)) ? true
                                                                    : false;
}
...
#define atomic_or_barrier32(V, M)                                              \
  OSAtomicOr32Barrier((M), (volatile uint32_t *)(V))
#define atomic_and_32(V, M) OSAtomicAnd32((M), (volatile uint32_t *)(V))
#define atomic_and_barrier32(V, M)                                             \
  OSAtomicAnd32Barrier((M), (volatile uint32_t *)(V))
#define atomic_xor_32(V, M) OSAtomicXor32((M), (volatile uint32_t *)(V))
#define atomic_xor_barrier32(V, M)                                             \
  OSAtomicXor32Barrier((M), (volatile uint32_t *)(V))
...
#define atomic_test_and_clear_barrier32(V, M)                                  \
  OSAtomicTestAndClearBarrier((M), (V))

#else // Just use C++11 std::atomic
#include <atomic>

inline s32 atomic_add_32(volatile s32 *V, s32 M) {
  return std::atomic_fetch_add_explicit<s32>(
             (volatile std::atomic<s32> *)V, M,
...
inline s32 atomic_or_32(volatile s32 *V, s32 M) {
  return std::atomic_fetch_or_explicit<s32>(
             (volatile std::atomic<s32> *)V, M,
             std::memory_order::memory_order_relaxed) |
         M;
}
...
inline s32 atomic_and_32(volatile s32 *V, s32 M) {
  return std::atomic_fetch_and_explicit<s32>(
             (volatile std::atomic<s32> *)V, M,
             std::memory_order::memory_order_relaxed) &
         M;
}
...
inline s32 atomic_xor_32(volatile s32 *V, s32 M) {
  return std::atomic_fetch_xor_explicit<s32>(
             (volatile std::atomic<s32> *)V, M,
             std::memory_order::memory_order_relaxed) ^
         M;
}
...
inline bool atomic_test_and_set_32(volatile s32 *V, s32 M) {
  return (std::atomic_fetch_or_explicit<s32>(
              (volatile std::atomic<s32> *)V, (0x80 >> ((M)&0x07)),
              std::memory_order::memory_order_relaxed) &
          (0x80 >> ((M)&0x07)))
             ? true
             : false;
...
inline bool atomic_test_and_clear_32(volatile s32 *V, s32 M) {
  return (std::atomic_fetch_and_explicit<s32>(
              (volatile std::atomic<s32> *)V, ~(s32)(0x80 >> ((M)&0x07)),
              std::memory_order::memory_order_relaxed) &
          (0x80 >> ((M)&0x07)))
             ? true
             : false;
...
#define MACRODO1(TRICK, TODO)                                                  \
  {                                                                            \
    const size_t X = TRICK;                                                    \
    TODO;                                                                      \
  }
#define MACRODO2(X, TODO)                                                      \
  { MACRODO1((X), TODO) MACRODO1(((X) + 1), TODO) }
#define MACRODO4(X, TODO)                                                      \
  { MACRODO2((X), TODO) MACRODO2(((X) + 2), TODO) }
#define MACRODO8(X, TODO)                                                      \
  { MACRODO4((X), TODO) MACRODO4(((X) + 4), TODO) }
#define MACRODO16(X, TODO)                                                     \
  { MACRODO8((X), TODO) MACRODO8(((X) + 8), TODO) }
#define MACRODO32(X, TODO)                                                     \
  { MACRODO16((X), TODO) MACRODO16(((X) + 16), TODO) }
#define MACRODO64(X, TODO)                                                     \
  { MACRODO32((X), TODO) MACRODO32(((X) + 32), TODO) }
#define MACRODO128(X, TODO)                                                    \
  { MACRODO64((X), TODO) MACRODO64(((X) + 64), TODO) }
#define MACRODO256(X, TODO)                                                    \
  { MACRODO128((X), TODO) MACRODO128(((X) + 128), TODO) }
...
#define MACRODO_N(N, TODO)                                                     \
  {                                                                            \
    if ((N)&0x100)                                                             \
      MACRODO256(0, TODO);                                                     \
    if ((N)&0x080)                                                             \
      MACRODO128((N) & (0x100), TODO);                                         \
    if ((N)&0x040)                                                             \
      MACRODO64((N) & (0x100 | 0x080), TODO);                                  \
    if ((N)&0x020)                                                             \
      MACRODO32((N) & (0x100 | 0x080 | 0x040), TODO);                          \
    if ((N)&0x010)                                                             \
      MACRODO16((N) & (0x100 | 0x080 | 0x040 | 0x020), TODO);                  \
    if ((N)&0x008)                                                             \
      MACRODO8((N) & (0x100 | 0x080 | 0x040 | 0x020 | 0x010), TODO);           \
    if ((N)&0x004)                                                             \
      MACRODO4((N) & (0x100 | 0x080 | 0x040 | 0x020 | 0x010 | 0x008), TODO);   \
    if ((N)&0x002)                                                             \
      MACRODO2((N) & (0x100 | 0x080 | 0x040 | 0x020 | 0x010 | 0x008 | 0x004),  \
               TODO);                                                          \
    if ((N)&0x001)                                                             \
      MACRODO1((N) & (0x100 | 0x080 | 0x040 | 0x020 | 0x010 | 0x008 | 0x004 |  \
                      0x002),                                                  \
               TODO);                                                          \
  }
