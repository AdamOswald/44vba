...
#define TOSTRING(X)                                                            \
  _Py_STRINGIZE(X) // Alias _Py_STRINGIZE so that we have a common macro name
#else
#define STRINGIFY(x) #x
#define TOSTRING(x) STRINGIFY(x)
#endif

// TODO - it isn't possible to build a core without a frontend, so really this
// belongs with frontend modules the only stuff that belongs in the core is
// major/minor/build versions which are (in principle) used for versioning
// savestates and movies and such..
#if (defined(HOST_WINDOWS) || defined(DESMUME_COCOA)) &&                       \
    !defined(TARGET_INTERFACE)
#include "scmrev.h"
...
#define DESMUME_CPUEXT_STRING                                                  \
  DESMUME_CPUEXT_PRIMARY_STRING DESMUME_CPUEXT_SECONDARY_STRING
...
#define DESMUME_COMPILER_DETAIL                                                \
  " (LLVM-Clang v" TOSTRING(__clang_major__) "." TOSTRING(                     \
      __clang_minor__) "." TOSTRING(__clang_patchlevel__) ")"
#elif defined(__llvm__)
#define DESMUME_COMPILER " (LLVM)"
#define DESMUME_COMPILER_DETAIL " (LLVM)"
#elif defined(                                                                 \
    __GNUC__) // Always make GCC the last check, since other compilers, such as
              // Clang, may define __GNUC__ internally.
#define DESMUME_COMPILER " (GCC)"

#if defined(__GNUC_PATCHLEVEL__)
#define DESMUME_COMPILER_DETAIL                                                \
  " (GCC v" TOSTRING(__GNUC__) "." TOSTRING(__GNUC_MINOR__) "." TOSTRING(      \
      __GNUC_PATCHLEVEL__) ")"
#else
#define DESMUME_COMPILER_DETAIL                                                \
  " (GCC v" TOSTRING(__GNUC__) "." TOSTRING(__GNUC_MINOR__) ")"
#endif
#else
...
#define DESMUME_VERSION_STRING                                                 \
  " "                                                                          \
  "0.9.12" DESMUME_SUBVERSION_STRING DESMUME_FEATURE_STRING                    \
      DESMUME_PLATFORM_STRING DESMUME_JIT DESMUME_CPUEXT_STRING
...
const char *EMU_DESMUME_SUBVERSION_STRING() {
  return DESMUME_SUBVERSION_STRING;
}
