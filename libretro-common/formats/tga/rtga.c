...
static uint8_t *rtga__tga_load(rtga__context *s, unsigned *x, unsigned *y,
...
static unsigned char *rtga__convert_format(unsigned char *data, int img_n,
...
static uint8_t *rtga__tga_load(rtga__context *s, unsigned *x, unsigned *y,
    ...
    /*   error check */
    if ((tga_width < 1) || (tga_height < 1) || (tga_image_type < 1) ||
            (tga_image_type > 3) ||
            ((tga_bits_per_pixel != 8) && (tga_bits_per_pixel != 16) &&
             (tga_bits_per_pixel != 24) && (tga_bits_per_pixel != 32)))
        return NULL; /* we don't report this as a bad TGA because we don't even know
                    if it's TGA */
    ...
    *y = tga_height;
    ...
    if (req_comp && req_comp != tga_comp)
        tga_data = rtga__convert_format(tga_data, tga_comp, req_comp, tga_width,
                                        tga_height);

    return tga_data;
}

int rtga_process_image(rtga_t *rtga, void **buf_data, size_t size,
    ...
    rtga->output_image = (uint32_t *)rtga_load_from_memory(
                             rtga->buff_data, size, width, height, &comp, 4);
    *buf_data = rtga->output_image;
    ...
    while (size_tex--) {
        ...
        ((unsigned int *)rtga->output_image)[size_tex] =
            A | (R << 16) | G | (B >> 16);
    };
